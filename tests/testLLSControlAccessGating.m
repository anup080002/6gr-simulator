...
% A time-varying TDD channel cannot claim exact same-realization SRS-to-DL
% reciprocity.  In that case SRS remains a causal UL authority, while DL is
% admitted independently and must obtain its own CSI from CSI-RS/data.
...
assert(~isempty(movingDLGrants), ...
    ["Moving TDD must not use stale/absent SRS as a DL admission gate when " + ...
    "the runtime channel cannot provide exact same-realization reciprocity."]);
