...
assert(logical(movingWithSRS.LatestDLFeedback(1).Valid) && ...
...
assert(double(sixgr.util.structGet(grant, "MCSIndex", NaN)) > 1 && ...
        grantOperatingPointSource == "runtime_cqi_to_amc" && ...
        contains(grantMeasurementSource, "srs_reciprocity") && ...
        contains(grantMeasurementSource, "scheduler"), ...
    "DL grants after reciprocal SRS feedback must consume measured CQI-derived AMC before PHY execution. Observed MCS=%g operating-source='%s' measurement-source='%s' status='%s'.", ...
