...
localWrite(fullfile(layout.ReportCSVDir, "scenario_summary.csv"), table( ...
    "ROOT_CAUSE_TRUTH", true, 0, 30, 20, 14, ...
    'VariableNames', {'ScenarioID','ResultOk','RequiredFailureCount', ...
    'SCS_kHz','SlotsPerFrame','SymbolsPerSlot'}));
...
controlledFolder = fullfile(tmp, "controlled_sweep");
controlledLayout = sixgr.report.resultLayout(controlledFolder);
localEnsureDirs({controlledLayout.ReportCSVDir, controlledLayout.PacketFlowCSVDir, ...
    controlledLayout.RFCSVDir, controlledLayout.SystemCSVDir, ...
    controlledLayout.ControlCSVDir, controlledLayout.AirInterfaceCSVDir, ...
    controlledLayout.BeamformingCSVDir});
localWrite(fullfile(controlledLayout.ReportCSVDir, "scenario_summary.csv"), table( ...
    "CONTROLLED_SWEEP", true, 0, 30, 20, 14, ...
    'VariableNames', {'ScenarioID','ResultOk','RequiredFailureCount', ...
    'SCS_kHz','SlotsPerFrame','SymbolsPerSlot'}));
controlledCoverage = coverageT;
controlledCoverage.LargeScaleWidebandSINR_dB(:) = NaN;
localWrite(fullfile(controlledLayout.ReportCSVDir, "live_coverage_layer.csv"), controlledCoverage);
localWrite(fullfile(controlledLayout.ReportCSVDir, "live_user_performance_snapshot.csv"), userPerfT);
controlledCfg = cfg;
controlledCfg.run.snrSweepEnabled = true;
controlledCfg.run.noiseOperatingMode = "standalone_awgn_snr_argument";
controlledScfg = struct("ScenarioID", "CONTROLLED_SWEEP", "ConfigHash", "controlled_hash");
sixgr.truth.exportLLSOutputCoverageArtifacts(controlledFolder, controlledScfg, controlledCfg);
controlledIssues = readtable( ...
    fullfile(controlledLayout.ReportCSVDir, "result_issue_registry.csv"), ...
    "VariableNamingRule", "preserve");
controlledHarq = controlledIssues(contains(string(controlledIssues.issue_id), ...
    "controlled_snr_sweep_harq_outage_observed"), :);
assert(~isempty(controlledHarq) && ...
    all(lower(string(controlledHarq.issue_status)) == "informational") && ...
    all(lower(string(controlledHarq.severity)) == "low"), ...
    "Expected low-SNR HARQ outages must remain visible but nonblocking in a controlled sweep.");
