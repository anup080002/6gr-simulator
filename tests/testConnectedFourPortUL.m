...
for rank=[1 2 4]
    ...
    [rx,rxInfo]=sixgr.phy.ul.PUSCH_Rx(wave,gnb,'NoiseVar',noise,'NoiseVarDomain','time');
    assert(rx.Ok && isequal(rx.TransportBlock,bits));
    localAssertDecoderNoiseDomain(rx,rxInfo);
    postCfg=gnb;
    postCfg.phy.pusch.measurements.decoderNoiseVarianceMode='post_equalization';
    [postRx,postInfo]=sixgr.phy.ul.PUSCH_Rx(wave,postCfg,'NoiseVar',noise,'NoiseVarDomain','time');
    assert(postRx.Ok && isequal(postRx.TransportBlock,bits), ...
        'The post-EQ mode must decode the same actual captured codeword.');
    localAssertDecoderNoiseDomain(postRx,postInfo);
    assert(rx.MeasuredDMRSPortCount==rank && ...
        rx.MeasuredDMRSAntennaPortCount==4 && ...
        string(rx.MeasuredDMRSPortSet)==string(mat2str(0:rank-1)) && ...
        string(rx.MeasuredDMRSPortSetSource)== ...
        "executed_nrPUSCHConfig.DMRS.DMRSPortSet", ...
        'test:PUSCHMeasuredDMRSPortDomain', ...
        ['Receiver DM-RS reference count must be %d, separate from four ' ...
         'precoded antenna-grid dimensions; observed %.0f/%.0f.'], ...
        rank,rx.MeasuredDMRSPortCount,rx.MeasuredDMRSAntennaPortCount);
    evidence=sixgr.link.deriveMeasuredPHYEvidence(rx);
    modulationEvidence=sixgr.link.deriveModulationTrackingMetrics(tx,rx,gnb,"UL");
    assert(numel(modulationEvidence.EVMPerLayer_rms)==rank && ...
        all(isfinite(modulationEvidence.EVMPerLayer_rms)) && ...
        modulationEvidence.EVMPerLayerSource=="paired_layer_symbols_average_reference_power_no_payload_fit", ...
        'Each decoded layer must retain its own paired-symbol EVM.');
    assert(evidence.MeasuredDMRSPortCount==rank && ...
        evidence.MeasuredDMRSAntennaPortCount==4 && ...
        string(evidence.MeasuredDMRSPortSet)==string(mat2str(0:rank-1)), ...
        'The primary trial evidence must retain both measured reference domains.');
    unavailable=sixgr.phy.ul.PUSCH_Rx(wave,gnb,'NoiseVar',NaN, ...
        'StrictNoiseVarianceRequired',false);
    assert(~unavailable.Ok && ~unavailable.DecodeAttempted && ...
        isnan(unavailable.NoiseVar) && ...
        unavailable.MeasuredDMRSPortCount==rank && ...
        unavailable.MeasuredDMRSAntennaPortCount==4 && ...
        string(unavailable.MeasuredDMRSPortSet)==string(mat2str(0:rank-1)), ...
        'Invalid noise must retain the reference domains without fabricating a decode.');
...
function localAssertDecoderNoiseDomain(rx,info)
assert(string(rx.LLRNoiseVarianceDomain)==string(info.DecoderNoiseVariance.Domain), ...
    'test:PUSCHDecoderNoiseDomainMismatch', ...
    'Exported LLR variance domain must be the actually executed decoder-noise resolver domain.');
if string(info.DecoderNoiseVariance.EffectiveMode)=="pre_equalization"
    assert(rx.LLRCSIWeightApplied && ...
        rx.LLRNoiseVariance==info.DecoderNoiseVariance.PreEqualizationNoiseVar);
else
    assert(string(info.DecoderNoiseVariance.EffectiveMode)=="post_equalization" && ...
        ~rx.LLRCSIWeightApplied && ...
        rx.LLRNoiseVariance==info.DecoderNoiseVariance.PostEqualizationNoiseVar);
end
end
