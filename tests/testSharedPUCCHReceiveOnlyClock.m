...
for item=items
    ...
    elseif item.Kind=="PreparePUCCH"
        ...
        assert(~isempty(noEvidence.CSIReportCalendar) && ...
            ~any(noEvidence.CSIReportResourceOverlap) && ...
            isempty(noEvidence.CSIReferenceEvidence) && ...
            isempty(noEvidence.CSIReportConfiguration) && ...
            noEvidence.Context.CSIPart1Bits==0 && ...
            noEvidence.Context.HARQACKBits==h.Context.HARQACKBits && ...
            noEvidence.Context.SRBits==h.Context.SRBits, ...
            'Configured CSI without a completed reference observation must not enter the receive layout.');
        calendarRow=noEvidence.CSIReportCalendar(1,:);
        sourceSlot=double(calendarRow.CSIReferenceSlot);
        evidence=struct('UEIndex',1,'ServingCell',double(h.ServingCell),'Slot',sourceSlot, ...
            'Transmitted',true,'Observed',true,'Consumed',true, ...
            'ResourceExtractionAvailable',true,'ChannelEstimateAvailable',true, ...
            'CSIMeasurementAvailable',true,'ObservationDeliverySlot',sourceSlot, ...
            'MeasurementClockDomain',"shared_receiver_sample_clock/v1", ...
            'ObservationStartSample',0,'ObservationEndSampleExclusive',1, ...
            'ObservationSampleRateHz',1,'ResultAvailableAtSample',1, ...
            'MeasurementClockEpoch',1,'DeliverySlotStartSample',0, ...
