...
assert(~any(logical(dl.TrialTable.LinkAdaptationApplied(1:2))), ...
    "DL pre-feedback bootstrap rows must not claim that link adaptation was applied.");
bootstrapArtifact = dl.TrialTable(1, :);
bootstrapArtifact.LinkAdaptationApplied(:) = true;
bootstrapArtifact = sixgr.truth.canonicalizeLLSLiveSignalChainTable( ...
    "dl_pdsch_trials", bootstrapArtifact);
assert(~logical(bootstrapArtifact.LinkAdaptationApplied(1)), ...
    "Final artifact canonicalization must not promote a decoded bootstrap grant to feedback-applied LA.");
...
assert(~any(logical(ul.TrialTable.LinkAdaptationApplied(1:2))), ...
    "UL pre-feedback bootstrap rows must not claim that link adaptation was applied.");
