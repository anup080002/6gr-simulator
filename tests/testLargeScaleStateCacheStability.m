...
assert(string(stateOff.PathlossModelSource) == "pathloss_disabled_fixed_snr_identity" && ...
    string(stateOff.PathlossComplianceStatus) == "not_applicable_pathloss_disabled" && ...
    ~logical(stateOff.FallbackUsedForPathloss), ...
    "A disabled pathloss identity must not be labelled as an executed TR 38.901 pathloss result.");
