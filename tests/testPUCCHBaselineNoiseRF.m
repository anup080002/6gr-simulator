...
mkdir(fullfile(outputRoot,'noise_replays'));
...
while count<v.noise_occasions
    ...
    if ulAllowed && ~dlAllowed
        ...
        preHit=find(string({outputs.ID})==string(v.receiver_id)+":pre_rf");
        assert(isscalar(preHit)); preWaveform=outputs(preHit).Chunk.Samples;
        assert(isequal(size(waveform),[lengthSamples nr]) && ...
            isequal(size(preWaveform),size(waveform)));
        assert(all(isfinite(waveform),'all') && all(isfinite(preWaveform),'all'));
        assert(outputs(hit).Chunk.StartSample==first && ...
            outputs(hit).Chunk.EndSample+1==stop && ...
            outputs(preHit).Chunk.StartSample==first && ...
            outputs(preHit).Chunk.EndSample+1==stop);
        iq.IQ(iqFirst:iqFirst+lengthSamples-1,1:nr)=waveform;
        % Actual pre/post-RF samples share one row and absolute-clock map.
        % Pre-RF IQ is audit-only and must never be given to the receiver.
        iq.PreRFIQ(iqFirst:iqFirst+lengthSamples-1,1:nr)=preWaveform;
        rxHit=find(string({execution.RX.ID})==string(v.receiver_id));
        assert(isscalar(rxHit)); replay=execution.RX(rxHit).Replay;
        assert(replay.RequestedAWGNReferenceSNR_dB==s.Data.simulation.snr_db);
        prePower=mean(abs(double(preWaveform)).^2,1);
        postPower=mean(abs(double(waveform)).^2,1);
        assert(isequaln(prePower,replay.PreRFMeanSamplePowerPerBranch_mW));
        replayRelativePath=string(sprintf('noise_replays/occasion_%06d.mat',count));
        replayPath=fullfile(outputRoot,replayRelativePath);
        observationIdentity=struct('Occasion',count,'AbsoluteSlot0',slot, ...
            'ReceiverID',string(v.receiver_id),'ConfigurationEpoch',epoch, ...
            'StartSample',first,'EndSampleExclusive',stop,'IQFirstRow',iqFirst, ...
            'IQRowCount',lengthSamples,'ReceiveBranches',nr, ...
            'PreRFIQField',"PreRFIQ",'PostRFIQField',"IQ", ...
            'PreRFChunkSHA256',outputs(preHit).Chunk.SHA256, ...
            'PostRFChunkSHA256',outputs(hit).Chunk.SHA256);
        % Persist EVERY complete replay, including time-varying AGC gains;
        % retaining only terminal.mat cannot close earlier sample accounting.
        save(replayPath,'replay','observationIdentity');
        replaySHA256=sixgr.util.sha256File(replayPath);
        for bits=double(v.harq_bit_counts(:).')
            ...
            rows{rowIndex}=struct('Occasion',count,'AbsoluteSlot0',slot, ...
                ...
                'InjectedNoiseVarianceDomain',string(replay.InjectedNoiseVarianceDomain), ...
                'PreRFIQField',"PreRFIQ",'PostRFIQField',"IQ", ...
                'PreRFChunkSHA256',outputs(preHit).Chunk.SHA256, ...
                'PostRFChunkSHA256',outputs(hit).Chunk.SHA256, ...
                'ReplayRelativePath',replayRelativePath,'ReplaySHA256',replaySHA256, ...
                'MeasuredPreRFSamplePowerPerBranch_mW',prePower, ...
                'MeasuredPostRFSamplePowerPerBranch_mW',postPower, ...
                'SamplePowerMeasurementDefinition',"noise_only_total_time_sample_power_not_RSRP_RSSI", ...
