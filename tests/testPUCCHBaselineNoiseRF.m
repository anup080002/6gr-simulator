...
while count<v.noise_occasions
    ...
    if ulAllowed && ~dlAllowed
        ...
        rxGrid=sixgr.phy.waveform.ofdmDemodulate(carrier,waveform);
        symbols=nrExtractResources(nrPUCCHIndices(carrier,pucch),rxGrid);
        ...
        for bits=double(v.harq_bit_counts(:).')
            [decoded,~,metric]=nrPUCCHDecode(carrier,pucch,bits,symbols,'DetectionThreshold',threshold);
            detected=~isempty(decoded{1}); ack=sum(decoded{1}==1);
            assert(detected==(metric>=threshold) && (~detected || numel(decoded{1})==bits));
            rowIndex=rowIndex+1;
