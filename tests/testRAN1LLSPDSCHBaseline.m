...
assert(all(result.TrialTable.MCSTable == "calibration_explicit") && ...
    all(result.TrialTable.MCSTable == string(result.AssumptionsTable.MCSTable)), ...
    "Fixed-MCS PDSCH trials must report the PDSCH calibration table authority.");
assert(~any(result.TrialTable.DMRSResidualPostEqSINRBoundEnabled) && ...
    ~any(result.TrialTable.DecisionDirectedPostEqSINRBoundEnabled) && ...
    ~any(result.TrialTable.PostEqSINRDMRSResidualBoundApplied));
assert(all(isfinite(result.TrialTable.DecoderNoiseVariance) & ...
    result.TrialTable.DecoderNoiseVariance > 0));
assert(all(result.TrialTable.DecoderNoiseVarianceConfiguredMode == "post_equalization"));
...
assert(publicationCfg.provenance.requireCleanWorktree && ...
