...
assert(contains(body,"strcmp(ME.identifier,'sixgr:phy:pdcch:NoFreeCandidate')") && ...
    contains(body,'control_blocked_no_nonoverlapping_pdcch_candidate') && ...
    contains(body,'cancelUnexecutedHARQGrantRuntime'), ...
    ['Shared-clock candidate exhaustion must become an explicit blocked ' ...
     'grant disposition, not terminate the entire scenario or overlap REs.']);
