...
pa = sixgr.rf.applyRFImpairmentChain(xPA, cfg, ...
    ...
    "ApplyPA", true, ...
    "ApplyADC", false);
% Normalized Rapp p=2 has exponent 1/(2*p). The former square-root
% expectation duplicated the production foldback defect instead of testing
% an amplitude response that saturates at unity.
expectedPA = xPA ./ (1 + abs(xPA).^4).^(1/4);
...
% A configured resolution does not authorize execution.  This guards the
% YAML feature switch independently from ADC parameters.
bitsOnly = struct();
bitsOnly.rf.adcBits = 4;
bitsOnly.phy.impairments.adcQuantizationBits = 4;
bitsOnlyOut = sixgr.rf.applyRFImpairmentChain(xAdc, bitsOnly, ...
    "SampleRateHz", fs, ...
    "Endpoint", "rx", ...
    "UseLegacyGlobalConfig", true, ...
    "StrictMutationRequired", false, ...
    "ApplyADC", true);
assert(~logical(bitsOnlyOut.Replay.ADCQuantizationApplied), ...
    "ADC bit depth without an explicit enable flag must remain disabled.");
assert(localMaxAbs(bitsOnlyOut.Waveform - xAdc) < 1e-12, ...
    "Disabled ADC parameters must not mutate the receive waveform.");
