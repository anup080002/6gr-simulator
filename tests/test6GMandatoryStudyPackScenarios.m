...
for i = 1:numel(scenarioFiles)
    ...
    for k = 1:numel(overrides)
        overrideConfig = sixgr.util.structGet(overrides(k), "config", struct());
        merged = sixgr.util.mergeStruct(scfg.toStruct(), overrideConfig);
        merged = sixgr.lls6g.config.normalizeScenarioAliases(merged, ...
            "SourceFiles", scfg.SourceFiles, "ConfigPath", scenarioPath, ...
