...
assert(all(double(heatmap.occupancy_fraction) >= 0 & ...
    double(heatmap.occupancy_fraction) <= 1), ...
    "Time-frequency PRB occupancy fractions must remain in [0,1] under spatial MU-MIMO reuse.");
assert(all(ismember(["spatial_reuse_peak","allocation_count"], ...
    string(heatmap.Properties.VariableNames))), ...
    "PRB heatmaps must report spatial reuse separately from time-frequency occupancy.");
...
assert(all(ismember(["qcl_accuracy","qcl_type","qcl_source_rs","qcl_status", ...
    "tci_state_id","unified_tci_state_id","tci_validity_timer_slots","tci_status"], ...
    string(beamPrecoder.Properties.VariableNames))), ...
    "Beam/precoder output must retain explicit QCL/TCI schema even when the active path correctly reports it unavailable.");
...
assert(height(csiReport)==1 && string(csiReport.ReportIdentity)=="coverage_receiver_fixture" && ...
    csiReport.RI==2 && string(csiReport.CQIDerivedModulation)=="64QAM" && ...
    string(csiReport.source_artifact_ref)=="control/csv/received_csi_reports.csv", ...
    "Public CSI reports must come from receiver rows, not scheduler observations.");
...
publicIdentityTables = {pdschRuntimeEvent, puschRuntimeEvent, pdcchPublic, ...
    pucchPublic, prachDetection, srsMeasurement, csiRsRuntime, csiReport, ...
    trsPublic, ssbPbchCellSearch, noiseEvidence, mcsCqiDecision};
for identityIndex = 1:numel(publicIdentityTables)
    identityT = publicIdentityTables{identityIndex};
    if isempty(identityT) || ~ismember("RunId", string(identityT.Properties.VariableNames))
        continue;
    end
    assert(all(strtrim(string(identityT.RunId)) == "unit_output_coverage"), ...
        "Every public LLS table must use the logical run tag as RunId, never a missing database key.");
end
...
assert(~isempty(plotManifest) && ~any(strcmp(string(plotManifest.PlotId), ...
    "power_energy_cumulative")), ...
    "The retired cumulative-power raster contract must not return; runtime power CSVs feed the post-run contract materializer.");
...
assert(~isempty(plotRenderStatus) && ...
    ~any(strcmp(string(plotRenderStatus.PlotRenderStatus), "rendered_real_plot")), ...
    "The pre-materialization coverage exporter must not claim that legacy MATLAB plots were rendered.");
...
% Explicit declared receiver fixture, independent of scheduler CQI rows.
localWrite(fullfile(layout.ControlCSVDir, "received_csi_reports.csv"), ...
    struct2table(struct('SourceSignal',"received_CSI_UCI",'Direction',"DL", ...
    'ReportIdentity',"coverage_receiver_fixture",'CSIUCIDecodeOk',1, ...
    'CSIUCICRCPass',1,'CSIUCITransport',"pucch_independently_received", ...
    'UEIndex',1,'RNTI',320,'SourceSlot',1,'DueSlot',6,'DeliveredSlot',6, ...
    'CQI',15,'RI',2,'PMI',0,'CRI',0, ...
