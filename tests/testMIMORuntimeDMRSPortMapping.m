...
for direction=["DL","UL"]
    ...
    raw.(direction).MinimumSnapshotRank=[1;1;4];
    raw.(direction).SpatialSpanRank=[2;4;4];
    raw.(direction).SpatialRankDimensionLimit=4*ones(3,1);
    raw.(direction).RankEstimateSource=repmat( ...
        "received_channel_estimate_numerical_svd_not_RI:maximum_snapshot",3,1);
end
...
assert(isequal(out.RankLayerTrials.MinimumSnapshotRank,[1;1;4;1;1;4]) && ...
    isequal(out.RankLayerTrials.SpatialSpanRank,[2;4;4;2;4;4]) && ...
    all(out.RankLayerTrials.SpatialRankDimensionLimit==4) && ...
    all(contains(out.RankLayerTrials.RankEstimateSource,"numerical_svd_not_RI")), ...
    'Do not discard spatial span or relabel numerical channel rank as decoded RI.');
...
withIdentity=raw;
for direction=["DL","UL"]
    withIdentity.(direction).MeasuredDMRSPortSet=["[0]";"[0 1]";"[0 1 2 3]"];
    withIdentity.(direction).MeasuredDMRSPortSetSource=repmat( ...
        "executed_receiver_dmrs_configuration",3,1);
end
identified=sixgr.mimo.resolveNominalVsEffectiveMIMO(cfg,withIdentity,'StrictMode',true);
assert(isequal(identified.RankLayerTrials.DMRSPorts, ...
    ["0";"0|1";"0|1|2|3";"0";"0|1";"0|1|2|3"]) && ...
    isequal(identified.LayerMetrics.DMRSPort, ...
    [0;0;1;0;1;2;3;0;0;1;0;1;2;3]), ...
    ['Explicit receiver-measured DM-RS identities must survive rank/layer ' ...
     'reconciliation and expand one-to-one into layer metrics.']);
