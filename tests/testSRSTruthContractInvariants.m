...
assert(all(isfinite(double(passing.NMSE_dB)) & double(passing.NMSE_dB) <= -8), ...
    "Strict SRS pass rows must carry finite, threshold-compliant NMSE.");
finiteNmse = double(T.NMSE_dB(isfinite(double(T.NMSE_dB))));
assert(isempty(finiteNmse) || all(finiteNmse <= -8), ...
    "Primary SRS NMSE evidence must not report positive/unusable estimates as valid NMSE.");
