...
% These rates already have a time denominator. Spectral efficiency must
% use that same basis, not divide again by a fixed DL/UL duty fraction.
for name=["DLThroughputCurve","ULThroughputCurve"]
    curve=res.Tables.(name);
    assert(all(abs(curve.SpectralEfficiency_bps_Hz_mean- ...
        curve.Goodput_Mbps_mean*1e6/100e6)<1e-12), ...
        'test:SpectralEfficiencyDoubleDutyFactor', ...
        '%s spectral efficiency must equal its goodput divided by channel bandwidth.',name);
end
assert(all(abs(res.Tables.Summary.SpectralEfficiency_mean_bps_Hz- ...
    res.Tables.Summary.Goodput_Mbps_mean*1e6/100e6)<1e-12));
assert(all(abs(res.Tables.KPISummary.SpectralEfficiency_bps_Hz- ...
    res.Tables.KPISummary.Goodput_Mbps*1e6/100e6)<1e-12));
...
% Both target bandwidths use identical rate/SE units, independently of the
% configured DL/UL ratio. These are declared measurement fixtures, not PHY
% throughput or full-scenario acceptance results.
for bandwidth=[5e6 400e6]
    targetRoot=tempname; mkdir(targetRoot);
    targetCleanup=onCleanup(@()rmdir(targetRoot,'s')); %#ok<NASGU>
    target=sixgr.analytics.generateMeasuredSINRCurves(targetRoot,'se_units', ...
        'TrialData',trialData,'ScenarioConfig', ...
        struct('global_radio_scope',struct('channel_bandwidth_hz',bandwidth)), ...
        'WriteKPISummary',true,'UpdateAnchorKPIs',false);
    for name=["DLThroughputCurve","ULThroughputCurve"]
        curve=target.Tables.(name);
        assert(all(abs(curve.SpectralEfficiency_bps_Hz_mean- ...
            curve.Goodput_Mbps_mean*1e6/bandwidth)<1e-12));
        assert(all(curve.SpectralEfficiencyDefinition== ...
            "goodput_over_channel_bandwidth_same_time_basis"));
    end
    assert(all(abs(target.Tables.Summary.SpectralEfficiency_mean_bps_Hz- ...
        target.Tables.Summary.Goodput_Mbps_mean*1e6/bandwidth)<1e-12));
    assert(all(abs(target.Tables.KPISummary.SpectralEfficiency_bps_Hz- ...
        target.Tables.KPISummary.Goodput_Mbps*1e6/bandwidth)<1e-12));
    fprintf('MEASURED_SINR_SE_UNITS_PASS bandwidth_hz=%g directions=DL,UL\n',bandwidth);
    clear targetCleanup;
end
