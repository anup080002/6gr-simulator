...
assert(double(srsCfg.ExpectedNumRB) == 273 && double(srsCfg.NumRB) == 272, ...
    ["The configured 273-RB request and the standardized 272-RB resolved " ...
     "allocation must remain separately observable."]);
assert(double(cov.OccupiedPRBCount) == double(srsCfg.NumRB), ...
    "Configured-band coverage must be measured from the actual mapped SRS PRBs.");
