...
diagnosticCtx = llsRootGateFixture("pass");
diagnosticScenario = diagnosticCtx.ScenarioConfig;
if isa(diagnosticScenario, "sixgr.lls6g.config.ScenarioConfig")
    diagnosticScenario = diagnosticScenario.toStruct();
end
diagnosticScenario = sixgr.util.structSet(diagnosticScenario, ...
    "validation.run_class", "adaptive_system_diagnostic");
diagnosticScenario = sixgr.util.structSet(diagnosticScenario, ...
    "canonical_control.run.min_trials_per_sinr_bin", 5);
diagnosticCfg = sixgr.util.structSet(diagnosticCtx.InternalConfig, ...
    "validation.run_class", "adaptive_system_diagnostic");
diagnosticCfg = sixgr.util.structSet(diagnosticCfg, ...
    "canonical_control.run.min_trials_per_sinr_bin", 5);
diagnosticVerdict = sixgr.truth.evaluateLLSRuntimeTruthContract( ...
    diagnosticCtx.RunFolder, diagnosticScenario, diagnosticCfg);
assert(logical(diagnosticVerdict.Ok) && ...
    ~logical(diagnosticVerdict.CheckDetails.MeasuredSINR.PublicationSampleAdequacyRequired) && ...
    double(diagnosticVerdict.CheckDetails.MeasuredSINR.RequiredTrialsPerAggregateDLBin) == 1, ...
    ["A bounded adaptive diagnostic must require real measured waveform rows " + ...
    "without falsely applying publication sample-adequacy thresholds."]);

% A configured-SINR LLS with geometry explicitly disabled has no physical
% distance authority. It must not fabricate distance-vs-SINR rows merely to
% satisfy a geometry-only plot contract.
diagnosticDistancePath = fullfile(diagnosticCtx.Layout.AirInterfaceCSVDir, ...
    "distance_vs_sinr.csv");
if isfile(diagnosticDistancePath)
    delete(diagnosticDistancePath);
end
diagnosticScenario = sixgr.util.structSet(diagnosticScenario, ...
...
diagnosticCfg = sixgr.util.structSet(diagnosticCfg, ...
    "canonical_control.launch.geometry_enabled", false);
configuredSINRVerdict = sixgr.truth.evaluateLLSRuntimeTruthContract( ...
    diagnosticCtx.RunFolder, diagnosticScenario, diagnosticCfg);
assert(logical(configuredSINRVerdict.Ok) && ...
    ~logical(configuredSINRVerdict.CheckDetails.MeasuredSINR.DistanceSINRApplicable) && ...
    string(configuredSINRVerdict.CheckDetails.MeasuredSINR.DistanceSINRApplicabilityAuthority) == ...
        "sixgr.validation.geometryEvidenceRequired", ...
    ["Geometry-disabled configured-SINR LLS must mark distance-vs-SINR " + ...
     "not applicable rather than fabricating a distance artifact."]);

geometryCtx = llsRootGateFixture("pass");
geometryDistancePath = fullfile(geometryCtx.Layout.AirInterfaceCSVDir, ...
    "distance_vs_sinr.csv");
if isfile(geometryDistancePath)
    delete(geometryDistancePath);
end
geometryScenario = geometryCtx.ScenarioConfig;
if isa(geometryScenario, "sixgr.lls6g.config.ScenarioConfig")
    geometryScenario = geometryScenario.toStruct();
end
geometryScenario = sixgr.util.structSet(geometryScenario, ...
    "validation.geometry_evidence_required", true);
geometryCfg = sixgr.util.structSet(geometryCtx.InternalConfig, ...
    "validation.geometry_evidence_required", true);
geometryVerdict = sixgr.truth.evaluateLLSRuntimeTruthContract( ...
    geometryCtx.RunFolder, geometryScenario, geometryCfg);
assert(~logical(geometryVerdict.Ok) && ...
    logical(geometryVerdict.CheckDetails.MeasuredSINR.DistanceSINRApplicable) && ...
    any(string(geometryVerdict.Failures) == "distance_vs_sinr_missing_or_empty"), ...
    "Geometry LLS must remain fail-closed when measured distance-vs-SINR evidence is absent.");
...
fixedScenario = sixgr.util.structSet( ...
    fixedScenario, "canonical_control.run.min_trials_per_sinr_bin", 40);
fixedCfg = sixgr.util.structSet( ...
    fixedCfg, "canonical_control.run.min_trials_per_sinr_bin", 40);
...
% One fixed applied-SNR point can legitimately spread over adjacent
% measured-SINR histogram bins.  Sample adequacy must use the applied-SNR
% point summary (40 trials), not incorrectly require all 40 observations in
% one 1 dB measured-SINR bin (17 + 23).
splitMeasuredCurve = table( ...
    [NaN; NaN], [17; 23], [1; 1], [0.04; 0.05], ...
    'VariableNames', {'UEIndex','TrialCount','BLER','BER'});
sixgr.util.csvWriteTable(fullfile(fixedCtx.Layout.AirInterfaceCSVDir, ...
    "dl_measured_sinr_bler_curve.csv"), splitMeasuredCurve);
fixedPointSummary = table( ...
    ["DL"; "UL"], [40; 40], [18; 18], ...
    'VariableNames', {'Direction','TrialCount','AppliedSNR_dB'});
sixgr.util.csvWriteTable(fullfile(fixedCtx.Layout.ReportCSVDir, ...
    "fixed_snr_sweep_curve_summary.csv"), fixedPointSummary);
...
assert(string(fixedVerdict.CheckDetails.MeasuredSINR.MeasuredSINRSampleAdequacySource) == ...
    "fixed_snr_sweep_curve_summary_applied_snr_points" && ...
    double(fixedVerdict.CheckDetails.MeasuredSINR.DLBinsMeetingRequiredTrials) == 1, ...
    ["Fixed-sweep sample adequacy must use complete applied-SNR points, " ...
    "not fragmented measured-SINR histogram bins."]);
