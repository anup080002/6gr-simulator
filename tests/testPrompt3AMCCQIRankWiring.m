...
assert(abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.ollaMarginMinDb", NaN)) + 10) < 1e-12 && ...
    abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.ollaMarginMaxDb", NaN)) - 10) < 1e-12 && ...
    abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.deltaMCSMin", NaN)) + 10) < 1e-12 && ...
    abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.deltaMCSMax", NaN)) - 10) < 1e-12, ...
    "OLLA bounds must resolve to [-10,+10] dB margins; deltaMCSMin/Max are legacy aliases.");
...
assert(logical(ollaPolicy.Enabled) && ...
    ...
    "The geometry master must resolve an exact 10%%-BLER ACK/NACK OLLA control law.");
...
row = struct( ...
    ...
    "ReceiverHestSINR_dB", 14, ...
    "ReceiverHestSINRSource", "receiver_hest_runtime_channel_estimate", ...
    "ReceiverHestSINRValueRole", "measured_channel_estimate_scheduling_input", ...
    "ReceiverHestSINRValueStatus", "OK", ...
    "CRCPass", 1);
csiRuntime = sixgr.truth.CoupledTruthRuntime.resolveMeasuredRuntimeCSIForRowRuntime(row, cfgDecision, "DL");
expectedFeedback = sixgr.link.resolveWidebandCQI(struct( ...
    "WidebandSINR_dB", 14, ...
    "SINRSource", "measured_scheduler_csi_conservative_min_channel_estimate_posteq", ...
    "SINRValueRole", "measured_post_equalization_scheduling_input", ...
    "SINRValueStatus", "OK"), cfgDecision, "DL");
expectedCQI = double(sixgr.util.normalizeReportedCQI( ...
    sixgr.util.structGet(expectedFeedback, "WidebandCQI", NaN)));
assert(isfinite(expectedCQI) && expectedCQI > 1, ...
    "Focused regression requires a measured SINR that maps above CQI 1.");
assert(double(csiRuntime.CQI) == expectedCQI && double(csiRuntime.CQI) > 1, ...
    "Runtime CSI feedback must derive CQI from eligible measured scheduler SINR rather than retaining placeholder CQI 1.");
assert(double(csiRuntime.MCSIndex) > 0 && ...
    strcmpi(char(string(csiRuntime.CQISource)), "measured_post_equalization_sinr_to_cqi"), ...
    "Measured runtime CSI must produce a nonzero CQI-derived MCS with measured-SINR provenance.");
assert(~contains(lower(string(csiRuntime.SINRSource)), "receiver_hest"), ...
    "Scheduler-facing CQI resolver provenance must not contain receiver_hest tokens that the CQI mapper rejects.");
