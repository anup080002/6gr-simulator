...
verifyEqual(testCase,[candidates.PMI],[0 1], ...
    "Strict codebook PMIs must retain the 3GPP zero-based identifier domain.");
verifyEqual(testCase,[candidates.BeamIndices],[1 2], ...
    "Runtime beam indices must use the one-based internal array domain.");
verifyGreaterThanOrEqual(testCase,min([candidates.BeamIndices]),1, ...
    "PMI zero must never be discarded as a missing runtime beam.");
