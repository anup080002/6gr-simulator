function bundle = prachStrictAnchorResult(varargin)
%PRACHSTRICTANCHORRESULT Cached strict PRACH mini-anchor execution for tests.
...
scenarioPath = fullfile(repoRoot, "configs", "lls", "lls_prach_strict_mini_anchor.yaml");
runFolder = tempname;
mkdir(runFolder);
cfg = sixgr.lls6g.config.loadScenarioConfig(scenarioPath);
internalCfg = sixgr.lls6g.buildInternalConfig(cfg, runFolder);
result = sixgr.phy.prach.runStrictPRACHValidation(internalCfg, ...
    "RunFolder", runFolder, ...
