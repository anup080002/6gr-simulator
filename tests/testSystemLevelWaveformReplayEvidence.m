...
assert(all(contains(lower(string(T.DecoderTruthProxySINRSource)), "not_decoder_truth") | ...
    contains(lower(string(T.DecoderTruthProxySINRSource)), "not_materialized")), ...
    "%s must disclose decoder SINR proxy quarantine/unavailability.", contextLabel);
assert(all(strcmpi(string(T.DecoderTruthProxySINRValueStatus), "unavailable")), ...
    "%s must mark decoder-side SINR proxy unavailable in primary truth.", contextLabel);
