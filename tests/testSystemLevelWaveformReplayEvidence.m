...
cfg.run.runTag = "system_level_waveform_replay_evidence";
cfg.run.executionID = "system_level_waveform_replay_evidence_execution";
cfg.meta.scenarioID = "unit_test_system_level_export";
cfg.meta.configHash = repmat('a', 1, 64);
...
cfg.phy.duplex.fdd = struct( ...
    "dlCenterFrequencyHz", 4.0e9, ...
    "ulCenterFrequencyHz", 3.9e9);
...
required = ["ReceiverHestSINR_dB","PostEqSINR_dB","DecoderIterations", ...
    ...
    "NoiseVariance","PostEqualizationNoiseVariance","LLRNoiseVariance", ...
    "EVM_rms","EVMProxySINR_dB","StrictReceiverEvidenceOk","TruthStatus", ...
...
if contains(lower(string(contextLabel)), "raw trial")
    required = [required, ...
        "Rank","EffectiveMCSIndex","EffectiveModulation","EffectiveLayers","EffectiveRank", ...
        "ScheduledMCSIndex","ScheduledModulation","ScheduledOperatingPointMatchesTransmitted", ...
        "OfferedBits","GoodBits","OfferedThroughput_Mbps","AirInterfaceObservation_ms", ...
        "RunID","ExecutionID", rawTrialEvidenceRequired];
else
    required = [required, "MCSIndex","Modulation","NumLayers","TargetCodeRate"];
end
...
assert(all(double(T.NoiseVariance) > 0) && ...
    all(double(T.PostEqualizationNoiseVariance) > 0) && ...
    all(double(T.LLRNoiseVariance) > 0), ...
    "%s must retain positive receiver noise-variance lineage.", contextLabel);
assert(all(double(T.EVM_rms) > 0) && ...
    all(abs(double(T.EVMProxySINR_dB) + 20 .* log10(double(T.EVM_rms))) < 1e-8), ...
    "%s must retain measured EVM and its exact diagnostic SINR transform.", contextLabel);
assert(all(logical(T.StrictReceiverEvidenceOk)), ...
    "%s must keep CRC outcome separate from receiver-evidence validity.", contextLabel);
if contains(lower(string(contextLabel)), "raw trial")
    assert(all(double(T.OfferedBits) == double(T.TBSize_bits)) && ...
        all(double(T.GoodBits) == double(T.OfferedBits) .* double(T.CRCPass)), ...
        "%s must reconcile offered/good bits to TBS and CRC.", contextLabel);
    assert(all(strlength(strtrim(string(T.RunID))) > 0) && ...
        all(strlength(strtrim(string(T.ExecutionID))) > 0), ...
        "%s must bind immutable runtime identities.", contextLabel);
end
end
