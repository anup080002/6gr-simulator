...
postRuntimeStatus = status;
postRuntimeStatus.Stage = "rf_ready";
sixgr.util.csvWriteTable(fullfile(layout.ReportCSVDir, "live_stage_status.csv"), postRuntimeStatus);
failurePath = fullfile(layout.MetaDir, "failure_debug_report.txt");
sixgr.util.writeTextFile(failurePath, "post-waveform finalization failure");
postRuntimeCheckpoint = sixgr.truth.assertCompletedWaveformRuntimeCheckpoint( ...
    runFolder, "ExpectedTotalSlots", 12, "ExpectedSweepPointCount", 3);
assert(postRuntimeCheckpoint.Ok && ...
    string(postRuntimeCheckpoint.HistoricalFailureClassification) == ...
    "post_waveform_finalization_failure_preserved_not_runtime_blocker" && ...
    any(string(postRuntimeCheckpoint.HistoricalFailureArtifacts) == string(failurePath)), ...
    "A post-waveform finalization failure must be preserved without blocking resume.");

runtimeFailureStatus = status;
runtimeFailureStatus.Stage = "control_gating_streaming";
sixgr.util.csvWriteTable(fullfile(layout.ReportCSVDir, "live_stage_status.csv"), runtimeFailureStatus);
localAssertIdentifier(@()sixgr.truth.assertCompletedWaveformRuntimeCheckpoint( ...
    runFolder, "ExpectedTotalSlots", 12, "ExpectedSweepPointCount", 3), ...
