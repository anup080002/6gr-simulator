...
noBeamCfg = localCfg(1, 20, "256QAM");
noBeamRaw = raw;
for direction = ["DL","UL"]
    noBeamRaw.(direction).Layers(:) = 1;
    noBeamRaw.(direction).RankEstimate(:) = 1;
    noBeamRaw.(direction).PostEqSINRPerLayer_dB(:) = "18";
    noBeamRaw.(direction).SelectedBeamIndex(:) = NaN;
end
noBeamOut = sixgr.mimo.resolveNominalVsEffectiveMIMO( ...
    noBeamCfg, noBeamRaw, "RunId", "mimo_rank1_no_selected_beam", ...
    "StrictMode", true);
% Two precoder output ports carrying one input layer are not rank two.
twoPortRankOne=noBeamRaw;
twoPortRankOne.UL.PrecodingNumLogicalPorts=2*ones(height(twoPortRankOne.UL),1);
twoPortRankOne.UL.PrecodingNumLayers=ones(height(twoPortRankOne.UL),1);
twoPortCfg=noBeamCfg; twoPortCfg.phy.pusch.NumAntennaPorts=2;
portOut=sixgr.mimo.resolveNominalVsEffectiveMIMO(twoPortCfg,twoPortRankOne, ...
    "RunId","two_port_rank_one","StrictMode",true);
ulPort=portOut.AntennaArrayConfig(portOut.AntennaArrayConfig.Direction=="UL",:);
assert(ulPort.ObservedLogicalTxPortCount==2 && ulPort.LogicalPortLayerMatch, ...
    'Precoder ports must not be forced equal to the transmitted layer count.');
for invalidPort=[0,1.5,65,NaN]
    badPort=twoPortRankOne;
    badPort.UL.PrecodingNumLogicalPorts(end)=invalidPort;
    badOut=sixgr.mimo.resolveNominalVsEffectiveMIMO(twoPortCfg,badPort, ...
        "RunId","invalid_port_minority_row","StrictMode",true);
    badUL=badOut.AntennaArrayConfig(badOut.AntennaArrayConfig.Direction=="UL",:);
    assert(~badUL.LogicalPortLayerMatch,'Every row must satisfy the port/layer contract.');
end
assert(all(string(noBeamOut.BeamCodebook.BeamId) == "not_selected") && ...
    all(string(noBeamOut.BeamCodebook.Status) == "pass") && ...
    all(string(noBeamOut.BeamSweepMeasurements.SelectedBeamId) == "not_selected") && ...
    all(string(noBeamOut.BeamSweepMeasurements.Status) == "pass"), ...
    ["A rank-one non-beamformed runtime must record the categorical " ...
    "not_selected state consistently; NaN and inactive raw sentinels are not beam IDs."]);
...
assert(height(exported.Evidence.StrictGateSummary) == 8 && ...
    all(logical(exported.Evidence.StrictGateSummary.Pass)), ...
    "A passing export must report every strict MIMO sub-gate explicitly.");
layout = sixgr.report.resultLayout(exportRoot);
rankExport = readtable(fullfile(layout.BeamformingCSVDir, ...
    "rank_layer_trials.csv"), "VariableNamingRule", "preserve");
summaryExport = readtable(fullfile(layout.BeamformingCSVDir, ...
    "mimo_configured_vs_effective.csv"), "VariableNamingRule", "preserve");
assert(ismember("AdaptivePolicyFailureReason", ...
    string(rankExport.Properties.VariableNames)), ...
    "Passing adaptive-policy rows must retain their declared empty failure-reason column.");
assert(all(ismember(["SpatialChannelRankEstimate", ...
    "SpatialChannelTxPorts", "SpatialChannelRxAntennas", ...
    "SpatialChannelRankDomain", "SpatialChannelRankSource", ...
    "MinimumSnapshotRank", "SpatialSpanRank", ...
    "SpatialRankDimensionLimit", "RankEstimateSource"], ...
    string(rankExport.Properties.VariableNames))), ...
    "MIMO evidence must separate measured spatial-channel rank from decoded data rank.");
assert(ismember("RequiredExactMatchPercent", ...
    string(summaryExport.Properties.VariableNames)), ...
    "AMC summaries must retain the not-applicable exact-match threshold column.");
...
assert(any(~logical(failedExport.Evidence.StrictGateSummary.Pass)) && ...
    contains(string(failedExport.FailureReason), "configured_vs_effective"), ...
    "A failed export must identify the exact failing strict MIMO sub-gate.");
...
rankAdaptiveCfg = localCfg(1, 20, "256QAM");
rankAdaptiveCfg.link_adaptation.fixed_or_amc = "amc";
rankAdaptiveCfg.mimo.rank_adaptation_policy = "adaptive";
rankAdaptiveCfg.phy.pdsch.maxLayers = 2;
rankAdaptiveCfg.phy.pusch.maxLayers = 2;
rankAdaptiveCfg.phy.pusch.NumAntennaPorts = 2;
rankAdaptiveRaw = struct( ...
    "DL", localAddAdaptivePolicyEvidence(localRank2Rows("DL")), ...
    "UL", localAddAdaptivePolicyEvidence(localRank2Rows("UL")));
for direction = ["DL","UL"]
    n = height(rankAdaptiveRaw.(direction));
    rankAdaptiveRaw.(direction).NumTxPorts = repmat(2,n,1);
    rankAdaptiveRaw.(direction).NumRxAntennas = repmat(4,n,1);
    rankAdaptiveRaw.(direction).PrecodingNumLogicalPorts = repmat(2,n,1);
    rankAdaptiveRaw.(direction).NumLogicalRxBranches = repmat(2,n,1);
end
rankAdaptiveOut = sixgr.mimo.resolveNominalVsEffectiveMIMO( ...
    rankAdaptiveCfg, rankAdaptiveRaw, "RunId", "mimo_rank1_bootstrap_rank2_runtime", ...
    "StrictMode", true);
assert(all(~logical(rankAdaptiveOut.RankLayerTrials.ExactSpatialMatch)) && ...
    all(logical(rankAdaptiveOut.RankLayerTrials.SpatialContractMatch)) && ...
    all(logical(rankAdaptiveOut.RankLayerTrials.ExecutionContractOk)) && ...
    all(string(rankAdaptiveOut.RankLayerTrials.Status) == "pass"), ...
    "An adaptive rank-two grant must pass against a rank-two capability " + ...
    "while remaining visibly different from the rank-one bootstrap.");
assert(all(double(rankAdaptiveOut.RankLayerTrials.ConfiguredRank) == 1) && ...
    all(double(rankAdaptiveOut.RankLayerTrials.ConfiguredMaximumRank) == 2), ...
    "Initial rank and maximum supported rank must remain separate export fields.");
...
assert(all(double(lowSNROut.ConfiguredVsEffective.DominantEffectiveDecodedRank) == 2) && ...
    all(double(lowSNROut.ConfiguredVsEffective.DominantEffectiveDecodedLayers) == 2), ...
    ["Aggregate decoded-rank evidence must be reduced over successful " ...
     "receiver rows; CRC-failed rank-zero sentinels belong to BLER, not " ...
     "the dominant decoded spatial-rank statistic."]);
...
assert(all(double(failedRows.TransmittedRank) == 2) && ...
    ...
    all(logical(failedRows.ExecutionContractOk)) && ...
    all(logical(failedRows.StrictOk)) && ...
    all(~logical(failedRows.DecodeReliabilityOk)) && ...
    all(string(failedRows.DecodeReliabilityStatus) == "crc_fail") && ...
    all(string(failedRows.Status) == "pass") && ...
    all(strlength(string(failedRows.FailureReason)) == 0), ...
    "MIMO evidence must preserve transmitted rank two while keeping failed decoded rank explicitly zero.");
assert(all(logical(lowSNROut.RankUtilization.StrictObjectivePass)) && ...
    any(double(lowSNROut.RankUtilization.BLER) > 0), ...
    ["Rank-utilization evidence must pass the transmitted spatial contract " ...
     "while retaining the independently measured nonzero BLER."]);
...
assert(all(string(noRaw.ConfiguredVsEffective.Status) == "not_validated") && ...
    all(string(noRaw.ConfiguredVsEffective.FailureReason) == "no_runtime_trials") && ...
    all(isnan(noRaw.ConfiguredVsEffective.DominantTransmittedRank)), ...
    "No trials must not become a bootstrap failure or an applied rank.");
assert(all(string(noRaw.AntennaArrayConfig.Status)=="not_validated") && ...
    all(string(noRaw.AntennaArrayConfig.FailureReason)=="no_runtime_trials") && ...
    all(~noRaw.AntennaArrayConfig.ExactRuntimeAntennaMatch) && ...
    all(string(noRaw.AntennaPortMapping.Status)=="not_validated") && ...
    all(string(noRaw.AntennaPortMapping.FailureReason)=="no_runtime_trials"), ...
    "Absent trials are not measured antenna mismatches or validated port mappings.");
oneDirectionRaw = adaptiveRaw;
oneDirectionRaw.UL = adaptiveRaw.UL([], :);
oneDirection = sixgr.mimo.resolveNominalVsEffectiveMIMO( ...
    adaptiveCfg, oneDirectionRaw, "RunId", "mimo_no_ul", "StrictMode", true);
ulSummary = oneDirection.ConfiguredVsEffective( ...
    string(oneDirection.ConfiguredVsEffective.Direction) == "UL", :);
assert(~oneDirection.StrictOk && height(ulSummary) == 1 && ...
    string(ulSummary.Status) == "not_validated" && ...
    string(ulSummary.FailureReason) == "no_runtime_trials" && ...
    ~ulSummary.ScenarioObjectivePass && ~ulSummary.RuntimePopulated && ...
    ulSummary.RuntimeTrialCount == 0 && isnan(ulSummary.DominantTransmittedRank), ...
    "An empty UL ledger must remain unvalidated, not pass or invent rank.");
ulAntenna=oneDirection.AntennaArrayConfig(string(oneDirection.AntennaArrayConfig.Direction)=="UL",:);
ulPorts=oneDirection.AntennaPortMapping(string(oneDirection.AntennaPortMapping.Direction)=="UL",:);
assert(string(ulAntenna.Status)=="not_validated" && ...
    string(ulPorts.Status)=="not_validated" && ~ulAntenna.ExactRuntimeAntennaMatch);
assert(any(string(badOut.AntennaArrayConfig.Status)=="fail"), ...
    "A real wrong-dimension waveform must still fail.");
...
% This declared algebraic fixture models two receiver-observed DM-RS port
% dimensions. Keep the count explicit; do not let the reducer infer it from
% Layers or construct port identities.
...
T.MUMIMOSpatialDesignContractVersion = repmat( ...
    "MeasuredMUMIMOPairDesign/v13", n, 1);
T.MUMIMOSpatialSignatureSubspaceMode = repmat( ...
    "complete_detectable_subspace", n, 1);
if upper(string(T.Direction(1))) == "DL"
    ...
    T.MUMIMOSpatialDesignEvidenceSource = T.MUMIMOPairingEvidenceSource;
    T.MUMIMOTransmitArchitecture = repmat("hybrid_rf_baseband", n, 1);
    ...
    T.InterferenceCovarianceSource = repmat( ...
        "oracle_separated_shared_slot_per_prb_symbol_contribution_grid_covariance", n, 1);
else
    T.MUMIMOPairingMetricSource = repmat( ...
        "measured_srs_complete_peer_subspace_admission_projection_leakage_full_dimensional_per_re_irc", n, 1);
    T.MUMIMOPairingEvidenceSource = repmat( ...
        "causal_measured_srs_complete_peer_subspace_pairing_with_full_receiver_observation_and_runtime_per_re_irc_covariance", n, 1);
    T.MUMIMOSpatialDesignEvidenceSource = T.MUMIMOPairingEvidenceSource;
    T.MUMIMOTransmitArchitecture = repmat("receive_full_dimensional_per_re_irc", n, 1);
