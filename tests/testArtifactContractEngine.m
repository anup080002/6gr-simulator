...
% The canonical master-YAML policy must override the disabled global
% output default.  With no registered evidence, an enabled policy must
% reach the fail-closed generator instead of returning DISABLED_BY_YAML.
...
try
    ...
    error("testArtifactContractEngine:ExpectedCanonicalAuthority", ...
        "Enabled canonical YAML authority must not be masked by the disabled global default.");
catch ME
    assert(strcmp(ME.identifier, "sixgr:artifact:RequiredArtifactGenerationFailed"), ...
        "Unexpected canonical artifact-authority error: %s | %s", ME.identifier, ME.message);
end
try
