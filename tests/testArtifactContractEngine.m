...
assert(height(runtimeCatalog) == 3 && ...
    ...
    "Same-execution runtime publication must contain exactly 3 CSV contracts.");
...
assert(~isfile(fullfile(runRoot, "artifact_generation", ...
    ...
    "A successful artifact build must not emit an empty failure CSV.");
...
% A CSV-only runtime catalog must not leave a header-only visual lineage or
% failure registry. Their presence would falsely advertise raster evidence
% and fail the strict runtime semantic audit despite containing no events.
csvOnlyRoot = tempname();
mkdir(csvOnlyRoot);
csvOnlyCleanup = onCleanup(@() localRemove(csvOnlyRoot)); %#ok<NASGU>
csvOnlyResult = sixgr.artifact.ContractArtifactGenerator.generate( ...
    csvOnlyRoot, evidence, "Catalog", miniCatalog(1, :));
assert(csvOnlyResult.Ok && csvOnlyResult.CSVCount == 1 && ...
    csvOnlyResult.PNGCount == 0);
assert(~isfile(fullfile(csvOnlyRoot, "components", ...
    "contract_plot_lineage.csv")), ...
    "CSV-only publication must omit an empty visual-lineage CSV.");
assert(~isfile(fullfile(csvOnlyRoot, "artifact_generation", ...
    "artifact_generation_failures.csv")), ...
    "CSV-only success must omit an empty failure registry.");
