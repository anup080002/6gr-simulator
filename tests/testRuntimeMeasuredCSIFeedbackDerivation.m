...
referenceIsolatedCSI = sixgr.truth.CoupledTruthRuntime.resolveMeasuredRuntimeCSIForRowRuntime(row, cfg, "DL");

assert(abs(double(referenceIsolatedCSI.SINR_dB) - 18.5) < 1e-9 && ...
    referenceIsolatedCSI.MCSIndex==csi.MCSIndex && referenceIsolatedCSI.CQI==csi.CQI, ...
    "An independent pilot-reference measurement must not replace data post-equalization SINR.");
assert(string(referenceIsolatedCSI.SINRSource)==row.MeasuredTrialSINRSource && ...
    string(referenceIsolatedCSI.SINRValueRole)==row.MeasuredTrialSINRValueRole, ...
    "Preserve actual receiver provenance instead of relabelling mixed measurement planes.");
