...
assert(double(srsCfg.Periodicity) == 20 && double(srsCfg.Offset) == 4, ...
    ["SRS waveform authority must preserve the 20-slot period and the " + ...
     "zero-based offset 4 authored by the geometry master YAML."]);
assert(isequal(double(srsCfg.ExpectedSlotSet(:).'), 4), ...
    "Strict SRS waveform resource must resolve the authored slot-5 occasion to zero-based slot 4.");
