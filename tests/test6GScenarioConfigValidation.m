...
coupledUsersFile = fullfile(tmp, "coupled_users.yaml");
fid = fopen(coupledUsersFile, "w");
fprintf(fid, "%s", ['{' ...
    '"inherits":["' strrep(fullfile(pwd, "simulator", "configs", "defaults", "global.yaml"), '\', '\\') '"],' ...
    '"meta":{"scenario_id":"coupled_users","description":"coupled users","version":"1","owner":"t","maturity_tag":"smoke"},' ...
    '"simulation":{"link_direction":"both"},' ...
    '"users":{"enabled":true,"n_users":2,"rnti_start":11,"seed_stride":3,' ...
    '"execution_model":"slot_coupled_truth","beam_selection_strategy":"fixed_first_beam","save_user_tables":true}}']);
fclose(fid);

scfgCoupled = sixgr.lls6g.config.loadScenarioConfig(coupledUsersFile);
assert(string(scfgCoupled.get("users.execution_model")) == "slot_coupled_truth", ...
    "Coupled truth execution mode should be accepted by schema validation.");

strictIndependentFile = fullfile(tmp, "strict_independent_truth_users.yaml");
fid = fopen(strictIndependentFile, "w");
fprintf(fid, "%s", ['{' ...
    '"inherits":["' strrep(fullfile(pwd, "simulator", "configs", "defaults", "global.yaml"), '\', '\\') '"],' ...
    '"meta":{"scenario_id":"strict_independent_truth_users","description":"bad strict truth users","version":"1","owner":"t","maturity_tag":"smoke","tags":["truth"]},' ...
    '"scenario":{"honesty_mode":"strict"},' ...
    '"simulation":{"link_direction":"both"},' ...
    '"mimo":{"n_tx_ant":2,"n_rx_ant":2,"n_layers":1,"precoder_type":"wideband"},' ...
    '"users":{"enabled":true,"n_users":2,"rnti_start":11,"seed_stride":3,' ...
    '"execution_model":"independent_link_sweep","beam_selection_strategy":"fixed_first_beam","save_user_tables":true}}']);
fclose(fid);
...
threw = false;
try
    sixgr.lls6g.config.loadScenarioConfig(strictIndependentFile);
catch ME
    threw = contains(string(ME.identifier), "IndependentSweepForbiddenForTruth");
end
assert(threw, "Strict/truth-tagged multi-user configs must reject independent_link_sweep.");
