...
assert(first.RuntimeChannelReplay.RuntimeChannelOutputSampleAlignment=="continuous_raw_samples");
[expectedGuard,expectedGuardSource]=sixgr.phy.sync.resolveTimingSearchGuard( ...
    cfg,first.RuntimeChannelReplay.SampleRate_Hz);
assert(first.SSBTimingSearchGuardSamples==expectedGuard && ...
    first.SSBTimingSearchGuardSource==expectedGuardSource && expectedGuard>0);
assert(~first.RuntimeChannelReplay.RuntimeChannelAlignmentLookaheadExecutedOnFork && ...
    first.RuntimeChannelReplay.RuntimeChannelAlignmentLookaheadSamples==0 && ...
    first.RuntimeChannelReplay.RuntimeChannelAppliedAlignmentTrimSamples==0);
...
for k = 1:numel(indices)
    ...
    assert(candidate.SSBIndex==indices(k) && ...
        candidate.ObservedSSBOccasionIndex==indices(k) && ...
        candidate.SSBIndexSource=="received_pss_candidate_window", ...
        "Every received timing window retains its identity even when PBCH decoding fails.");
    assert(candidate.PBCHHypothesisSSBIndex==candidate.PBCH.SSBIndex);
