...
runtimeEqT = sixgr.util.csvReadTable(runtimeEqCsv);
for rawField = ["RawEqualizedReal", "RawEqualizedImag"]
    assert(~ismember(rawField, string(runtimeEqT.Properties.VariableNames)) || ...
        all(isnan(runtimeEqT.(rawField))), ...
        "A legacy fitted constellation without receiver provenance must not be relabeled as raw receiver samples.");
end
...
relationshipT = sixgr.util.csvReadTable(fullfile(tmpConstRuntime, "reports", "csv", "ber_vs_bler.csv"), ...
    "TextType", "string");
...
beamOutputsT = sixgr.util.csvReadTable(fullfile(tmpConstRuntime, "reports", "csv", "beam_management_outputs.csv"), ...
    "TextType", "string");
assert(ismember("MetricKey", string(beamOutputsT.Properties.VariableNames)), ...
    "Beam export must retain its metric schema when quoted notes contain spaces and semicolons.");
...
assert(all(beamFailure.Availability == "not_available") && ...
    ~any(logical(beamFailure.CountsTowardCoverage)), ...
    "Statistics whose raw beam trace is absent must not count as measured runtime coverage.");
trackingT = sixgr.util.csvReadTable(fullfile(tmpConstRuntime, "reports", "csv", "cfo_to_tracking_traces.csv"), ...
    "TextType", "string");
...
data = sixgr.util.structSet(data, "frequency.duplex_mode", "TDD");
