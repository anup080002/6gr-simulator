...
REQUIRED_CSV: dict[str, CsvContract] = {
    ...
    "ssb_timing_matrix.csv": CsvContract(
        ("TestID", "Case", "FrequencyRange", "CarrierFrequency_Hz", "SSBSCS_kHz",
         "Lmax", "CandidateIndex", "StartSymbol", "Slot", "Symbol",
         "ExpectedValid", "ActualValid", "ReasonCode", "Status"),
        ("TestID",),
    ),
    "prach_occasion_matrix.csv": CsvContract(
        ("TestID", "FrequencyRange", "DuplexMode", "ConfigurationIndex", "Format",
         "PeriodFrames", "Frame", "AbsoluteSlot", "StartSymbol", "DurationSymbols",
         "FrequencyIndex", "ULAvailable", "ExpectedValid", "ActualValid",
         "ReasonCode", "Status"),
        ("TestID",),
    ),
    "fdd_grid_trace.csv": CsvContract(
        ("TestID", "CCID", "Direction", "GridID", "BWPID", "AbsSlot", "Symbol",
         "StartRB", "NumRB", "ResourceKey", "ExpectedValid", "ActualValid",
         "ReasonCode", "Status"),
        ("TestID",),
    ),
    "shortcut_removal_audit.csv": CsvContract(
        ("CheckID", "Behavior", "SearchScope", "MatchCount", "ExpectedCount",
         "Status", "Reason"),
        ("CheckID",),
    ),
...
def verify_csvs(root: Path) -> tuple[list[dict[str, object]], dict[str, list[dict[str, str]]]]:
    ...
    for name, contract in REQUIRED_CSV.items():
        ...
        try:
            ...
            if "IndexConvention" not in header:
                reasons.append("missing_columns:IndexConvention")
            if rows and not missing:
                ...
                bad_index_convention = [
                    str(index) for index, row in enumerate(rows, start=2)
                    if str(row.get("IndexConvention", "")).strip()
                    != "zero_based_phy_indices"
                ]
                if bad_index_convention:
                    reasons.append(
                        "bad_index_convention_rows:"
                        + ",".join(bad_index_convention[:50])
                    )
            ...
            if name == "shortcut_removal_audit.csv" and rows:
                for index, row in enumerate(rows, start=2):
                    observed = finite_number(row.get("MatchCount", ""))
                    expected = finite_number(row.get("ExpectedCount", ""))
                    if observed != 0 or expected != 0:
                        reasons.append(
                            f"shortcut_hit:line={index}:"
                            f"observed={row.get('MatchCount','')}"
                        )
            if name == "fdd_grid_trace.csv" and rows:
                grids_by_direction: dict[str, set[str]] = {}
                for row in rows:
                    grids_by_direction.setdefault(
                        str(row.get("Direction", "")), set()
                    ).add(str(row.get("GridID", "")))
                dl = grids_by_direction.get("DL", set())
                ul = grids_by_direction.get("UL", set())
                if not dl or not ul or dl & ul:
                    reasons.append("fdd_dl_ul_grid_identity_not_separate")
...
def write_report(root: Path, rows: list[dict[str, object]]) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not root.is_dir():
        ...
        return 66
    expected_csv = set(REQUIRED_CSV)
    observed_csv = {path.name for path in root.glob("*.csv")}
    expected_images = set(IMAGE_SOURCE)
    observed_images = {path.name for path in root.glob("*.png")}
    count_results: list[dict[str, object]] = []
    csv_set_reasons = []
    if observed_csv != expected_csv:
        csv_set_reasons.append(
            "csv_set_mismatch:"
            f"missing={sorted(expected_csv - observed_csv)}:"
            f"unexpected={sorted(observed_csv - expected_csv)}"
        )
    count_results.append(row_result(
        "required_csv_set", "ARTIFACT_SET",
        "PASS" if not csv_set_reasons else "FAIL", csv_set_reasons,
        Expected=15, Observed=len(observed_csv),
    ))
    image_set_reasons = []
    if observed_images != expected_images:
        image_set_reasons.append(
            "image_set_mismatch:"
            f"missing={sorted(expected_images - observed_images)}:"
            f"unexpected={sorted(observed_images - expected_images)}"
        )
    count_results.append(row_result(
        "required_png_set", "ARTIFACT_SET",
        "PASS" if not image_set_reasons else "FAIL", image_set_reasons,
        Expected=9, Observed=len(observed_images),
    ))
    ...
    all_results = count_results + csv_results + image_results + link_results
