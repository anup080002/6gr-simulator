...
loaded={}
for c in rows(pack/'desired_rf_impact_csv_contract.csv'):
...
if 'rf_impact_raw_trials.csv' in loaded:ck(len(loaded['rf_impact_raw_trials.csv'])>=768,'all_experiments')
if 'rf_impact_rule_evaluation.csv' in loaded:ck(len(loaded['rf_impact_rule_evaluation.csv'])>=96 and all(truth(r['Passed']) for r in loaded['rf_impact_rule_evaluation.csv']),'all_rules')
if 'rf_impact_operating_points.csv' in loaded:ck(all(truth(r['Complete']) for r in loaded['rf_impact_operating_points.csv']),'all_pairs_complete')
if 'rf_impact_summary.csv' in loaded:ck(all(int(float(r['Failed']))==0 and int(float(r['Incomplete']))==0 for r in loaded['rf_impact_summary.csv']),'impact_complete')
audit={r['ImageFile']:r for r in loaded.get('rf_impact_image_semantic_audit.csv',[])}
for c in rows(pack/'desired_rf_impact_image_contract.csv'):
 ...
 ck(w>=int(c['MinimumWidth']) and h>=int(c['MinimumHeight']),'png_dimensions:'+c['ImageFile']);ck(nb,'png_nonblank:'+c['ImageFile']);a=audit.get(c['ImageFile']);ck(a is not None,'png_audit:'+c['ImageFile'])
 if a:
