...
def verify(root: Path) -> int:
    ...
    csv_contract = read_contract("desired_pdsch_impact_csv_contract.csv")
    image_contract = read_contract("desired_pdsch_impact_image_contract.csv")
    matrix = read_contract("pdsch_impact_experiment_matrix.csv")
    rules = {
        row["RuleID"]: row
        for row in read_contract("pdsch_impact_acceptance_rules.csv")
    }
    ...
    expected_experiments = {row["ExperimentID"] for row in matrix}
    raw = parsed.get("pdsch_impact_raw_trials.csv", [])
    completed = {row.get("ExperimentID", "") for row in raw}
    missing_experiments = sorted(expected_experiments - completed)
    ...
    for line, row in enumerate(raw, 2):
        ...
        if "not_waveform_truth" not in row.get("ApproximationMode", ""):
            failures.append(f"missing_evidence_label:line={line}")
        source = root.parent / "pdsch_dlsch_phase" / row.get("SourceCSV", "")
        if not source.exists():
            failures.append(f"missing_bound_source:line={line}")
        elif digest(source) != row.get("SourceCSV_SHA256"):
            failures.append(f"source_digest_mismatch:line={line}")

    evaluation = parsed.get("pdsch_impact_rule_evaluation.csv", [])
    ...
    effects = parsed.get("pdsch_impact_pairwise_effects.csv", [])
    if len(effects) != 12:
    ...
    if len(parsed.get("pdsch_impact_summary.csv", [])) != 12:
        failures.append("summary_count")
    if len(parsed.get("pdsch_impact_finding_coverage.csv", [])) != 12:
        failures.append("coverage_count")

    manifest = parsed.get("pdsch_impact_run_manifest.csv", [])
    if len(manifest) == 1:
        row = manifest[0]
        if truth(row.get("TruthQualified")):
    ...
    audits = {
        ...
        for row in parsed.get("pdsch_impact_image_semantic_audit.csv", [])
    }
    for contract in image_contract:
        ...
        for actual, required, label in (
            (audit.get("AxesCount", "0"), contract["MinAxesCount"], "axes"),
            (audit.get("SeriesCount", "0"), contract["MinSeriesCount"], "series"),
            (
                audit.get("FinitePointCount", "0"),
                contract["MinFinitePointCount"],
                "points",
            ),
        ):
            if int(float(actual)) < int(required):
                failures.append(f"{label}_count:" + name)
    ...
    report = {
        "required_experiments": len(expected_experiments),
