...
CSV_CONTRACT='desired_protocol_csv_contract.csv';IMG_CONTRACT='desired_protocol_image_contract.csv';AUDIT='protocol_image_semantic_audit.csv'
...
def verify(root):
 ...
 # Technical gates.
 for i,r in enumerate(parsed.get('rlc_pdu_encoding.csv',[]),2):
  if integer(r.get('MismatchCount'))!=0:fail.append('rlc_mismatch:'+str(i))
 for i,r in enumerate(parsed.get('pdcp_pdu_encoding.csv',[]),2):
  if integer(r.get('MismatchCount'))!=0:fail.append('pdcp_mismatch:'+str(i))
 for i,r in enumerate(parsed.get('pdcp_security_results.csv',[]),2):
  if integer(r.get('MismatchCount'))!=0 or r.get('ExpectedHex')!=r.get('ActualHex'):fail.append('security:'+str(i))
 for i,r in enumerate(parsed.get('rrc_asn1_messages.csv',[]),2):
  if integer(r.get('MismatchCount'))!=0 or not r.get('IndependentVectorSHA256'):fail.append('rrc_asn1:'+str(i))
 for i,r in enumerate(parsed.get('radio_bearer_config.csv',[]),2):
  if not all(truth(r.get(x)) for x in ['RRCValidated','RLCCommitted','PDCPCommitted','MACCommitted','Atomic']):fail.append('bearer_atomic:'+str(i))
 for i,r in enumerate(parsed.get('protocol_conservation.csv',[]),2):
  if any(integer(r.get(x))!=0 for x in ['UnownedBytes','DuplicateDeliveredBytes','EquationErrorBytes']):fail.append('conservation:'+str(i))
 for i,r in enumerate(parsed.get('protocol_negative_tests.csv',[]),2):
  if r.get('ExpectedError')!=r.get('ActualError') or any(truth(r.get(x)) for x in ['StateChanged','PDUProduced','DeliveryCounted']) or not truth(r.get('Passed')):fail.append('negative:'+str(i))
 required={'rlc_um_header','rlc_am_header','rlc_status','rlc_state','pdcp_header','pdcp_count','pdcp_security','pdcp_reordering','sdap_header','sdap_mapping','rrc_semantics','bearer_config','handover','traffic','lineage'};seen=set()
 for i,r in enumerate(parsed.get('protocol_independent_vector_results.csv',[]),2):
  seen.add(r.get('VectorFamily',''))
  if integer(r.get('MismatchCount'))!=0 or len(r.get('OracleArtifactSHA256',''))!=64:fail.append('oracle:'+str(i))
  if any(x in r.get('OracleClass','').lower() for x in ['same_toolbox','self_consistency','same_implementation']):fail.append('oracle_independence:'+str(i))
 if required-seen:fail.append('missing_oracles:'+','.join(sorted(required-seen)))
 for i,r in enumerate(parsed.get('protocol_test_summary.csv',[]),2):
...
def selftest():
 cc=contracts(CSV_CONTRACT);ic=contracts(IMG_CONTRACT);oracles=['rlc_um_header','rlc_am_header','rlc_status','rlc_state','pdcp_header','pdcp_count','pdcp_security','pdcp_reordering','sdap_header','sdap_mapping','rrc_semantics','bearer_config','handover','traffic','lineage']
 with tempfile.TemporaryDirectory() as td:
  ...
  for c in cc:
   ...
   for i in range(max(1,int(c['MinRows']))):
    ...
    if fn=='protocol_test_summary.csv':r.update(Mandatory='true',Executed='true',Passed='true',Total='1',Failed='0',Skipped='0',Blocked='0')
    rs.append(r)
