...
loaded={}
for c in rows(pack/"desired_channel_impact_csv_contract.csv"):
...
manifest=loaded.get("channel_impact_run_manifest.csv",[])
ck(len(manifest)>=768,"impact_manifest_768")
raw=loaded.get("channel_impact_raw_trials.csv",[])
ck(len(raw)>=768,"impact_raw_trials_768")
if raw:
    groups={}
    for r in raw: groups.setdefault((r["FamilyID"],r["PairID"],r["Seed"]),[]).append(r)
    ck(len(groups)>=384,"impact_pairs_384")
    inv=["GeometryHash","ChannelHash","NoiseHash"]
...
rules=loaded.get("channel_impact_rule_evaluation.csv",[])
ck(len(rules)>=96,"impact_rules_96")
if rules: ck(all(r["Result"].upper()=="PASS" for r in rules),"impact_rules_pass")
points=loaded.get("channel_impact_operating_points.csv",[])
if points: ck(all(r["Incomplete"].lower()=="false" for r in points),"no_incomplete_points")
summary=loaded.get("channel_impact_summary.csv",[])
if summary: ck(all(r["CompletionStatus"].upper()=="COMPLETE" for r in summary),"all_families_complete")
runtime=loaded.get("channel_impact_runtime.csv",[])
if runtime: ck(all(r["SerialSHA256"]==r["ParallelSHA256"] and r["Reproducible"].lower()=="true" for r in runtime),"serial_parallel_reproducibility")
audit={r["ImageFile"]:r for r in loaded.get("channel_impact_image_semantic_audit.csv",[])}
for c in rows(pack/"desired_channel_impact_image_contract.csv"):
