...
adaptiveCfg = cfg;
adaptiveCfg.validation.RunClass = "adaptive_system_diagnostic";
adaptiveCfg.phy.linkAdaptation.mode = "adaptive";
adaptiveCfg.run.snrSweepEnabled = true;
adaptiveCfg.channel.model = "AWGN";
adaptiveCfg.channel.fading.model = "AWGN";
adaptiveCfg.channel.awgnOnly = true;
adaptiveT = localDegradedTrialTable();
adaptiveT = adaptiveT(1:4, :);
adaptiveT.SNR_dB = [-20; -20; 20; 20];
adaptiveT.CRCPass = [false; false; true; true];
adaptiveT.BitErrors = [500000; 500000; 0; 0];
adaptive = sixgr.truth.evaluatePDSCHObjectiveStrict(adaptiveT, adaptiveCfg, ...
    "RunId", "adaptive_sweep_case", ...
    "ScenarioName", "adaptive_sweep_objective_scope", ...
    "StrictMode", true);
assert(logical(adaptive.ObjectivePass), ...
    "Adaptive sweep must apply unchanged objectives at its highest configured SNR bin. Failures: %s", ...
    strjoin(string(adaptive.FailureCodes), "|"));
assert(abs(double(adaptive.Summary.RawBLER(1)) - 0.5) < eps && ...
    double(adaptive.Summary.ObjectiveBLER(1)) == 0, ...
    "Global sweep BLER must retain outage rows while objective BLER uses the high-SNR bin.");
assert(string(adaptive.Summary.ObjectiveEvaluationScope(1)) == ...
    "highest_configured_snr_bin" && ...
    double(adaptive.Summary.ObjectiveEvaluationSNR_dB(1)) == 20, ...
    "Adaptive objective scope and operating point must be explicit.");

fixedSweep = sixgr.truth.evaluatePDSCHObjectiveStrict(adaptiveT, cfg, ...
    "RunId", "fixed_anchor_case", ...
    "ScenarioName", "fixed_anchor_global_objective", ...
    "StrictMode", true);
assert(~logical(fixedSweep.ObjectivePass), ...
    "Fixed-anchor objectives must continue to score all positive attempts.");
