...
exactSeedCfg = cfg;
exactSeedCfg.validation.fixed_link_campaign.seeds = [7001 7002 7003];
exactSeedPub = sixgr.analytics.evaluatePublicationReadinessGates( ...
    exactSeedCfg, tmp);
assert(logical(exactSeedPub.Flags.LongRunStabilityOk), ...
    "The exact YAML seed identities must satisfy long-run pairing.");

% A persisted pre-lineage table may have valid BLER/drop rows but no
% configured seed index/value columns.  It is not publication evidence,
% but the reducer must classify it fail-closed instead of throwing an
% array-size exception during recovery/refinalization.
dropPath = fullfile(tmp, "air_interface", "csv", ...
    "multi_seed_drop_statistics.csv");
legacyDrop = readtable(dropPath, "VariableNamingRule", "preserve");
legacyDrop = removevars(legacyDrop, ...
    ["FixedLinkSeedIndex","FixedLinkSeedValue","SeedLineageComplete"]);
sixgr.util.csvWriteTable(dropPath, legacyDrop);
legacyPub = sixgr.analytics.evaluatePublicationReadinessGates( ...
    exactSeedCfg, tmp);
assert(~logical(legacyPub.Flags.LongRunStabilityOk) && ...
    string(legacyPub.Tables.LongRunStability.ConfiguredSeedPairingFailureCode(1)) == ...
    "configured_seed_pairing_schema_invalid", ...
    ["Legacy drop statistics without configured-seed lineage must fail " ...
     "closed without crashing the terminal publication reducer."]);
localWriteTerminalEvidence(tmp);

wrongSeedCfg = exactSeedCfg;
wrongSeedCfg.validation.fixed_link_campaign.seeds = [8001 8002 8003];
wrongSeedPub = sixgr.analytics.evaluatePublicationReadinessGates( ...
    wrongSeedCfg, tmp);
wrongSeedLongRun = wrongSeedPub.Tables.LongRunStability;
wrongSeedFailure = string( ...
    wrongSeedLongRun.ConfiguredSeedPairingFailureCode(1));
assert(~logical(wrongSeedPub.Flags.LongRunStabilityOk) && ...
    wrongSeedFailure == "configured_seed_set_does_not_match_yaml", ...
    ["Long-run evidence with the right seed count but different seed " + ...
     "identities must fail the YAML binding contract."]);

collapsedSeeds = readtable(dropPath, "VariableNamingRule", "preserve");
collapsedSeeds.FixedLinkSeedIndex(:) = 1;
collapsedSeeds.FixedLinkSeedValue(:) = 7001;
sixgr.util.csvWriteTable(dropPath, collapsedSeeds);
collapsed = sixgr.analytics.evaluatePublicationReadinessGates(cfg, tmp);
assert(~logical(collapsed.Flags.LongRunStabilityOk) && ...
    double(collapsed.Tables.LongRunStability.SeedCount(1)) == 1, ...
    ["Distinct drop/task seeds must not masquerade as independent " + ...
     "configured seeds in the long-run stability gate."]);
localWriteTerminalEvidence(tmp);

disjointSeeds = readtable(dropPath, "VariableNamingRule", "preserve");
secondPoint = double(disjointSeeds.FixedLinkPointIndex) == 2;
disjointSeeds.FixedLinkSeedValue(secondPoint) = ...
    double(disjointSeeds.FixedLinkSeedValue(secondPoint)) + 10000;
sixgr.util.csvWriteTable(dropPath, disjointSeeds);
disjoint = sixgr.analytics.evaluatePublicationReadinessGates(cfg, tmp);
assert(~logical(disjoint.Flags.LongRunStabilityOk) && ...
    ~logical(disjoint.Tables.LongRunStability.ConfiguredSeedPairingOk(1)) && ...
    string(disjoint.Tables.LongRunStability.ConfiguredSeedPairingFailureCode(1)) == ...
    "configured_seed_index_value_map_not_bijective", ...
    ["Equal per-point seed counts with different YAML seed identities " + ...
     "must fail the long-run pairing contract."]);
localWriteTerminalEvidence(tmp);

nestedCfg = rmfield(cfg, "canonical_control");
nestedCfg.lls6g.resolvedConfig.canonical_control = cfg.canonical_control;
nestedCfg.lls6g.resolvedConfig.canonical_control.run.num_seeds = 30;
nested = sixgr.analytics.evaluatePublicationReadinessGates(nestedCfg, tmp);
assert(~logical(nested.Flags.LongRunStabilityOk) && ...
    double(nested.Tables.LongRunStability.RequiredSeedCount(1)) == 30 && ...
    double(nested.Tables.LongRunStability.SeedCount(1)) == 3, ...
    ["Long-run stability must retain the resolved master-YAML seed " ...
     "requirement after buildInternalConfig nesting; three observed seeds " ...
     "cannot satisfy a configured 30-seed campaign."]);
...
tmpConfiguredOnly = tempname;
mkdir(tmpConfiguredOnly);
cleanupConfiguredOnly = onCleanup(@() rmdir(tmpConfiguredOnly, "s")); %#ok<NASGU>
configuredOnlyLayout = sixgr.report.resultLayout(tmpConfiguredOnly);
sixgr.util.ensureFolder(configuredOnlyLayout.MetaDir);
sixgr.util.jsonWrite(fullfile(configuredOnlyLayout.MetaDir, "runtime_summary.json"), ...
    struct("ElapsedSeconds", 12.5, "StartedUTC", "2026-06-29T00:00:00Z", ...
    "CompletedUTC", "2026-06-29T00:00:12Z"));
configuredOnly = sixgr.analytics.evaluatePublicationReadinessGates(cfg, tmpConfiguredOnly);
assert(~logical(configuredOnly.Flags.PerformanceProfileOk), ...
    "Enabling profiling in configuration must not substitute for persisted profiler evidence.");
assert(string(configuredOnly.Tables.Performance.FailureReason(1)) == ...
    "persisted_profiling_evidence_missing", ...
    "The performance gate must identify the missing measured profiler artifact exactly.");

tmpInvalidProfiler = tempname;
mkdir(tmpInvalidProfiler);
cleanupInvalidProfiler = onCleanup(@() rmdir(tmpInvalidProfiler, "s")); %#ok<NASGU>
invalidLayout = sixgr.report.resultLayout(tmpInvalidProfiler);
sixgr.util.ensureFolder(invalidLayout.MetaDir);
sixgr.util.ensureFolder(invalidLayout.ReportCSVDir);
sixgr.util.jsonWrite(fullfile(invalidLayout.MetaDir, "runtime_summary.json"), ...
    struct("ElapsedSeconds", 12.5, "StartedUTC", "2026-06-29T00:00:00Z", ...
    "CompletedUTC", "2026-06-29T00:00:12Z"));
invalidProfiler = table("", 0, NaN, ...
    'VariableNames', {'FunctionName','NumCalls','TotalTime_s'});
sixgr.util.csvWriteTable(fullfile(invalidLayout.ReportCSVDir, ...
    "runtime_function_profile.csv"), invalidProfiler);
invalidProfilerGate = sixgr.analytics.evaluatePublicationReadinessGates(cfg, tmpInvalidProfiler);
assert(~logical(invalidProfilerGate.Flags.PerformanceProfileOk) && ...
    double(invalidProfilerGate.Tables.Performance.ProfilerArtifactRows(1)) == 1, ...
    "Header/nonempty profiler shape without finite measured time must fail closed.");
assert(startsWith(string(invalidProfilerGate.Tables.Performance.FailureReason(1)), ...
    "persisted_profiling_evidence_invalid:"), ...
    "Invalid profiler evidence must be distinguished from a missing artifact.");
...
emptyEarlierAlias = table('Size', [0 3], ...
    'VariableTypes', {'string','double','double'}, ...
    'VariableNames', {'FunctionName','NumCalls','TotalElapsed_s'});
sixgr.util.csvWriteTable(fullfile(layout.ReportCSVDir, "time_profile_summary.csv"), ...
    emptyEarlierAlias);

drops = table(repmat("DL", 6, 1), [1;1;1;2;2;2], ...
    [-4;-4;-4;8;8;8], [1;2;3;1;2;3], ...
    [101;102;103;201;202;203], [1;2;3;1;2;3], ...
    [7001;7002;7003;7001;7002;7003], true(6,1), ...
    [0.90;0.91;0.89;0.10;0.11;0.09], ...
    repmat("executed_trial_rows", 6, 1), ...
    'VariableNames', {'Direction','FixedLinkPointIndex','SNR_dB', ...
    'FixedLinkDropIndex','FixedLinkDropSeed','FixedLinkSeedIndex', ...
    'FixedLinkSeedValue','SeedLineageComplete','BLER','EvidenceStatus'});
