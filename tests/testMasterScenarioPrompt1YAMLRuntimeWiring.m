...
expectedSpeedKmh = localResolvedSpeedKmh(resolved);
...
assert(abs(double(sixgr.util.structGet(resolved, "channels.doppler_hz", NaN)) - expectedDopplerHz) < 1e-6, ...
    "%s must derive Doppler from the configured UE speed at 4 GHz.", label);
...
assert(abs(double(sixgr.util.structGet(resolved, "mobility.ue_speed_kmh", NaN)) - expectedSpeedKmh) < 1e-12, ...
    "%s must preserve configured UE mobility speed.", label);
...
assert(abs(double(sixgr.util.structGet(resolved, "link_adaptation.olla_step_down", NaN)) - 0.09) < 1e-12, ...
    "%s must use the exact 10%%-BLER OLLA NACK step 0.09 dB.", label);
...
assert(abs(double(sixgr.util.structGet(resolved, "link_adaptation.target_bler", NaN)) - 0.1) < 1e-12, ...
    "%s must expose the OLLA target BLER explicitly.", label);
...
assert(abs(double(sixgr.util.structGet(cfg, "channel.doppler_Hz", NaN)) - expectedDopplerHz) < 1e-6, ...
    "%s must apply derived configured-speed Doppler internally.", label);
...
assert(all(abs(double(sixgr.util.structGet(cfg, "scenario.mobility.speed_kmh", NaN)) - expectedSpeedKmh) < 1e-12), ...
    "%s must apply configured UE mobility internally.", label);
...
assert(abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.ollaStepDown", NaN)) - 0.09) < 1e-12, ...
...
assert(abs(double(sixgr.util.structGet(cfg, "phy.linkAdaptation.targetBLER", NaN)) - 0.1) < 1e-12, ...
    "%s must wire the explicit OLLA target BLER internally.", label);
...
function speedKmh = localResolvedSpeedKmh(resolved)
speedKmh = double(sixgr.util.structGet(resolved, "mobility.ue_speed_kmh", NaN));
if ~isfinite(speedKmh)
    speedKmh = double(sixgr.util.structGet(resolved, "channels.mobility_kmph", NaN));
end
assert(isfinite(speedKmh) && speedKmh >= 0, ...
    "Scenario must resolve a finite nonnegative UE mobility speed.");
end
...
required = [ ...
    ...
    "random_access.timing.rar_processing_delay_slots"
    "random_access.timing.msg3_k2_slots"
    "random_access.timing.msg4_processing_delay_slots"
    "random_access.timing.setup_complete_k2_slots"
