...
if any(proxyNamedUntracked == allowedProxyNamed)
    ...
    assert(contains(helperText, "evm_proxy_quarantined_not_decoder_truth") && ...
        contains(helperText, "not a") && contains(helperText, "decoder-truth SINR measurement"), ...
        "Allowed proxy-named SINR helper must remain explicitly quarantined as a non-truth proxy.");
...
reportingPath = fullfile(repoRoot, "+sixgr", "+truth", "exportLLSReportingBundle.m");
reportingText = string(fileread(reportingPath));
assert(~contains(reportingText, "chartT = measuredChartT") && ...
    ~contains(reportingText, "useMeasuredBins = true"), ...
    "BLER/throughput-vs-SNR plots must not silently substitute measured-quality bins for controlled SNR sweep data.");
assert(~contains(reportingText, "measured_quality_binning") && ...
    ~contains(reportingText, "measured_sinr_bin_center_db") && ...
    ~contains(reportingText, "localBuildMeasuredQualitySweepChart"), ...
    "BLER/throughput-vs-SNR plot code must not keep a dormant measured-SINR binning shortcut.");
