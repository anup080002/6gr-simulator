...
% After an actual nonlinear/composite RX front end and measured gain
% compensation, an exact scalar post-front-end injected variance is not
% generally defined.  Accept that honest N/A only when strict receiver
% evidence and both downstream reference-derived variance domains exist.
estimatedNoiseTrials = disabledTrials;
for direction = ["DL", "UL"]
    candidate = estimatedNoiseTrials.(char(direction));
    n = height(candidate);
    candidate.ReceiverInputSampleNoiseVariance(:) = NaN;
    candidate.ReceiverInputSampleNoiseVarianceDomain = repmat( ...
        "receiver_sample_waveform_post_composite_front_end", n, 1);
    candidate.ReceiverInputSampleNoiseVarianceSource = repmat( ...
        "unavailable_after_actual_RF_ADC_gain_compensation_receiver_reference_RE_estimation_required", n, 1);
    candidate.StrictReceiverEvidenceOk = true(n, 1);
    estimatedNoiseTrials.(char(direction)) = candidate;
end
estimatedNoiseRoot = fullfile(tmp, "reference_estimated_downstream_noise");
sixgr.analytics.writeRFInterferenceReconciliation(disabledCfg, estimatedNoiseRoot, ...
    estimatedNoiseTrials, mobilityArtifacts, struct());
estimatedNoise = readtable(fullfile(estimatedNoiseRoot, "reports", "csv", ...
    "noise_reconciliation.csv"), "VariableNamingRule", "preserve", "TextType", "string");
assert(all(localAsLogical(estimatedNoise.NoiseReconciliationOk)) && ...
    all(localAsLogical(estimatedNoise.DownstreamReferenceEstimatedNoisePath)) && ...
    all(string(estimatedNoise.ReceiverInputNoiseEvaluationStatus) == ...
    "not_scalar_after_actual_front_end_downstream_reference_estimates_strictly_valid"), ...
    "Honest post-front-end scalar N/A must qualify only through strict downstream receiver estimates.");

ambiguousNoiseTrials = estimatedNoiseTrials;
ambiguousNoiseTrials.DL.ReceiverInputSampleNoiseVarianceSource(1) = "unavailable";
ambiguousNoiseRoot = fullfile(tmp, "ambiguous_receiver_input_noise");
sixgr.analytics.writeRFInterferenceReconciliation(disabledCfg, ambiguousNoiseRoot, ...
    ambiguousNoiseTrials, mobilityArtifacts, struct());
ambiguousNoise = readtable(fullfile(ambiguousNoiseRoot, "reports", "csv", ...
    "noise_reconciliation.csv"), "VariableNamingRule", "preserve", "TextType", "string");
dlAmbiguous = upper(string(ambiguousNoise.Direction)) == "DL";
assert(~localAsLogical(ambiguousNoise.NoiseReconciliationOk(dlAmbiguous)) && ...
    localAsLogical(ambiguousNoise.NoiseReconciliationOk(~dlAmbiguous)), ...
    "A generic unavailable token must not qualify missing receiver-input variance.");
