        ...
        if item.Kind=="PBCH"
            ...
            for candidate=receivedPBCH.CandidateResults
                ...
                fprintf('ssb=%g pbch=%d sib1=%d rsrp=%g sinr=%g failure=%s\n', ...
                    a.SSBIndex,a.PBCH.Ok,a.Ok,a.SS_RSRP_dBm,a.MeasuredTrialSINR_dB,a.FailureReason);
            end
            ...
            receivedTRS=sixgr.link.completeTRSReception(trs,post,replay,ch);
...
assert(isfinite(receivedTRS.MeasuredTrialSINR_dB) && ...
    all(cellfun(@(x)isfinite(x.SS_RSRP_dBm),receivedPBCH.CandidateResults)));
try
