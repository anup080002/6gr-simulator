...
for item=events
    ...
    elseif item.Kind=="PUCCH"
        ...
        % Schema-only sentinel for an earlier untransmitted SR observation.
        % This recreates the integrated slot-34 append after NaN-valued SR
        % content evidence. It is not a physical SR episode or an export row.
        assert(isempty(state.ControlTrials.PUCCH));
        state.ControlTrials.PUCCH=table(NaN,"declared_SR_schema_only", ...
            'VariableNames',{'UCIContentMatch','UCIType'});
        state=sixgr.truth.CoupledTruthRuntime.completeSharedPUCCHFeedbackRuntime(state,item);
        assert(height(state.ControlTrials.PUCCH)==2 && ...
            isnan(state.ControlTrials.PUCCH.UCIContentMatch(1)) && ...
            state.ControlTrials.PUCCH.UCIContentMatch(2)==1, ...
            'Missing SR content must survive appending actual decoded HARQ evidence.');
        state.ControlTrials.PUCCH=state.ControlTrials.PUCCH(2,:);
