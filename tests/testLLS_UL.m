...
assertMeasuredPAPRTrialEvidence(puschLow.TrialTable);
assertMeasuredPAPRTrialEvidence(puschHigh.TrialTable);
assertMeasuredEVMTrialEvidence(puschLow.TrialTable);
assertMeasuredEVMTrialEvidence(puschHigh.TrialTable);
...
if ~(logical(puschLow.Skipped) || logical(puschHigh.Skipped))
    assertMeasuredEqualizerWork(puschLow.TrialTable);
    assertMeasuredEqualizerWork(puschHigh.TrialTable);
    ...
    for measured = {puschLow.TrialTable,puschHigh.TrialTable}
        T = measured{1};
        assert(all(ismember(["PilotReconstructionResidualRatio_dB", ...
            "PilotCommonPhaseRate_Hz","PilotPhaseFitResidualRMS_deg", ...
            "PilotPhasePairCount","PilotPhaseUnaliasedHalfWidth_Hz", ...
            "PilotPhaseEvolutionSource","PilotPhaseEvolutionStatus","NMSESource"], ...
            string(T.Properties.VariableNames))), ...
            "The throughput runtime must retain typed pilot and phase measurements.");
        observed = isfinite(T.PilotReconstructionResidualRatio_dB);
        assert(any(observed) && all(isnan(T.NMSE_dB(observed))) && ...
            all(contains(string(T.NMSESource(observed)),"independent_applied_channel_reference_not_bound")), ...
            "Noisy-pilot residuals must not be published as independent channel NMSE.");
        assert(all(strlength(string(T.PilotPhaseEvolutionStatus(observed)))>0), ...
            "Unavailable temporal phase observations require an explicit status.");
        timed = isfinite(T.PilotCommonPhaseRate_Hz);
        assert(all(T.PilotPhasePairCount(timed)>0) && ...
            all(isfinite(T.PilotPhaseUnaliasedHalfWidth_Hz(timed)) & ...
            T.PilotPhaseUnaliasedHalfWidth_Hz(timed)>0), ...
            "Export the observation count and alias interval with every measured phase rate.");
    end
...
assert(all(ismember(["ResidualInterLayerPowerMean","ResidualInterLayerPowerPerLayer", ...
    ...
    string(T.Properties.VariableNames))), ...
    '%s must retain receiver inter-layer evidence through the throughput wrapper.',label);
