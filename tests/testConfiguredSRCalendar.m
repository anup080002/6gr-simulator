...
% Preparation now selects PUCCH versus scheduled PUSCH independently. The
% PUCCH-only wrapper remains a completion guard, not the preparation entry.
assert(isscalar(caller) && contains(caller,'execution.GNBReception=sixgr.truth.buildScheduledHARQTransportReception(') && ...
    ~contains(caller,"isempty(sixgr.util.structGet(cfg,'validation.pucch_resources.sr_resource_ids'"));
selector=string(fileread(fullfile(root,'+sixgr','+truth','buildScheduledHARQTransportReception.m')));
assert(contains(selector,'sixgr.truth.buildConfiguredSRCalendar(cfg,ueData,targetSlot,targetSlot)') && ...
    contains(selector,'sixgr:truth:UnresolvedSRPUCCHReceiveHypothesis'), ...
    'Independent transport selection must retain the installed SR calendar and overlap guard.');
