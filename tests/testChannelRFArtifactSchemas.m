...
assert(height(T) > 0 && all(ismember(["TrialId","StrictOk","ExpectedOk", ...
    "ExpectedOutcomeMatch","TruthStatus"], string(T.Properties.VariableNames))) && ...
    all(logical(T.ExpectedOutcomeMatch)), ...
...
assert(height(strictCfg) == 1 && logical(strictCfg.ConfigValidationOk) && ...
    ...
    "Strict O2I validation must preserve the explicitly enabled YAML pathloss/O2I authority.");
