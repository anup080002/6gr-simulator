...
timingCfg = withCanonicalSchedulerTiming(sixgr.config.defaultConfig());
grant = struct("Direction", "DL", "ControlAbsoluteSlot", 0, ...
    "ControlSymbolAllocation", [0, 2], ...
    "SymbolAllocation", [0, 14], ...
    "HARQFeedbackSymbolAllocation", [0, 2], ...
    "TimingAdvanceTicks", int64(0), "HARQProcessID", 0);
[k1, decision] = sixgr.l2.mac.resolveHARQFeedbackK1( ...
    timingCfg, grant);
assert(k1 == 2 && decision.Valid, ...
    "HARQ K1 must come from the attached canonical timing policy.");
...
rateLow = sixgr.l2.mac.SchedulerPF.cqiToApproxThroughputBps( ...
    3, 10, 0.5e-3, 14);
rateHigh = sixgr.l2.mac.SchedulerPF.cqiToApproxThroughputBps( ...
    13, 10, 0.5e-3, 14);
localAssertError(@() ...
    sixgr.l2.mac.SchedulerPF.cqiToApproxThroughputBps(3, 10), ...
    "sixgr:SchedulerPF:MissingSlotDuration");
...
try
    ...
    assert(string(cause.identifier) == string(expectedIdentifier), ...
        ...
        expectedIdentifier, cause.identifier);
    return;
end
