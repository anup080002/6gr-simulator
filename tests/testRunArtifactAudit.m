...
assert(~any(ismember(string(csvAudit.relative_path), [ ...
        "reports/csv/all_csv_artifact_audit.csv"
        "reports/csv/all_image_artifact_audit.csv"
        "reports/csv/artifact_issue_registry.csv"])), ...
    "Recursive audit products must not audit their own previous bytes.");
csvAuditHash = sixgr.util.sha256File(fullfile(tmp, "reports", "csv", ...
    "all_csv_artifact_audit.csv"));
imageAuditHash = sixgr.util.sha256File(fullfile(tmp, "reports", "csv", ...
    "all_image_artifact_audit.csv"));
repeatAudit = sixgr.validation.auditRunArtifacts(tmp, "Strict", false, ...
    "WriteOutputs", true);
assert(logical(repeatAudit.Ok) && ...
    csvAuditHash == sixgr.util.sha256File(fullfile(tmp, "reports", "csv", ...
        "all_csv_artifact_audit.csv")) && ...
    imageAuditHash == sixgr.util.sha256File(fullfile(tmp, "reports", "csv", ...
        "all_image_artifact_audit.csv")), ...
    "An unchanged artifact tree must produce byte-identical recursive audits.");
...
scfg = scfg.toStruct();
scfg = sixgr.util.structSet(scfg, "mimo.reciprocity_mode", "tdd");
scfg.harq.k2 = 2;
...
% auditFixedSNRSweepRun consumes the canonical CSV-renderer lineage.  The
% recursive visual-audit portions of this test separately exercise the
% legacy helper's exact image/source map below.
lineageT = table( ...
