...
dl = table( ...
    ...
    repmat(12.0, n, 1), repmat(0.0, n, 1), double(crcPass) .* 2048, ...
...
snr = readtable(fullfile(layout.AirInterfaceCSVDir, "lls_snr_sweep.csv"), "VariableNamingRule", "preserve");
assert(numel(unique(snr.SNR_dB)) >= 5, "SNR sweep table did not preserve multiple SNR points.");
assert(any(string(snr.Properties.VariableNames) == "BLER_CI95_Low"), "SNR sweep table missing BLER CI low column.");
assert(any(string(snr.Properties.VariableNames) == "bler_ci_high"), "SNR sweep table missing lowercase CI alias.");
