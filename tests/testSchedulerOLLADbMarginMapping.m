...
% This payload is transported asynchronously from a receiver measurement to
% a later grant. A nonzero OLLA delta without finite threshold lineage is an
% invalid partial decision.
assert(isfinite(double(mappedMCS)) && ...
    isfinite(double(detail.BaseRequiredSINR_dB)) && ...
    isfinite(double(detail.TargetRequiredSINR_dB)) && ...
    strlength(string(detail.ThresholdSource)) > 0, ...
    "The asynchronous OLLA decision payload must contain finite threshold lineage.");

feedback = struct( ...
    "Valid", true, "CQI", 12, "RI", 1, "MCSIndex", double(mappedMCS), ...
    "RawCQIDerivedMCS", double(baseMCS), ...
    "MCSSelectionSource", "runtime_link_adaptation_decision", ...
    "MCSValueStatus", "measured_cqi_mapped_olla_db_margin_adjusted", ...
    "OuterLoopEnabled", true, "LinkAdaptationStateUpdateCount", 12, ...
    "OLLAUpdateCount", 6, ...
    "OLLADeltaDb", double(deltaDb), "DeltaMCS", double(deltaDb), ...
    "OLLAAdjustedMCSBeforeCQICeiling", double(mappedMCS), ...
    "OLLABaseRequiredSINR_dB", double(detail.BaseRequiredSINR_dB), ...
    "OLLATargetRequiredSINR_dB", double(detail.TargetRequiredSINR_dB), ...
    "OLLAThresholdSource", char(string(detail.ThresholdSource)));
grant = struct("RNTI", 9301, "NumLayers", 1, "Layers", 1, ...
    "MUMIMOEnabled", false, "MUMIMOGroupSize", 1);
transported = sixgr.truth.CoupledTruthRuntime.applyMeasuredFeedbackAMCToGrantRuntime( ...
    grant, feedback, [], cfg, "DL");
assert(double(transported.OLLAAdjustedMCSBeforeCQICeiling) == double(mappedMCS) && ...
    double(transported.OLLABaseRequiredSINR_dB) == double(detail.BaseRequiredSINR_dB) && ...
    double(transported.OLLATargetRequiredSINR_dB) == double(detail.TargetRequiredSINR_dB) && ...
    string(transported.OLLAThresholdSource) == string(detail.ThresholdSource), ...
    "Receiver-owned OLLA thresholds must survive the causal feedback-to-grant handoff.");
