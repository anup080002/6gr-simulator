...
missingPolarizationT = removevars(antennaT, ...
    {'BSAntennaPolarization','UEAntennaPolarization'});
sixgr.util.csvWriteTable(fullfile(tmp, "reports", "csv", ...
    "antenna_runtime_evidence.csv"), missingPolarizationT);
reportMissingPolarization = sixgr.analytics.buildPhase7ReadinessArtifacts(cfg, tmp);
polarizationRows = reportMissingPolarization.PhysicalChannelReconciliation.Polarization;
assert(height(polarizationRows) == 2 && ...
    ~any(logical(polarizationRows.PolarizationReconciliationOk)) && ...
    all(contains(string(polarizationRows.FailureReason), ...
    "runtime_polarization_column_missing")), ...
    ["Missing runtime polarization columns must produce explicit failing " ...
     "rows without configured-value substitution or an exception."]);

% Restore the complete same-flow antenna evidence for the remaining
% independent channel-realization negative case.
sixgr.util.csvWriteTable(fullfile(tmp, "reports", "csv", ...
    "antenna_runtime_evidence.csv"), antennaT);
