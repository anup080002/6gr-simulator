...
for geometry=[25 15;264 120].'
    ...
    for layers=[1 2 4]
        ...
        amplitudeScale=1e-10;
        scaled=sixgr.phy.ul.measureULLinkState(h*amplitudeScale,variance*amplitudeScale^2,cfg, ...
            'ReceivedGrid',received*amplitudeScale,'ReferenceIndices',indices, ...
            'ReferenceSymbols',symbols,'ChannelEstimateDomain', ...
            'pusch_dmrs_effective_layer_domain');
        assert(abs(scaled.PilotSINR_dB-expected)<1e-9, ...
            'Physical power units changed UL reference SINR.');
        assert(isequal(isfinite(scaled.PilotPerRBSINR_dB),isfinite(actual.PilotPerRBSINR_dB)) && ...
            max(abs(scaled.PilotPerRBSINR_dB-actual.PilotPerRBSINR_dB),[],'omitnan')<1e-9, ...
            'UL per-RB measurement retained an absolute numerical noise floor.');
        assert(abs(scaled.ChannelGain_dB-actual.ChannelGain_dB-20*log10(amplitudeScale))<1e-9, ...
            'Measured channel gain must retain physical attenuation below eps(1).');
    end
end
ok=true;
fprintf('UL_REFERENCE_PORT_UNION_PASS geometries_and_ranks=6 measurements=24 physical_scale_invariant=1 declared_channel_algebra=1 integrated=0\n');
end
