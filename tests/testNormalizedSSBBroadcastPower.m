...
assert(string(read.SSBWindowRelativePowerMeasurementJSON)==row.SSBWindowRelativePowerMeasurementJSON && ...
    string(read.SSBWindowReferenceRSRQPerReceiveAntenna_dB)==row.SSBWindowReferenceRSRQPerReceiveAntenna_dB);
window=jsondecode(read.SSBWindowRelativePowerMeasurementJSON);
assert(string(window.Scope)=="ssb_240_subcarrier_four_symbol_window_not_full_carrier_RSSI" && ...
    string(window.ReferenceEstimator)=="nrSSBMeasurements_not_primary_noise_debiased_SSS_RSRP");
assert(max(abs(10*log10(window.NumRB)+window.ReferenceRSRPPerAntenna_dB_re_UnitOccupiedRE_Es(:)- ...
    window.RSSIPerAntenna_dB_re_UnitOccupiedRE_Es(:)-window.ReferenceRSRQPerAntenna_dB(:)))<1e-4);
