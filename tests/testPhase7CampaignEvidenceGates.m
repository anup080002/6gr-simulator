...
executionOnlyLayout = sixgr.report.resultLayout(tmpExecutionOnly);
...
sixgr.util.csvWriteTable(fullfile(executionOnlyLayout.ReportCSVDir, ...
    "checkpoint_resume_equivalence.csv"), checkpoint);
sixgr.util.csvWriteTable(fullfile(executionOnlyLayout.ReportCSVDir, ...
    "serial_parallel_determinism.csv"), determinism);
sixgr.analytics.buildPhase7ReadinessArtifacts(cfg, tmpExecutionOnly);
executionOnlySummary = readtable(fullfile(tmpExecutionOnly, "reports", "final", ...
    "final_campaign_summary.csv"), "VariableNamingRule", "preserve", "TextType", "string");
executionOnlyGates = readtable(fullfile(tmpExecutionOnly, "reports", "csv", ...
    "phase7_truth_gates.csv"), "VariableNamingRule", "preserve", "TextType", "string");
assert(localAsLogical(executionOnlySummary.CheckpointResumeEquivalenceOk(1)) && ...
    localAsLogical(executionOnlySummary.SerialParallelDeterminismOk(1)), ...
    "Independent execution proofs must survive when the statistical campaign table is absent.");
assert(localAsLogical(executionOnlyGates.CheckpointResumeEquivalenceOk(1)) && ...
    localAsLogical(executionOnlyGates.SerialParallelDeterminismOk(1)), ...
    "Phase-7 truth gates must retain valid checkpoint and determinism execution evidence.");
assert(~localAsLogical(executionOnlyGates.CampaignCompletionOk(1)) && ...
    ~localAsLogical(executionOnlyGates.SampleAdequacyOk(1)) && ...
    strcmp(string(executionOnlySummary.Status(1)), "no_phase7_campaign_runs_provided"), ...
    "Independent execution proofs must not make unexecuted statistical campaign gates pass.");

tmpFixedApplicability = tempname;
mkdir(tmpFixedApplicability);
cleanupFixedApplicability = onCleanup(@() rmdir(tmpFixedApplicability, "s")); %#ok<NASGU>
fixedCfg = cfg;
fixedCfg.validation.run_class = "fixed_snr_sweep_lls";
fixedCfg.channel.model = "AWGN";
sixgr.analytics.buildPhase7ReadinessArtifacts(fixedCfg, tmpFixedApplicability);
fixedGates = readtable(fullfile(tmpFixedApplicability, "reports", "csv", ...
    "phase7_truth_gates.csv"), "VariableNamingRule", "preserve", "TextType", "string");
assert(contains(string(fixedGates.NotApplicableGateNames(1)), ...
    "GeometryValidationOk"), ...
    ["A fixed-SNR-only run must explicitly classify geometry validation " ...
     "as not applicable."]);
assert(~contains(string(fixedGates.FailureCodes(1)), ...
    "GeometryValidationOk_false"), ...
    ["A fixed-SNR-only run must not report an inapplicable geometry " ...
     "validation failure."]);
assert(~localAsLogical(fixedGates.GeometryValidationOk(1)), ...
    "Not-applicable geometry evidence must remain false, never become a synthetic pass.");
