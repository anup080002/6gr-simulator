...
assert(double(summary.ExecutedTransportBlocks(1)) > 0 && ...
    double(summary.NDLTrialsTotal(1)) > 0 && ...
    double(summary.BlockErrorCount(1)) == 2 * sum([15 10 5 2 0]), ...
    "Campaign summary must derive executed trial totals from fixed-link rows.");
assert(~ismember("FinalRuns", string(summary.Properties.VariableNames)) && ...
    ~ismember("FailedRuns", string(summary.Properties.VariableNames)), ...
    ["Transport-block samples and block errors must not be mislabeled " ...
     "as successful or failed campaign runs."]);
...
assert(height(curve) == 5 && all(double(curve.TrialCount) >= 30), ...
    "Derived DL BLER curve must keep all configured SNR bins and trial counts.");
assert(all(double(curve.BLER_CI_Width) <= 0.40), ...
...
targetQualifiedCfg = cfg;
targetQualifiedCfg.validation.fixed_link_campaign.target_bler = [0.1 0.01];
targetQualifiedCfg.validation.fixed_link_campaign.max_target_crossing_bracket_db = 2;
targetQualified = sixgr.analytics.buildPhase7ReadinessArtifacts( ...
    targetQualifiedCfg, tmp);
assert(~logical(targetQualified.Gates.SweepDataQualityOk), ...
    ["Numerical sample/CI adequacy must not pass SweepDataQualityOk when " ...
     "the YAML-requested BLER crossings have no qualified fixed-sweep audit."]);
...
tmpAdaptiveApplicability = tempname;
mkdir(tmpAdaptiveApplicability);
cleanupAdaptiveApplicability = onCleanup(@() rmdir(tmpAdaptiveApplicability, "s")); %#ok<NASGU>
adaptiveCfg = cfg;
adaptiveCfg.validation.run_class = "adaptive_system_diagnostic";
sixgr.analytics.buildPhase7ReadinessArtifacts(adaptiveCfg, tmpAdaptiveApplicability);
adaptiveGates = readtable(fullfile(tmpAdaptiveApplicability, "reports", "csv", ...
    "phase7_truth_gates.csv"), "VariableNamingRule", "preserve", "TextType", "string");
adaptiveNA = string(adaptiveGates.NotApplicableGateNames(1));
for name = ["CheckpointResumeEquivalenceOk","SeedHierarchyOk","CampaignDesignOk", ...
        "CampaignCompletionOk","MultiSeedDropStatisticsOk","ConfidenceIntervalsOk", ...
        "SampleAdequacyOk","SweepDataQualityOk","SerialParallelDeterminismOk", ...
        "LongRunStabilityOk"]
    assert(contains(adaptiveNA, name) && ...
        ~contains(string(adaptiveGates.FailureCodes(1)), name + "_false"), ...
        "Adaptive single-run diagnostics must explicitly exclude %s from functional Phase-7 reduction.", name);
end
assert(~localAsLogical(adaptiveGates.CampaignCompletionOk(1)) && ...
    ~localAsLogical(adaptiveGates.LongRunStabilityOk(1)), ...
    "Not-applicable campaign evidence must remain false, never become a synthetic pass.");
...
cfg.canonical_control.run.min_trials_per_sinr_bin = 30;
cfg.canonical_control.run.max_ci_width = 0.40;
...
snr = (-2:2:6).';
n = numel(snr);
trialCount = repmat(30, n, 1);
failures = [15; 10; 5; 2; 0];
fixed = table(snr, repmat("fixed_link_monte_carlo", n, 1), ...
    ...
    zeros(n, 1), repmat(0.20, n, 1), repmat(0.20, n, 1), ...
    repmat(30, n, 1), false(n, 1), trialCount, failures, failures ./ trialCount, ...
    repmat(0.20, n, 1), repmat(30, n, 1), false(n, 1), ...
...
T.FixedLinkSeedIndex = double(taskPlan.DropIndex);
T.FixedLinkSeedValue = 104728 + double(taskPlan.DropIndex);
end
