...
for k=1:4
    ...
    for raw=0:3
        ...
        producers=table(e.UEId,e.RNTI,e.DataAbsoluteSlot+1,e.HARQProcess,e.TargetSlot, ...
            'VariableNames',{'UEIndex','RNTI','SourceSlot','HarqID','ScheduledAbsoluteSlot'});
        binding=sixgr.truth.bindReceivedPUSCHCodebookRows(book,producers);
        assert(binding.ProducerCount==1 && binding.BitCount==n && ...
            binding.ProducerBitIndices==e.DAI && binding.EventRowIndices==1 && ...
            isequal(binding.ProtocolGapBitIndices,find(positions==0)));
        poison=producers; poison.Ack=~(e.State=="ACK");
        assert(isequaln(binding,sixgr.truth.bindReceivedPUSCHCodebookRows(book,poison)));
        ...
        emptyBinding=sixgr.truth.bindReceivedPUSCHCodebookRows(none,table());
        assert(emptyBinding.ProducerCount==0 && emptyBinding.BitCount==emptyCount && ...
            isempty(emptyBinding.ProducerBitIndices) && ...
            isequal(emptyBinding.ProtocolGapBitIndices,(1:emptyCount).'));
        localReject(@()sixgr.truth.bindReceivedPUSCHCodebookRows(none,producers), ...
            'sixgr:truth:ReceivedPUSCHProducerMismatch');
        localReject(@()sixgr.truth.bindReceivedPUSCHCodebookRows(book,[producers;producers]), ...
