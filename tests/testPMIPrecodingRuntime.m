...
cfg = sixgr.util.structSet(cfg, "phy.beamManagement.dlCodebookSize", 9);

[twoPortCandidates, twoPortInfo] = sixgr.phy.dl.pmiCodebookCandidates( ...
    sixgr.util.structSet(cfg, "phy.beamManagement.dlCodebookSize", 2), ...
    2, 2, "Mode", "type1_su_mimo");
assert(double(twoPortInfo.NumBeams) == 2 && ~isempty(twoPortCandidates), ...
    "The minimum two-beam, two-port Type-I catalog must support rank 2.");
assert(all(arrayfun(@(candidate) ...
    norm(candidate.W' * candidate.W - eye(2), "fro") <= 1e-10, ...
    twoPortCandidates)), ...
    "Two-port rank-2 Type-I candidates must be semi-unitary.");

[oddBeamCandidates, oddBeamInfo] = sixgr.phy.dl.pmiCodebookCandidates( ...
    cfg, 2, 4, "Mode", "type1_su_mimo");
assert(double(oddBeamInfo.NumBeams) == 9 && ~isempty(oddBeamCandidates), ...
    "A nine-beam four-port Type-I catalog must materialize rank-2 candidates.");
assert(all(arrayfun(@(candidate) ...
    isequal(size(candidate.W), [4 2]) && ...
    norm(candidate.W' * candidate.W - eye(2), "fro") <= 1e-10, ...
    oddBeamCandidates)), ...
    "Every four-port rank-2 Type-I candidate must be semi-unitary.");
...
cfgUL.phy.pusch.dmrs.portSet = [0 1];
cfgUL.phy.pusch.dmrs.DMRSPortSet = [0 1];
