...
actual = sixgr.phy.ul.validatePUSCHPrecoderEvidence(tx);
assert(actual.NumLogicalPorts == 2 && actual.NumPorts == 4 && ...
        isempty(actual.BeamIndices), ...
    "Actual hybrid precoder evidence must distinguish logical ports, elements, and spatial beams.");
...
actual = sixgr.phy.ul.validatePUSCHPrecoderEvidence(tx);
assert(actual.NumLogicalPorts == 4 && actual.NumPorts == 4 && isnan(actual.PMI), ...
    "Actual non-codebook waveform evidence must not invent a TPMI.");
...
tx = sixgr.phy.ul.PUSCH_Tx(cfg, "PUSCH", pusch, "CompactOutput", false);
actual = sixgr.phy.ul.validatePUSCHPrecoderEvidence(tx);
assert(actual.PMI == pusch.TPMI && actual.TransformPrecodingApplied && ...
        isempty(actual.BeamIndices), ...
    "Actual transform-precoded waveform must expose TPMI without fabricated spatial beams.");
