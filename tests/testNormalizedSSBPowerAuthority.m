...
raw=struct('SS_RSRP_dBm',-5,'SS_RSRPPerReceiveAntenna_dBm',"-5;-6", ...
    ...
    'SS_SINR_dB',12,'BCHCrcPass',true);
% Declared power-domain unit fixture, not a received RF episode. Retain a
% complete window schema now that normalization preserves its evidence.
branchRSRP=[-5 -6]; symbolPower=240*10.^((branchRSRP-30)/10);
window=struct('Source',"declared_window_metadata_unit_fixture", ...
    'Scope',"ssb_240_subcarrier_four_symbol_window_not_full_carrier_RSSI", ...
    'AmplitudeUnit',"sqrt_W",'CPIncluded',false,'NumReceiveAntennas',2, ...
    'NumRB',20,'NumSubcarriers',240,'SymbolIndicesWithinSSB0Based',[0 1 2 3], ...
    'SymbolPowerPerAntenna_W',repmat(symbolPower,4,1), ...
    'RSSIPerAntenna_dBm',10*log10(symbolPower)+30, ...
    'ReferenceRSRPPerAntenna_dBm',branchRSRP, ...
    'ReferenceRSRQPerAntenna_dB',repmat(10*log10(20/240),1,2));
raw.SSBWindowPowerMeasurementJSON=string(jsonencode(window));
...
malformed=raw; malformed.SSBWindowPowerMeasurementJSON='{"dBm":-5}';
localReject(@()sixgr.link.normalizeReceivedSSBPowerReference(malformed,cfg), ...
