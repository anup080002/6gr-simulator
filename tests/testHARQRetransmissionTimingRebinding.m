...
for originalSlot = 0:38
    ...
    for replayDelta = 1:10
        ...
        stale.HARQACKPUSCHTimingConstraints=struct('PreviousAttemptOnly',true);
        rebound = sixgr.l2.mac.rebindHARQRetransmissionTiming(stale);
        assert(~isfield(rebound,'HARQACKPUSCHTimingConstraints'), ...
            'A new retransmission must not inherit the old PDSCH/UL overlap endpoints.');
