...
expired=cfg; expired.CurrentSlot=measurement.Slot+measurement.MaxAgeSlots+1;
try
    sixgr.phy.mimo.NRCSIReportEngine.run(carrier,csirs,dmrs,H,nVar,expired,measurement);
    error('test:ExpectedFailure','Expired Type-I channel measurement was accepted.');
catch ex
    assert(string(ex.identifier)=="sixgr:mimo:StaleMeasurementState",ex.message);
end
...
assert(abs(actual.CQIEffectiveSINR_dB-double(expectedCQIInfo.EffectiveSINR(1)))<1e-12, ...
    'Preserve the CQI mapping statistic separately, in its original dB units.');
expectedReceiverSINR=10*log10(expm1(mean(log1p(expectedPMIInfo.SINRPerREPMI),'all')));
assert(abs(actual.WidebandSINR_dB-expectedReceiverSINR)<1e-12, ...
    'Receiver SINR must come from selected-PMI layer measurements, not the CQI mapping.');
