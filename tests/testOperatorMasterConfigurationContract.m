...
assert(logical(cfg.phy.pusch.measurements.dmrsResidualPostEqSINRBoundEnabled) == ...
    logical(raw.pusch.dmrs_residual_post_eq_sinr_bound_enabled), ...
    "%s did not preserve the configured PUSCH DM-RS residual policy.", fileName);
assert(string(cfg.phy.pusch.measurements.decoderNoiseVarianceMode) == ...
    string(raw.pusch.decoder_noise_variance_mode), ...
    "%s did not preserve the configured PUSCH decoder-noise policy.", fileName);
...
localAssertEqual(double(cfg.phy.schedulingTiming.pdcchToPDSCHK0), ...
    double(timing.pdcch_to_pdsch_k0), fileName + " K0");
localAssertEqual(double(cfg.phy.schedulingTiming.dlHARQFeedbackK1Candidates), ...
    double(timing.dl_harq_feedback_k1), fileName + " K1");
localAssertEqual(double(cfg.phy.schedulingTiming.ulGrantK2), ...
    double(timing.ul_grant_k2), fileName + " K2");
assert(~isfield(cfg.phy, "tddTiming"), ...
    "%s exposed the removed duplicate tddTiming runtime authority.", fileName);
...
assert(string(fixed.Cfg.phy.pdsch.executionProfile) == "phy_calibration" && ...
    string(geometry.Cfg.phy.pdsch.executionProfile) == "scheduler_truth", ...
