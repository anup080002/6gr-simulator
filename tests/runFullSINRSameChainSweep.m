function out = runFullSINRSameChainSweep(outputRoot, runTag, configuredSNR, existingRunFolder)
...
function localVerify(out, configuredSNR)
...
dl = raw.DL;
ul = raw.UL;
assert(all(double(dl.Layers) == 2) && all(double(ul.Layers) == 2), ...
    "Every executed PDSCH/PUSCH allocation must use the YAML rank/layers=2 anchor.");
assert(all(double(dl.TxWaveformColumns) == 64) && ...
    all(double(dl.PhysicalTxAntennas) == 64) && ...
    all(double(dl.RxWaveformBranches) == 4), ...
    "PDSCH must propagate 64 gNB elements to four UE branches.");
assert(all(double(ul.TxWaveformColumns) == 4) && ...
    all(double(ul.PhysicalTxAntennas) == 4) && ...
    all(double(ul.RxWaveformBranches) == 64), ...
    "PUSCH must propagate four UE elements to 64 gNB branches.");
...
assert(height(muDL) >= 2 && height(muUL) >= 2);
assert(all(string(muDL.InterferenceMode) == "shared_slot_waveform_superposition") && ...
    all(string(muUL.InterferenceMode) == "shared_slot_waveform_superposition") && ...
    all(double(muDL.InterferenceContributorCount) >= 1) && ...
    all(double(muUL.InterferenceContributorCount) >= 1), ...
    "MU-MIMO must be measured from the shared desired-plus-peer waveform, not a scalar proxy.");
...
% A 40-slot adaptive point must consume real feedback instead of leaving
% every allocation at the YAML bootstrap.  This is a causal-chain check,
% not a demand that MCS equal a configured maximum.
...
fprintf(["FULL_SAME_CHAIN_PASS dl=%d ul=%d pbch=%d prach=%d pdcch=%d " ...
    "pucch=%d srs=%d csirs=%d trs=%d snr=[%s]\n"], ...
