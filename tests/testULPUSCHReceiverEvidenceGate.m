...
required = ["StrictReceiverEvidenceOk","StrictOk","TruthStatus", ...
    ...
    "SecondHopStartPRB", ...
    "ConfiguredMCSIndex","ConfiguredModulation","ConfiguredRank", ...
...
assert(all(double(T.EffectiveMCSIndex) == double(T.MCS)) && ...
    all(string(T.EffectiveModulation) == string(T.Modulation)) && ...
    all(double(T.EffectiveLayers) == double(T.Layers)) && ...
    all(double(T.EffectiveRank) == double(T.Layers)), ...
    "UL effective operating-point columns must expose the exact transmitted waveform point.");
scheduledRows = logical(T.AdaptiveMode) & logical(T.LinkAdaptationScheduled);
assert(all(double(T.ScheduledMCSIndex(scheduledRows)) == double(T.MCS(scheduledRows))) && ...
    all(string(T.ScheduledModulation(scheduledRows)) == string(T.Modulation(scheduledRows))) && ...
    all(logical(T.ScheduledOperatingPointMatchesTransmitted(scheduledRows))), ...
    "UL scheduled and transmitted operating-point evidence must reconcile for scheduled rows.");
assert(all(contains(string(T.ScheduledOperatingPointEvidenceStatus(~scheduledRows)), ...
        "not_applicable_no_adaptive_scheduled_decision")) && ...
    ~any(logical(T.ScheduledOperatingPointMatchesTransmitted(~scheduledRows))), ...
    "Standalone fixed-mode rows must not fabricate scheduler-grant operating-point evidence.");
