...
required = ["StrictReceiverEvidenceOk","StrictOk","TruthStatus", ...
    ...
    "ReceiverHestSINRApplicable","FrequencyHoppingApplied", ...
...
assert(all(isfinite(double(T.MeasuredTrialSINR_dB))) && ...
    all(abs(double(T.MeasuredTrialSINR_dB) - double(T.PostEqSINR_dB)) < 1e-9), ...
    "Canonical UL trial SINR must equal the receiver-derived PUSCH post-equalization SINR.");
assert(all(string(T.MeasuredTrialSINRSource) == string(T.PostEqSINRSource)) && ...
    all(string(T.MeasuredTrialSINRValueRole) == "measured_post_equalization_scheduling_input"), ...
    "UL trial reduction must not replace or clip post-equalization SINR with DM-RS/SRS diagnostics.");
assert(~any(contains(lower(string(T.MeasuredTrialSINRSource)), ["pilot","reference","hest","limited"])), ...
    "Reference-signal/Hest diagnostics must remain separate from canonical UL scheduling SINR.");
...
assert(~any(logical(T.FrequencyHoppingApplied)) && ...
    all(string(T.FrequencyHoppingMode) == "none") && ...
    all(string(T.FrequencyHoppingToolboxMode) == "neither") && ...
    all(isnan(double(T.SecondHopStartPRB))), ...
    "UL trial rows must expose the transmitter-applied disabled hopping state.");
