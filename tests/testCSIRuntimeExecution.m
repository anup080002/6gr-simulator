...
expectedPhysicalRSRP = double(dl.CSIRSTrialTable.MeasurementRelativeRSRP_dB) - ...
    20 .* log10(double(dl.CSIRSTrialTable.MeasurementGridScaleToSqrtW));
...
trialForCommit = dl.TrialTable(end, :);
state.CurrentSlot = double(trialForCommit.Slot(end));
state.CurrentCanonicalSlot = state.CurrentSlot;
state.CurrentFrame = double(trialForCommit.Frame(end));
state.CurrentFrameLocal = state.CurrentFrame;
[state, ~] = sixgr.truth.CoupledTruthRuntime.completeSlot( ...
    state, cfgRun, 1, "DL", trialForCommit, ...
    struct("CSIRSTrialTable", csiForCommit, "HARQ", struct()));
