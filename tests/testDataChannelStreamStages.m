...
for caseIndex = reshape(caseIndices,1,[])
    ...
    if caseIndex==4
        ...
        observed=completed.HARQ.IndependentHARQObservation;
        assert(observed.ReceiverContextDigest==context.UCIReceiveContext.Digest && ...
            observed.DecodeOk && ~observed.DTXFlag && observed.DecodeAttempted && ...
            observed.InformationBitCount==2 && isequal(observed.DecodedBits,int8([1;0])));
