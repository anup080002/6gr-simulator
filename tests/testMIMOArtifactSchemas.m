...
assert(all(ismember(["ConfiguredRank","ConfiguredMaximumRank", ...
    "ConfiguredMaximumLayers","TransmittedRank","EffectiveDecodedRank", ...
    "MeasuredDMRSPortCount","SourceRowsHash","StrictOk"], ...
    string(rankT.Properties.VariableNames))) && ...
    all(double(rankT.MeasuredDMRSPortCount)==2), ...
...
% Explicit receiver evidence for this algebraic rank-2 fixture. Port
% identities remain unavailable because the fixture measures only count.
T.MeasuredDMRSPortCount = repmat(2, height(T), 1);
end
