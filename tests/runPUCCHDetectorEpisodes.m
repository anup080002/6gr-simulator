function result=runPUCCHDetectorEpisodes(configPath,outputRoot)
% Actual shared-clock TDD detector episodes; never a qualification decision.
...
for k=1:numel(cases)
    ...
    summary{k}=struct('CaseID',string(c.id),'PlannedEpisodes',v.episodes,'ExecutedEpisodes',executed, ...
        ...
        'DetectorQualified',false,'Scope',string(v.stage)+"_not_detector_qualification");
end
