...
fprintf(fid, "%s", ['{' ...
    ...
    '"simulation":{"link_direction":"both","n_frames":1,"n_slots":5,"monte_carlo_iterations":1,"random_seed":23,"snr_db":50,"noise_operating_mode":"standalone_awgn_snr_argument"},' ...
    '"scenario":{"study_mode":"smoke","target_cases":["dl","ul","pdsch","pusch","pdcch","pucch","srs","dmrs"]},' ...
    '"run_control":{"total_slots":5,"warmup_slots":0,"measurement_slots":5,"total_time_ms":5.0,"warmup_time_ms":0,"measurement_time_ms":5.0,"batch_size_links":1,"num_workers":1},' ...
    ...
    '"rf_frontend":{"ul_power_control":{"enabled":false,"open_loop_enabled":false,"closed_loop_enabled":false,"require_measured_reference_rs":false}},' ...
    '"initial_access":{"sib1":{"enabled":false},"ssb":{"waveform_domain":"logical_rf_chain_post_analog_precoder"}},' ...
    '"mimo":{"n_tx_ant":2,"n_rx_ant":2,"n_layers":1,"max_dl_layers":1,"max_ul_layers":1,"mu_mimo_enable":false,"ul_mu_mimo_enable":false,"beam_sweep_enabled":false,"beam_management_enable":false,"phase07_strict":{"max_rank":1,"rank_domain":[1]}},' ...
    '"mimo_and_beam_management":{"beam_sweeping":false,"beam_refinement":false,"beam_switching":false,"beam_tracking":false,"beam_prediction":false},' ...
    '"system":{"beam":{"enable":false}},' ...
    '"pdsch":{"execution_profile":"scheduler_truth","layer_count":1,"rank":1,"num_antenna_ports":1},' ...
    '"pusch":{"execution_profile":"scheduler_truth","layer_count":1,"num_layers":1,"num_antenna_ports":1},' ...
    ...
    '"reference_signals":{"srs_periodicity_slots":1,"srs_slot_within_period":1,"srs_max_ues_per_slot":2,"srs_scheduling_policy":"multiplex_due_users","srs":{"period_offset":0},"trs_enabled":false,"tracking_rs_enabled":false},' ...
    '"scheduling_timing":{"dl_harq_feedback_k1":[2]},' ...
    '"harq":{"feedback_timing_slots":2},' ...
    '"pucch_resources":{"power_control":{"require_measured_reference_rs":false},"overlap_policy":{"uci_on_pusch_enabled":false}},' ...
    ...
    '"live_publish_frame_interval":1,"live_heavy_refresh_interval_frames":5,"profiler_enabled":false}}']);
fclose(fid);

out = run_6g_phy_lls_single(scenarioPath, tmp, "smoke");
if ~out.Ok
    kpiDiagnostic = sixgr.util.structGet(out, "Result.Link.KPITable", table());
    if istable(kpiDiagnostic) && ~isempty(kpiDiagnostic)
        disp(kpiDiagnostic);
    end
    disp(out.ScenarioStatus);
    diagnosticFile = fullfile(char(string(out.RunFolder)), ...
        "air_interface", "csv", "pdcch_trials.csv");
    if exist(diagnosticFile, "file") == 2
        diagnostic = readtable(diagnosticFile, "VariableNamingRule", "preserve");
        wanted = intersect(["Slot","UEIndex","Direction","Status","CRCPass", ...
            "FailureReason","ReceiverHestSINR_dB","NoiseVar","NoiseVarSource", ...
            "TimingOffset_samples","AppliedTimingCorrection_samples", ...
            "ObservationStartSample","ObservationEndSampleExclusive"], ...
            string(diagnostic.Properties.VariableNames), "stable");
        disp(diagnostic(:, wanted));
    end
    for relativePath = [ ...
            fullfile("beamforming","csv","mimo_configured_vs_effective.csv"), ...
            fullfile("beamforming","csv","mimo_strict_gate_summary.csv"), ...
            fullfile("beamforming","csv","rank_layer_trials.csv")]
        diagnosticFile = fullfile(char(string(out.RunFolder)), relativePath);
        if exist(diagnosticFile, "file") == 2
            disp(readtable(diagnosticFile, "VariableNamingRule", "preserve"));
        end
    end
end
