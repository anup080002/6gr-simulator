function ok=testPDSCHChannelReferenceOperators()
...
projection=diag(exp(1i*[.2 -.7 .4 -.1]));
B=exp(-2i*pi*(0:1)'*(0:1)/2)/sqrt(2);
cases=0;
for layers=[1 2]
  ...
  for fraction=[0 .5 1]
    for ports=1:layers
      ...
      input=[complex(zeros(origin,4));physical];
      ...
      initial=sixgr.channel.ChannelFactory.materializeRuntimeChannelState(initial,cfg,input, ...
          struct('OFDM',info),'NumTxAnt',4,'NumRxAnt',2);
      ...
      snapshot=struct('Contract',"PDSCH_pilot_channel_capture/v1", ...
          ...
          'PilotIndices',sub2ind([K L layers],bin,symbol,ports));
      tx=struct('Carrier',carrier,'Waveform',logical,'PrecodeInfo',struct('MatrixPorts',W), ...
          'PowerContext',struct('AmplitudeScale',amplitude));
      prepared=struct('Direction',"DL",'Tx',tx,'SampleRateHz',fs,'StartSample',origin,'ReceiveStartSample',0);
      captures={struct('Reference',ra,'LinkID',"basis",'TX',"gNB",'RX',"UE"), ...
          struct('Reference',rb,'LinkID',"basis",'TX',"gNB",'RX',"UE")};
      ...
      % A direction reversal after symbol 10 leaves later OFDM symbols
      % without this DL operator. Score the same retained symbol-9 pilot;
      % do not demand a fabricated DL channel during the UL tail.
      slotFirst=mod(double(carrier.NSlot),double(info.SlotsPerSubframe))*L;
      lengths=double(info.SymbolLengths(slotFirst+(1:L)));
      stop=origin+sum(lengths(1:10));
      partial=captures;
      partial{2}.Reference=truncateReference(rb,stop);
      [short,shortEvidence]=sixgr.truth.sharedDataChannelReference( ...
          snapshot,prepared,partial,replays,projection);
      assert(isequaln(reference,short) && ...
          all(isnan(short(:,symbol+1:end,:,:)),'all') && ...
          isequal(shortEvidence.ScoredPilotSymbolIndices0,symbol-1), ...
          'Inactive tail symbols cannot change scored pilot coefficients.');
      % Cut one sample inside the ACTUAL scored FFT window: still reject.
      cp=lengths(symbol)-info.Nfft;
      stopInside=origin+sum(lengths(1:symbol-1))+floor(cp*fraction)+info.Nfft-1;
      incomplete=partial;
      incomplete{2}.Reference=truncateReference(rb,stopInside);
      caught=false;
      try, sixgr.truth.sharedDataChannelReference(snapshot,prepared,incomplete,replays,projection);
      catch ex, caught=strcmp(ex.identifier,'sixgr:truth:IncompleteFFTChannelReference'); end
      assert(caught,'A missing scored FFT sample must never be replaced.');
      bad=captures; bad{2}.Reference.StartSample=bad{2}.Reference.StartSample+1;
      caught=false;
      try, sixgr.truth.sharedDataChannelReference(snapshot,prepared,bad,replays,projection);
...
fprintf('PDSCH_CHANNEL_REFERENCE_OPERATORS_PASS cases=%d actual_waveforms=1 fitted_gain_or_phase=0\n',cases);
ok=true;
end

function ref=truncateReference(ref,stop)
count=stop-ref.StartSample;
assert(count>0 && stop<=ref.EndSampleExclusive);
ref.EndSampleExclusive=stop;
ref.PathGains=ref.PathGains(1:count,:,:,:);
ref.SampleTimes_s=ref.SampleTimes_s(1:count);
end
