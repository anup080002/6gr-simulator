...
for scs=[15 30 60 120]
    ...
    cfg.phy.trs.slotNumbers=[2 7];
    p=sixgr.link.prepareTRSTransmission(cfg,12);
    tx=p.Tx;
    T=tx.SlotTable;
    assert(isequal(T.Slot,[2;7]));
    ...
    assert(size(tx.PortGrid,2)==6*tx.GridSlots(1).Carrier.SymbolsPerSlot);
    ...
    if scs==15
        assert(abs(diff(T.StartSample1Based)/tx.SampleRateHz-0.005)<1e-14);
        ...
        assert(abs(freq.Table.DeltaT_s(1)-0.005)<1e-14);
    end
end
