...
assert(double(txp.HARQ.HARQRound) == 0, ...
    "A new HARQ transport block must be exported as round zero.");
grant = localHARQGrant(txp.HARQ);
...
assert(double(retx.HARQ.HARQRound) == 1, ...
    "The first HARQ retransmission must be exported as round one.");
grant = localHARQGrant(retx.HARQ);
...
function grant = localHARQGrant(harqState)
tbsBits = 800;
targetCodeRate = 0.5;
rateMatchedBitCount = 1648;
layout = sixgr.phy.phycode.resolveCodingLayout( ...
    "Direction", "DL", ...
    "TransportBlockSize", tbsBits, ...
    "TargetCodeRate", targetCodeRate, ...
    "RV", double(harqState.RV), ...
    "Modulation", "QPSK", ...
    "NumLayers", 1, ...
    "RateMatchedBitCount", rateMatchedBitCount);
grant = struct( ...
    "Direction", "DL", ...
    "CellId", 1, ...
    "ServingCell", 1, ...
    ...
    "Modulation", "QPSK", ...
    "NumLayers", 1, ...
    "CodedBitCountG", rateMatchedBitCount, ...
    "CodingLayout", layout, ...
    "HARQ", harqState);
end
