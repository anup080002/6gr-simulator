...
assert(logical(sixgr.util.structGet(cfg, "run.controlGating.pucchRequired", false)), ...
    "The master scenario must resolve PUCCH as a required control-gating block.");
...
assert(logical(policy.EnablePUCCH), ...
    "Bundle scenarios with required PUCCH must enable sixgr.phy.pucch.runStrictPUCCHValidation.");
