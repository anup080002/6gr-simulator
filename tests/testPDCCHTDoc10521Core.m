...
c=sixgr.phy.pdcch.tdoc.loadPDCCHTDocStudyCampaignConfig( ...
    "simulator/configs/pdcch_tdoc10521/master.yaml");
assert(c.Config.strict_mode&&c.Config.output.png_only);
a=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyAnalyticalSuite.run(c.Config);
assert(all(a.Acceptance.Pass));
assert(height(sixgr.phy.pdcch.tdoc.PDCCHTDocStudyScenarioRegistry.catalog())==39);
assert(height(sixgr.phy.pdcch.tdoc.PDCCHTDocStudyScenarioRegistry.figures())==15);
