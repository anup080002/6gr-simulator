...
## Repository checkpoint: consolidated candidate, explicitly unqualified

### 16:47 IST: IDE consolidation and two bounded repairs pass

The IDE branch `work/tdd-normalized-ssb-20260914` was fast-forwarded from `e6341805` to the existing `08f9ec60` checkpoint. The newer audit was preserved at `logs/tdd_12db_root_cause_audit_pre_ff_20260916.md` and restored after the fast-forward; a no-index comparison verified identical text. The pre-existing `logs/README.md` deletion and patch reconciliation inventory were preserved. The 17 modified and six new candidate source/test/config files were then imported from `sixgr_tdd_sr_lifecycle_20260916` using patches. All 23 normalized file texts matched at `2026-09-16T11:07:31.1646757Z`, before new repairs. This is a content transfer, not byte-identical source qualification or a claim that every historical worktree/archived proposal is reconciled. No live MATLAB checkout's source or HEAD was changed.

Two further bounded repairs are now in the IDE source:

- `BSRStateMachine` gives each successful preparation its own exact-integer local serial before hashing. Identical payloads no longer overwrite preparation ownership. The extended lifecycle test verifies discard isolation, reverse completion order, preservation of a later trigger, immutable serials and duplicate rejection. A local serial is not physical execution or configuration-epoch authority; normal MAC-PDU/TX binding is still required.
- Both maintained master YAMLs now explicitly declare the existing PUSCH CSI-presence algorithm and Format-2/3/4 policies. Fixed-sweep feedback remains disabled, with inactive multiplex permissions false; the enabled geometry master explicitly permits combined feedback. Coding rate 0.35 is an authored profile choice, not a universal 3GPP constant. `testOperatorMasterConfigurationContract` additionally checks config propagation and enabled receiver RRC policy. This does not change the requested 12 dB scenario to geometry or resume FDD implementation.

Root batch `logs/root_bsr_identity_master_candidate_20260916.log` is **terminal, exit 0, all nine focused checks passed**: two named MAC BSR tests, BSR lifecycle/wire/fixed-CE layout, operator-master contract, PUCCH multiplex permission/code-rate allocation and PUSCH CSI-presence decision. At `2026-09-16T11:16:08.4369552Z`, both processes were absent and all **39** captured source hashes matched. Log SHA-256 `EC3DE72A72504B132E8E6349536144C10D1CB5497E0497BAEEDEC706FC65684A`. Static analysis and `git diff --check` found no syntax/whitespace errors; existing style suggestions remain. These are focused checks, not unfiltered final-source qualification.

The earlier development batch is also **terminal, exit 0**, with both named MAC checks and all ten selected tests passing, including both E2E guards. The packet-semantic campaign completed seeds 11, 22 and 33 in 1168.86 s; truth/proxy guard took 743.35 s. At `2026-09-16T11:16:31.5647981Z`, both processes were absent and all **187** captured source hashes matched. Log SHA-256 `8FE431A6F1FC5F29C35C7C128270C74388AA6C3D384BCF3A80424752C7B5BF88`. This earlier batch does not cover the subsequent root preparation-identity or master-YAML edits.

Still open: normal identity-bound SR/MAC/PDU integration, independent combined PUCCH and missing-DCI/transport closure, detector physical qualification, SRS/measurement/export corrections, final-source unfiltered required suites, R2023b execution, integrated 5 MHz TDD 12 dB acceptance and complete historical-edit consolidation. No qualified-main push or successful 12 dB run is implied by this checkpoint.

### 16:27 IST: source-frozen validation and next repair boundaries

The current BSR candidate has passed both named MAC checks, its four selected helper/codec/lifecycle tests, link export, artifact integrity/preservation, scheduler consistency and `testE2E_FastVsTruth` (743.35 s). `testE2E_TruthPacketSemanticCampaign` remains running; session 53433 has not terminated. At `2026-09-16T10:57:11.9817436Z`, all **187** prelaunch source hashes matched. No complete guard-batch pass or final log hash is claimed. No production source, test source or checkout HEAD was changed during this checkpoint.

The separate, earlier frozen full suite had **247 top-level test passes and six failures** at 16:26:50 IST. The new sixth failure, `testSharedCSIReportClockVariants`, is specifically its **two FDD cases**, both rejecting absent Format-2 `max_code_rate` at `PUCCHRRCContext.maxCodeRate`. Its TDD combined-HARQ/CSI and TDD stale-metadata cases passed, each delivering the seven-bit CSI report in slot 5 for due slot 4. FDD implementation remains deferred; this is not a new TDD clock defect. The other five failures retain their previously recorded classifications, including two detector pilots rejected before RF by the clean-Git guard.

Read-only comparison against the exact 17 catalog files loaded by `loadParameterCatalog` found the following **four missing catalog entries in each** maintained operator master. This is structural evidence, not a MATLAB configuration-build pass:

| Missing entry | Existing authority and bounded repair | Required proof |
| --- | --- | --- |
| `pusch.csi_presence_decision_algorithm` | Declare the already-supported `unique_current_tb_crc` policy; reuse the existing scenario catalog and snake-to-camel config mapping. Do not add a receiver algorithm or silent default. | Both master configurations build and preserve the declared receiver policy; malformed policy still rejects. |
| `pucch_resources.format2` | Install explicit per-format multiplex permission and coding-rate policy, consistent with the master's intended enabled features. | Nonempty policy propagates into the installed RRC context and actual allocation; absent/invalid policy still rejects. |
| `pucch_resources.format3` | Same existing per-format authority; do not use a global permission or an empty struct merely to satisfy field coverage. | Same propagation/negative checks for Format 3. |
| `pucch_resources.format4` | Same existing per-format authority; do not enable a feature merely to complete the catalog surface. | Same propagation/negative checks for Format 4. |

Audited source hashes: `master_sinr_sweep.yaml` SHA-256 `5E34CF602645072C792DFEB5BA60B6492D80EBCD8252BCECF78E6202D2999466`; `master_geometry_based.yaml` SHA-256 `696C37B878A8C5DAE469D5978E8349A404C9142F91DF0F47AA4C78A4B220E9A9`. The latter is maintained-surface parity, not a change of the requested 12 dB operating mode to geometry. The currently frozen guard source is not edited to apply this repair.

Normal SR integration remains a distinct missing boundary: `queueUplinkControl` schedules a pre-sample `ULTransmitBoundary` with empty context, but supplies no identity-bound post-sample control-TX callback to MAC. The existing data path already has post-sample `DataTX` evidence and a callback dispatch. Reuse that mechanism in `CoupledWaveformStream` and the existing `runWaveformLinkBundle` observation dispatcher; do not count queuing, planned RE ownership or successful reception as the actual control transmission event. Then wire the existing MAC state through real PDU assembly and received BSR consumption. The tested BSR helper alone does not close this path.

Additional candidate review item, **runtime-reproduced at 16:30 IST**: `BSRStateMachine.buildReport` hashes decision content without a preparation-instance identifier. Two identical preparations in slot 3 returned the same ID. Discarding the first then caused `recordTransmission` for the second to reject with `sixgr:mac:UnknownBSRPreparation`; the pending regular trigger remained and no transmission was counted. Pure-MAC diagnostic session 67666 terminated with exit 0; no matching process remained. Log `logs/bsr_preparation_identity_reproducer_v2_20260916.log` SHA-256 `891B9562DF1F4A819FB151D849736AFC326A6D5DBDFB9B2CE8F54721317C6D8D`; source `BSRStateMachine.m` SHA-256 `FC88CA4CDC5D85C8016E9268DB44333B6C9747FD270C7A67193965864144DEFD`. The first launcher attempt had a PowerShell/MATLAB string-quoting error before execution; its separate failed log is retained (SHA-256 `66C968DEBB197F4EC36570693931D8609B8716B938A9089087B7B95C57642B90`).

Repair boundary: distinguish preparation instances within the existing helper and bind them to actual MAC-PDU/transmission ownership in the coordinator. Preserve immutable-payload checks and rejection of repeated callbacks for the same preparation. Payload equality alone is not an ownership identity. A preparation serial is not, by itself, physical execution evidence or cross-epoch authority. Add tests for two equal-content preparations, discard isolation, out-of-order completion, newer trigger preservation and duplicate rejection. Do not claim this helper defect caused the historical 12 dB abort. No production edit was made while session 53433 remains live.

The integration contract was rechecked against [TS 38.321 V18.9.0, sections 5.4.4-5.4.5](https://www.etsi.org/deliver/etsi_ts/138300_138399/138321/18.09.00_60/ts_138321v180900p.pdf): BSR-triggered SR cancellation needs the qualifying transmitted BSR's trigger coverage or a grant accommodating all pending data; an arbitrary received grant is insufficient. Active BWP, measurement gaps, prohibit timing and overlap permissions also gate the SR occasion. Keep these conditions in the normal coordinator and its tests; do not treat the legacy helper's unconditional `UL_GRANT` transition as sufficient normal-path semantics. Multi-preparation handling must still respect at most one BSR CE per MAC PDU.

### 16:10 IST: BSR wire and declared-event lifecycle repaired; final focused/guard batch running

Development remains isolated in `C:/Users/anup0/AppData/Local/Temp/sixgr_tdd_sr_lifecycle_20260916`, detached at `08f9ec60` plus preserved candidate changes. No qualified-main merge, new push, clean-tree claim or successful integrated 12 dB run is claimed. The IDE checkout still contains the audit rather than the production edits.

The original BSR diagnostic is terminal: zero physical transmissions, short report index **74 instead of 15** for 1000 bytes, trigger cleared by preparation, and a **five-byte Long BSR selected for a four-byte allowance**. Budgets 0-2 also exposed wrong Short Truncated LCID 60 instead of 59. Receipt confirms all ten original source hashes unchanged; log SHA-256 `9F5D98D84F19821FD720608EF3805D28E91D0868AD94DFF234DF1591BB6CC115`.

The bounded candidate now reuses `BSRTableR18` and the existing MAC subheader/assembler. `BSR_PHR` removes logarithmic long-BSR quantization, budgets complete subPDUs, selects padding truncation by explicit LCP priorities, rejects malformed/reserved inputs and returns quantization bounds rather than exact queue estimates. Its PHR approximation is untouched and is **not** qualified for strict normal integration. `testBSRWireContract` covers 287 non-reserved table entries, all 256 bitmaps and 80 actual assembler/demultiplexer byte-budget cases. The two-test codec batch ended with exit 0 at 15:59:48 IST; all 74 source hashes matched and both processes were absent. Log SHA-256 `A00567AF18A6026C43DDF226371B2DC60C03C53A7B64AB5077AB306FD47EF4AE`. The failed missing-test-path launch and the candidate's empty-list shape failure are retained separately, not erased.

`BSRStateMachine` now uses the correct short/long tables and byte budget, preserves pending triggers during generation, tracks periodic/retx expiry, restarts retx timing on a declared received new-data grant, and validates immutable preparations at transmission/discard. Full-report transmission cancels only covered triggers; later triggers survive, and padding-truncated reports do not cancel regular/periodic state. This is a **declared MAC-event helper**, not proof of physical transmission or normal SR integration. LCP-priority trigger generation, sufficient-grant cancellation policy, normal MAC-PDU carriage/received BSR consumption and identity-bound physical callbacks remain coordinator work. Timer generation versus transmission cancellation follows TS 38.321 V18.9.0 section 5.4.5, rather than moving all timer effects to TX.

Coverage root found and repaired: `MACSelfTest.runOne` previously routed the named BSR format/trigger/timer tests to only `assert(vectorResult.Passed)`. Those two names now exercise the state machine directly. `testBSRReportLifecycle` is registered in `testAll`; the phase-state writer now takes `TableID` from the actual encoded report. This table-label correction does not qualify the broader phase artifact generator or its other evidence claims.

The first lifecycle batch terminated with **all six checks passing**, exit 0, all 181 source hashes unchanged, processes absent at 16:08:52 IST. Log SHA-256 `48BDA2CC2B79E131C41FC6AAFFB2B4196AC10EF227757E7F4AF4D2264E436DFC`. It covered lifecycle, wire, fixed CE layout, both repaired named MAC checks and `testCodexLLSRemediationHelpers`. A subsequent review added integer-typed buffer/timer arithmetic protection and a corresponding saturation regression. Final selected validation is therefore **running**, not yet passed: session 53433, `logs/bsr_lifecycle_candidate_v2_guards_20260916.log`, with 187 prelaunch source hashes. It repeats the six checks and includes link export, artifact integrity/preservation, scheduler consistency and both required E2E guards. No source/HEAD edits are permitted in that checkout while it runs. Unfiltered final-source `testAll` remains due.

The old clean `d05c1441` full suite is now terminal: **47 failures / 706 tests**, MATLAB and launcher exit 1, launcher completion `2026-09-16T10:34:08.0542702Z`. Original log SHA-256 `9086414D6AC61A155E3EAFC8458074F84F7DC00475151BD5642BBD0A115DF31D`; `failures.csv` retains the complete list. Its existing queue automatically started the old-revision 27-test guard batch (engine 26692); no new old-revision queue was created. The separate frozen combined-receiver suite remains live (engine 6248); at 16:07 its snapshot had 177 top-level passes and five failures. The fifth is an explicit master-YAML coverage omission: `pusch.csi_presence_decision_algorithm` is missing from `master_sinr_sweep.yaml`. Repair the maintained operator surfaces from the existing catalog authority, not the assertion. Neither suite qualifies the current BSR source.

At 16:12 IST, all six focused BSR checks have passed again on the final integer-safe candidate. The broader selected export/scheduler/E2E batch remains live as session 53433, engine 14668/launcher 7540. At `2026-09-16T10:41:47.9911550Z`, all 187 prelaunch source hashes matched. No terminal log hash or complete guard-batch pass is claimed while it runs.

### 15:26 IST: CRC and CSI clock-boundary focused batches terminal, all selected tests pass

The live `08f9ec60` plus 13-file candidate remains frozen in `sixgr_tdd_combined_receiver_20260916`. The latest counted unfiltered-suite snapshot had **134 top-level passes and four failures**: CRC and periodic-CSI fixtures, plus `testPUCCHDetectorPilot` and `testPUCCHDetectorModelCandidatePilot`. Both pilots stop at `runPUCCHDetectorEpisodes.m:15`, the unchanged clean-Git guard, **before RF execution**. Launching this suite from an uncommitted snapshot was a validation-workflow error; matching source hashes does not satisfy that guard. Preserve the failure and guard, and run final qualification from a committed clean candidate. These are not two new statistical detector results and do not replace the retained false-ACK failure. All 53 source hashes still matched at 15:01:46 IST. Full-suite engines 6248 and 8316 remain live. No existing suite was stopped. After memory availability improved, one single-threaded focused diagnostic was permitted at a time with a prelaunch minimum of 5 GiB commit headroom; this is not another full-suite queue.

A separate detached worktree, `C:/Users/anup0/AppData/Local/Temp/sixgr_tdd_sr_lifecycle_20260916`, was prepared at `08f9ec60`. Its `logs/source_snapshot_20260916.json` records all 53 relevant source files byte-identical to the frozen candidate before new edits (13 working changes copied; 40 additional differences were checked as line-ending-only and copied exactly). This preserves the live test source and HEAD, rather than testing against files changing underneath the process.

`tests/testPUCCHReceiverCRC.m` now explicitly tests both configured-CSI and dynamic-HARQ procedures, independently declares the receiver schema, asserts four/two PRBs and 128/64 coded bits, and retains desired-RNTI pass and wrong-RNTI CRC-failure assertions. The HARQ transmitter's existing power formula is bound to its selected two-PRB allocation; detector threshold, sample-noise variance and original seeds remain unchanged. `git diff --check` passed; standalone MATLAB `mlint.exe -id` reported only the existing scalar-length style suggestion. Candidate SHA-256: `39CACC314C9A6A5EB688D7A92A25B8D3FDBDE561785918F6C7ADC898F6CC4CFA`.

**Runtime proof:** `crc_fixture_focused_jvm_20260916.log` reproduced desired-RNTI failure for original TX/RX PRBs 4/2 and coded lengths 128/64. Keeping the actual TX and aligning only the declared CSI receiver yields the desired pass and wrong-RNTI failure. The repaired CSI and HARQ fixtures both pass, as do `testUCIDecoderCRC`, `testPUCCHCodeRateAllocation` and `testPUCCHReceiverOnlyAssignment`. Session 1429 terminated with exit 0; both process IDs were absent and all 54 source hashes matched at 15:10:29 IST. Log SHA-256: `6B2ED1C1D5A9C122E1AA095ADE1CBAB4A2738D5CA6CC70BE3F6A3EBE56280F22`. This closes the bounded CRC fixture regression, not detector qualification or final-source `testAll`/12 dB acceptance. A prior no-JVM attempt exited 1 before PHY execution because the YAML external-Python path uses `java.io.File`; its separate failed log/receipt is retained, SHA-256 `043A0FE2999CA10CF83BDEE66B3464BC4FC78D78E992977B31E14D15FE5A6F97`. No configuration workaround was added.

The second failure is now runtime-reproduced: `testSharedPeriodicCSIProducer` expects `MeasurementAvailableAtSample` in the delivered scheduler-facing row, although `receivedCSISchedulerReport` retains the original UE record in `UEReferenceRecordJSON` and leaves the projected timestamp NaN. The unchanged original assertion failed after printing **measurement=7680, projected=NaN, audit=7680, audit/expected epoch=1, matching shared-clock domain**. Session 5661 exited 1, both processes were absent and all 55 source hashes matched at 15:14:44 IST. Original log SHA-256: `5E42B52800345F14F398173D612E0B6D2C9FEF0BE3A79CCA03B6EE04ED278ECF`.

After termination, the periodic test was changed to check exact timing before transmission and in the identity-bound audit after publication, while rejecting raw UE timing/SINR in the scheduler-facing fields. `testSharedPUSCHChannelArtifacts.m::localVerifyCSI` had the same old post-publication expectation; its CSV check now validates the exact retained audit clock/epoch/identity and unavailable projected fields. The late-PUSCH wrapper retains its actual artifacts beneath `logs/tdd_shared_pusch_late_csi/`. Production receiver/scheduler code is unchanged. The three-test candidate batch (session 12243) is **terminal, exit 0, all three tests passed**: `testSharedPeriodicCSIProducer` 115.67 s, `testCSIReportPublicationClock` 20.97 s, `testSharedPUSCHLateCSIDelivery` 274.14 s. Both processes were absent and all 57 source hashes matched at 15:25:57 IST. Log SHA-256: `ABEEF35374F4892DC4EA6483FAA863ABF580BF8E26B7092E8E10EB8DE0D87352`.

Independent readback of `logs/tdd_shared_pusch_late_csi/tp8a6c8e9a_8529_48f4_879f_dd88bd918c91/late_csi_delivery.csv` confirms one row, actual `pusch_decoded` transport, source slot 7, due slot 10, delivery slot 11, CQI/RI/PMI 15/1/3, unavailable projected UE timestamp, and retained audit completion sample **53775**, epoch 1 and matching report identity. CSV SHA-256: `1F8D27E00D384568F4019FCA070E64F1ED1E59AB804FA6A2F7798C8CC4F4BF85`. This legacy transport fixture explicitly uses a three-slot feedback-delay input and has `CSIReferenceSlot=NaN`; it does not qualify the standard periodic-reference calendar or integrated 12 dB scenario. The separate independent shared-PUSCH CSI fixture uses its authored slot-6/slot-10 calendar. Full-source unfiltered qualification remains due on a committed clean candidate, not the dirty frozen suite.

Next bounded execution: reproduce and repair the diagnosed SR/BSR wire/lifecycle dependency. Do not count preparation as passing or launch another full-suite queue. The normal SR/BSR coordinator, independent combined PUCCH, detector qualification, SRS/measurement/export repairs and final consolidation remain open as detailed below. No qualified-main merge, push, clean-tree claim or successful integrated 12 dB run follows from these focused passes.

At 15:28 IST the BSR byte-budget/state diagnostic started as session 57692, engine 2244/launcher 6276, with a prelaunch receipt covering ten directly used source/script files. It subsequently terminated with exit 0, reproducing the defects described in the 16:10 checkpoint; it is no longer running. It exercised unchanged codec/state helpers and the strict assembler with declared MAC inputs, not RF or normal SR integration.

### Independent shared CSI: focused repair and ten selected broader guards passed

The new combined fixture reached real slot-6 DL reception (ACK=1), actual CSI-RS acquisition and successfully decoded slot-10 PUSCH, then failed exactly at `test:IndependentCSIPublicationMissing`: common independent completion had no CSI publication. `logs/independent_shared_csi_reproducer_20260916.log` exited 1, SHA-256 `0672F079F6046A70C66C390FF6DEC8AEE540152186F8C4FD4FA5D976840BE1B5`; all 49 captured source hashes matched. The actual receive result is retained at `logs/tdd_shared_pusch_independent_csi/tpef9a0a8d_94a0_485c_a0a4_8cc987bd5738/independent_csi_completion.mat`. This is high-margin component evidence, not 12 dB performance.

The ensuing candidate changes the existing completion boundary, not the waveform/receiver thresholds:

- `normalizeReceivedPUSCHCSI.m` validates receiver-owned bits, context, presence, per-part evidence and received-Part-1 sizing. Unknown lengths stay NaN; absent/unresolved CSI does not become a received report.
- `CoupledTruthRuntime.completeSharedPUSCHHARQFeedbackRuntime` preflights CSI and value-state publication/bookkeeping before shared HARQ mutation. The existing scheduler-publication body is moved into `publishCSIReport`, eliminating its mandatory UE-row lookup for this independent path. Publication after HARQ retains the real feedback/OLLA update order.
- The new `SharedGNBCSIReportTable` uses configured report identity/reference timing and completed observation provenance. Raw UE measurements remain unavailable to the scheduler. `SourceSlotAuthority` explicitly distinguishes the configured CSI reference slot from an unknown actual UE measurement slot. Available UE references join only the audit; missing references do not fabricate bit-comparison results. `received_csi_reports.csv` preserves this schema in control and air-interface exports.
- The independent completion receipt prevents later legacy reducers from publishing the same CSI again. The test exercises normal publication, malformed CSI rejection before HARQ mutation, and a repeated physical fixture with UE bookkeeping removed and TX CSI references poisoned **after actual transmission/reception**. That last case is a metadata-independence test, not a no-transmitter noise trial.

`logs/independent_shared_csi_candidate_v1_20260916.log` terminated with seven passes and one duplicate-guard ordering failure. After correcting that ordering, v2 terminated with **exit 0, all eight selected tests passed**, including both actual shared-waveform cases (normal and UE CSI bookkeeping removed/TX references poisoned), identical scheduler feedback, original HARQ-only completion and export regression. All 50 recorded source hashes matched at completion. The v2 log SHA-256 is `CC2E2301CCEFE86A0CC40101CE9E0BFC3AE43EF807C107C3CFC668654D4964E5`; detailed receipts and artifact roots appear in the 14:01 entry below. This is high-margin component evidence, not integrated 12 dB acceptance.

The independent-CSI required guard batch is now **terminal, exit 0, all ten selected tests passed**. `testE2E_FastVsTruth` took 515.63 s and `testE2E_TruthPacketSemanticCampaign` 801.92 s. All 50 captured source hashes matched, and process absence was verified at 14:31 IST. Log `logs/independent_shared_csi_required_guards_20260916.log` SHA-256: `2E89D39FE4202FA65A7B326CBD5112650AC63274771F1985E9F52545536587D8`. These passes predate the SR selector patch below. Unfiltered `testAll`, remaining feedback integration, measurement closure and integrated 12 dB acceptance remain due.

### 14:35 IST: SR selector defect reproduced and bounded repair under test

The SR entry diagnostic completed against unchanged production source with exit 0. Log `logs/sr_runtime_entry_reproducer_v2_20260916.log`, SHA-256 `E1555C176945309ACA2A3DE21F26DA6B235406063409E8072B0AB0C53E7FD34A`, records **configured SR resource 37 selecting dynamic HARQ resource 10 with `decoded_dci` provenance**. All 50 previous source hashes still matched before patching. The earlier diagnostic launcher failed because MATLAB `run` temporarily changed to `logs` before setup; its separate original log is preserved and is not defect evidence.

The probe also establishes limits on the diagnosis: NaN pending state was already rejected by MATLAB (`MATLAB:nologicalnan`), not converted into a positive SR. A negative SR assignment alone is not an erroneous over-air transmission: the existing transmitter suppresses its IQ/RE/power ownership. Raw MAC transitions accept caller-declared timer/max/grant events; those probes show why a real coordinator is needed, not proof that the low-level helper should independently infer physical events. Three raw SR flags serialize into three bits; the normal caller still needs the ordered multi-SR identity encoding, rather than treating that bit-packer as a completed multiplexing procedure.

Bounded changes after both MATLAB processes terminated:

- `PUCCHConfigBuilder` now preserves the existing installed SR identity/occasion associations in its RRC context; no YAML default or policy namespace was added.
- `PUCCHResourcePlan` selects standalone SR from the installed SR resource association, binds typed identity/period/offset/priority and exact target slot, checks format and UL symbols, and marks configured-occasion provenance. HARQ PRI/K1 cannot choose or shift it. Explicit bit-level component calls require a unique installed SR resource; multiple resources require typed identity.
- `PUCCHTransmissionAssignment.fromSR` retains SR priority and can return the exact bound report for transmission. Negative SR continues to produce no waveform energy; it is not changed into a positive request or treated as a received report.
- PHY `SchedulingRequestState` now validates finite integer clock/identity fields and scalar binary flags before conversion. This is input-contract hardening, not the MAC lifecycle implementation.
- `testPUCCHConfiguredSRResource` uses different SR/HARQ resources and poisoned unrelated DCI fields, actual Format-0/1 positive/negative waveforms, public waveform references, independent RX, and invalid configuration/state/timing cases. The existing SR-calendar test checks configuration propagation; the new test is registered in unfiltered `testAll`.

At 14:37, selected batch `logs/sr_configured_resource_candidate_v1_20260916.log` is **terminal, exit 0, all nine selected tests passed**. This includes the new resource/waveform test, existing short-SR behavior, installed SR calendar, combined component receiver, power-independent planning, power-resource binding, coding-rate allocation, multiplexing permissions and CSI-only resource selection. All 13 changed/new MATLAB/YAML source hashes match, process absence was verified, and `git diff --check` passed. Log SHA-256: `DA8D0B38B59E90867AA361CFBDC0D92F53A7F2C1EE92B33AB8CE2CEC5447C914`.

The mandatory **unfiltered** `setup6GRSimToolkit('Verbose',false); testAll;` was launched next (session 73552), with `logs/tdd_sr_resource_full_testall_20260916.log` and a prelaunch receipt covering 53 relevant changed/tested source files over `08f9ec60`. No test-name filter or exclusions were supplied; the log name denotes the candidate, not a TDD-only suite. Source remains frozen and uncommitted during this run. **Normal SR lifecycle, ordered multi-SR multiplexing, combined-PUCCH integration, detector qualification, all-measurement closure and integrated 12 dB acceptance are still open.** This bounded selector repair does not substitute for those requirements.

### 14:47 IST: normal SR requires an explicit scenario/MAC boundary; new full-suite failure

The unfiltered suite is live on engine 6248/session 73552. At 14:45, 42 top-level tests passed and one genuinely failed: `testPUCCHReceiverCRC`, line 22. Four `regressionHarnessProbeTest` failures earlier in the log are intentional harness tests whose owning top-level tests passed; they are not four additional simulator defects. All 53 captured source hashes still match. The current full-suite candidate is **not passing**, and production source has not been changed beneath it.

**New CRC failure, source-localized, runtime repair not yet verified:** the fixture factory puts the 32 transmitted bits in `CSIReports`, while `testPUCCHReceiverCRC` declares 32 HARQ bits and zero CSI at RX. The installed Format-2 resource has four PRBs, two symbols and max code rate 0.80. `PUCCHResource.selectPRBAllocation` preserves all four PRBs for configured CSI but selects two for this dynamic-HARQ length (43 information-plus-CRC bits fit 0.80 times 64 coded bits). The receiver therefore does not observe the same coded allocation as the intended transmitter. The failure is consistent with this fixture mismatch; the log does not record which RNTI iteration failed, so it is not yet an executed proof of the complete cause. Repair the fixture's declared procedure, add explicit TX/RX resource/coded-length equality, and exercise both CSI and HARQ allocations while retaining desired-pass/wrong-RNTI-fail assertions. Do not change CRC outcomes or detector thresholds to make it pass. A diagnostic must first record the actual widths, coded lengths and CRCs.

**Correction to the earlier SR policy inventory:** `master_sinr_sweep.yaml` contains `mac_phase08.sr`, and the internal mapping exists. However, the saved SR-calendar fixture that directly inherits the current 12 dB scenario contains **neither `mac_phase08` nor `protocol`**. Its actual saved internal `cfg` also contains neither `mac.phase08` nor `protocol`; `traffic.model` is `fullBuffer`. The fixture changes metadata and repeats the same installed SR calendar, not the MAC/protocol surface. Thus the master policy is reusable, but it is not already installed in this scenario. Proof is retained in `logs/sr_bsr_integration_audit_20260916.json`; its read-only script inspects the actual resolved JSON and saved MATLAB `cfg` using SciPy, with source/artifact hashes. This is artifact/source evidence, not a new MATLAB/RF run.

`logs/diagnose_pucch_crc_allocation_20260916.m` is prepared to record the original and CSI-aligned receiver allocation, actual coded lengths and both RNTI CRC outcomes while preserving the original assertions. It is **not executed** at the 14:49 checkpoint. The same full-suite engine remains live and source remains unchanged; a diagnosis is not a passing repaired fixture.

The normal-source dependency chain is now explicit:

| Existing boundary | Evidence and required replacement |
| --- | --- |
| `CoupledTruthRuntime.buildSchedulerUEState` | UL demand comes directly from UE `state.ULQueueBits`, is copied into `ueState.ULBufferBytes`, then read by `SchedulerPF`. Full-buffer demand is a benchmark assumption, not evidence of received SR/BSR. A claimed closed-loop MAC mode needs a separate gNB state updated from received reports; do not relabel the current queue as a decoded BSR. |
| `WaveformProtocolBridge.encodeFragment` / `deliverDecoded` | Assembly currently contains one data SDU. Delivery requires exactly one subPDU and uses pending TX hashes as integrity checks. A normal BSR path must carry a real CE in the same PHY TB, budget its header/payload, parse received bytes, and keep optional TX comparison separate from receiver acceptance. Reuse `MACPDUAssembler` / `MACPDUDemultiplexer`, which already support CE subPDUs. |
| `BSRStateMachine.buildReport` | Chooses the 8-bit table even for short BSR and clears pending state during construction, before actual transmission. Separate report preparation from TX commit; use the appropriate short/long table and preserve trigger coverage until physical transmission or a sufficient received grant. |
| `BSR_PHR` legacy codec | Its long-BSR index is explicitly a logarithmic approximation. Source-formula comparison gives 1000 bytes -> index 192 versus index 74 in existing `BSRTableR18`; 10 bytes -> 64 versus 1. Its long decoder also silently breaks on truncated payload. Do not connect this helper unchanged or add another approximate table. Reuse `BSRTableR18` and reject malformed payloads; retain quantization intervals rather than calling a bound an exact measured queue size. |

The applicable SR/BSR procedures and buffer-size tables are in [TS 38.321 V18.9.0 sections 5.4.4, 5.4.5 and 6.1.3.1](https://www.etsi.org/deliver/etsi_ts/138300_138399/138321/18.09.00_60/ts_138321v180900p.pdf). The next normal-path repair must therefore: (1) install and validate the existing policy through a reusable scenario fragment, (2) repair/reuse exact BSR preparation and wire parsing, (3) connect UE triggers/actual TX/received grant cancellation and independent gNB demand updates, then (4) integrate SR/HARQ/CSI resource arbitration and reception. Merely inserting an SR bit, emitting an SR row, or treating the full-buffer queue as a received report would not meet this boundary. No protocol stack was enabled and no source patch was applied during the live suite.

Read-only refinement at 15:20 IST: before wiring the legacy BSR helper, also repair its **LCID and complete MAC-subPDU byte budget**. `BSR_PHR.buildBSR` labels its one-octet Short Truncated BSR as LCID 60, while the existing `MACCESchemaRegistry` correctly maps 59 to Short Truncated and 60 to Long Truncated, matching TS 38.321 Table 6.2.1-2. For two active LCGs, Long BSR requires one bitmap plus two buffer indices and a two-octet variable-length subheader: five bytes, not four. `BSRStateMachine.buildReport` budgets `2 + number_of_active_LCGs`; `BSR_PHR.buildBSR(...,4)` selects Long BSR, and `testCodexLLSRemediationHelpers` incorrectly treats that as sufficient without assembling the wire PDU. Reuse the existing strict subheader/assembler for budget proof; correct the helper and its expectation rather than relaxing `MACPDUCapacityExceeded`. Truncation must use configured logical-channel priority, not largest buffer or first active LCG, and distinguish regular/periodic from padding triggers (§5.4.5). `logs/diagnose_bsr_wire_contract_20260916.m` in the new worktree is prepared to exercise budgets 0–5, actual assembler rejection and prepare-versus-transmit state. It is **not executed** at this checkpoint. No BSR production edit has been applied.

### 14:22 IST: next SR integration boundary, source-proved versus untested

No further production changes were made while the above batch runs. This review narrows the next repair; it does not mark SR or combined PUCCH complete.

| Boundary | Direct source evidence | Minimal repair / acceptance proof |
| --- | --- | --- |
| Existing policy | `master_sinr_sweep.yaml` already supplies `mac_phase08.sr`; `buildInternalConfig.m:2232` already maps it to `cfg.mac.phase08`. `scenario_parameter_catalog_extension_12.yaml` accepts SR only as a generic struct; `MACRuntime.validateConfig` checks section presence, not SR fields. | Keep the existing policy and mapping. Validate its enabled/ID/timer/max/fallback fields and consistency with installed SR resources. Do not add a duplicate default namespace. Test malformed and mismatched configurations. |
| UE SR owner | MAC `SchedulingRequestState` is a low-level declared-event transition helper, not a normal runtime coordinator. `PUCCHConfigBuilder.m:112` supplies an empty SR input. | Add the missing normal owner using existing UE queues, configured SR identity, actual received grants and transmitted MAC-PDU evidence. Preserve declared-event tests; a raw `MAX_TX` event alone does not prove incorrect caller behavior. Test repeated arrival, timer expiry, maximum count, sufficient/insufficient grant, BSR coverage and configuration epoch. |
| Physical event | `CoupledWaveformStream.queueUplinkControl` schedules `ULTransmitBoundary`; its handler at line 929 only retargets/checks timing, then continues without a MAC callback. Existing `DataTX` commits after the first active sample was physically consumed. | Reuse that physical event mechanism for an identity-bound control-TX receipt. Do not increment SR state merely when future IQ is prepared, nor wait for gNB decode success. Test queued-but-untransmitted, exactly-once physical TX, dropped reception and duplicate callbacks. Account for applicable lower-layer failure policy explicitly. |
| Multi-SR wire representation | `UCIReportSerializer.sr` concatenates supplied `Bits`/`Value`; the existing overlap resolver separately calculates the ordered identity width. | Resolve eligible overlapping resources before serialization and provide one typed wire representation. Test no/one/multiple positive requests, ordering, negative indication and HARQ/CSI bit positions. Keep format-specific short-resource behavior distinct. |
| SR-only resource selection | `PUCCHTransmissionAssignment.fromSR` passes SR state into `PUCCHResourcePlan`; the plan has configured-CSI and dynamic-PRI branches but no explicit configured-SR selection. | First run a two-resource counterexample where installed SR and PRI select different resources. This is a source-identified risk, **not yet a runtime-reproduced failure**. Repair the existing selector if reproduced; do not invent PRI authority for SR-only. |
| Combined gNB reception | Normal combined PUCCH still lacks independent receive context/publication; configured CSI/SR overlap guards remain. | Only after UE wire/resource semantics are fixed, replace strict-runtime TX-context fallback using configured obligations and actual captures. Keep absent/ambiguous interpretations unresolved and preserve the guards until implemented tests replace them. |

The pinned MAC profile uses [TS 38.321 V18.9.0 section 5.4.4](https://www.etsi.org/deliver/etsi_ts/138300_138399/138321/18.09.00_60/ts_138321v180900p.pdf). It requires pending SR, timer/counter and eligible-resource handling; BSR-related cancellation depends on adequate grants or a transmitted BSR covering the relevant triggers. A scheduled grant or report-construction event is insufficient evidence by itself. The proposed post-sample receipt is this simulator's causal implementation choice, not a prescribed 3GPP API.

`logs/diagnose_sr_runtime_entry_20260916.m` is prepared in the development checkout but **not executed**. Its low-level event, malformed-flag and raw serializer probes are not RF qualification and must not be cited as passing or reproduced runtime results. Run it after the frozen guard batch terminates, then add the two-resource case before production edits.

The next bounded deliverable is therefore the SR wire/resource/physical-event contract, not another decoder framework. Its entry gate is the above reproducer evidence; its exit gate is focused tests plus a real normal-path waveform case. The broader all-gap repair map remains under "Ordered work packages". No guaranteed total completion time or successful 12 dB run is established by this checkpoint.

### 13:22 IST: selected E2E/export batch complete; combined-CSI reproducer started

The frozen `08f9ec60` batch `csi_wire_authority_v2_20260916.log` completed with **exit 0, all seven selected tests passing**, including both required E2E guards. `testE2E_FastVsTruth` took 585.02 s; `testE2E_TruthPacketSemanticCampaign` took 834.20 s and exercised seeds 11, 22 and 33. All 46 captured source hashes matched at completion; the tracked development tree was clean before the next fixture edit. Log SHA-256: `2F85808638C1E0A8B774DCDB36E6B4B457EE5D95435BD5263D19E1E3B5A6D763`. This is not unfiltered `testAll` or integrated 12 dB acceptance.

After the process exited, the existing shared-PUSCH fixture was extended with a CSI-enabled independent mode and registered as `testTDDSharedPUSCHIndependentCSICompletion`. Its YAML explicitly aligns slot-6 CSI-RS and the periodic slot-10 report while retaining slot-6 HARQ feedback on that same PUSCH. It inherits the existing high-margin component point; it is not 12 dB performance evidence. Reference timing is checked against the installed calendar and the single-resource/equal-numerology rule in [TS 38.214 V18.8.0 section 5.2.2.5](https://www.etsi.org/deliver/etsi_ts/138200_138299/138214/18.08.00_60/ts_138214v180800p.pdf).

`logs/independent_shared_csi_reproducer_20260916.log` is running against unchanged production code plus this fixture. It requires common independent completion to publish received CSI without the legacy producer-bound adapter. The separate source-binding receipt explicitly records capture immediately after launch, not a prelaunch snapshot. Production preflight/publication drafts are not applied while this reproducer runs. No existing assertion, RF margin, power rule or detector gate was relaxed.

**Reading this ledger:** dated entries below preserve what was known at each checkpoint, including failed proposals and then-uncommitted work. They are not all current pending items. This front section supersedes their repository/validation status. The file-by-file repair map remains under “Ordered work packages”; an implemented component is not automatically integrated or qualified.

### 13:07 IST reconciliation and analysis checkpoint

The six top-level `docs/lls/pending_*.patch` files are now reconciled against `08f9ec60`: **all six changes are already present**, two by exact reverse-application check and four by source/manifest comparison. Do not apply them again. The receipt is `docs/lls/pending_patch_reconciliation_20260916.json` in the IDE checkout; it records patch hashes, failed check diagnostics, semantic evidence and vector-file hashes. Three files use `apply_patch` syntax, so `git apply` rejecting their format was not evidence that their changes were missing. The HARQ-calendar patch's QCL context has newer runtime-authority checks that must be preserved. This closes this bounded six-patch inventory only; seven archived proposals, stash content and final checkout consolidation remain separate.

No production code was changed during this read-only checkpoint. Current development source remains frozen for the already-running E2E guards. At 13:07 IST, all five preceding checks and `testE2E_FastVsTruth` passed (the latter in 585.02 seconds); `testE2E_TruthPacketSemanticCampaign` is live. All 46 captured source hashes still match, and the development tracked tree is clean. Neither a completed E2E batch nor a successful 12 dB run is claimed.

| Current task boundary | Verified progress | Still required |
| --- | --- | --- |
| UCI wire/resource correctness | Short-format corrections, report-digest binding, rate/resource calculations and bounded whole-report selection have focused passes. | Normal SR lifecycle, guarded short-payload omission and applicable Part-2/multi-report boundaries; do not silently disable enabled features. |
| CSI presence and scheduler inputs | Actual PUSCH present/absent waveform checks and independent normal shared-PUSCH publication pass. Removing UE CSI bookkeeping/poisoning TX references leaves scheduler feedback unchanged. | Full coordinator, rejected-DCI/cross-transport and remaining absent/ambiguous/epoch cases; optional malformed-audit handling; independent PUCCH/SR publication. |
| Timing | Original clock/CSI fixture errors have focused repairs; tail drain has focused passing coverage. | Current-revision final-slot integrated completion and final artifact hydration. |
| Detector | Original post-RF false-ACK failure is retained and diagnosed; component passes do not erase it. | Invalid-input rejection, frozen policy, held-out acquired-timing noise and signal qualification. |
| SRS and measurements | Mathematical counterexamples and exact source repair boundaries are documented below. | SRS CDM/hopping/grid/energy fixes; consistent FER population, unique-TB goodput and final measurement/export verification. |
| Preservation | Cumulative receiver checkpoint pushed; six top-level patches reconciled. | Archived/stash semantic reconciliation, final-source unfiltered suite, accepted 12 dB execution and qualified consolidation. |

The demonstrated **independent CSI completion/publication on normal shared PUSCH** defect is now focused-test verified. The next boundary is independent combined PUCCH with its required SR wire/lifecycle dependency: the normal planner still supplies an empty SR report, so copying the already passing isolated combined receiver into that path would retain an incorrect wire contract. Inspect and repair the existing SR configuration/state/planner interfaces before extending receive hypotheses and common completion. Missed-DCI transport ownership guards remain until their actual capture/disposition replacement is tested. The historic boundary table below records the original PUSCH task; the 14:01 entry distinguishes its completed proof from remaining cases.

The earlier 1–2-day total is not a reliable estimate, and a 2–3-hour all-fixes-and-qualification guarantee is not supported either. Use 2–3 hours as a bounded implementation/review timebox with explicit results, not as a promised passing run. Test execution is separate: the prior failed integrated attempt took approximately 100 minutes including recovery, and final-source full-suite/detector qualification has no measured short completion bound. Re-estimate the remaining unit from its reproducer, patch and targeted runtime; retain failures and unknown roots explicitly.

The cumulative development repairs, audit, selected original failure/pass logs, source-binding receipts and the declared CSI audit CSV are saved on `work/tdd-uci-wire-receiver-20260916`, commit **`08f9ec6058f35ccaea86ea19ca02d4dfe1a28af3`**. `git ls-remote` confirmed that exact GitHub branch hash. The development checkout's tracked tree was clean at follow-up launch. Evidence copies retain their original bytes, with local Git attributes preventing line-ending conversion. Main, the two older validation checkouts, the IDE's existing `logs/README.md` deletion and all original logs were left unchanged. This is not completion of older patch-content reconciliation or qualified-main consolidation.

Follow-up `logs/csi_wire_authority_v2_20260916.log` is running against that frozen commit: the TDD consumer/CSV contract, export/integrity/preservation and scheduler checks, followed by both required E2E guards. Source-binding receipt: `logs/csi_wire_authority_v2_20260916_source_binding.json`. This is still a selected batch, not final-source unfiltered `testAll` or integrated 12 dB acceptance. The v1 pass below predates the two schema-preserving production export calls; v2 includes them.

At **12:58 IST**, all five pre-E2E checks have passed (consumer/CSV, link export, artifact integrity, E2E-artifact preservation, scheduler consistency). MATLAB PID 11784 is confirmed live in the E2E guards. All 46 captured source hashes still match and the development tracked tree remains clean at `08f9ec60`. The required generic E2E fixtures include FDD cases; this does not resume FDD implementation work or qualify a TDD 12 dB run. The five committed retained log blobs were also checked against the original local log bytes with `git hash-object --no-filters`.

### Next independent-CSI integration boundaries, verified by source trace

| Existing boundary | Remaining replacement | Required proof |
| --- | --- | --- |
| `completeSharedPUSCHHARQFeedbackRuntime` | Reuse its independently rebuilt receive context and completed-buffer identity to prevalidate CSI before shared HARQ handles mutate. Bind CSI acceptance to installed configuration and actual per-part evidence, not `UCIOnPUSCHCSIReportIdentity` or expected payload. | Present/absent/unresolved CSI; poisoned TX metadata; malformed CSI must not partly commit HARQ first. |
| `processDueFeedback` / `consumeDecodedPUSCHCSI` | Separate scheduler publication from the UE pending-report lookup. Reuse the new decoded-field projection, but supply receiver-owned report identity and configured reference/report timing. Keep optional producer joins solely for TX audit. The current projection intentionally does not solve timing/ownership. | No UE producer row must not suppress a real received observation; no raw UE SINR/source CRC may return; duplicate and stale deliveries must not update twice or replace newer feedback. |
| `completeSharedPUSCHAfterRejectedControl` | Preserve `RejectedULUCITransportOwnershipRequired` until UE PUCCH disposition and gNB selected/unselected captures are implemented for CSI. Removing that guard alone would hide a real missing transport owner. | Missed UL DCI with an actual UE CSI producer, and no-producer receive-only captures; no manufactured UL TB/BER/BLER/goodput rows. |
| `testSharedPUSCHChannelArtifacts` and TDD fixture YAML | Add an actual independently scheduled CSI-enabled case. Existing `withCSI` uses the legacy completion; existing independent modes explicitly require `~withCSI`. The legacy slot-7 source/slot-10 delay test does not prove periodic CSI reference-resource timing. | Use a configured report occasion with a valid preceding CSI-RS reference opportunity and ordinary received UL DCI; exercise the common receiver commit, not the legacy payload-sized adapter. |

Current-TB-CRC presence selection still does not implement receiver-owned multi-hypothesis retransmission combining. That remains a separate explicit receiver limitation, not a reason to import transmitter-selected UCI lengths or old soft-buffer assumptions.
...
## Independent shared-PUSCH CSI publication: evidence checkpoint, 16 Sep 13:47 IST

Current development checkout: `C:/Users/anup0/AppData/Local/Temp/sixgr_tdd_combined_receiver_20260916`, branch `work/tdd-uci-wire-receiver-20260916`, checkpoint `08f9ec6058f35ccaea86ea19ca02d4dfe1a28af3` plus an uncommitted candidate. The IDE checkout retains this audit and the user's deleted `logs/README.md`; it is not the development checkout. The remote checkpoint is explicitly unqualified.

### Reproduced defect and bounded repair

The actual shared-waveform fixture acquires DL timing, receives the slot-6 DL, creates both HARQ and CSI from that DL, and decodes their normal slot-10 PUSCH. On production checkpoint `08f9ec6`, common independent completion did not publish CSI: `test:IndependentCSIPublicationMissing`. The old producer-bound consumer was still required. Failure log `logs/independent_shared_csi_reproducer_20260916.log`, SHA-256 `0672F079F6046A70C66C390FF6DEC8AEE540152186F8C4FD4FA5D976840BE1B5`, is retained together with actual receive artifacts. This is a high-margin component fixture, not the 12 dB diagnostic.

| File | Candidate repair | What is removed or deliberately retained |
| --- | --- | --- |
| `+sixgr/+truth/normalizeReceivedPUSCHCSI.m` | Validate received binary vectors before conversion; bind installed report/context and receiver-derived presence/length; decode actual CSI fields. | No TX-expected bits, identity or lengths choose the report. Unknown/absent CSI is not promoted to success. |
| `+sixgr/+truth/CoupledTruthRuntime.m` | Common PUSCH completion prepares receiver-owned CSI, preflights value-state publication before HARQ handle mutation, then publishes after actual HARQ/OLLA updates. | The independent path no longer needs the producer-bound consumer; the legacy path remains for its isolated callers. Existing publication code is extracted and reused, not duplicated. |
| Same runtime, report construction/export | Store received CSI separately, with configuration/epoch, observation clock and explicit reference-resource timing authority. UE records are optional audit joins. | Raw UE SINR and source data CRC remain unavailable rather than copied into received scheduler evidence. No synthetic rows. |
| `tests/testSharedPUSCHChannelArtifacts.m`, new `testTDDSharedPUSCHIndependentCSICompletion.m` and focused fixture YAML | Use actual shared DL/PUSCH; check atomic malformed-input rejection, publication/CSV, duplicate rejection and normal reducer. Repeat after removing UE CSI bookkeeping and poisoning expected CSI metadata. | Original assertions remain. Removing bookkeeping after reception is not a no-transmitter RF episode. |

### Current result and next exact change

`logs/independent_shared_csi_candidate_v1_20260916.log` is still running; its source-binding receipt covers 50 files. The received-CSI waveform, partial-CSI reception, source-authority and causal-AMC tests passed. The new physical shared test passed its publication, source-authority and CSV assertions, then failed its unchanged duplicate-reception assertion: it expected `DuplicateSharedPUSCHHARQReception` but received `DuplicateIndependentCSIReport`.

Root cause is ordering in `completeSharedPUSCHHARQFeedbackRuntime`: `prepareIndependentPUSCHCSIReport` checks an already completed CSI occasion before the method checks its physical observation receipt. Move the existing physical duplicate-receipt guard ahead of CSI preparation, retaining both guards and all original assertions. Wait for this batch to terminate before changing its source. The removed-producer replay has not yet run; do not claim its pass.

Exit criterion for this work unit: both normal and removed-producer physical cases pass with identical scheduler feedback, original HARQ-only completion passes, focused CSI/AMC/export regressions pass, and source hashes remain frozen through execution. This is not complete HARQ/CSI/SR integration or full-suite/12 dB qualification. Malformed optional audit metadata, cross-transport disposition, SR lifecycle, detector qualification, SRS/measurement/export gaps and final-source mandatory regressions remain separately open.

At 13:47 IST, v1 terminated with exit 1: seven selected tests passed and the combined CSI test failed only at the duplicate-error assertion. All 50 captured source hashes matched at termination. Raw log SHA-256: `956BE22A56592EB00230FDF92024F38342F3168F995F8EA77DEABA137D699988`. After both MATLAB processes exited, the existing physical-receipt guard was moved before CSI normalization/preparation. No assertion, detector policy, payload, power or fixture parameter changed. The same eight-test v2 batch started at 13:47:52 IST; its 50-file receipt was captured before launch. Development source is frozen until it terminates.

### 14:01 IST result: this focused publication repair passes

The v2 batch terminated with **exit 0, all eight selected tests passed**. Both MATLAB processes exited, all 50 recorded file hashes remained unchanged, and HEAD remained `08f9ec6` with the explicitly recorded candidate edits. Log SHA-256: `CC2E2301CCEFE86A0CC40101CE9E0BFC3AE43EF807C107C3CFC668654D4964E5`. Receipt: `logs/independent_shared_csi_candidate_v2_20260916_source_binding.json` in the development checkout.

- The new paired shared-waveform test passed in 463.61 seconds. Both actual receptions completed normal HARQ and CSI publication, duplicate rejection and the normal reducer; removing UE CSI bookkeeping and poisoning expected CSI metadata left the full `LatestDLFeedback` value identical.
- Its two actual received-CSI CSVs have 122 columns. The only differing column is `UEReferenceRecordJSON`; receiver-owned fields are identical. CQI=15, RI=1, PMI=3; raw UE SINR and source-data CRC remain NaN. Reference slot=6, due/delivered slot=10, observation end and availability sample=76792. These are this high-margin fixture's results, not a 12 dB measurement.
- Actual artifact roots under `logs/tdd_shared_pusch_independent_csi/`: `tpcf13b7cb_9c97_432b_8f0e_085c936551e1` (normal) and `tpfafff5ae_997e_4723_9173_6c501421f09a` (removed producer record).
- The original shared-HARQ-only completion passed in 110.77 seconds. Received-CSI waveform, partial CSI, CSI source authority, causal AMC, grant consistency and export pipeline tests also passed.

This closes the demonstrated missing-publication defect and its duplicate-guard regression for the tested normal shared-PUSCH component path. It does **not** close full coordinator/PUCCH/SR integration, all missing-DCI/epoch/transport combinations, malformed optional-audit handling, detector qualification, SRS/measurement gaps, final-source unfiltered `testAll`, R2023b execution or integrated 12 dB acceptance. The original failed captures remain preserved. No qualified-main merge or push is claimed.

At 14:02:12 IST the same frozen source began `logs/independent_shared_csi_required_guards_20260916.log`: ten selected config/strict, DL/UL/reference-point, artifact-preservation and both required E2E tests. Main MATLAB PID 24704, launcher 13464, execution session 52661. Its prelaunch receipt binds the same 50 source files; all matched at 14:02:46 IST. This batch is running, not passed, and is not unfiltered `testAll`. The older `d05c1441` full suite remains separate and cannot qualify this candidate; the `5d20e64d` queue still reports waiting, not a successful 12 dB run. No additional full-suite queue was created.
