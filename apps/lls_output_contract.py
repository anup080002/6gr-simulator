...
DOMAIN_COLUMNS = {
    ...
    "measurement": "ConfiguredSNR_dB ConfiguredSNRSource SNRValueRole AppliedAWGNSNR_dB MeasuredTrialSINR_dB MeasuredTrialSINRSource EstimatedWidebandSINR_dB MeasuredWidebandSINR_dB LargeScaleWidebandSINR_dB LargeScaleSINR_dB LargeScaleSINRSource ServingRSRP_dBm ServingRSRPSource RSRP_dBm RSRPSource CSI_RSRP_dB CSI_RSRPSource AppliedLargeScaleGain_dB AppliedLargeScaleGainSource WidebandCQI PMI RI CRI SSBRI RankEstimate measurement_window_id filter_id report_trigger_reason report_status",
...
ANALYTICS_SECTIONS = [
    ...
    _s("channel-estimation-propagation-analytics", "Channel Estimation / Propagation Analytics", "channel", "channel", "channel_estimation_analytics channel_quality_analytics propagation_analytics", "true H(tau) if available;estimated Hhat(tau);true H(f) if available;estimated Hhat(f);channel magnitude heatmap;channel phase heatmap;tap power profile;delay spread chart;angle spread chart;pathloss distribution;shadowing distribution;O2I distribution;noise variance trend;NMSE vs SNR / SINR;estimator bias / variance summaries;serving vs interferer decomposition"),
    ...
    _s("beamforming-precoding-mimo-analytics", "Beamforming / Precoding / MIMO Analytics", "beam_mimo", "beam_mimo", "beam_analytics mimo_analytics user_grouping_analytics precoder_analytics", "beam id timeline;beam pair timeline;selected vs best beam gap;beam hit rate / top-K hit rate;beam gain gap histogram;precoder mode distribution;combiner summary;rank distribution;per-layer SINR;inter-user leakage;MU grouping analytics;condition number distribution;calibration / reciprocity diagnostics if modeled"),
    ...
    _s("power-energy-efficiency-analytics", "Power / Energy / Efficiency Analytics", "power_energy", "power_energy", "power_analytics energy_efficiency_analytics runtime_power_analytics sleep_state_analytics", "DL Tx power per cell / beam / UE;UL Tx power per UE;power control behavior;PA backoff distribution;RF chain power;baseband power;CPU power if available;power vs throughput;power vs BLER;energy/bit;joules/GB;energy efficiency by UE;energy efficiency by cell;sleep/idle/active state occupancy;PAPR vs power;thermal/throttling analytics if available"),
    _s("runtime-compute-complexity-parallelism-analytics", "Runtime / Compute / Complexity / Parallelism Analytics", "runtime", "power_energy", "compute_analytics runtime_latency_analytics determinism_analytics parallelism_analytics", "block execution time;stage latency;end-to-end latency;compute latency;decode latency;CPU cycles;memory usage;worker timelines;lock/contention observations;DB write latency;export lag;single-thread vs multi-thread determinism;dropped row / duplicate write analytics;decoder complexity units;normalized decoder complexity"),
