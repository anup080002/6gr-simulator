...
testInterface = localRequireEnumText(entry.test_interface, ...
    ["conducted", "radiated_RIB"], ...
...
if testInterface == "radiated_RIB"
    if ~isfield(entry, "test_interface_contract")
        localInvalid(catalogPath, scope + ".test_interface_contract", ...
            "is required for a radiated_RIB reference point");
    end
    localValidateRadiatedRIBContract( ...
        entry.test_interface_contract, mimo, scope, catalogPath);
end
...
allocationSymbols = allocation.start_symbol + ...
    (0:allocation.symbol_length-1);
if any(~ismember(dmrs.symbol_indices, allocationSymbols))
    localInvalid(catalogPath, scope + ".dmrs.symbol_indices", ...
        "must lie inside the configured time-domain allocation");
end
if string(entry.allocation.mapping_type) == "A" && ...
        dmrs.symbol_indices(1) ~= double(entry.dmrs.type_a_position)
    localInvalid(catalogPath, scope + ".dmrs.symbol_indices", ...
        "must start at dmrs.type_a_position for mapping Type A");
elseif string(entry.allocation.mapping_type) == "B" && ...
        dmrs.symbol_indices(1) ~= allocation.start_symbol
    localInvalid(catalogPath, scope + ".dmrs.symbol_indices", ...
        "must start at allocation.start_symbol for mapping Type B");
end
dmrsLength = 1 + double(string(entry.dmrs.duration) == "double_symbol");
expectedDMRSSymbolCount = ...
    (double(entry.dmrs.additional_position) + 1) * dmrsLength;
if numel(dmrs.symbol_indices) ~= expectedDMRSSymbolCount
    localInvalid(catalogPath, scope + ".dmrs.symbol_indices", sprintf( ...
        "must contain %d symbol(s) for duration '%s' and additional_position=%d", ...
        expectedDMRSSymbolCount, string(entry.dmrs.duration), ...
        double(entry.dmrs.additional_position)));
end
...
localRequireFields(input, { ...
    ...
    "ports", "symbol_indices", "data_to_dmrs_epre_db"}, scope, catalogPath);
...
dmrs.symbol_indices = localRequireIntegerVector( ...
    input.symbol_indices, scope + ".symbol_indices", catalogPath, false);
if isempty(dmrs.symbol_indices) || ...
        any(dmrs.symbol_indices < 0 | dmrs.symbol_indices > 13) || ...
        numel(unique(dmrs.symbol_indices)) ~= numel(dmrs.symbol_indices) || ...
        ~issorted(dmrs.symbol_indices)
    localInvalid(catalogPath, scope + ".symbol_indices", ...
        "must be a nonempty, strictly increasing vector in the range 0..13");
end
...
mimo.rx_antennas = localRequirePositiveInteger( ...
    input.rx_antennas, scope + ".rx_antennas", catalogPath);
...
function localValidateRadiatedRIBContract(input, mimo, recordScope, catalogPath)
scope = recordScope + ".test_interface_contract";
localRequireScalarStruct(input, scope, catalogPath);
localRequireFields(input, { ...
    "reference_plane", "rib_count", "rib_labels", ...
    "receiver_branch_by_rib", "polarization_basis", ...
    "noise_variance_policy", "standard_reference"}, scope, catalogPath);
localRequireEnumText(input.reference_plane, ...
    "radiated_interface_boundary", scope + ".reference_plane", catalogPath);
ribCount = localRequirePositiveInteger(input.rib_count, ...
    scope + ".rib_count", catalogPath);
labels = localRequireTextVector(input.rib_labels, ...
    scope + ".rib_labels", catalogPath);
branches = localRequireIntegerVector(input.receiver_branch_by_rib, ...
    scope + ".receiver_branch_by_rib", catalogPath, false);
localRequireEnumText(input.polarization_basis, ...
    "orthogonal_test_polarizations", ...
    scope + ".polarization_basis", catalogPath);
localRequireEnumText(input.noise_variance_policy, ...
    "equal_independent_variance_per_rib", ...
    scope + ".noise_variance_policy", catalogPath);
localRequireNonemptyText(input.standard_reference, ...
    scope + ".standard_reference", catalogPath);
if ribCount ~= double(mimo.rx_antennas) || numel(labels) ~= ribCount || ...
        ~isequal(branches, 1:ribCount) || numel(unique(labels)) ~= ribCount
    localInvalid(catalogPath, scope, ...
        "must map each distinct RIB label bijectively to one receiver branch");
end
end
...
selectedDuplex = string(input.selected_duplex_mode);
if selectedDuplex == "TDD"
    localRequireFields(input, { ...
        "tdd_pattern_period_slots", "tdd_slot_formats", ...
        "first_ul_opportunity_slot", ...
        "special_slot_downlink_symbols", ...
        "special_slot_guard_symbols", ...
        "special_slot_uplink_symbols"}, scope, catalogPath);
    periodSlots = localRequirePositiveInteger( ...
        input.tdd_pattern_period_slots, ...
        scope + ".tdd_pattern_period_slots", catalogPath);
    slotFormats = upper(localRequireTextVector( ...
        input.tdd_slot_formats, scope + ".tdd_slot_formats", catalogPath));
    if numel(slotFormats) ~= periodSlots || ...
            any(~ismember(slotFormats, ["D", "S", "U"]))
        localInvalid(catalogPath, scope + ".tdd_slot_formats", ...
            "must contain exactly one D, S, or U token per TDD pattern slot");
    end
    firstUL = localRequireNonnegativeInteger( ...
        input.first_ul_opportunity_slot, ...
        scope + ".first_ul_opportunity_slot", catalogPath);
    if firstUL >= periodSlots || slotFormats(firstUL + 1) ~= "U"
        localInvalid(catalogPath, scope + ".first_ul_opportunity_slot", ...
            "must identify a full U slot in tdd_slot_formats");
    end
    nDL = localRequireNonnegativeInteger( ...
        input.special_slot_downlink_symbols, ...
        scope + ".special_slot_downlink_symbols", catalogPath);
    nGuard = localRequireNonnegativeInteger( ...
        input.special_slot_guard_symbols, ...
        scope + ".special_slot_guard_symbols", catalogPath);
    nUL = localRequireNonnegativeInteger( ...
        input.special_slot_uplink_symbols, ...
        scope + ".special_slot_uplink_symbols", catalogPath);
    if nDL + nGuard + nUL ~= 14
        localInvalid(catalogPath, scope, ...
            "TDD special-slot D/G/U symbol counts must sum to 14");
    end
    if spacing ~= periodSlots
        localInvalid(catalogPath, scope + ".retransmission_spacing_slots", ...
            "must equal tdd_pattern_period_slots for successive full-UL opportunities");
    end
end
