...
%   consumed by sixgr.conformance.addReferenceNoise.
%
...
%   exists. For the one-layer CP-OFDM FRCs supported here,
%
%       S = NdataRE * Edata + NdmrsRE * Edmrs
...
if numLayers ~= 1 || numPorts ~= 1
    error("sixgr:conformance:UnsupportedPUSCHSNREnergyMapping", ...
        ["Exact TS 38.104 connector-energy accounting currently supports " ...
        "one PUSCH layer on one antenna port; got NumLayers=%g and " ...
        "NumAntennaPorts=%g."], numLayers, numPorts);
end
...
dataRECount = numel(dataIndices);
dmrsRECount = numel(dmrsIndices);
dataSignalEnergy = dataRECount * dataEPRE;
dmrsSignalEnergy = dmrsRECount * dataEPRE * dmrsPowerScale;
...
[rxWaveform, base] = sixgr.conformance.addReferenceNoise( ...
...
provenance = base;
provenance.Version = "ts38104_pusch_reference_noise/v1";
...
provenance.SignalEnergyEquation = ...
    "S = PUSCHDataRECount*DataEPRE + PUSCHDMRSRECount*DMRSEPRESameUnits";
