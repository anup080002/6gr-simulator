    ...
    for speedIndex = 1:numel(speeds)
        for ageIndex = 1:numel(ages)
            uePositionBound = double(cfg.gnss_degraded.initial_ue_position_error_los_m) + ...
                speeds(speedIndex)*ages(ageIndex)*cos(elevation) + ...
...
for ei=1:numel(elev)
    ...
    rtt=2*geo.Range_m/c; minimumRTT=min(rtt); differentialRTT=rtt-minimumRTT;
    for si=1:numel(scs)
        tslot=1e-3/(scs(si)/15); ue=ceil(rtt/tslot).*tslot;
        beam=ceil(max(rtt)/tslot)*tslot+zeros(n,1); part=part+1;
        rawParts{part}=table(repmat(elev(ei),n,1),repmat(scs(si),n,1), ...
            rtt,repmat(minimumRTT,n,1),differentialRTT,ue,beam,ue-rtt,beam-rtt, ...
            'VariableNames',{'Elevation_deg','SCS_kHz','RTT_s','BeamMinimumRTT_s', ...
            'DifferentialRTT_s','UEConfiguredDelay_s','BeamConfiguredDelay_s', ...
            'UEExcessDelay_s','BeamExcessDelay_s'});
    end
end
raw=vertcat(rawParts{:});
for si=1:numel(scs)
    mask=raw.SCS_kHz==scs(si); tslot=1e-3/(scs(si)/15);
    cellDelay=ceil(max(raw.RTT_s(mask))/tslot)*tslot;
    raw.CellConfiguredDelay_s(mask)=cellDelay;
end
raw.CellExcessDelay_s=raw.CellConfiguredDelay_s-raw.RTT_s;
raw.CoverageOk=raw.UEConfiguredDelay_s+eps>=raw.RTT_s & ...
...
raw.Properties.VariableUnits={'deg','kHz','s','s','s','s','s','s','s','s','s','',''};
...
summary=groupsummary(raw,{'Elevation_deg','SCS_kHz'}, ...
    {'min','max','mean'},{'RTT_s','DifferentialRTT_s','UEExcessDelay_s', ...
    'BeamExcessDelay_s','CellExcessDelay_s'});
summary.Provenance=repmat("SCHEDULER_SYSTEM",height(summary),1);
summary.Properties.VariableUnits=repmat({''},1,width(summary));
slotDuration=1e-3./(raw.SCS_kHz/15);
processing=(double(cfg.k_offset.processing_delay_ms)+ ...
    double(cfg.k_offset.configured_wait_ms))*1e-3;
pipeline=table(raw.Elevation_deg,raw.SCS_kHz,raw.RTT_s, ...
    raw.BeamConfiguredDelay_s,raw.CellConfiguredDelay_s, ...
    ceil((raw.RTT_s+processing)./slotDuration), ...
    ceil((raw.BeamConfiguredDelay_s+processing)./slotDuration), ...
    ceil((raw.CellConfiguredDelay_s+processing)./slotDuration), ...
    repmat(processing,height(raw),1), ...
    repmat("per_ue_per_beam_global_cell_absolute_rtt",height(raw),1), ...
    repmat("SCHEDULER_SYSTEM",height(raw),1), ...
    'VariableNames',{'Elevation_deg','SCS_kHz','UERTT_s','BeamReferenceRTT_s', ...
    'CellReferenceRTT_s','UEPipelineSlots','BeamPipelineSlots', ...
    'CellPipelineSlots','ProcessingDelay_s','Interpretation','Provenance'});
pipeline.Properties.VariableUnits={'deg','kHz','s','s','s','slot','slot','slot','s','',''};

updateElev=double(cfg.k_offset.rtt_update_elevations_deg(:));
updateRows=cell(numel(updateElev)*numel(scs),1); updateIndex=0;
for ei=1:numel(updateElev)
    sat=sixgr.ntn.resilientsync.geometry.solveSatelliteStateForElevation( ...
        R,h,deg2rad(updateElev(ei)),mu);
    center=sixgr.ntn.resilientsync.geometry.slantRangeAndRate( ...
        sat.SatellitePositionECEF_m,sat.SatelliteVelocityECEF_m_s, ...
        sat.UEPositionECEF_m,sat.UEVelocityECEF_m_s);
    absDRTT=2*abs(double(center.RangeRate_m_s))/c;
    for si=1:numel(scs)
        tslot=1e-3/(scs(si)/15); updateIndex=updateIndex+1;
        if absDRTT<=eps, timeToSlot=Inf; else, timeToSlot=tslot/absDRTT; end
        updateRows{updateIndex}={updateElev(ei),scs(si),tslot, ...
            double(center.RangeRate_m_s),absDRTT,timeToSlot,"ANALYTICAL"};
    end
end
rttUpdate=cell2table(vertcat(updateRows{:}),'VariableNames', ...
    {'Elevation_deg','SCS_kHz','SlotDuration_s','RangeRate_m_s', ...
    'AbsoluteRTTRate_s_s','TimeToOneSlot_s','Provenance'});
rttUpdate.Properties.VariableUnits={'deg','kHz','s','m/s','s/s','s',''};
result=struct("Campaign","E","Status","COMPLETE","Tables", ...
    struct("k_offset_samples",raw,"k_offset_summary",summary, ...
    "timing_pipeline",pipeline,"rtt_update_timescale",rttUpdate), ...
    "Evidence",localEvidence(["k_offset_samples";"k_offset_summary"; ...
    "timing_pipeline";"rtt_update_timescale"], ...
    [repmat("SCHEDULER_SYSTEM",3,1);"ANALYTICAL"]));
end
