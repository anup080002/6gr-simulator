...
samplingPlan = lower(string(localAllowedText(cfg, "simulation.samplingPlan", ...
    ["fixed_sample","adaptive_error_count_diagnostic"])));
intervalMethod = lower(string(localAllowedText(cfg, ...
...
if string(cfg.simulation.statisticalClass) == "publication_candidate" && ...
        samplingPlan ~= "fixed_sample"
    error("sixgr:lls:PublicationSamplingPlanInvalid", ...
        "Publication-candidate LLS requires simulation.samplingPlan=fixed_sample. " + ...
        "Adaptive peeking at fixed-sample Wilson intervals is not permitted.");
end
if string(cfg.simulation.statisticalClass) == "publication_candidate" && ...
        intervalMethod ~= "wilson_two_sided_fixed_sample"
    error("sixgr:lls:PublicationIntervalMethodInvalid", ...
        "Publication-candidate LLS requires confidenceIntervalMethod=wilson_two_sided_fixed_sample.");
end
...
if logical(sixgr.util.structGet(cfg,"linkAdaptation.enabled",false))
    ...
    thresholdComparisonToleranceDb = localRequiredFinite(cfg, ...
        "linkAdaptation.thresholdComparisonToleranceDb");
    if thresholdComparisonToleranceDb < 0 || thresholdComparisonToleranceDb > 1e-3
        error("sixgr:lls:InvalidLinkAdaptationPolicy", ...
            ["linkAdaptation.thresholdComparisonToleranceDb must be between " + ...
            "0 and 1e-3 dB. It is a floating-point equality guard, not a link margin."]);
    end
    ...
    feedbackDelay = localRequiredInteger(cfg, ...
        "linkAdaptation.feedbackDelaySlots",1,1024); %#ok<NASGU>
    outOfRangePolicy = lower(string(localAllowedText(cfg, ...
        "linkAdaptation.outOfRangeCQIPolicy", ...
        ["block_scheduling","minimum_mcs_waveform_probe"])));
    if outOfRangePolicy == "minimum_mcs_waveform_probe" && ...
            string(cfg.simulation.statisticalClass) ~= "diagnostic_only"
        error("sixgr:lls:DiagnosticCQIProbeForbidden", ...
            "linkAdaptation.outOfRangeCQIPolicy=minimum_mcs_waveform_probe " + ...
            "is permitted only when simulation.statisticalClass=diagnostic_only. " + ...
            "Publication and production scheduling must use block_scheduling.");
    end
    localRequiredLogical(cfg,"linkAdaptation.olla.enabled");
    ollaTargetBLER = localRequiredFinite(cfg,"linkAdaptation.olla.targetBLER");
    if ~(ollaTargetBLER > 0 && ollaTargetBLER < 1)
        error("sixgr:lls:InvalidLinkAdaptationPolicy", ...
            "linkAdaptation.olla.targetBLER must be strictly between zero and one.");
    end
    ...
    impliedTargetBLER = ackStep / (ackStep + nackStep);
    if logical(sixgr.util.structGet(cfg,"linkAdaptation.olla.enabled",false)) && ...
            abs(impliedTargetBLER-ollaTargetBLER) > 1e-12
        error("sixgr:lls:OLLATargetBLERStepMismatch", ...
            "For the configured OLLA sign convention, targetBLER must equal " + ...
            "ackStepDb/(ackStepDb+nackStepDb). Configured target=%.12g, " + ...
            "step-implied target=%.12g.",ollaTargetBLER,impliedTargetBLER);
    end
end
