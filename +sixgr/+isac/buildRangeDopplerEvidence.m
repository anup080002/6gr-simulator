...
for i=1:numel(symbols)
    symbol=symbols(i);
    active=mask(:,symbol);
    product=rxGrid(active,symbol).*conj(bundle.SensingGrid(active,symbol));
...
mapTable=table(repmat(delayClass,numel(power),1),rangeGrid(:),dopplerGrid(:), ...
    ...
    'VariableNames',{'DelayClass','RangeM','DopplerHz','PowerLinear', ...
