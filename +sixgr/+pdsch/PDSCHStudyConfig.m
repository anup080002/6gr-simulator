...
tdra = struct();
tdra.MappingType = char(upper(string(sixgr.util.structGet(pdsch6gr, "MappingType", "A"))));
...
function context = localResolveMCSContext(pdsch6gr)
context = struct( ...
    "UECapability1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSUECapability1024QAM",[]), ...
    "RRCEnabled1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSRRCEnabled1024QAM",[]), ...
    "DCIEnabled1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSDCIEnabled1024QAM",[]), ...
    "DeploymentAllows1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSDeploymentAllows1024QAM",[]), ...
    "FrequencyRange", string(sixgr.util.structGet( ...
        pdsch6gr,"MCSFrequencyRange","")), ...
    "OperatingBand", string(sixgr.util.structGet( ...
        pdsch6gr,"MCSOperatingBand","")), ...
    "DeploymentClass", string(sixgr.util.structGet( ...
        pdsch6gr,"MCSDeploymentClass","")), ...
    "FrequencyRangeAllows1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSFrequencyRangeAllows1024QAM",[]), ...
    "BandAllows1024QAM", sixgr.util.structGet( ...
        pdsch6gr,"MCSBandAllows1024QAM",[]));
logicalFields = [ ...
...
for idx = 1:numel(logicalFields)
    name = logicalFields(idx);
    if ~(islogical(context.(name)) && isscalar(context.(name)))
        error("sixgr:pdsch:PDSCHStudyConfig:MissingMCSCapabilityContext", ...
            "%s must be configured as an explicit logical scalar.",name);
    end
end
textFields = ["FrequencyRange","OperatingBand","DeploymentClass"];
for idx = 1:numel(textFields)
    name = textFields(idx);
    if ~isscalar(context.(name)) ...
            || strlength(strtrim(context.(name))) == 0
        error("sixgr:pdsch:PDSCHStudyConfig:MissingMCSDeploymentContext", ...
            "%s must be configured as an explicit nonempty token.",name);
    end
end
end
