...
addParameter(p, "InterfererActivePreamblePattern", [], @(x) isempty(x) || isnumeric(x) || islogical(x));
addParameter(p, "InterfererPreambleIndex", [], @(x) isempty(x) || isnumeric(x));
...
prachCfg.InterfererActivePreamblePattern = localResolveArray(localFirstNonEmpty( ...
    opts.InterfererActivePreamblePattern,localResolveField(cfg, ...
    {"prach_lls.InterfererActivePreamblePattern", ...
    "random_access.interferer_active_preamble_pattern"}, ...
    prachCfg.ActivePreamblePattern)));
prachCfg.InterfererPreambleIndex = localResolveArray(localFirstNonEmpty( ...
    opts.InterfererPreambleIndex,localResolveField(cfg, ...
    {"prach_lls.InterfererPreambleIndex", ...
    "random_access.interferer_preamble_index"}, ...
    mod(double(prachCfg.PreambleIndex)+7,64))));
...
if isfield(serializable, "InterfererActivePreamblePattern")
    serializable.InterfererActivePreamblePattern = double(serializable.InterfererActivePreamblePattern);
end
end
...
sixgr.config.publishConfigApplicationEvidence("record", ...
    "random_access.enabled", "Random_Access_PRACH", "phy.prach.enable", consumer, sixgr.util.structGet(cfg, "phy.prach.enable", false), ...
...
sixgr.config.publishConfigApplicationEvidence("record", ...
    "random_access.msg1_fdm", "Random_Access_PRACH", "prach_lls.Msg1FDM", consumer, prachCfg.Msg1FDM, ...
    "RuntimeObjectType", objectType, "RuntimeObjectPath", "prachCfg.Msg1FDM", "ApplicationScope", scope);
sixgr.config.publishConfigApplicationEvidence("record", ...
