...
for sourceIndex = 1:numel(sources)
    ...
    [csvT, encodedColumns] = localCSVSerializableTable(T);
    ...
    rows(end + 1, 1) = struct( ... %#ok<AGROW>
        ...
        "LogicalColumnCount", width(T), ...
        "SerializationContract", "sixgr_raw_table_csv/v2", ...
        "EncodedColumns", strjoin(encodedColumns, "|"), ...
...
function [csvT, encodedColumns] = localCSVSerializableTable(T)
csvT = T;
encodedColumns = strings(0, 1);
...
for variableIndex = 1:numel(names)
    ...
    for rowIndex = 1:height(T)
        ...
        tokens(rowIndex) = sixgr.runtime.RawCSVArrayCodec.encode(rowValue);
    end
    csvT.(char(name)) = tokens;
    encodedColumns(end + 1, 1) = name; %#ok<AGROW>
end
end

function tf = localRequiresArrayToken(value)
...
elseif ischar(value)
    ...
    if tf
        error("sixgr:runtime:RawCSVArrayTypeUnsupported", ...
            "Raw table cell contains unsupported class %s.", class(value));
    end
end
end
if isempty(rows)
...
row = struct("SchemaName", "", "SchemaVersion", "", ...
    ...
    "ColumnCount", 0, "LogicalColumnCount", 0, ...
    "SerializationContract", "", "EncodedColumns", "", ...
    "ExcludedNonInPathRows", 0, ...
