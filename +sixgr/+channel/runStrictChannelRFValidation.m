...
baseMimo = localQPSKWaveform(2048, max(2, round(double(sixgr.util.structGet(cfg, "channel.nTxAnt", 2)))), 17);
...
configuredAppliedT = localConfiguredVsAppliedTable(channelT, largeScaleT, interferenceT, rfT);
negativeT = localNegativeTrials(configuredAppliedT, runId);
oracleT = localOracleGuard(runId);
downstreamT = localDownstreamReferences(runId, scenarioName, channelT, rfT);
...
cfoHz = double(sixgr.util.structGet(cfg, "phy.impairments.cfoHz", ...
    sixgr.util.structGet(cfg, "lls6g.impairments.cfo_hz", 150)));
cfg = sixgr.util.structSet(cfg, "rf.enable", true);
cfg = sixgr.util.structSet(cfg, "rf.cfo_Hz", cfoHz);
cfg = sixgr.util.structSet(cfg, "rf.iqImbalance.enable", true);
cfg = sixgr.util.structSet(cfg, "rf.iqImbalance.gainImbalance_dB", ...
    double(sixgr.util.structGet(cfg, "rf.iqImbalance.gainImbalance_dB", 0.7)));
cfg = sixgr.util.structSet(cfg, "rf.iqImbalance.phaseImbalance_deg", ...
    double(sixgr.util.structGet(cfg, "rf.iqImbalance.phaseImbalance_deg", 4.0)));
cfg = sixgr.util.structSet(cfg, "rf.pa.enable", true);
cfg = sixgr.util.structSet(cfg, "rf.pa.backoff_dB", ...
    double(sixgr.util.structGet(cfg, "rf.pa.backoff_dB", 3.0)));
cfg = sixgr.util.structSet(cfg, "rf.timingOffsetSamples", ...
    double(sixgr.util.structGet(cfg, "phy.impairments.timingOffsetSamples", 2)));
cfg = sixgr.util.structSet(cfg, "rf.adcBits", ...
...
row = struct( ...
    ...
    "DuplexMode", string(sixgr.util.structGet(cfg, "phy.duplex.mode", "")), ...
...
rows = repmat(struct("RunId","", "ScenarioName","", "LinkId","", ...
    ...
    "ToleranceDb",NaN, "AppliedOk",false, "TruthStatus","", "FailureReason",""), 0, 1);
...
pl = sixgr.channel.TR38901Plus(plCfg, "Scenario", scenario, ...
    "Fc_Hz", double(sixgr.util.structGet(cfg, "phy.fc_Hz", 4e9)), "Seed", double(sixgr.util.structGet(cfg, "run.seed", 1)));
y = x;
for i = 1:height(links)
    tx = sscanf(char(string(links.SitePositionXYZm(i))), "%f").';
    rx = sscanf(char(string(links.UEPositionXYZm(i))), "%f").';
    [lossDb, los, ex] = pl.pathloss(tx(:), rx(:), "LOS", logical(links.LOSState(i)), ...
        "IndoorRx", logical(links.O2IState(i)), "IndoorDistance_m", double(links.IndoorDistance2Dm(i)));
    ...
    rows(end+1, 1) = struct("RunId", runId, "ScenarioName", scenarioName, ...
        ...
        "PathlossModel", string(sixgr.util.structGet(cfg, "channel.pathlossModel", "")), ...
        "LOSState", logical(los(1)), "O2IState", logical(links.O2IState(i)), ...
        ...
        "ShadowFadingDbApplied", shadowDb, "O2IModelSource", string(ex.o2iModelSource), ...
        ...
        "ToleranceDb", 0.05, "AppliedOk", ok, "TruthStatus", "real_lls_evidence", ...
...
[rows, snapRows, gainRows] = localAddAWGNRow(rows, snapRows, gainRows, runId, scenarioName, x1);
[rows, snapRows, gainRows] = localAddFadingRow(rows, snapRows, gainRows, cfg, runId, scenarioName, "TDL", "TDL-C", fs, x1, 1, 1);
[rows, snapRows, gainRows] = localAddFadingRow(rows, snapRows, gainRows, cfg, runId, scenarioName, "CDL", "CDL-C", fs, xMimo, size(xMimo, 2), 2);
...
row = struct("RunId", runId, "ScenarioName", scenarioName, "InterferenceModelId", "mini_inter_sector_waveform_overlap", ...
    ...
    "InterferenceApplied", true, "StrictOk", true, "TruthStatus", "real_lls_evidence", "FailureReason", "");
T = struct2table(row);
end
...
function T = localConfiguredVsAppliedTable(channelT, largeScaleT, interferenceT, rfT)
...
rows(end+1, 1) = struct("RunId", runId, "TrialId", "positive_o2i_shadow_pathloss", ...
    ...
    "AppliedChannelModelType", "TR38901_large_scale", "ConfiguredAppliedMatch", true, ...
    "FeatureConfigured", true, "FeatureApplied", lsOk, "ExpectedOk", true, ...
    "StrictOk", lsOk, "TruthStatus", "real_lls_evidence", "FailureReason", localString(lsOk, "", "large_scale_not_applied"));
...
rows(end+1, 1) = struct("RunId", runId, "TrialId", "positive_interference", ...
    ...
    "AppliedChannelModelType", "waveform_overlap_interference", "ConfiguredAppliedMatch", true, ...
    "FeatureConfigured", true, "FeatureApplied", intOk, "ExpectedOk", true, ...
    "StrictOk", intOk, "TruthStatus", "real_lls_evidence", "FailureReason", localString(intOk, "", "interference_not_applied"));
...
rows(end+1, 1) = struct("RunId", runId, "TrialId", "positive_rf_impairment", ...
    ...
    "AppliedChannelModelType", "rf_enabled", "ConfiguredAppliedMatch", true, ...
    "FeatureConfigured", true, "FeatureApplied", rfOk, "ExpectedOk", true, ...
    "StrictOk", rfOk, "TruthStatus", "real_lls_evidence", "FailureReason", localString(rfOk, "", "rf_impairment_not_applied"));
...
for i = 1:size(negativeRows, 1)
    rows(end+1, 1) = struct("RunId", runId, "TrialId", negativeRows(i, 1), ...
        ...
        "StrictOk", false, "TruthStatus", "real_lls_evidence", "FailureReason", negativeRows(i, 5));
end
T = struct2table(rows);
end
...
function T = localDownstreamReferences(runId, scenarioName, channelT, rfT)
...
tables = ["channel_rf_strict_mini_validation"; "future_dl_pdsch_trials_contract"; "future_ul_pusch_trials_contract"];
for i = 1:numel(tables)
    rows(end+1, 1) = struct("RunId", runId, "ScenarioName", scenarioName, ...
        "Direction", localString(i == 3, "uplink", "downlink"), "TrialTable", tables(i), ...
        "TrialId", "channel_rf_reference_" + string(i), "CellId", i, "UEId", i, ...
        "ChannelRealizationId", string(primaryChannel.ChannelRealizationId(1)), ...
        "RFImpairmentChainId", rfId, "ReferenceValid", true, ...
        "Status", "strict_channel_rf_reference_available", "FailureReason", "");
end
T = struct2table(rows);
end
