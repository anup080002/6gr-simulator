...
switch cmd
    case "reset"
        ...
        out = state.Context;
    case "record"
        row = localBuildRow(state.Context, varargin{:});
...
ctx = struct( ...
    "RunId", double(sixgr.util.structGet(meta, "RunId", NaN)), ...
...
function row = localBuildRow(ctx, parameterId, featureFamily, internalCfgPath, consumerFunction, appliedValue, varargin)
p = inputParser;
