...
if isfield(cfg,'phy') && isa(cfg.phy,'struct')
    ...
    defaultDLMCS = char(localNormalizeMCSTable(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.pdsch.mcsTable", []), ...
        localDefaultMCSTableForModulation(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.pdsch.maxModulation", []), ...
        sixgr.util.structGet(cfg, "phy.pdsch.modulation", "16QAM"))))));
    defaultULMCS = char(localNormalizeMCSTable(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.pusch.mcsTable", []), ...
        localDefaultMCSTableForModulation(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.pusch.maxModulation", []), ...
        sixgr.util.structGet(cfg, "phy.pusch.modulation", "16QAM"))))));
    inferredDLCQI = char(localDefaultCQITableForMCSTable(defaultDLMCS));
    inferredULCQI = char(localDefaultCQITableForMCSTable(defaultULMCS));
    globalCQITableRaw = sixgr.util.structGet(cfg, "phy.csi.cqiTable", []);
    ...
    cfg.phy.csi.maxTrustedReferenceSINR_dB = double(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.csi.maxTrustedReferenceSINR_dB", []), 45));
    cfg.phy.linkAdaptation = localStructEnsure(cfg.phy, 'linkAdaptation');
    ...
    cfg.phy.linkAdaptation.muMIMOFirstDataBootstrapEnabled = logical(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.linkAdaptation.muMIMOFirstDataBootstrapEnabled", []), false));
    cfg.phy.linkAdaptation.muMIMOFirstDataBootstrapPolicy = char(lower(strtrim(string(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.linkAdaptation.muMIMOFirstDataBootstrapPolicy", []), ...
        "yaml_mcs_until_first_data_feedback")))));
    cfg.phy.linkAdaptation.dlMUMIMOFirstDataBootstrapMCSIndex = max(0, min(31, round(double(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.linkAdaptation.dlMUMIMOFirstDataBootstrapMCSIndex", []), ...
        cfg.phy.linkAdaptation.bootstrapMCSIndex)))));
    cfg.phy.linkAdaptation.ulMUMIMOFirstDataBootstrapMCSIndex = max(0, min(31, round(double(localFirstNonEmpty( ...
        sixgr.util.structGet(cfg, "phy.linkAdaptation.ulMUMIMOFirstDataBootstrapMCSIndex", []), ...
        cfg.phy.linkAdaptation.bootstrapMCSIndex)))));
...
function token = localDefaultCQITableForMCSTable(mcsTable)
scheme = lower(char(string(mcsTable)));
if contains(scheme, "256") || contains(scheme, "table2")
    token = 'table2';
else
    token = 'table1';
end
