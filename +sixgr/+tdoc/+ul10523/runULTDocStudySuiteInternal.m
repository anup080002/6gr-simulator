function result = runULTDocStudySuiteInternal(mode, options)
...
if any(mode==["audit","figure_replay"])
    ...
    [audit,passed,publicationPassed]=sixgr.tdoc.ul10523.ULTDocStudyArtifactAuditor.run(options.RunFolder);
...
deterministic=sixgr.tdoc.ul10523.ULTDocStudyDeterministicSuite.run();
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(runFolder,"tables","deterministic_checks.csv"), ...
...
figureManifest=sixgr.tdoc.ul10523.ULTDocStudyFigurePublisher.publish( ...
...
[audit,passed,publicationPassed]=sixgr.tdoc.ul10523.ULTDocStudyArtifactAuditor.run(runFolder);
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(runFolder,"reports","artifact_audit.csv"),audit,context);
...
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(root,"manifests", ...
...
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(root,"manifests","unit_scope.csv"),T,context);
end
...
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(root,"reports","limitations.csv"),T,context);
end
...
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(root,"manifests","environment_manifest.csv"),T,context);
end
...
for idx=1:numel(paths),hashes(idx)=sixgr.csi.studyFileSHA256(paths(idx));end
T=table(kind,paths(:),hashes(:),repmat(string(mode),3,1),repmat("PASS",3,1), ...
    'VariableNames',{'InputKind','SourcePath','SHA256','RunMode','Status'});
sixgr.tdoc.ul10523.ULTDocStudyResultWriter.write(fullfile(root,"manifests","input_manifest.csv"),T,context);
end
...
for idx=1:numel(files)
    ...
    rows(end+1,1)=struct("RelativePath",rel,"ArtifactType",erase(ext,"."), ... %#ok<AGROW>
        "SHA256",sixgr.csi.studyFileSHA256(full),"Bytes",files(idx).bytes, ...
...
for idx=1:numel(inputPaths),hashes(idx)=sixgr.csi.studyFileSHA256(inputPaths(idx));end
