classdef RARetryState
    ...
    methods
        ...
        function [obj,row]=responseExpired(obj,receiver,nowTicks)
            ...
            row.FailurePhase="rar_response_window";
            row.ContentionResolutionTicks=NaN;
            if isempty(obj.Events), obj.Events=row; else, obj.Events=[obj.Events;row]; end
        end
        function [obj,row]=contentionFailed(obj,receiver,rarReceiver,nowTicks)
            now=sixgr.phy.frame.AbsoluteTime.fromTicks(nowTicks);
            assert(obj.Status=="waiting_contention" && ...
                isa(receiver,'sixgr.phy.ra.Msg4ReceiveWindow') && ...
                any(receiver.MAC.Status==["expired","identity_mismatch"]) && ...
                receiver.RAConfig.RunId==obj.RunId && receiver.MAC.ResolutionTicks==now.Ticks && ...
                isa(rarReceiver,'sixgr.phy.ra.RARReceiveWindow') && rarReceiver.Status=="matched" && ...
                rarReceiver.RAConfig.RunId==obj.RunId, ...
                'sixgr:mac:ra:MissingUEContentionFailure',"Retry requires this UE's actual MAC failure and received RAR backoff.");
            previous=obj.TransmissionCounter; obj.TransmissionCounter=previous+1;
            exhausted=obj.TransmissionCounter==obj.PreambleTransMax+1;
            draw=NaN; backoffTicks=int64(0); earliest=NaN;
            if exhausted
                obj.Status="exhausted";
            else
                stream=RandStream('mt19937ar','Seed',obj.BackoffSeed); stream.State=obj.BackoffRNGState;
                draw=rand(stream); obj.BackoffRNGState=stream.State;
                backoffTicks=int64(ceil(rarReceiver.PreambleBackoff_ms*draw* ...
                    double(sixgr.phy.frame.AbsoluteTime.TicksPerSecond)/1000));
                next=now.plusTicks(backoffTicks); obj.EarliestNextAttemptTicks=next.Ticks;
                earliest=double(next.Ticks); obj.Status="ready";
            end
            row=table(obj.RunId,double(receiver.RAConfig.UEId),previous,obj.TransmissionCounter, ...
                obj.PowerRampingCounter,obj.PreambleTransMax,obj.CurrentReference, ...
                double(obj.CurrentTransmissionEndTicks),NaN,rarReceiver.PreambleBackoff_ms, ...
                string(rarReceiver.BackoffSource),obj.BackoffSeed,draw,double(backoffTicks),earliest, ...
                exhausted,obj.Status,"actual_ue_contention_failure_and_retry_state",false,false, ...
                'VariableNames',{'RunId','UEId','CompletedAttempt','NextTransmissionCounter', ...
                'PowerRampingCounter','PreambleTransMax','SelectedReference', ...
                'PreambleTransmissionEndTicks','ResponseExpiryTicks','PreambleBackoff_ms', ...
                'BackoffSource','BackoffSeed','UniformDraw','BackoffTicks','EarliestRetryTicks', ...
                'PreambleTransMaxExhausted','Status','Source','ProxyUsed','FallbackUsed'});
            row.FailurePhase="contention_"+receiver.MAC.Status;
            row.ContentionResolutionTicks=double(now.Ticks);
            if isempty(obj.Events), obj.Events=row; else, obj.Events=[obj.Events;row]; end
        end
