...
mandatory = [ ...
    ...
    "rar_grant.field_layout"
    "rar_grant.frequency_hopping"
    "rar_grant.time_resource_assignment"
    "rar_grant.tpc_command"
...
ra.RARGrantConfig = raNode.rar_grant;
ra.NCellID = double(cellId);
ra.NStartGrid = double(sixgr.util.structGet(cfg,"phy.carrier.NStartGrid",0));
...
ra.CarrierCyclicPrefix = string(sixgr.util.structGet(cfg, "phy.carrier.CyclicPrefix", "normal"));
% A received common TDRA list takes precedence over default A. Until its
% explicit list codec is supported, reject it instead of silently replacing
% the receiver's RRC authority with the default table.
commonPUSCH = sixgr.util.structGet(cfg, ...
    "UECommonCellConfiguration.PUSCHConfigCommon", struct());
for field = ["pusch_TimeDomainAllocationList", "pusch-TimeDomainAllocationList", ...
        "puschTimeDomainAllocationList"]
    if isfield(commonPUSCH, field) && ~isempty(commonPUSCH.(field))
        error("sixgr:mac:ra:UnsupportedRARCommonTDRAList", ...
            "Decoded PUSCH common TDRA list must be consumed; default A cannot replace it.");
    end
end
...
ra.PRACHOccasionEndSlot = ra.PRACHAbsoluteSlot + ...
    floor((startSymbol + durationSymbols - 1) / frame.SymbolsPerSlot);
for symbol = startSymbol:(startSymbol+durationSymbols-1)
    slot = ra.PRACHAbsoluteSlot + floor(symbol/frame.SymbolsPerSlot);
    if ~frame.IsULAllocation(slot,[mod(symbol,frame.SymbolsPerSlot) 1])
        error("sixgr:mac:ra:ConfiguredPRACHOccasionNotUL", ...
            "Configured PRACH occasion occupies a non-UL symbol at absolute slot %d symbol %d.", ...
            slot,mod(symbol,frame.SymbolsPerSlot));
    end
end
...
ra.Msg2PDSCH = localSched(raNode.msg2_pdsch, 0, false, false);
...
% Plan from the very grant fields that the transmitter will encode. Delays
% are minimum processing times; they do not override signalled K2 or turn
% fixed DL/flexible symbols into UL allocations.
grantPlan = sixgr.mac.ra.buildRARULGrant(ra);
if frame.Mu ~= grantPlan.TimeDomainAllocation.Mu
...
ra.TimingSchedule = sixgr.phy.ia.RAEventScheduler.resolve( ...
    ...
    "Msg3TimeDomainAllocation", grantPlan.TimeDomainAllocation, ...
    "Msg4ProcessingDelaySlots", double(timingNode.msg4_processing_delay_slots), ...
    "ContentionResolutionTimerSlots", ra.RAContentionResolutionTimerSlots, ...
    "SetupCompleteK2Slots", double(timingNode.setup_complete_k2_slots), ...
    "RequireSetupComplete", ra.RequireRRCSetupComplete, ...
    "FrameEngine", frame, ...
    "Msg2SymbolAllocation", [ra.Msg2PDSCH.SymbolStart ra.Msg2PDSCH.NumSymbols], ...
...
ra.SetupCompleteSlot = double(ra.TimingSchedule.SetupCompleteSlot);
localValidateProcedureSlots(ra);
ra.RARNTI = double(sixgr.phy.ra.computeRARNTI( ...
    "SymbolIndex", double(resolvedOccasion.RARNTISymbolIndex), ...
    "SlotIndex", double(resolvedOccasion.RARNTISlotIndex), ...
    "FrequencyIndex", ra.PRACHFrequencyIndex));

ra.RARDCIReference = sixgr.phy.ra.resolveRARDCIReference(cfg);
rarContext = sixgr.phy.pdcch.RARDCIContext.create(ra.RARDCIReference,ra.RARNTI);
...
ra.RACHConfigHash = sixgr.rrc.asn1.asn1SHA256Hex(uint8(unicode2native(hashSource, "UTF-8")));
