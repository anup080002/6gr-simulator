classdef FullStackRegressionRunner
    %FULLSTACKREGRESSIONRUNNER Execute eleven gates within the SC-30 budget.
    %
    ...
    methods (Static)
        ...
        function T = run(ctx)
            ...
            code = localTestAllCode(countPath, ...
                "testAll('ExcludeNames',{'test6GScenarioMatrixRunner'," + ...
                "'test6GScenarioMatrixRunnerNoSummary'}," + ...
                "'Verbose',false,'ShowSlowest',10)");
            ...
            rows(6) = localBoundedGate(ctx,rows(6).Suite, ...
                ["python","-m","compileall","apps","backend","tools", ...
                "tests"],"",localSuiteTimeout(ctx,budgetSeconds, ...
                budgetTimer,rows(6).Suite),6,numel(rows));
            localWrite(ctx,rows);
            countPath = fullfile(countDir,"python_full_counts.csv");
            pythonGate = fullfile(ctx.Profile.RepositoryRoot,"tools", ...
                "run_pytest_gate.py");
            rows(7) = localBoundedGate(ctx,rows(7).Suite, ...
                ["python",pythonGate,"--count-output",countPath,"tests"], ...
                countPath, ...
                localSuiteTimeout(ctx,budgetSeconds,budgetTimer, ...
                rows(7).Suite),7,numel(rows));
            localWrite(ctx,rows);
            countPath = fullfile(countDir,"webgui_unit_counts.csv");
            rows(8) = localBoundedGate(ctx,rows(8).Suite, ...
                ["python",pythonGate,"--count-output",countPath, ...
                "--marker","webgui_unit","tests"],countPath, ...
                localSuiteTimeout(ctx,budgetSeconds,budgetTimer, ...
                rows(8).Suite),8,numel(rows));
            localWrite(ctx,rows);
            secureE2E=fullfile(ctx.Profile.RepositoryRoot,"tools", ...
                "run_secure_webgui_e2e.py");
            rows(9) = localBoundedGate(ctx,rows(9).Suite, ...
                ["python",secureE2E,"--base-url",ctx.WebGUIURL, ...
                "--run-tag",ctx.RunID], ...
                "", ...
                localSuiteTimeout(ctx,budgetSeconds,budgetTimer, ...
                rows(9).Suite),9,numel(rows));
            localWrite(ctx,rows);
            matrixResults=fullfile(countDir, ...
                "matlab_scenario_matrix_results.csv");
            countPath=fullfile(countDir, ...
                "matlab_scenario_matrix_counts.csv");
            code="setup6GRSimToolkit('Verbose',false);" + ...
                "r=sixgr.integration.qualification." + ...
                "ScenarioMatrixRegressionSharder.run('" + ...
                localMATLABLiteral(matrixResults)+"','" + ...
                localMATLABLiteral(countPath)+"');" + ...
                "assert(all(string(r.Status)=='PASS'));";
...
row = struct("Suite","","MandatoryTests",0,"ExecutedTests",0, ...
    ...
    "ShardIndex",0,"ShardCount",11,"SuiteTimeoutSeconds",NaN, ...
