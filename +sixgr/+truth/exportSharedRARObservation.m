function exportSharedRARObservation(runFolder,cfg,context,planes,result)
...
folder=fullfile(layout.AirInterfaceMATDir,'rar_monitoring_observations');
sixgr.util.ensureFolder(folder);
name=string(matlab.lang.makeValidName(char(context.RunId)))+"_RAR_"+context.AbsoluteSlot;
...
capture=struct('Source',"actual_shared_physical_RAR_monitoring", ...
    ...
    'ExecutionReplay',replay,'ReceiverObservation',result.RARMonitoringObservations(end,:), ...
...
if ~ok, error('sixgr:truth:RARObservationPublishFailed','%s',message); end
end
