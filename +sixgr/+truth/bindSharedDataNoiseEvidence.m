function replay=bindSharedDataNoiseEvidence(planes,prepared,planeID,replay,configuredSNR_dB)
...
if nargin<5 || isempty(configuredSNR_dB)
    configuredSNR_dB=sixgr.truth.resolveWaveformOperatingPointMetadata( ...
        prepared.InputConfig);
end
validateattributes(configuredSNR_dB,{'numeric'}, ...
    {'real','scalar','finite'});
...
elseif mode=="standalone_awgn_snr_argument"
    ...
    assert(abs(requested-double(configuredSNR_dB))<=1e-12*max(1,abs(double(configuredSNR_dB))), ...
        'sixgr:truth:FixedAWGNOperatingPointMismatch', ...
        ['The physical receiver was calibrated at %.12g dB but the active ' ...
         'fixed-SNR point is %.12g dB. Configured labels cannot replace ' ...
         'executed noise calibration.'],requested,double(configuredSNR_dB));
    [configuredEnergy,referencePolicy]=sixgr.link.resolveAWGNReferenceEnergy(prepared.InputConfig);
    assert(signalEnergy==configuredEnergy && ...
        string(sixgr.util.structGet(replay,'SharedNoiseCalibrationSource',''))== ...
        referencePolicy.CalibrationSource, ...
