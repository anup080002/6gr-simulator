...
% TS 38.213 7.4: decoded referenceSignalPower minus UE-filtered RSRP.
...
commonUsable=string(sixgr.util.structGet(common,'Source',''))=="decoded_sib1" && ...
    ...
    isfinite(powerSlot) && powerSlot>=1 && powerSlot<=slot && ...
...
if measurement.Usable
    ...
    if string(row.Direction)~="DL" || any(~isfinite([rsrp filteredRSRP pathloss])) || pathloss<0
        error('sixgr:truth:InvalidRAPowerReference','Selected UE SSB reference must have finite received powers and nonnegative pathloss.');
    end
