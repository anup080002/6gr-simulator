...
for i = 1:numel(specs)
    ...
    try
        % These are canonical comma-separated artifacts. Several genuine
        % runtime fields (for example LDPC bit/parity vectors) use '|'
        % inside a quoted cell. Automatic delimiter inference can mistake
        % that payload separator for a second table delimiter and expand an
        % 813-column trial table into tens of thousands of Var* columns.
        % Force the file contract instead of interpreting payload content.
...
out.IssueCount = double(sum(double(auditT.LabelTokenCellCount) + ...
    double(auditT.ProvenanceColumnCount) + ...
    double(auditT.UnexpectedGeneratedVariableCount) + ...
    double(~logical(auditT.SchemaExpansionOk))));
end
...
function row = localAuditRow(measurementRel, measurementT, kind, sourceT)
if nargin == 0
    row = struct("MeasurementArtifact","", "MeasurementRows", NaN, ...
        "MeasurementColumnCount", NaN, "ProvenanceColumnCount", NaN, ...
        "LabelTokenCellCount", NaN, "SourceColumnCount", NaN, ...
        "UnexpectedGeneratedVariableCount", NaN, "SchemaExpansionOk", false, ...
        "MeasurementViewClean", false, "SplitKind", "");
    return;
end
[provColCount, tokenCount] = localMeasurementViewIssues(measurementT);
sourceWidth = double(localWidth(sourceT));
measurementWidth = double(localWidth(measurementT));
unexpectedVars = 0;
if istable(measurementT)
    variableNames = cellstr(string(measurementT.Properties.VariableNames));
    unexpectedVars = sum(~cellfun(@isempty, regexp( ...
        variableNames, '^Var[0-9]+$', 'once')));
end
schemaExpansionOk = measurementWidth <= sourceWidth && unexpectedVars == 0;
row = struct( ...
    "MeasurementArtifact", string(measurementRel), ...
    "MeasurementRows", double(localHeight(measurementT)), ...
    "MeasurementColumnCount", measurementWidth, ...
    "ProvenanceColumnCount", double(provColCount), ...
    "LabelTokenCellCount", double(tokenCount), ...
    "SourceColumnCount", sourceWidth, ...
    "UnexpectedGeneratedVariableCount", double(unexpectedVars), ...
    "SchemaExpansionOk", logical(schemaExpansionOk), ...
    "MeasurementViewClean", logical(provColCount == 0 && ...
        tokenCount == 0 && schemaExpansionOk), ...
