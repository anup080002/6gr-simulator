...
% Persist the failed producer receipt above, then expose its real error.
% Empty/malformed required trial evidence cannot be repaired by repeating
% status/hash sealing. Do not spend three passes and mislabel that failure
% as a non-convergent terminal status, or fabricate rows to get past it.
sixgr.artifact.assertBrowserMaterializationSucceeded(contractMaterialization);
