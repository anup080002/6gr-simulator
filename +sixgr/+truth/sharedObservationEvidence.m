...
if nargin>=2
    ...
    elseif isa(prepared,'sixgr.link.PreparedDataTransmission')
        ...
        assert(isstruct(prepared) && isscalar(prepared), ...
            'sixgr:truth:InvalidPhysicalObservationAuthority', ...
            'A shared observation requires retained preparation and clock authority.');
        stage=string(sixgr.util.structGet(prepared,'ExecutionStage',''));
        if stage=="csirs_waveform_prepared_not_received"
            assert(isfield(prepared,'TransmitStartSample') && ...
                isfield(prepared,'ReceiveStartSample') && ...
                isfield(prepared,'ReceiveEndSampleExclusive') && ...
                tx.isComplete() && post.isComplete() && pre.isComplete() && ...
                tx.SampleRateHz==prepared.SampleRateHz && ...
                post.SampleRateHz==prepared.SampleRateHz && ...
                pre.SampleRateHz==prepared.SampleRateHz && ...
                tx.StartSample==prepared.TransmitStartSample && ...
                post.StartSample==prepared.ReceiveStartSample && ...
                pre.StartSample==prepared.ReceiveStartSample && ...
                tx.EndSampleExclusive==prepared.TransmitStartSample+prepared.NumSamples && ...
                post.EndSampleExclusive==prepared.ReceiveEndSampleExclusive && ...
                pre.EndSampleExclusive==prepared.ReceiveEndSampleExclusive && ...
                post.EndSampleExclusive>=post.StartSample+prepared.NumSamples && ...
                tx.NumReceiveAntennas==prepared.NumPhysicalTransmitAntennas, ...
                'sixgr:truth:CSIRSPhysicalObservationMismatch', ...
                ['CSI-RS TX, pre-RF and post-RF evidence must cover the ' ...
                 'retained independently prepared physical waveform.']);
            buffers={post,pre,tx};
        else
            assert(stage=="pdcch_waveform_prepared_not_received" && ...
                ...
                'Distinct TX/RX intervals require retained PDCCH preparation and clock authority.');
...
replay=struct('RuntimeChannelStartSample',post.StartSample, ...
    'ReceiverID',rxID,'TransmitterID',txID, ...
...
mapping={'NoiseOperatingMode',rx,'NoiseOperatingMode'; ...
    'NoiseStreamSeed',rx,'NoiseStreamSeed'; ...
    'AppliedAWGNSNR_dB',rx,'AppliedAWGNSNR_dB'; ...
    'AppliedAWGNSNRSource',rx,'AppliedAWGNSNRSource'; ...
    'AppliedAWGNSNRValueRole',rx,'AppliedAWGNSNRValueRole'; ...
    ...
    'AWGNReferenceEnergySource',rx,'AWGNReferenceEnergySource'; ...
