...
tables.result_issue_registry_evaluation = localBuildResultIssueRegistryEvaluation( ...
    src, meta, tables.result_issue_registry);
...
logicalPaths = struct( ...
    ...
    "result_issue_registry_evaluation", "reports/csv/result_issue_registry_evaluation.csv", ...
...
for i = 1:numel(names)
    name = names{i};
    T = tables.(name);
    if istable(T) && (~isempty(T) || name == "result_issue_registry")
...
if grantDisposition.StandaloneFailedCount > 0
    rows(end+1, 1) = localIssueRow( ... %#ok<AGROW>
        "pucch_receiver_trials_decode_failed", "high", "FAIL", "control_channel_runtime", ...
        "UL", NaN, NaN, "PUCCH", "observed_receiver_trial_decode_failures", ...
        string(grantDisposition.StandaloneFailedCount), ...
        "Every observed standalone PUCCH trial must pass detection, decode/content and applicable CRC checks", ...
        "air_interface/csv/pucch_trials.csv", ...
        "The receiver trial exists, but its physical UCI decode did not pass", ...
        "Repair the physical receive/layout cause; do not fabricate success or classify the observed trial as missing.");
end
...
if isempty(rows)
    T = table('Size', [0 21], ...
        'VariableTypes', {'string','string','string','string','string','double','double','string','string','string', ...
        'string','string','string','double','double','double','logical','string','string','string','string'}, ...
        ...
        'root_cause_hint','frame','slot','harq_id','analytics_visible_flag','run_id','scenario_id', ...
        'config_hash','producer_module'});
    return;
end
T = struct2table(rows);
...
function T = localBuildResultIssueRegistryEvaluation(src, meta, issueT)
% Emit positive evidence that every configured issue detector ran, even
% when the correct result is an empty issue registry.  The strict root gate
% validates this receipt; a missing or header-only registry without it
% remains NOT_EVALUATED.
sourceNames = ["DLTrials","ULTrials","DLGrants","ULGrants", ...
    "PDCCHTrials","PUCCHGrants","PUCCHTrials","PRACHTrials", ...
    "PBCHTrials","SRSTrials","ReceiverTrackingTrace"];
sourceTableCount = 0;
sourceRowCount = 0;
for i = 1:numel(sourceNames)
    name = char(sourceNames(i));
    if isfield(src, name) && istable(src.(name))
        sourceTableCount = sourceTableCount + 1;
        sourceRowCount = sourceRowCount + height(src.(name));
    end
end
issueRowCount = 0;
if istable(issueT)
    issueRowCount = height(issueT);
end
T = table( ...
    localFirstNonEmptyString(string(meta.run_tag), string(meta.scenario_id)), ...
    string(meta.scenario_id), string(meta.config_hash), ...
    "EVALUATED", double(issueRowCount), double(sourceTableCount), double(sourceRowCount), ...
...
for i = 1:height(trials)
    ...
    rows(i).applied_precoder_pmi_basis = localTextTableValue(row,"AppliedPrecoderPMIBasis","");
    rows(i).equivalent_pdsch_port_basis_codebook_index = localNumericTableValue(row,"EquivalentPDSCHPortBasisCodebookIndex",NaN);
    rows(i).applied_csi_resource_index = localNumericTableValue(row,"AppliedCSIResourceIndex",NaN);
    rows(i).csi_port_pmi_matrix_sha256 = localTextTableValue(row,"PMICodebookMatrixCSIPortsSHA256","");
    rows(i).csi_port_to_element_matrix_sha256 = localTextTableValue(row,"CSIRSPortToElementMatrixSHA256","");
    rows(i).composed_element_matrix_sha256 = localTextTableValue(row,"ComposedElementMatrixSHA256","");
    ...
    rows(i).qcl_source_resource_id=localNumericTableValue(row,"QCLSourceResourceID",NaN);
    rows(i).qcl_source_slot0=localNumericTableValue(row,"QCLSourceSlot0",NaN);
    rows(i).qcl_timing_prior_samples=localNumericTableValue(row,"QCLTimingPriorSamples",NaN);
    rows(i).qcl_dmrs_delay_residual_samples=localNumericTableValue(row,"QCLDMRSDelayResidual_samples",NaN);
    rows(i).qcl_measurement_status=localTextTableValue(row,"QCLMeasurementStatus","");
    rows(i).tci_codepoint=localNumericTableValue(row,"TCICodepoint",NaN);
    rows(i).tci_initialization_source=localTextTableValue(row,"TCIInitializationSource","");
...
rows = repmat(struct("timestamp_sim_ms", NaN, "frame", NaN, "slot", NaN, "direction", "", ...
    ...
    "applied_precoder_pmi_basis","","equivalent_pdsch_port_basis_codebook_index",NaN, ...
    "applied_csi_resource_index",NaN,"csi_port_pmi_matrix_sha256","", ...
    "csi_port_to_element_matrix_sha256","","composed_element_matrix_sha256","", ...
    ...
    "qcl_source_resource_id",NaN,"qcl_source_slot0",NaN,"qcl_timing_prior_samples",NaN, ...
    "qcl_dmrs_delay_residual_samples",NaN,"qcl_measurement_status","", ...
    "tci_codepoint",NaN,"tci_initialization_source","", ...
...
try
    preserveDeclaredSchema = endsWith(lower(string(logicalPath)), ...
        "beamforming/csv/beam_precoder_table.csv");
    sixgr.util.csvWriteTable(csvPath, T, "PreserveSchema", preserveDeclaredSchema);
...
names = ["run_id","run_tag","scenario_id","scenario_variant_id","config_hash","code_commit","seed","drop_id", ...
    ...
    "qcl_accuracy","qcl_type","qcl_source_rs","qcl_status", ...
    "tci_state_id","unified_tci_state_id","tci_validity_timer_slots","tci_status"];
end
