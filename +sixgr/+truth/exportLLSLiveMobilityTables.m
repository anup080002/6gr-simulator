...
for slotIdx = 1:numSlots
    ...
    for u = 1:ue.K
        ...
        servingRows(servingCount) = struct( ...
            ...
            "Speed_mps", double(localUEColumn(ue, "speed_kmh", u)) / 3.6, ...
            "Heading_deg", double(localUEColumn(ue, "heading_deg", u)), ...
            "Heading_rad", double(localUEColumn(ue, "heading_deg", u)) * pi / 180, ...
            "CellID", double(cellIdx), ...
            ...
            "BasePathloss_dB", double(largeScaleState.BasePathloss_dB(u, cellIdx)), ...
            "Pathloss_dB", double(largeScaleState.Pathloss_dB(u, cellIdx)), ...
            "LOSFlag", logical(largeScaleState.LOS(u, cellIdx)), ...
            "LOSState", string(localLOSStateToken(largeScaleState.LOS(u, cellIdx))), ...
            "ShadowFading_dB", double(largeScaleState.Shadow_dB(u, cellIdx)), ...
            "O2I_dB", double(largeScaleState.O2I_dB(u, cellIdx)), ...
            "Distance2D_m", double(largeScaleState.d2d_m(u, cellIdx)), ...
            "Distance3D_m", double(largeScaleState.d3d_m(u, cellIdx)), ...
            "PropagationDelay_s", double(largeScaleState.PropagationDelay_s(u, cellIdx)), ...
            "RadialVelocity_mps", double(largeScaleState.RadialVelocity_mps(u, cellIdx)), ...
            "ExpectedDopplerHz", double(abs(localUEColumn(ue, "speed_kmh", u)) / 3.6 * localCarrierFrequencyHz(cfgMob) / 299792458), ...
            "AppliedDopplerHz", double(largeScaleState.Doppler_Hz(u, cellIdx)), ...
            "SignedDoppler_Hz", double(largeScaleState.SignedDoppler_Hz(u, cellIdx)), ...
            "ChannelComplianceMode", char(string(sixgr.util.structGet(largeScaleState, "ChannelComplianceMode", ""))), ...
            "PathlossModelSource", char(string(sixgr.util.structGet(largeScaleState, "PathlossModelSource", ""))), ...
            "PathlossComplianceStatus", char(string(sixgr.util.structGet(largeScaleState, "PathlossComplianceStatus", ""))), ...
            "FallbackUsedForPathloss", logical(sixgr.util.structGet(largeScaleState, "FallbackUsedForPathloss", false)), ...
            "O2IModelSource", char(string(sixgr.util.structGet(largeScaleState, "O2IModelSource", ""))), ...
            "O2IComplianceStatus", char(string(sixgr.util.structGet(largeScaleState, "O2IComplianceStatus", ""))), ...
            "O2IComplianceReason", char(string(sixgr.util.structGet(largeScaleState, "O2IComplianceReason", ""))), ...
            "LOSProbabilitySource", char(string(sixgr.util.structGet(largeScaleState, "LOSProbabilitySource", ""))), ...
            "LOSComplianceStatus", char(string(sixgr.util.structGet(largeScaleState, "LOSComplianceStatus", ""))), ...
            "LOSComplianceReason", char(string(sixgr.util.structGet(largeScaleState, "LOSComplianceReason", ""))), ...
...
row = struct( ...
    ...
    "RadialVelocity_mps", NaN, ...
    "ExpectedDopplerHz", NaN, ...
    "AppliedDopplerHz", NaN, ...
    "SignedDoppler_Hz", NaN, ...
...
function fcHz = localCarrierFrequencyHz(cfg)
fcHz = double(sixgr.util.structGet(cfg, "frequency.center_frequency_hz", NaN));
if ~(isfinite(fcHz) && fcHz > 0)
    fcHz = double(sixgr.util.structGet(cfg, "phy.fc_Hz", NaN));
end
if ~(isfinite(fcHz) && fcHz > 0)
    fcHz = double(sixgr.util.structGet(cfg, "channel.carrier_frequency_hz", NaN));
end
end
