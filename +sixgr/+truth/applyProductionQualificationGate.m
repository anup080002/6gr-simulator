...
functionalOk = localResolvedLogical(status, persistedStatus, "ResultOk", false) && ...
    ...
    localResolvedLogical(status, persistedStatus, "ScenarioObjectiveOk", false) && ...
...
scientificOk = functionalOk && phase7Ok && statisticalOk;
...
status.ProductionQualificationFailureReasons = strjoin(localReasons( ...
    functionalOk, wiringOk, phase7Ok, statisticalOk, referenceOk, publicationOk, ...
...
gateT = table( ...
    ["FunctionalRun";"RuntimeWiringCoverage";"Phase7NumericalValidation"; ...
     "StatisticalQualification";"IndependentReferenceQualification"; ...
     "TerminalPublicationEvidence";"ProductionGrade"], ...
    [true;true;true;true;true;true;true], ...
    [functionalOk;wiringOk;phase7Ok;statisticalOk;referenceOk;publicationOk;productionGradeOk], ...
    [localState(functionalOk,false);wiringStatus;phase7Status; ...
...
passColumn = intersect(["Pass","PASS","QualificationPass"], ...
    string(T.Properties.VariableNames), "stable");
if isempty(passColumn) || ~any(required)
    ok = false; state = "NOT_EVALUATED";
    reason = "independent_frc_reference_qualification_schema_invalid";
    return;
end
pass = arrayfun(@localToLogical, T.(char(passColumn(1))));
ok = all(~required | pass);
state = localState(ok, false);
reason = localReason(ok, "independent_frc_reference_point_failed");
end

function reasons = localReasons(functionalOk, wiringOk, phase7Ok, statisticalOk, referenceOk, publicationOk, wiringReason, phase7Reason, referenceReason, publicationReason)
reasons = strings(0,1);
