...
if isProxyOnlyStudy
    ...
    % Proxy-only studies are not PHY truth, but they still require the same
    % canonical root status lifecycle as every other production runner.
    % Omitting this call left finalization without its authoritative CSV/JSON
    % pair and caused the later reducer to fail with RootStatusArtifactMissing.
    ...
    rootResultOk = logical(sixgr.util.structGet( ...
        rootResultStatus, "ResultOk", false));
    verdict.CheckDetails.RootStatus = rootStatus;
    verdict.ResultStatus = rootResultStatus;
    verdict.Ok = rootResultOk;
    verdict.RuntimeTruthContractOk = rootResultOk;
    if ~rootResultOk
        rootReason = string(sixgr.util.structGet( ...
            rootResultStatus, "ResultStatusReason", "root_status_failed"));
...
decodedSIB1BindingRequired = required && localScenarioAnyTrue(scfg, cfg, [ ...
    "sib1_and_initial_access.sib1_decode_from_waveform_required", ...
    "initial_access.sib1.decode_from_waveform_required", ...
    "validation.sib1_and_initial_access.sib1_decode_from_waveform_required"]);
runtimeStagesRequired = required && localScenarioAnyTrue(scfg, cfg, [ ...
    "random_access_evidence.require_runtime_stage_waveforms", ...
...
stats = struct( ...
    ...
    "DecodedSIB1BindingRequired", logical(decodedSIB1BindingRequired), ...
    "DecodedSIB1BindingOk", false, ...
    "RuntimeStageWaveformsRequired", logical(runtimeStagesRequired), ...
    "RuntimeStageWaveformsOk", false, ...
    "RAAttemptRows", 0, ...
    "RAStateRows", 0, ...
    "Msg1Rows", 0, ...
    "Msg2Rows", 0, ...
...
if decodedSIB1BindingRequired
    requiredColumns = [requiredColumns, "RABindingSource", ...
        "SIB1RACHBindingApplied","SIB1RACHBindingSource", ...
        "SIB1RACHPayloadHash","SIB1RACHTreeHash"];
end
if runtimeStagesRequired
    requiredColumns = [requiredColumns, "RuntimeStageWaveformsRequired", ...
        "RuntimeStageWaveformsUsed","RuntimeSelfLoopWaveformsUsed", ...
        "RuntimeChannelStateUsed"];
end
...
if decodedSIB1BindingRequired
    bindingRows = localColumnBool(attemptT, "SIB1RACHBindingApplied") & ...
        string(attemptT.RABindingSource) == "decoded_sib1_rach_config_common" & ...
        string(attemptT.SIB1RACHBindingSource) == "decoded_sib1_rach_config_common" & ...
        strlength(strtrim(string(attemptT.SIB1RACHPayloadHash))) == 64 & ...
        strlength(strtrim(string(attemptT.SIB1RACHTreeHash))) == 64;
    stats.DecodedSIB1BindingOk = any(bindingRows) && all(bindingRows);
    strictRows = strictRows & bindingRows;
else
    stats.DecodedSIB1BindingOk = true;
end
if runtimeStagesRequired
    runtimeRows = localColumnBool(attemptT, "RuntimeStageWaveformsRequired") & ...
        localColumnBool(attemptT, "RuntimeStageWaveformsUsed") & ...
        ~localColumnBool(attemptT, "RuntimeSelfLoopWaveformsUsed") & ...
        localColumnBool(attemptT, "RuntimeChannelStateUsed");
    stats.RuntimeStageWaveformsOk = any(runtimeRows) && all(runtimeRows);
    strictRows = strictRows & runtimeRows;
else
    stats.RuntimeStageWaveformsOk = true;
end
...
elseif decodedSIB1BindingRequired && ~stats.DecodedSIB1BindingOk
    stats.RAStatus = "decoded_sib1_rach_config_not_causally_bound";
elseif runtimeStagesRequired && ~stats.RuntimeStageWaveformsOk
    stats.RAStatus = "required_runtime_stage_waveforms_missing_or_self_looped";
else
...
stats = struct( ...
    ...
    "PDCCHStatisticalRequired", false, ...
...
stats.PDCCHStatisticalRequired = localScenarioAnyTrue(scfg, cfg, [ ...
    "control.pdcch_strict.statistical_qualification.enabled", ...
    "lls6g.control.pdcch_strict.statistical_qualification.enabled", ...
...
switch component
    case "prach"
        ...
        if localScenarioAnyTrue(scfg, cfg, [ ...
                "sib1_and_initial_access.sib1_decode_from_waveform_required", ...
                "initial_access.sib1.decode_from_waveform_required", ...
                "validation.sib1_and_initial_access.sib1_decode_from_waveform_required"])
            requiredTrue = [requiredTrue, "SIB1RACHBindingApplied"];
        end
        if localScenarioAnyTrue(scfg, cfg, [ ...
                "random_access_evidence.require_runtime_stage_waveforms", ...
                "validation.random_access_evidence.require_runtime_stage_waveforms"])
            requiredTrue = [requiredTrue, "RuntimeStageWaveformsRequired", ...
                "RuntimeStageWaveformsUsed","RuntimeChannelStateUsed"];
        end
    case "srs"
...
if component == "prach" && localScenarioAnyTrue(scfg, cfg, [ ...
        "sib1_and_initial_access.sib1_decode_from_waveform_required", ...
        "initial_access.sib1.decode_from_waveform_required", ...
        "validation.sib1_and_initial_access.sib1_decode_from_waveform_required"])
    for field = ["RABindingSource","SIB1RACHBindingSource", ...
            "SIB1RACHPayloadHash","SIB1RACHTreeHash"]
        if ~localHasColumn(T, field)
    ...
    componentOk = componentOk & ...
        string(T.RABindingSource) == "decoded_sib1_rach_config_common" & ...
        string(T.SIB1RACHBindingSource) == "decoded_sib1_rach_config_common" & ...
        strlength(strtrim(string(T.SIB1RACHPayloadHash))) == 64 & ...
        strlength(strtrim(string(T.SIB1RACHTreeHash))) == 64;
end
if component == "prach" && localScenarioAnyTrue(scfg, cfg, [ ...
        "random_access_evidence.require_runtime_stage_waveforms", ...
        "validation.random_access_evidence.require_runtime_stage_waveforms"])
    if ~localHasColumn(T, "RuntimeSelfLoopWaveformsUsed")
        status = "integrated_prach_runtime_missing_column:RuntimeSelfLoopWaveformsUsed";
        return;
    end
    componentOk = componentOk & ...
        ~localFirstBoolColumn(T, "RuntimeSelfLoopWaveformsUsed", true);
end
...
positiveIdentityOk = localHasColumn(configuredAppliedT, "EvidenceScope") && ...
    ...
    all(strlength(strtrim(string(configuredAppliedT.ScenarioConfigHash(positiveMask)))) == 64);
negativeIdentityOk = localHasColumn(configuredAppliedT, "EvidenceScope") && ...
    all(lower(strtrim(string(configuredAppliedT.EvidenceScope(negativeMask)))) == "component_anchor") && ...
    localHasColumn(configuredAppliedT, "SameScenarioInPathEligible") && ...
    all(~localColumnBool(configuredAppliedT(negativeMask, :), "SameScenarioInPathEligible"));
...
interferenceOk = localHasColumn(interferenceT, "InterferenceConfigured") && ...
    ...
    localHasColumn(interferenceT, "InterferenceContributorCount") && ...
    localHasColumn(interferenceT, "StrictOk") && ...
    all(localColumnBool(interferenceT, "StrictOk"));
if interferenceOk
    configuredInterference = localColumnBool(interferenceT, "InterferenceConfigured");
    appliedInterference = localColumnBool(interferenceT, "InterferenceApplied");
    contributorCount = localColumnNumeric(interferenceT, "InterferenceContributorCount");
    interferenceOk = all(appliedInterference == (contributorCount > 0)) && ...
        all(~appliedInterference(~configuredInterference));
    if any(configuredInterference)
        interferenceOk = interferenceOk && any(appliedInterference(configuredInterference));
    end
end
...
noiseOk = localHasColumn(noiseT, "StrictOk") && all(localColumnBool(noiseT, "StrictOk")) && ...
    localHasColumn(noiseT, "InjectedNoiseVariance") && ...
    all(localColumnNumeric(noiseT, "InjectedNoiseVariance") > 0) && ...
    localHasColumn(noiseT, "NoiseOperatingMode") && ...
    all(strlength(strtrim(string(noiseT.NoiseOperatingMode))) > 0);
...
if boundedDiagnosticValidation
    ...
    requiredTrialsPerAggregateBin = 1;
end
geometryDistanceApplicable = ~logical(isFixedSNRSweep) && ...
    sixgr.validation.geometryEvidenceRequired(scfg, cfg);
stats = struct( ...
    ...
    "DistanceSINRApplicable", logical(geometryDistanceApplicable), ...
    "DistanceSINRApplicabilityAuthority", ...
        "sixgr.validation.geometryEvidenceRequired", ...
    "DistanceSINREvidenceOk", true, ...
    "SINRSummaryOk", true, ...
    "DLBinsWithAtLeast5Trials", NaN, ...
    "RequiredTrialsPerAggregateDLBin", requiredTrialsPerAggregateBin, ...
...
if logical(isFixedSNRSweep)
    ...
    if isempty(curveT) || height(curveT) == 0 || ...
        ...
        stats.DLBinsMeetingRequiredTrials = ...
            sum(dl & trials >= requiredTrialsPerAggregateBin);
    ...
    if isempty(curveT) || height(curveT) == 0 || ...
        ...
        stats.DLBinsMeetingRequiredTrials = ...
            sum(agg & trials >= requiredTrialsPerAggregateBin);
        stats.MeasuredSINRCurveOk = stats.DLBinsMeetingRequiredTrials >= 1;
        if ~stats.MeasuredSINRCurveOk
            failures(end+1, 1) = ...
...
if logical(geometryDistanceApplicable)
...
% The strict root reducer is the authority for whether an empty issue
% registry was actually evaluated.  A schema-only registry with a valid
% identity-bound evaluation receipt is PASS, whereas a missing or
% unevaluated registry is NOT_EVALUATED.  Do not collapse both cases to the
% local table reader's generic "missing" state.
issueRegistryStatus = string(sixgr.util.structGet(rootStatus, ...
    "IssueRegistryStatus", sixgr.util.structGet(issueRegistry, ...
    "IssueRegistryStatus", "missing")));
issueRegistryRows = double(sixgr.util.structGet(rootStatus, ...
    "IssueRegistryRowCount", sixgr.util.structGet(issueRegistry, ...
    "RegistryRows", NaN)));
activeMandatoryIssues = double(sixgr.util.structGet(rootStatus, ...
    "ActiveMandatoryIssueCount", sixgr.util.structGet(issueRegistry, ...
    "BlockingIssueCount", NaN)));
activeCriticalIssues = double(sixgr.util.structGet(rootStatus, ...
    "ActiveCriticalIssueCount", sixgr.util.structGet(issueRegistry, ...
    "ActiveCriticalCount", NaN)));
activeHighIssues = double(sixgr.util.structGet(rootStatus, ...
    "ActiveHighIssueCount", sixgr.util.structGet(issueRegistry, ...
    "ActiveHighCount", NaN)));
activeMediumIssues = double(sixgr.util.structGet(rootStatus, ...
    "ActiveMediumIssueCount", sixgr.util.structGet(issueRegistry, ...
    "ActiveMediumCount", NaN)));
