function [T, checkT] = buildPlannedREAllocation(cfg)
...
ssbEnabled = logical(sixgr.util.structGet(cfg, "phy.ssb.enable", false));
...
type0Enabled = ssbEnabled && logical(sixgr.util.structGet( ...
...
sib1Enabled = logical(sixgr.util.structGet(cfg, ...
...
pdschEnabled = logical(sixgr.util.structGet(cfg, "phy.pdsch.enable", true));
...
csirsEnabled = logical(sixgr.util.structGet(cfg, "phy.csirs.enable", false));
...
srsEnabled = logical(sixgr.util.structGet(cfg, "phy.srs.enable", false));
...
if isfield(strict,"FirstActiveOccasion") && isstruct(strict.FirstActiveOccasion)
    slots=double(sixgr.util.structGet(strict.FirstActiveOccasion,"AbsoluteSlot",[]));
end
if isempty(slots)
    for slot1=1:totalSlots
