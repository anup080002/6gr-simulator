...
%EVALUATEINPATHCONTROLEVIDENCE Evaluate current-run PDCCH/PUCCH rows only.
...
out = struct("Ok",logical(ok), "StrictOk",logical(ok), ...
    "PDCCH",pdcch, "PUCCH",pucch, "SummaryTable",summary, ...
...
row = struct("SignalFamily",string(name), ...
    ...
    "TrialRows",height(result.ArtifactTables.(lower(char(name)) + "_trials")), ...
