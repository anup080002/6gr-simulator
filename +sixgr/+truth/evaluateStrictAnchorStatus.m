...
switch runClass
    ...
    case "functional_waveform_validation"
        publicationEligible = false;
        objectiveGateOk = logical(configuredGate.ConfiguredEffectivePolicyOk) && eligibleCount > 0;
        if objectiveGateOk
            reason = "functional_waveform_validation has actual waveform rows and exact configured/effective agreement; publication eligibility remains disabled because bounded functional evidence is not statistical qualification.";
        else
            reason = "functional_waveform_validation requires actual waveform rows and exact configured/effective agreement; publication eligibility remains disabled.";
        end
...
if ~(required && reconSchemaOk)
    ...
    applicable = localToLogical(localColumnOrDefault(reconT, ...
        "Applicable", true));
    applicabilityReason = strtrim(string(localColumnOrDefault(reconT, ...
        "ApplicabilityReason", "")));
    invalidDuration = ismember(duration, ...
        ["", "unavailable", "nan", "<missing>"]);
    hasFailureReason = strlength(reason) > 0 & ~ismissing(reason);
    % Only applicable KPIs may be required to execute a formula.  A KPI
    % disabled by the resolved scenario (for example HARQ or scheduler
    % KPIs in an independent fixed-link campaign) is honest evidence only
    % when it is explicitly classified not_applicable with a reason.  It
    % must not be converted into a false reconstruction failure merely
    % because FormulaExecuted is correctly false.
    % Schema/raw/formula requirements apply only to KPIs that the resolved
    % scenario says are applicable.  A disabled HARQ or scheduler KPI has no
    % raw table schema to validate; its truth contract is instead the
    % explicit not_applicable classification and nonempty configuration-
    % derived reason below.  Applying ~schema globally made honest disabled
    % rows fail the root gate even though StrictOk/ReconciliationPass were
    % true and no proxy value had been exported.
    badApplicable = applicable & ( ...
        ~reconStrict | ~reconPass | ~schema | ...
        ~formula | missingRaw | invalidDuration | ...
        ismember(status, ["fail", "failed", "error", "not_applicable"]) | ...
        hasFailureReason);
    badNotApplicable = ~applicable & ( ...
        ~reconStrict | ~reconPass | ...
        status ~= "not_applicable" | ...
        strlength(applicabilityReason) == 0 | ...
        ismissing(applicabilityReason) | hasFailureReason);
    bad = badApplicable | badNotApplicable;
    reconOk = ~any(bad);
...
fixed = any(runClassProfile.RunClass == ["fixed_lls_anchor", "functional_waveform_validation"]);
...
mimoRequired = localStrictMIMOEvidenceRequired( ...
    scfg, cfg, runClassProfile.RunClass);
mimoEvidenceSource = "persisted_mimo_configured_vs_effective";
% Strict publication consumes the persisted four-gate artifact produced by
% the in-path MIMO exporter.  Recomputing a replacement inside the root
% reducer would hide a missing production stage and would let recovery
% claim artifact completeness without the required auditable CSV.  The
% ordinary branch below therefore remains deliberately fail closed.
...
function required = localStrictMIMOEvidenceRequired(scfg, cfg, runClass)
...
publicationRankContract = any(string(runClass) == [ ...
    "fixed_lls_anchor", "functional_waveform_validation", ...
    "hybrid_validation"]);
rankRequested = max([double(sixgr.util.structGet(cfg, ...
    ...
    localRunClassRankFixed(scfg, cfg) && publicationRankContract;
% A rank-adaptive diagnostic is allowed to schedule and transmit fewer
% layers than its configured maximum when the per-trial CQI/rank evidence
% proves that decision.  Requiring the publication-oriented four-gate MIMO
% summary merely because the maximum rank exceeds one would incorrectly
% turn a valid adaptive rank decision into a policy failure.  Fixed-rank,
% publication/high-rank, MU-MIMO and hybrid-beamforming contracts remain
% fail-closed here.  A diagnostic fixed-rank contract is still checked
% directly against transmitted rank/layers in the canonical trial rows.
...
requiredFields = ["SchemaName", "SchemaVersion", "RunID", "Status", ...
    "PublicationBackend", "DatabasePersisted", "FilesystemPersisted", ...
    "BrowserMaterialized"];
...
receiptBackend = lower(strtrim(string(receipt.PublicationBackend)));
backendMatches = receiptBackend == backend;
persistencePassed = ...
    (receiptBackend == "mysql_web" && localToLogical(receipt.DatabasePersisted)) || ...
    (receiptBackend == "filesystem" && localToLogical(receipt.FilesystemPersisted));
passed = upper(strtrim(string(receipt.Status))) == "PASS" && ...
    backendMatches && persistencePassed && ...
...
T = table( ...
    ...
    [true; true; exactMandatory; true; true; pdcchBindingMandatory; true; true], repmat(string(status.ScenarioMode), 8, 1), ...
