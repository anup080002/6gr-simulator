...
carrierFrequencyHz = double(sixgr.util.structGet(cfg, "phy.fc_Hz", ...
    sixgr.util.structGet(cfg, "carrier.fc_Hz", ...
...
rows = repmat(struct( ...
    ...
    "Speed_kmh", NaN, "Speed_mps", NaN, "UEHeading_deg", NaN, ...
    "Distance2D_m", NaN, "Distance3D_m", NaN, "Pathloss_dB", NaN, "BasePathloss_dB", NaN, ...
    "ShadowFading_dB", NaN, "O2I_dB", NaN, "LOSState", "", "PropagationDelay_s", NaN, ...
    "RadialVelocity_mps", NaN, "ExpectedDopplerHz", NaN, "AppliedDopplerHz", NaN, ...
    "SignedDoppler_Hz", NaN, "MeasurementSource", ""), 0, 1);
    for t = 1:ttiCount
        ...
        for u = 1:ueCount
            ...
            speedKmh = double(localMatrixValue(double(sixgr.util.structGet(details, "UESpeed_kmh", [])), t, u));
            speedMps = speedKmh / 3.6;
            radialVelocityMps = double(localMatrixValue(double(sixgr.util.structGet(details, "ServingRadialVelocity_mps", [])), t, u));
            expectedDopplerHz = abs(radialVelocityMps) * carrierFrequencyHz / lightSpeed_mps;
            losValue = double(localMatrixValue(double(sixgr.util.structGet(details, "ServingLOS", [])), t, u));
            losState = "";
            if isfinite(losValue)
                losState = "NLOS";
                if logical(losValue)
                    losState = "LOS";
                end
            ...
            rows(end + 1, 1) = struct( ... %#ok<AGROW>
                ...
                "Distance2D_m", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingDistance_m", [])), t, u)), ...
                "Distance3D_m", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingDistance3D_m", [])), t, u)), ...
                "Pathloss_dB", double(localMatrixValue(double(sixgr.util.structGet(details, "Pathloss_dB", [])), t, u)), ...
                "BasePathloss_dB", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingBasePathloss_dB", [])), t, u)), ...
                "ShadowFading_dB", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingShadowFading_dB", [])), t, u)), ...
                "O2I_dB", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingO2I_dB", [])), t, u)), ...
                "LOSState", losState, ...
                "PropagationDelay_s", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingPropagationDelay_s", [])), t, u)), ...
                "RadialVelocity_mps", radialVelocityMps, ...
                "ExpectedDopplerHz", expectedDopplerHz, ...
                "AppliedDopplerHz", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingAppliedDopplerHz", [])), t, u)), ...
                "SignedDoppler_Hz", double(localMatrixValue(double(sixgr.util.structGet(details, "ServingSignedDoppler_Hz", [])), t, u)), ...
