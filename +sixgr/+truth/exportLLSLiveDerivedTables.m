...
runtimeHARQLifecyclePath = fullfile(layout.HARQCSVDir,"harq_transmitter_lifecycle.csv");
if any(isfield(slotTrace,{'DLHarq','ULHarq','HARQTransmitterLifecycleArchive'}))
    runtimeHARQLifecycleT = sixgr.truth.captureHARQTransmitterLifecycle(slotTrace);
    sixgr.util.csvWriteTable(runtimeHARQLifecyclePath,runtimeHARQLifecycleT,"PreserveSchema",true);
elseif exist(runtimeHARQLifecyclePath,'file')==2
    % Persisted finalization retains the exact previously executed ledger;
    % it must not reconstruct process release from the receiver CRC table.
    runtimeHARQLifecycleT = sixgr.util.csvReadTable(runtimeHARQLifecyclePath);
else
    runtimeHARQLifecycleT = table();
end
...
antennaRuntimeT = localBuildAntennaRuntimeEvidenceTable(trialCombinedT, cfg);
arrayConsistencyT = localBuildChannelArrayConsistencyTable(trialCombinedT, cfg);
...
artifacts.RuntimeHARQ = struct( ...
    "TransmitterLifecycleTable", runtimeHARQLifecycleT, ...
    "TransmitterLifecycleCSV", runtimeHARQLifecyclePath, ...
    "FeedbackObservationTable", sixgr.util.structGet(slotTrace,"SharedGNBUCIHARQTable",table()), ...
...
specs = [ ...
    ...
    localBeamMetricSpec("P1SS_RSRP_dB_re_UnitOccupiedRE_Es", "SS_RSRP_dB_re_UnitOccupiedRE_Es"); ...
    localBeamMetricSpec("P1SS_RSRPRawObserved_dB_re_UnitOccupiedRE_Es", "SS_RSRPRawObserved_dB_re_UnitOccupiedRE_Es"); ...
...
function T = localBuildAntennaRuntimeEvidenceTable(trialT, cfg)
fields = { ...
    'Direction','UEIndex','RNTI','BaseStationID','Frame','Slot','GrantContextId', ...
    'NumTxPorts','NumRxAntennas','ConfiguredTxAntennas','ConfiguredRxAntennas','TxWaveformColumns','RxWaveformBranches','PhysicalTxAntennas','PhysicalRxAntennas', ...
    ...
    'ChannelUsesCountOnlyAntennaModel','ChannelUsesSameRuntimeAntennaAssumptions','ElementPatternChannelApplicability','AntennaChannelConsistencyStatus','AntennaChannelConsistencyReason','AntennaChannelConsistencyOk','TransmitElementPatternApplied','ReceiveElementPatternApplied','TransmitElementPatternSource','ReceiveElementPatternSource','InterferenceChannelObjectSource','InterferenceChannelObjectClass','InterferenceChannelArrayHandlingStatus','InterferenceChannelArrayHandlingBlocker','InterferenceChannelArrayValueRole','InterferenceChannelArrayValueStatus', ...
...
T = localClassifyAntennaChannelEvidence(T, cfg);
T.UEID = double(T.UEIndex);
T = movevars(T, 'UEID', 'After', 'Direction');
end

function T = localClassifyAntennaChannelEvidence(T, cfg)
% Keep channel applicability separate from antenna-object creation.  AWGN
% executes a sample-domain identity/no-fading operator; it does not consume
% element patterns, array pose, or a count-only spatial channel model.
fields = {'ChannelArrayModel','ChannelObjectSource','ChannelObjectClass', ...
    'ChannelArrayHandlingStatus','ChannelArrayHandlingBlocker', ...
    'ChannelGeometryCouplingLevel','ChannelUsesCountOnlyAntennaModel', ...
    ...
    'ConfiguredTxAntennas','ConfiguredRxAntennas','TxWaveformColumns', ...
    'RxWaveformBranches','PhysicalTxAntennas','PhysicalRxAntennas'};
T = localEnsureEvidenceTable(T, fields);
if isempty(T)
    return;
end

model = lower(strtrim(string(T.ChannelArrayModel)));
isAWGN = model == "awgn_no_array_channel";
nonAWGN = ~isAWGN;
T.ElementPatternChannelApplicability(nonAWGN) = "applicable_spatial_channel";
T.AntennaChannelConsistencyOk(nonAWGN) = ...
    logical(T.ChannelUsesSameRuntimeAntennaAssumptions(nonAWGN));
T.AntennaChannelConsistencyStatus(nonAWGN & T.AntennaChannelConsistencyOk) = ...
    "pass_runtime_channel_uses_same_antenna_assumptions";
T.AntennaChannelConsistencyStatus(nonAWGN & ~T.AntennaChannelConsistencyOk) = ...
    "fail_runtime_channel_antenna_assumptions_not_shared";
T.AntennaChannelConsistencyReason(nonAWGN & T.AntennaChannelConsistencyOk) = ...
    "runtime_spatial_channel_reports_shared_antenna_assumptions";
T.AntennaChannelConsistencyReason(nonAWGN & ~T.AntennaChannelConsistencyOk) = ...
    "runtime_spatial_channel_does_not_report_shared_antenna_assumptions";

if ~any(isAWGN)
    return;
end

configuredTx = double(T.ConfiguredTxAntennas);
configuredRx = double(T.ConfiguredRxAntennas);
observedTx = localFirstFiniteVector(double(T.PhysicalTxAntennas), ...
    double(T.TxWaveformColumns));
observedRx = localFirstFiniteVector(double(T.PhysicalRxAntennas), ...
    double(T.RxWaveformBranches));
dimensionEvidence = isfinite(configuredTx) & isfinite(configuredRx) & ...
    isfinite(observedTx) & isfinite(observedRx);
dimensionMatch = dimensionEvidence & ...
    abs(configuredTx - observedTx) < 1e-9 & ...
    abs(configuredRx - observedRx) < 1e-9;
patternAbsent = ~logical(T.TransmitElementPatternApplied) & ...
    ~logical(T.ReceiveElementPatternApplied);
awgnOk = isAWGN & dimensionMatch & patternAbsent;

identityEnabled = logical(sixgr.util.structGet(cfg, ...
    "channel.sharedIdentityAWGNEnabled", false));
T.ChannelUsesCountOnlyAntennaModel(isAWGN) = false;
T.ChannelUsesSameRuntimeAntennaAssumptions(isAWGN) = false;
T.ChannelArrayHandlingStatus(isAWGN) = ...
    "awgn_identity_spatial_dimensions_no_array_kernel";
T.ChannelArrayHandlingBlocker(isAWGN) = "";
T.ChannelGeometryCouplingLevel(isAWGN) = ...
    "not_applicable_no_fading_channel_object";
T.ElementPatternChannelApplicability(isAWGN) = ...
    "not_applicable_awgn_identity_channel";
T.TransmitElementPatternSource(isAWGN) = ...
    "not_applied_not_applicable_awgn_identity_channel";
T.ReceiveElementPatternSource(isAWGN) = ...
    "not_applied_not_applicable_awgn_identity_channel";
if identityEnabled
    T.ChannelObjectSource(isAWGN) = ...
        "sixgr.channel.IdentityAWGNRuntime.materialize";
    T.ChannelObjectClass(isAWGN) = "explicit_identity_sample_operator";
else
    T.ChannelObjectSource(isAWGN) = ...
        "sixgr.channel.ChannelFactory.create:awgn_shortcut";
    T.ChannelObjectClass(isAWGN) = "no_fading_channel_object";
end
T.AntennaChannelConsistencyOk(isAWGN) = awgnOk(isAWGN);
T.AntennaChannelConsistencyStatus(awgnOk) = ...
    "pass_not_applicable_awgn_identity_channel";
T.AntennaChannelConsistencyReason(awgnOk) = ...
    "configured_physical_dimensions_match_waveform_branches_and_no_element_pattern_was_applied";
if ~isempty(sixgr.util.structGet(cfg,'channel.awgnSpatialMatrixDL',[]))
    T.ChannelObjectSource(isAWGN)="sixgr.channel.IdentityAWGNRuntime.materialize";
    T.ChannelObjectClass(isAWGN)="explicit_fixed_matrix_sample_operator";
    T.ChannelArrayHandlingStatus(isAWGN)="awgn_configured_spatial_matrix_no_array_kernel";
    T.ElementPatternChannelApplicability(isAWGN)="not_applicable_awgn_fixed_matrix_channel";
    T.TransmitElementPatternSource(isAWGN)="not_applied_awgn_fixed_matrix_channel";
    T.ReceiveElementPatternSource(isAWGN)="not_applied_awgn_fixed_matrix_channel";
    T.AntennaChannelConsistencyStatus(awgnOk)="pass_not_applicable_awgn_fixed_matrix_channel";
end
missingDimensions = isAWGN & ~dimensionEvidence;
T.AntennaChannelConsistencyStatus(missingDimensions) = ...
    "fail_awgn_runtime_dimension_evidence_missing";
T.AntennaChannelConsistencyReason(missingDimensions) = ...
    "awgn_classification_requires_configured_and_observed_tx_rx_dimensions";
mismatchedDimensions = isAWGN & dimensionEvidence & ~dimensionMatch;
T.AntennaChannelConsistencyStatus(mismatchedDimensions) = ...
    "fail_awgn_runtime_dimension_mismatch";
T.AntennaChannelConsistencyReason(mismatchedDimensions) = ...
    "configured_physical_dimensions_do_not_match_waveform_branches";
patternViolation = isAWGN & ~patternAbsent;
T.AntennaChannelConsistencyStatus(patternViolation) = ...
    "fail_awgn_element_pattern_claimed_applied";
T.AntennaChannelConsistencyReason(patternViolation) = ...
    "awgn_has_no_spatial_kernel_in_which_an_element_pattern_could_be_applied";
end

function T = localBuildChannelArrayConsistencyTable(trialT, cfg)
rows = repmat(struct( ...
    "Direction", "", ...
    "ObservedRows", NaN, ...
    "ChannelArrayModel", "", ...
    ...
    "PhysicalArrayConsistencyOk", false, ...
        ...
        "AntennaChannelConsistencyStatus", "", ...
        "AntennaChannelConsistencyReason", "", ...
        "AntennaChannelConsistencyOk", false, ...
...
for di = 1:numel(dirList)
    ...
    rows(end+1,1) = struct( ... %#ok<AGROW>
        ...
        "ElementPatternChannelApplicability", char(localLastToken(slice, "ElementPatternChannelApplicability")), ...
        "AntennaChannelConsistencyStatus", char(localLastToken(slice, "AntennaChannelConsistencyStatus")), ...
        "AntennaChannelConsistencyReason", char(localLastToken(slice, "AntennaChannelConsistencyReason")), ...
        "AntennaChannelConsistencyOk", logical(all(logical(slice.AntennaChannelConsistencyOk))), ...
...
if isempty(rows)
    T = struct2table(repmat(struct( ...
        ...
        "ElementPatternChannelApplicability", "", "AntennaChannelConsistencyStatus", "", "AntennaChannelConsistencyReason", "", "AntennaChannelConsistencyOk", false, ...
...
elseif any(status == ["no_fading_channel_object", "awgn_identity_spatial_dimensions_no_array_kernel"])
    note = string(direction) + " uses an AWGN identity/no-fading sample operator. Configured Tx/Rx dimensions remain enforced, but no spatial channel kernel consumed array geometry or element patterns.";
else
