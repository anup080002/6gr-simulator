...
methods(Static)
    function state = initialize(cfg, runFolder, multiUser, controlTrials, totalTrafficFrames)
        ...
        state.PUCCHCurrentSlotGrantSnapshot = table();
        ...
        state.SweepPointStartSlot = 1;
        state.SweepPointLinkStateResetCount = 0;
        state.SweepPointInitialAccessResetCount = 0;
    ...
    function state = resetIndependentSweepPointLinkState(state, cfg, sweepPointIndex, startSlot)
        % Reset state that can causally carry decoding or adaptation gain
        % between fixed-SNR operating points. Geometry, accumulated truth
        % tables and application-delivery ledgers remain intact. Initial
        % access is reset only when the YAML explicitly requests an
        % independent access trial at every SNR point.
        if nargin < 3 || ~(isnumeric(sweepPointIndex) && isscalar(sweepPointIndex) && ...
                isfinite(sweepPointIndex) && sweepPointIndex >= 1)
            error("sixgr:truth:InvalidSweepPointIndex", ...
                "Independent sweep-point reset requires a positive finite sweep-point index.");
        end
        if nargin < 4 || ~(isnumeric(startSlot) && isscalar(startSlot) && ...
                isfinite(startSlot) && startSlot >= 1)
            error("sixgr:truth:InvalidSweepPointStartSlot", ...
        ...
        if accessPolicy == "independent_per_point"
            state.CellAcquisitionState = repmat( ...
                string(state.ControlGating.PBCHInitialState), nUsers, 1);
            state.AccessState = repmat( ...
                string(state.ControlGating.PRACHInitialState), nUsers, 1);
            state.LastPDCCHStatus = repmat("not_attempted", nUsers, 1);
            state.ControlEligibility = false(nUsers, 1);
            state.SchedulingEligibility = false(nUsers, 1);
            state.LastSuccessfulPBCHSlotByUE = nan(nUsers, 1);
            ...
            state.TimingAdvanceUpdateRequiredByUE = false(nUsers, 1);
            state.TimingAdvanceUpdateStatusByUE = repmat("not_evaluated", nUsers, 1);
            state.SweepPointInitialAccessResetCount = double(sixgr.util.structGet( ...
                state, "SweepPointInitialAccessResetCount", 0)) + 1;
        elseif accessPolicy ~= "continuous_runtime"
            error("sixgr:truth:InvalidSNRSweepInitialAccessStatePolicy", ...
                "Unsupported SNR-sweep initial-access state policy '%s'.", ...
                char(accessPolicy));
        end

        % A new operating point must obtain fresh causal CSI/SRS/TRS. The
        % measurement table is retained as evidence; its consumer filters
        % pre-boundary rows using SweepPointStartSlot.
        state.SRSValidityState = repmat(string(state.ControlGating.SRSInitialState), nUsers, 1);
        state.CSIValidityState = repmat(string(state.ControlGating.CSIInitialState), nUsers, 1);
        state.LastSuccessfulSRSSlotByUE = nan(nUsers, 1);
        state.LastSRSObservedSlotByUE = nan(nUsers, 1);
        ...
        state.LastTRSObservedSlotByCell = nan(nCells, 1);
        state.TRSFailureCountByCell = zeros(nCells, 1);
        state.RuntimeChannelStates = repmat( ...
            sixgr.channel.ChannelFactory.emptyRuntimeChannelState(), 0, 1);
        state.PUCCHChannelStateByUE = cell(nUsers, 1);

        % Feedback sourced by the preceding independent operating point
        % must not enter the fresh HARQ entities above. Preserve the grant
        % as evidence, but close it explicitly without claiming that a
        % PUCCH waveform was executed.
        pucchTrace = sixgr.util.structGet(state, "PUCCHGrantTraceTable", table());
        if istable(pucchTrace) && ~isempty(pucchTrace)
            executed = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                pucchTrace, "GrantExecutedFlag", false(height(pucchTrace), 1)));
            sourceSlot = double(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                pucchTrace, "SourceSlot", nan(height(pucchTrace), 1)));
            cancelMask = ~executed & isfinite(sourceSlot) & ...
                sourceSlot < round(double(startSlot));
            pucchTrace = sixgr.truth.CoupledTruthRuntime.setLogicalColumn( ...
                pucchTrace, "CanceledAtSweepBoundary", false(height(pucchTrace), 1));
            pucchTrace = sixgr.truth.CoupledTruthRuntime.setNumericColumn( ...
                pucchTrace, "CancellationSweepPoint", nan(height(pucchTrace), 1), false);
            pucchTrace = sixgr.truth.CoupledTruthRuntime.setNumericColumn( ...
                pucchTrace, "CancellationBoundarySlot", nan(height(pucchTrace), 1), false);
            pucchTrace = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                pucchTrace, "CancellationReason", repmat("", height(pucchTrace), 1), false);
            if any(cancelMask)
                cancelIdx = find(cancelMask);
                pucchTrace.CanceledAtSweepBoundary(cancelMask) = true;
                pucchTrace.CancellationSweepPoint(cancelMask) = round(double(sweepPointIndex));
                pucchTrace.CancellationBoundarySlot(cancelMask) = round(double(startSlot));
                pucchTrace = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    pucchTrace, "CancellationReason", cancelIdx, ...
                    "independent_snr_point_boundary_harq_state_reset");
                pucchTrace = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    pucchTrace, "PUCCHGrantState", cancelIdx, ...
                    "canceled_independent_sweep_boundary");
                pucchTrace = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    pucchTrace, "Status", cancelIdx, "CANCELED");
                pucchTrace = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    pucchTrace, "Notes", cancelIdx, ...
                    "Scheduled feedback was not executed because its source transmission belongs to the preceding independent SNR point.");
            end
            state.PUCCHGrantTraceTable = pucchTrace;
        end

        state.DLSchedulers = sixgr.truth.CoupledTruthRuntime.createSchedulers( ...
            state.CfgMobility, nCells, "DL", state.DLHarq);
        state.ULSchedulers = sixgr.truth.CoupledTruthRuntime.createSchedulers( ...
            state.CfgMobility, nCells, "UL", state.ULHarq);
        state.LastDLGrantCount = 0;
        state.LastULGrantCount = 0;
        state.LastDLGrantedUsers = 0;
        state.LastULGrantedUsers = 0;
        ...
        % Do not carry a traffic backlog from an outage point into the next
        % throughput operating point. Cumulative offered/delivered ledgers
        % remain untouched; the next canonical frame supplies new traffic.
        state.DLQueueBits = zeros(nUsers, 1);
        state.ULQueueBits = zeros(nUsers, 1);
        state.LastTrafficFrameApplied = round(double(startSlot)) - 1;
        state.SweepPointIndex = round(double(sweepPointIndex));
        state.SweepPointStartSlot = round(double(startSlot));
        state.SweepPointLinkStateResetCount = double(sixgr.util.structGet( ...
            state, "SweepPointLinkStateResetCount", 0)) + 1;
        state = sixgr.truth.CoupledTruthRuntime.refreshControlStateImpl(state);
    end
    ...
    function [state, channelState] = acquireRuntimeChannelStateForControl(state, cfg, ueIdx, direction)
        % Use the same per-link ChannelFactory state for causal control
        % measurements and their later data waveform.  A separate SRS
        % channel realization cannot authorize an exact MU precoder or IRC
        % combiner for the scheduled PUSCH/PDSCH channel.
        [state, channelState] = ...
            sixgr.truth.CoupledTruthRuntime.acquireRuntimeChannelStateForControlImpl( ...
            state, cfg, ueIdx, direction, NaN);
    end
    ...
    function grant = refreezeReplayGrantPHYContractRuntime(cfg, grant, direction, state, harqContext)
        % Public boundary for focused frozen-architecture HARQ replay tests.
        grant = sixgr.truth.CoupledTruthRuntime.refreezeReplayGrantPHYContract( ...
            cfg, grant, direction, state, harqContext);
    end
    ...
    function state = applyDecodedULHARQOutcomeRuntime(state, feedbackRow)
    ...
    function [state, grantsOut] = multiplexDueHARQACKOnPUSCHRuntime(state, grantsIn, dueSlot)
        [state, grantsOut] = sixgr.truth.CoupledTruthRuntime. ...
    ...
    function state = enqueueCSIReportRuntime(state, ueIdx, direction, row, cfgExecuted, csiMeasurementRow)
        if nargin < 5
            cfgExecuted = struct();
        end
        if nargin < 6
            csiMeasurementRow = table();
        end
        state = sixgr.truth.CoupledTruthRuntime.enqueueCSIReport( ...
            state, ueIdx, direction, row, cfgExecuted, csiMeasurementRow);
    end
    ...
    function state = finalizeFiniteHorizonRuntime(state, terminalSlot, reason)
        if nargin < 2 || ~(isnumeric(terminalSlot) && isscalar(terminalSlot) && ...
                isfinite(terminalSlot) && terminalSlot >= 0)
            terminalSlot = double(sixgr.util.structGet(state, "CurrentSlot", 0));
        end
        if nargin < 3 || strlength(strtrim(string(reason))) == 0
            reason = "finite_runtime_horizon_reached";
        end
        state = sixgr.truth.CoupledTruthRuntime.rightCensorPendingCausalEventsImpl( ...
...
methods(Static, Access=private)
    ...
    function state = advanceFrameImpl(state, absoluteFrame, snr_dB)
        ...
        state.CurrentSNR_dB = double(snr_dB);
        % Large-scale geometry/channel state exists for every UE in every
        % canonical slot, irrespective of whether the scheduler allocates
        % a data grant. Persist that runtime state now; a later decoded
        % waveform trial enriches the same (slot,UE) row in place.
        state = sixgr.truth.CoupledTruthRuntime.appendPerSlotServingPhysics(state);
    end
    ...
    function [cfgU, state] = applyUserContextImpl(cfgIn, state, ueIdx, direction)
        ...
        % TRS is a DL reference observed by the UE receiver.  Preserve the
        % observation for audit, but authorize timing/CFO correction only
        % for a DL receiver.  In particular, TDD reciprocity must not turn a
        % UE-side TRS acquisition delay into a gNB-side PUSCH timing shift.
        % The same endpoint/direction rule applies to FDD and TDD.
        trsConsumerContext = sixgr.phy.rx.authorizeReceiverTrackingDirection( ...
            trsContext, "DL", direction);
        ...
        userMeta.RuntimeTRSMeasurementDirection = char(string(trsConsumerContext.TrackingMeasurementDirection));
        userMeta.RuntimeReceiverTrackingConsumerDirection = char(string(trsConsumerContext.TrackingConsumerDirection));
        userMeta.RuntimeReceiverTrackingDirectionCompatible = logical(trsConsumerContext.TrackingDirectionCompatible);
        userMeta.RuntimeReceiverTrackingAuthorityPolicy = char(string(trsConsumerContext.TrackingAuthorityPolicy));
        userMeta.RuntimeReceiverTrackingAuthorityStatus = char(string(trsConsumerContext.TrackingAuthorityStatus));
        userMeta.RuntimeReceiverTrackingAuthorityReason = char(string(trsConsumerContext.TrackingAuthorityReason));
        userMeta.RuntimeTRSTimingApplicationScope = char(string(trsConsumerContext.TRSTimingApplicationScope));
        userMeta.RuntimeTRSCFOApplicationScope = char(string(trsConsumerContext.TRSCFOApplicationScope));
        userMeta.RuntimeTRSObservedTimingEstimateAvailable = logical(trsConsumerContext.ObservedTRSTimingEstimateAvailable);
        userMeta.RuntimeTRSObservedTimingEstimate_samples = double(trsConsumerContext.ObservedTRSTimingEstimate_samples);
        userMeta.RuntimeTRSObservedCFOEstimateAvailable = logical(trsConsumerContext.ObservedTRSCFOEstimateAvailable);
        userMeta.RuntimeTRSObservedEstimatedCFO_Hz = double(trsConsumerContext.ObservedTRSEstimatedCFO_Hz);
        userMeta.RuntimeTRSTimingEstimateAvailable = logical(trsConsumerContext.TRSTimingEstimateAvailable);
        userMeta.RuntimeTRSTimingEstimate_samples = double(trsConsumerContext.TRSTimingEstimate_samples);
        userMeta.RuntimeTRSCFOEstimateAvailable = logical(trsConsumerContext.TRSCFOEstimateAvailable);
        userMeta.RuntimeTRSEstimatedCFO_Hz = double(trsConsumerContext.TRSEstimatedCFO_Hz);
        userMeta.RuntimeTRSEstimatedOscillatorCFO_Hz = double(trsConsumerContext.TRSEstimatedOscillatorCFO_Hz);
        ...
        for cellId = 1:nCells
            ...
            for gi = 1:numel(cellGrants)
                ...
                if ~logical(sixgr.util.structGet(grant, ...
                        ...
                        ~logical(sixgr.util.structGet(grant, "Valid", true))
                    % AMC/rank feedback is applied after the scheduler
                    % candidate is formed. A failed exact refreeze is not an
                    % executable grant and must never be promoted to DCI or a
                    % primary grant trace. Release tentative new-data HARQ;
                    % retransmissions remain pending for a later feasible
                    % allocation.
                    state = sixgr.truth.CoupledTruthRuntime. ...
                        cancelUnexecutedHARQGrantImpl(state, grant, direction);
                    info.RejectedExactPHYGrants = double(sixgr.util.structGet( ...
                        info, "RejectedExactPHYGrants", 0)) + 1;
                    info.LastExactPHYRejectionReason = char(string( ...
    ...
    function [state, context, grantRow] = buildTrialContextFromGrantImpl(state, cfg, ueIdx, direction, grant)
        ...
        if isRetx
            ...
            % The stored HARQ snapshot owns the original transport block,
            % coding layout and soft-combining context.  The grant passed to
            % this method owns the current-slot scheduling, exact resource
            % feasibility and decoded-PDCCH authority.  Never execute a
            % retransmission using stale or missing control authority from
            % the stored first-transmission snapshot.
            replayGrant = sixgr.truth.CoupledTruthRuntime. ...
                mergeReplayCurrentGrantAuthority(replayGrant, grant);
            % The stored first-transmission snapshot owns the TB/coding
            % contract, but it must never own UCI placed on a later UL
            % occasion.  Re-apply the current grant's occasion-local
            % HARQ-ACK binding after the replay merge so a retransmission
            % cannot resurrect feedback already consumed on its original
            % PUSCH.
            replayGrant = sixgr.truth.mergeCachedGrantWithLiveRuntime( ...
                replayGrant, grant);
            % Reconcile every spatial alias after joining the immutable
            % HARQ transport-block snapshot with current-slot control
            % authority.  Stored snapshots can predate Rank/RankIndicator
            % fields, and current scheduler rows can retain stale rank-one
            % aliases even though NumLayers/Layers is the executable
            % waveform contract.
            replayGrant = sixgr.truth.CoupledTruthRuntime.normalizeGrantSnapshot( ...
                replayGrant, direction, state, ueIdx);
            ...
            % Bind the transport-block identity before the PHY/HARQ
            % context is frozen.  This is an immutable runtime identity
            % derived from the actual scheduled grant, not a reporting
            % fallback.  Retransmissions subsequently carry this same ID
            % from the stored HARQ context.
            runtimeTBId = sixgr.truth.CoupledTruthRuntime. ...
                transportBlockIdFromGrant(state, grant, direction, ueIdx);
            if ~(isscalar(runtimeTBId) && strlength(strtrim(runtimeTBId)) > 0)
                error("sixgr:truth:MissingRuntimeTransportBlockIdentity", ...
                    "%s new transmission has no immutable transport-block identity.", ...
                    char(direction));
            end
            grant.TransportBlockId = char(runtimeTBId);
            grant.TBId = char(runtimeTBId);
        ...
        % Keep the explicit trial argument, HARQ context and executable
        % grant snapshot on one current-occasion UCI payload.  Assign even
        % an empty vector so a stored retransmission context cannot retain
        % an earlier occasion's ACK bits.
        expectedUCI = sixgr.truth.CoupledTruthRuntime. ...
            resolveGrantExpectedUCIBits(context.GrantSnapshot);
    ...
    function grant = refreezeReplayGrantPHYContract(cfg, grant, direction, state, harqContext)
        ...
        frozenPHYGrant = sixgr.util.structGet(grant, "PHYGrant", struct());
        cfgReplay = cfg;
        if isstruct(frozenPHYGrant) && ~isempty(fieldnames(frozenPHYGrant))
            % A retransmission inherits the exact first-transmission antenna
            % and precoding architecture.  The transient per-user cfg can
            % carry only that user's selected 64x2 beam while the frozen
            % cell architecture is 64x4.  Project the frozen grant into the
            % replay cfg before refreezing so a stale per-user mirror cannot
            % redefine the hardware matrix.
            cfgReplay = sixgr.phy.grant.applyPHYGrantToConfig( ...
                cfgReplay, frozenPHYGrant);
        end
        if isfield(grant, "PHYGrant")
            grant = rmfield(grant, "PHYGrant");
        end
        phyGrant = sixgr.phy.grant.freezePHYGrant(cfgReplay, direction, grant, ...
            "Frame", double(sixgr.util.structGet(grant, "Frame", state.CurrentFrame)), ...
            "Slot", double(sixgr.util.structGet(grant, "Slot", state.CurrentSlot)), ...
            "HARQContext", harqContext);
        grant.PHYGrant = phyGrant;
        grant.PHYGrantContextId = char(string(phyGrant.GrantContextId));
    end

    function replayGrant = mergeReplayCurrentGrantAuthority(replayGrant, currentGrant)
        if ~(isstruct(replayGrant) && isstruct(currentGrant) && ...
                ~isempty(fieldnames(currentGrant)))
            return;
        end
        authorityFields = [ ...
            ...
            ... % Current measured spatial design and executable matrices.
            "MUMIMOPrecoderType", ...
            "MUMIMORequiredLeakageThreshold_dB", ...
            "MUMIMODesiredSubspaceGain_dB", ...
            "MUMIMORequiredMinimumDesiredGain_dB", ...
            "MUMIMOSpatialDesignStatus", ...
            ...
            "PrecodingMatrixLogicalPorts", "LogicalPrecodingMatrix", ...
            "PrecodingMatrix", "HybridElementToPortMatrix", ...
            "HybridElementToPortMatrixSHA256", ...
            "BaseHybridElementToPortMatrixSHA256", ...
            "MUMIMOHybridRFDesignPolicy", "MUMIMOHybridRFDesignStatus", ...
            "PrecodingActive", "PrecodingMode", "PrecoderSource", ...
            "PrecodingApplicationStage", ...
            "PriorMUMIMOGroupId", "PriorMUMIMOGroupSize", ...
            "PriorMUMIMOPairingEvidenceSource", ...
            ... % Current-slot causal UL sounding authority. A HARQ
            ... % retransmission reuses the original TB/soft buffer, but
            ... % its executable codebook/TPMI decision must be bound to
            ... % the fresh SRS state owned by the current scheduler grant.
            "SRSValid", "SRSCausalUsable", ...
            "SRSCausalMeasurementId", "LastSuccessfulSRSSlot", ...
            "SRSAgeSlots", "SRSCausalAgeSlots", "SRSCausalStatus", ...
            "SRSMeasurementAuthoritySource", ...
            "SRI", "SRSResourceIndicator", ...
            "SRSValidityState", "CSIValidityState", ...
            ...
            "ExecutableTBSMode", "ExactAllocationCapacityBits", ...
            "ExactAllocationCapacityBytes", "ExactTBSBits", ...
            "ExactTBSBytes", "ExactNREPerPRB", ...
            "ExactTBSUsedFastNREApprox", "ExactTBSInfo", ...
            "PlanningOnlyApproximation", "TimingDecision", ...
            "K0", "K1", "K2", "ControlAbsoluteSlot", ...
            "ScheduledAbsoluteSlot", "HARQFeedbackAbsoluteSlot", ...
            "SchedulingCCID", "ScheduledCCID", "CarrierIndicator", ...
            "SourceBWPID", "TargetBWPID", "BWPId", ...
            "GrantContextId", "ControlDecodeOk", "DCICrcPass", ...
            "PDCCHPayloadMatch", "PDCCHCausalGrantDecodeOk", ...
            "PDCCHMissedDetection", "PDCCHFalseAlarm", ...
            "GrantValid", "NegativeExpectedOk", ...
            "PDCCHGrantBindingRequired", "PDCCHGrantBindingOk", ...
            "PDCCHGrantBindingStatus", "PDCCHGrantBindingFailureCode", ...
            "PDCCHGrantDCIId", "PDCCHGrantDCIFieldsHash", ...
            "PDCCHGrantFieldsHash", "PDCCHGrantSearchSpaceId", ...
            "PDCCHGrantCORESETId", "PDCCHGrantAggregationLevel", ...
            "PDCCHGrantCandidateIndex", "PDCCHGrantDCIFormat", ...
            "GrantControlState", "PDCCHControlFailureReason", ...
            "PDCCHControlEvidenceSource", "ControlDecodeSource"];
        for fieldIdx = 1:numel(authorityFields)
    ...
    function [state, channelState] = acquireRuntimeChannelStateForControlImpl(state, cfg, ueIdx, direction, servingCell)
        ...
        direction = upper(string(direction));
        if nargin < 5 || ~(isnumeric(servingCell) && isscalar(servingCell) && isfinite(servingCell) && servingCell >= 1)
            servingCell = double(sixgr.truth.CoupledTruthRuntime.currentServingCellForUE(state, ueIdx));
        end
        if ~(isfinite(servingCell) && servingCell >= 1)
            servingCell = 1;
        end
        ...
        slotStartTime_s = max(0, double(sixgr.util.structGet(state, "CurrentSlot", 1)) - 1) * ...
            slotDuration_s;
        channelState.TargetSlotStartTime_s = double(slotStartTime_s);
        channelState.TargetSlot = double(sixgr.util.structGet(state, "CurrentSlot", NaN));
        channelState.TargetFrame = double(sixgr.util.structGet(state, "CurrentFrame", NaN));
        channelState.TargetUEIndex = double(ueIdx);
        channelState.TargetServingCell = double(servingCell);
        channelState.Direction = char(direction);
    end
    ...
    function [state, context] = attachRuntimeChannelStateToGrantContext(state, cfg, ueIdx, direction, context)
        ...
        [state, chState] = ...
            sixgr.truth.CoupledTruthRuntime.acquireRuntimeChannelStateForControlImpl( ...
            state, cfg, ueIdx, direction, servingCell);
        linkKey = string(sixgr.util.structGet(chState, "LinkKey", ""));
        slotStartTime_s = double(sixgr.util.structGet(chState, ...
    ...
    function state = writeTablesImpl(state, runFolder)
        ...
        standalonePUCCHRequired = logical(sixgr.util.structGet(state.CfgMobility, ...
            "control_gating.pucch_required", sixgr.util.structGet(state.CfgMobility, ...
            "run.controlGating.pucchRequired", false)));
        if standalonePUCCHRequired || (istable(pucchT) && ~isempty(pucchT))
            sixgr.truth.CoupledTruthRuntime.writeMirroredTable(layout, "pucch_trials.csv", pucchT);
        else
            % An explicitly transferred PUSCH-UCI scenario has no standalone
            % PUCCH primary table. Skip it instead of writing a headerless or
            % synthetic placeholder CSV.
            for stalePath = [string(fullfile(layout.ControlCSVDir, "pucch_trials.csv")), ...
                    string(fullfile(layout.AirInterfaceCSVDir, "pucch_trials.csv"))]
                if isfile(stalePath)
                    delete(stalePath);
                end
            end
        ...
        srsDecisionT = sixgr.util.structGet( ...
            state.ControlTrials, "SRSResourceDecisions", table());
        if istable(srsDecisionT) && ~isempty(srsDecisionT)
            sixgr.truth.CoupledTruthRuntime.writeMirroredTable( ...
                layout, "srs_resource_decisions.csv", srsDecisionT);
        end
        sixgr.truth.CoupledTruthRuntime.writeOptionalFeatureMirroredTable( ...
        ...
        protocolPayloadT = sixgr.util.structGet(state, "PacketSDULedgerTable", table());
        if logical(sixgr.util.structGet(state, "ProtocolWaveformEnabled", false)) && ...
                istable(protocolPayloadT) && ~isempty(protocolPayloadT)
            sixgr.util.csvWriteTable(fullfile(layout.PacketFlowCSVDir, ...
                "protocol_in_path_payload_delivery.csv"), protocolPayloadT);
        end
        ...
        slotTraceTable = sixgr.util.structGet(state, "SlotTraceTable", table());
        sixgr.util.csvWriteTable(fullfile(layout.ReportCSVDir, "run_state.csv"), ...
            runStateTable, "PreserveSchema", true);
        % SlotTrace is a typed causal contract.  Empty no-grant reason
        % columns mean that every scheduled direction produced a grant;
        % dropping those columns would destroy the distinction between
        % "observed empty" and "producer did not implement the field".
        sixgr.util.csvWriteTable(fullfile(layout.ReportCSVDir, "slot_trace.csv"), ...
            slotTraceTable, "PreserveSchema", true);
        sixgr.util.csvWriteTable(fullfile(layout.PacketFlowCSVDir, "slot_trace.csv"), ...
            slotTraceTable, "PreserveSchema", true);
    ...
    function state = commitGrantExecutionImpl(state, ueIdx, direction, grant)
        ...
        exactTextFields = ["TBSInputModulation","TBSInputSource"];
        for fieldName = exactTextFields
            if ismember(fieldName, string(traceT.Properties.VariableNames))
                traceT = sixgr.truth.CoupledTruthRuntime.localAssignTraceTextValue( ...
                    traceT, char(fieldName), idx, sixgr.util.structGet(grant, fieldName, ""));
            end
        end
        exactNumericFields = ["TBSInputNumLayers","TBSInputNPRB", ...
            "TBSInputNREPerPRB","TBSInputTargetCodeRate","TBSInputXOverhead"];
        for fieldName = exactNumericFields
            if ismember(fieldName, string(traceT.Properties.VariableNames))
                traceT.(char(fieldName))(idx) = double(sixgr.util.structGet(grant, fieldName, NaN));
            end
        end
        ...
        for digestField = ["AppliedPrecoderMatrixSHA256","RequestedPrecoderSHA256", ...
                "AppliedPrecoderSHA256","PrecoderDigestDomain","FrozenGrantContextId"]
            if ismember(digestField, string(traceT.Properties.VariableNames))
                traceT = sixgr.truth.CoupledTruthRuntime.localAssignTraceTextValue( ...
                    traceT, digestField, idx, sixgr.util.structGet(grant, digestField, ""));
            end
        end
    ...
    function [allowDL, allowUL, slotLabel, partition] = slotDuplexState(cfg, canonicalSlot)
        ...
        if duplex == "FDD"
            ...
            configuredDLAllocation = sixgr.truth.CoupledTruthRuntime. ...
                requiredConfiguredSymbolAllocation(cfg, "DL");
            configuredULAllocation = sixgr.truth.CoupledTruthRuntime. ...
                requiredConfiguredSymbolAllocation(cfg, "UL");
            symbolsPerSlot = double(context.SymbolsPerSlot);
            fullDirectionAllocation = [0, symbolsPerSlot];
            if ~sixgr.truth.CoupledTruthRuntime.symbolAllocationFitsPartition( ...
                    configuredDLAllocation, fullDirectionAllocation) || ...
                    ~sixgr.truth.CoupledTruthRuntime.symbolAllocationFitsPartition( ...
                    configuredULAllocation, fullDirectionAllocation)
                error("sixgr:truth:CoupledTruthRuntime:FDDAllocationOutsideCarrierSlot", ...
                    "Configured FDD DL/UL allocations must fit their separate %d-symbol carrier slots.", ...
                    round(symbolsPerSlot));
            end
            partition = struct( ...
                ...
                "AllowUL", true, ...
                "IsSpecialSlot", false, ...
                "DLSymbolAllocation", fullDirectionAllocation, ...
                "GuardSymbolAllocation", [0, 0], ...
                "ULSymbolAllocation", fullDirectionAllocation, ...
                "ConfiguredDLDataSymbolAllocation", configuredDLAllocation, ...
                "ConfiguredULDataSymbolAllocation", configuredULAllocation, ...
    ...
    function [state, trialT] = completeSlotImpl(state, cfgU, ueIdx, direction, trialT, res)
        ...
        if upper(string(direction)) == "UL"
            ...
            % standalone PUCCH.  Consume only the bits decoded from this
            % PUSCH waveform and close the corresponding DL HARQ process.
            state = sixgr.truth.CoupledTruthRuntime.consumeDecodedPUSCHHARQACK( ...
                state, sixgr.util.structGet(res, "HARQ", struct()));
        end
            ...
            if istable(csiT) && ~isempty(csiT)
                ...
                if ismember("Slot", string(csiT.Properties.VariableNames))
                    mask = mask & isfinite(double(csiT.Slot)) & ...
                        round(double(csiT.Slot)) == round(sourceSlot);
                end
                if ismember("UEIndex", string(csiT.Properties.VariableNames))
                    mask = mask & isfinite(double(csiT.UEIndex)) & ...
                        round(double(csiT.UEIndex)) == round(double(ueIdx));
                end
                hit = find(mask, 1, "last");
                if ~isempty(hit)
                    csiMeasurementRow = csiT(hit, :);
                end
            end
        end
    ...
    function state = processDueFeedback(state)
        ...
        if ~(istable(grantTrace) && ~isempty(grantTrace))
            ...
            if ~(currentULAllowed && isfinite(currentULSymbols) && currentULSymbols > 0)
                ...
                canceled = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                    grantTrace, "CanceledAtSweepBoundary", false(height(grantTrace), 1)));
                rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                    grantTrace, "RightCensored", false(height(grantTrace), 1)));
                multiplexedOnPUSCH = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                    grantTrace, "MultiplexedOnPUSCH", false(height(grantTrace), 1)));
                dueGrantMask = ~logical(grantTrace.GrantExecutedFlag) & ~canceled & ...
                    ~rightCensored & ~multiplexedOnPUSCH & ...
                    abs(double(grantTrace.ScheduledAbsoluteSlot) - double(state.CurrentSlot)) < 1e-9;
            end
        end
        if any(dueGrantMask)
            dueGrantRows = grantTrace(dueGrantMask, :);
            % Freeze the simultaneous emission set before any receiver
            % consumes feedback.  Reciprocal same-slot interference must
            % not depend on the serial loop order used for state updates.
            state.PUCCHCurrentSlotGrantSnapshot = dueGrantRows;
        ...
        csiRightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
            state.PendingCSITable, "RightCensored", false(height(state.PendingCSITable), 1)));
        dueCSIMask = ~logical(state.PendingCSITable.Processed) & ~csiRightCensored & ...
            double(state.PendingCSITable.DueSlot) <= double(state.CurrentSlot);
        if ~any(dueCSIMask)
            return;
        end
        ...
        state.PendingCSITable.DeliveredSlot(dueCSIMask) = double(state.CurrentSlot);
        state.PendingCSITable.DeliveryStatus(dueCSIMask) = repmat("delivered_to_runtime_scheduler", nnz(dueCSIMask), 1);
    end
    ...
    function [state, harqFields] = updateHARQState(state, ueIdx, direction, cfgU, row, res)
        ...
        if direction == "UL"
            % NR uplink HARQ has no UE-to-gNB ACK/NACK transmission on
            % PUCCH.  The gNB already owns the PUSCH decoder result and
            % applies it to the UL HARQ process before forming a later UL
            % scheduling/DCI decision.  Treating this result as PUCCH UCI
            % fabricates an air-interface transmission and can illegally
            % place it in a TDD special slot.
            % Runtime slots are one-based while slotStartTimeSec(k+1) is
            % the end boundary of slot k.  Publish the gNB decoder outcome
            % at that next scheduling boundary so it cannot precede the
            % completed PUSCH delivery in the protocol/HARQ ledger.
            feedbackDueSlot = double(slotIdx + 1);
            feedbackMechanism = "gnb_pusch_decoder";
            feedbackEvidenceSource = "same_waveform_pusch_rx_crc";
            ...
            % Downlink HARQ feedback is UE-originated UCI and therefore
            % must traverse the configured PUCCH waveform at the exact
            % scheduler-resolved K1 occasion.
            fb.UCIBitCount = 1;
            ...
            feedbackMechanism = "pucch_uci";
            feedbackEvidenceSource = "same_waveform_pucch_rx_uci";
        end
        ...
        runtimeTBId = string(sixgr.util.structGet(grantSnapshot, "HARQTBContext.TBId", ""));
        if ~(isscalar(runtimeTBId) && strlength(strtrim(runtimeTBId)) > 0)
            error("sixgr:truth:MissingRuntimeHARQTransportBlockIdentity", ...
                "%s executed HARQ attempt has no immutable transport-block identity.", ...
                char(direction));
        end
        t.TBId = runtimeTBId;
    ...
    function state = appendTelemetry(state, ueIdx, row)
        ...
        if isfinite(double(csi.SINR_dB))
            estimatedSINR = double(csi.SINR_dB);
            widebandSINRSource = string(csi.SINRSource);
            widebandSINRValueRole = string(csi.SINRValueRole);
        end
        cqi = double(csi.CQI);
        ...
        r.ExpectedDopplerHz = abs(double(r.RadialVelocity_mps)) * double(sixgr.truth.CoupledTruthRuntime.resolveCarrierFrequencyHz(state)) / 299792458;
    ...
    function state = updateUserStats(state, ueIdx, direction, row)
        ...
        scheduledBits = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            row, "TBSize_bits", sixgr.truth.CoupledTruthRuntime.rowValue( ...
            row, "CurrentTBSBits", NaN)));
        if ~(isfinite(scheduledBits) && scheduledBits >= 0)
            scheduledBits = 0;
        end
        offeredBits = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            row, "OfferedBits", NaN));
        if ~(isfinite(offeredBits) && offeredBits >= 0)
            offeredBits = 0;
        end
        stats(ueIdx).ScheduledBitsSum = ...
            stats(ueIdx).ScheduledBitsSum + scheduledBits;
        stats(ueIdx).OfferedBitsSum = ...
            stats(ueIdx).OfferedBitsSum + offeredBits;
    ...
    function T = buildUserPerformanceTable(state)
        ...
        for ueIdx = 1:double(state.NumUsers)
            ...
            r.DL_Throughput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(dl.ScheduledBitsSum, duration_s, dl.Frames);
            r.UL_Throughput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(ul.ScheduledBitsSum, duration_s, ul.Frames);
            r.DL_OfferedThroughput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(dl.OfferedBitsSum, duration_s, dl.Frames);
            r.UL_OfferedThroughput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(ul.OfferedBitsSum, duration_s, ul.Frames);
            r.DL_Goodput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(dl.GoodBitsSum, duration_s, dl.Frames);
            r.UL_Goodput_Mbps = sixgr.truth.CoupledTruthRuntime.directionThroughputMbps(ul.GoodBitsSum, duration_s, ul.Frames);
            ...
            r.UserOfferedThroughput_Mbps = sum([r.DL_OfferedThroughput_Mbps r.UL_OfferedThroughput_Mbps], "omitnan");
            r.UserGoodput_Mbps = sum([r.DL_Goodput_Mbps r.UL_Goodput_Mbps], "omitnan");
    ...
    function [state, ueState] = buildSchedulerUEState(state, cfg, ueIdx, direction, servingCell)
        ...
        ueState.SchedulerCQIRawCQI = double(sixgr.util.structGet(feedback, "SchedulerCQIRawCQI", NaN));
    ...
    function state = applyPRACHTrialImpl(state, ueIdx, trialT)
        ...
        if ok
            ...
            rrcSetupCompleteRequired = logical(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            ...
            if fullRA
                ...
                if rrcSetupCompleteRequired
                    eventSource = "slot_coupled_initial_access_rrc_waveform_chain";
                    eventNote = eventNote + " Waveform-backed SRB1 RRCSetupComplete is required for terminal access.";
                end
            end
            ...
            if fullRA
                ...
                if rrcSetupCompleteRequired
                    rrcOk = logical(sixgr.truth.CoupledTruthRuntime.rowValue(row, "RRCSetupRequestDecoded", false)) && ...
                        ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SRB1Installed", false)) && ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowValue(row, "RRCSetupCompleteCRC", false)) && ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowValue(row, "RRCSetupCompleteDecoded", false)) && ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowValue(row, "RRCConnected", false));
                    payloadHash = strtrim(string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        row, "RRCSetupCompletePayloadSHA256", "")));
                    if ~rrcOk || strlength(payloadHash) ~= 64
                        error("sixgr:truth:MissingRRCSetupCompleteEvidence", ...
                            ["A strict in-path PRACH row requiring RRCSetupComplete passed without " ...
                            "complete waveform receiver, SRB1, state-machine, and payload-hash evidence."]);
                    end
    ...
    function result = consumeReferenceSignalMeasurementImpl(state, signalType, targetType, targetId, consumerSlot, maxAgeSlots)
        ...
        pointStartSlot = double(sixgr.util.structGet(state, "SweepPointStartSlot", 1));
        if istable(T) && ~isempty(T) && isfinite(pointStartSlot) && ...
    ...
    function state = updateLatestULFeedbackFromSRSTrial(state, ueIdx, row, slotIdx)
        ...
        if feedsDLFromSRS
            ...
            [dlCqi, dlMCS, dlMod, dlRate, dlAdjustedSINR, dlBackoff, dlSchedSource] = ...
                sixgr.truth.CoupledTruthRuntime.resolveDLSRSReciprocitySchedulerCQI( ...
                state.CfgMobility, sinrDb, rawSrsCQI, row);
            if isfinite(dlCqi) && dlCqi > 0 && srsWidebandMCSUsable
                dlLatest.CQI = double(dlCqi);
                previousDLMCS = double(sixgr.util.structGet(dlLatest, "MCSIndex", NaN));
                previousDLCRCPass = double(sixgr.util.structGet(dlLatest, "FeedbackCRCPass", NaN));
                if isfinite(previousDLCRCPass) && previousDLCRCPass == 0 && ...
                        isfinite(previousDLMCS) && previousDLMCS >= 0 && ...
                        isfinite(dlMCS) && dlMCS > previousDLMCS
                    dlMCS = double(previousDLMCS);
                    profile = sixgr.link.resolveMCSProfile( ...
                        sixgr.link.resolveConfiguredMCSTable(state.CfgMobility, "DL"), dlMCS);
                    if logical(sixgr.util.structGet(profile, "Valid", false))
                        dlMod = char(string(profile.Modulation));
                        dlRate = double(profile.TargetCodeRate);
                    end
                    dlSchedSource = char(string(dlSchedSource) + "_capped_by_recent_dl_nack");
                end
                if isfinite(dlMCS) && dlMCS >= 0
                    ...
                    dlLatest.MCSSelectionSource = char(dlSchedSource);
                    dlLatest.MCSValueStatus = "measured_srs_reciprocity_adjusted_for_pdsch_data_channel";
                    dlLatest.SchedulerCQIRawCQI = double(rawSrsCQI);
                    dlLatest.SchedulerAdjustedSINR_dB = double(dlAdjustedSINR);
                    dlLatest.SchedulerSINRBackoff_dB = double(dlBackoff);
                    dlLatest.SchedulerCQISource = char(dlSchedSource);
                end
            end
            if isfinite(ri)
    ...
    function [cqi, mcsIndex, modulation, targetCodeRate, adjustedSINR_dB, backoff_dB, source] = ...
        ...
        elseif layerPolicy == "reference_signal_wideband_with_backoff"
            % This is a measured SRS receiver quantity after the YAML-owned
            % SRS-to-PUSCH scheduling margin.  Do not include the generic
            % token "configured" in the provenance label: that token is
            % intentionally rejected by resolveWidebandCQI because it is
            % also used by non-measured configured-SNR inputs.
            source = "measured_ul_srs_wideband_after_srs_to_pusch_scheduler_margin";
        else
        ...
        if ~(isfinite(cqi) && cqi > 0)
            ...
            % the unadjusted reference-signal CQI.  That bypasses the
            % configured SRS-to-PUSCH margin and can over-schedule PUSCH.
            cqi = NaN;
            source = "ul_srs_adjusted_scheduler_cqi_unavailable";
        end
        if ~(isfinite(cqi) && cqi > 0)
            return;
        end
    ...
    function [cqi, mcsIndex, modulation, targetCodeRate, adjustedSINR_dB, backoff_dB, source] = ...
            resolveDLSRSReciprocitySchedulerCQI(cfg, sinrDb, fallbackCQI, row)
        cqi = NaN;
        mcsIndex = NaN;
        modulation = "";
        targetCodeRate = NaN;
        adjustedSINR_dB = NaN;
        backoff_dB = NaN;
        source = "measured_dl_srs_reciprocity_after_scheduler_margin";
        sinrDb = double(sinrDb);
        if ~(isscalar(sinrDb) && isfinite(sinrDb))
            return;
        end
        ...
        backoff_dB = double(sixgr.truth.CoupledTruthRuntime.firstFiniteScalar( ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.dlSRSReciprocitySchedulingBackoff_dB", NaN), ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.dlSRSToPDSCHSINRBackoff_dB", NaN), ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.dlReferenceSignalSchedulingBackoff_dB", NaN), ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.dlMCSBackoff_dB", NaN), ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.maxCSIAgingPenalty_dB", NaN), ...
            6));
        if ~(isfinite(backoff_dB) && backoff_dB >= 0)
            backoff_dB = 6;
        end
        ...
        try
            feedback = sixgr.link.resolveWidebandCQI(struct( ...
                ...
                "SINRValueStatus", "PASS", ...
                "RankIndicator", double(ri)), cfg, "DL");
            cqi = double(sixgr.util.normalizeReportedCQI( ...
                sixgr.util.structGet(feedback, "WidebandCQI", NaN)));
        catch
            cqi = NaN;
        ...
        if ~(isfinite(cqi) && cqi > 0)
            % Reciprocity scheduling is allowed only from the adjusted,
            % provenance-accepted measurement.  Do not substitute raw SRS
            % CQI when that conversion is unavailable.
            cqi = NaN;
            source = "dl_srs_reciprocity_adjusted_scheduler_cqi_unavailable";
        end
        if ~(isfinite(cqi) && cqi > 0)
            return;
        end
    ...
    function tf = shouldPreservePendingULDataMCS(state, ueIdx, srsMCSIndex, slotIdx)
        ...
        rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
            pendingT, "RightCensored", false(height(pendingT), 1)));
        mask = ~processed & ~rightCensored & upper(string(pendingT.Direction)) == "UL" & ...
    ...
    function [state, grant, allowExecution] = applyPDCCHGrantTrialImpl(state, grant, direction, trialT)
        ...
        % This method only receives scheduler-owned data grants. Their
        % decoded DCI is intrinsic authority for PHY execution regardless
        % of whether PDCCH was also selected as an initial-access gate.
        gatingActive = true;
    ...
    function state = recordInitialAccessGrantCompletion(state, ueIdx, direction, row)
        ...
        elseif direction == "DL"
            if sixgr.truth.CoupledTruthRuntime.hasInitialAccessEvent(state, ueIdx, "MSG3_PUSCH_COMPLETED") && ...
                    ~sixgr.truth.CoupledTruthRuntime.hasInitialAccessEvent(state, ueIdx, "MSG4_CONTENTION_RESOLUTION_COMPLETED") && ...
                    ~sixgr.truth.CoupledTruthRuntime.hasInitialAccessEvent(state, ueIdx, "RRC_SETUP_COMPLETE_ACCEPTED") && ...
    ...
    function state = recordInitialAccessEvent(state, ueIdx, eventName, direction, sourceArtifact, stageName, slotIdx, valueSource, notes, localRASlot)
        ...
        row.SweepPointIndex = double(sixgr.util.structGet(state, ...
            "SweepPointIndex", 1));
        ...
        slotsPerFrame = double(sixgr.util.structGet(state, "SlotsPerFrame", NaN));
        if isfinite(slotsPerFrame) && slotsPerFrame >= 1 && slotsPerFrame == fix(slotsPerFrame)
            [frame1, frameSlot1] = sixgr.time.canonicalSlotToFrameSlot( ...
                double(slotIdx), slotsPerFrame);
            row.Frame = double(frame1);
            row.FrameSlot = double(frameSlot1);
        else
    ...
    function state = finalizeInitialAccessLifecycleIfReady(state, ueIdx)
        ...
        cfg = sixgr.util.structGet(state, "CfgMobility", struct());
        requireRRCSetupComplete = logical(sixgr.util.structGet( ...
            cfg, "initial_access.rrc.require_setup_complete", false));
        state.InitialAccessLifecycleTraceTable = ...
            sixgr.truth.deriveInitialAccessProcedureDelay(T, ...
            "SlotDuration_s", double(sixgr.util.structGet(state, "SlotDuration_s", NaN)), ...
            "RequireRRCSetupComplete", requireRRCSetupComplete);
    end

    function state = appendPerSlotServingPhysics(state)
        if isempty(fieldnames(sixgr.util.structGet(state, "LargeScaleState", struct()))) || ...
                ~isfield(state, "UE") || ~isfield(state.UE, "pos_m")
            return;
        end
        nUE = size(state.UE.pos_m, 1);
        fcHz = double(sixgr.truth.CoupledTruthRuntime.resolveCarrierFrequencyHz(state));
        interferenceMode = sixgr.truth.CoupledTruthRuntime.resolveInterferenceExecutionMode( ...
            state.CfgMobility, state.MultiUser);
        for ueIdx = 1:nUE
            servingCell = double(state.CurrentServingIdx(ueIdx));
            if ~(isfinite(servingCell) && servingCell >= 1)
                continue;
            ...
            r = sixgr.truth.CoupledTruthRuntime.emptyServingRow();
            r.Slot = double(state.CurrentCanonicalSlot);
            r.Time_s = (double(r.Slot) - 1) * double(state.SlotDuration_s);
            r.UEID = sixgr.truth.CoupledTruthRuntime.resolveUEID(state.UE, ueIdx);
            r.Lat = double(state.CurrentUELat(ueIdx));
            r.Lon = double(state.CurrentUELon(ueIdx));
            r.X_m = double(state.UE.pos_m(ueIdx, 1));
            ...
            r.Heading_deg = sixgr.truth.CoupledTruthRuntime.ueColumn(state.UE, "heading_deg", ueIdx);
            r.Heading_rad = double(r.Heading_deg) * pi / 180;
            r.CellID = servingCell;
            r.ServingCell = servingCell;
            r.ServingSite = double(state.Layout.bs.siteId(servingCell));
            r.ServingSector = double(state.Layout.bs.sectorId(servingCell));
            r.ServingBeamIndex = double(state.LargeScaleState.BeamIndex(ueIdx, servingCell));
            r.ServingBeamGain_dB = double(state.LargeScaleState.BeamGain_dB(ueIdx, servingCell));
            ...
            r.AppliedDopplerHz = double(state.LargeScaleState.Doppler_Hz(ueIdx, servingCell));
            r.SignedDoppler_Hz = double(state.LargeScaleState.SignedDoppler_Hz(ueIdx, servingCell));
            r.ExpectedDopplerHz = abs(double(r.RadialVelocity_mps)) * fcHz / 299792458;
            r.ChannelComplianceMode = char(string(sixgr.util.structGet(state.LargeScaleState, "ChannelComplianceMode", "")));
            r.PathlossModelSource = char(string(sixgr.util.structGet(state.LargeScaleState, "PathlossModelSource", "")));
            r.PathlossComplianceStatus = char(string(sixgr.util.structGet(state.LargeScaleState, "PathlossComplianceStatus", "")));
            r.FallbackUsedForPathloss = logical(sixgr.util.structGet(state.LargeScaleState, "FallbackUsedForPathloss", false));
            ...
            r.LOSComplianceStatus = char(string(sixgr.util.structGet(state.LargeScaleState, "LOSComplianceStatus", "")));
            r.LOSComplianceReason = char(string(sixgr.util.structGet(state.LargeScaleState, "LOSComplianceReason", "")));
            r.LargeScaleWidebandSINR_dB = sixgr.truth.CoupledTruthRuntime.estimateRuntimeWidebandSINR( ...
                state, ueIdx, servingCell, interferenceMode);
            r.LargeScaleSINR_dB = r.LargeScaleWidebandSINR_dB;
    ...
    function state = upsertServingTraceRow(state, row)
        row = sixgr.truth.CoupledTruthRuntime.normalizeStructRowToPrototype( ...
            row, sixgr.truth.CoupledTruthRuntime.emptyServingRow());
        newT = struct2table(row, "AsArray", true);
        current = sixgr.util.structGet(state, "ServingTraceTable", table());
        if istable(current) && ~isempty(current) && ...
                all(ismember(["Slot","UEID"], string(current.Properties.VariableNames)))
            mask = double(current.Slot) == double(row.Slot) & ...
                double(current.UEID) == double(row.UEID);
            if any(mask)
                current(find(mask, 1, "last"), :) = newT;
                state.ServingTraceTable = current;
                return;
            end
        end
        state.ServingTraceTable = sixgr.truth.CoupledTruthRuntime.appendCompatTable(current, newT);
    end
    ...
    function tf = hasInitialAccessEvent(state, ueIdx, eventName)
        ...
        mask = double(T.UEIndex) == double(ueIdx) & ...
            upper(strtrim(string(T.EventName))) == upper(strtrim(string(eventName)));
        if ismember("SweepPointIndex", string(T.Properties.VariableNames))
            mask = mask & double(T.SweepPointIndex) == double(sixgr.util.structGet( ...
                state, "SweepPointIndex", 1));
        end
        tf = any(mask);
    end
    ...
    function state = updatePacketDeliveryFromHARQ(state, direction, grant, harqRow, decodedTbBits)
        direction = upper(string(direction));
        ...
        if logical(sixgr.truth.CoupledTruthRuntime.rowLogical(harqRow, "CombinedDecodeOK", false))
            ...
            if logical(sixgr.util.structGet(state, "ProtocolWaveformEnabled", false))
                ...
                if isempty(decodedTbBits)
                    error("sixgr:protocol:MissingDecodedTransportBlockBits", ...
                        ["A protocol-bound PHY decode passed for transport block %s, " + ...
                         "but the receiver did not return its decoded bits."], string(tbId));
                end
                [protocolDelivered, protocolEvidence] = bridge.deliverDecoded( ...
                    string(tbId), decodedTbBits, double(deliveryTime));
                protocolStatus = string(sixgr.util.structGet(protocolEvidence, "Status", ""));
                sduT.ProtocolDecodedBitCount(mask) = double(sixgr.util.structGet( ...
                    protocolEvidence, "DecodedBitCount", NaN));
                sduT.ProtocolDecodedBitExact(mask) = logical(sixgr.util.structGet( ...
                    protocolEvidence, "DecodedBitExact", false));
                sduT.ProtocolDecodedMACSHA256(mask) = string(sixgr.util.structGet( ...
                    protocolEvidence, "DecodedMACSHA256", ""));
                sduT.ProtocolDecodedRLC_SHA256(mask) = string(sixgr.util.structGet( ...
                    protocolEvidence, "DecodedRLC_SHA256", ""));
                sduT.ProtocolReceiverEvidenceSource(mask) = string(sixgr.util.structGet( ...
                    protocolEvidence, "EvidenceSource", ""));
    ...
    function state = refreshApplicationPacketDeliveries(state)
        ...
        for k = 1:numel(packetIds)
            ...
            if ~protocolRequired
                % PHY/HARQ delivery is not application delivery.  Without
                % the configured same-waveform SDAP/PDCP/RLC/MAC bridge,
                % retain the packet as an offered/scheduled application
                % object and make the missing end-to-end boundary explicit.
                if fullyScheduled && allDelivered
                    packetT.ProtocolReassemblyStatus(pidx) = "protocol_stack_not_enabled";
                    packetT.DeliverySource(pidx) = "not_available_without_same_waveform_protocol";
                    packetT.Status(pidx) = "phy_delivery_complete_protocol_not_enabled";
                end
                continue;
            end
            ...
            packetT.SameWaveformProtocolComplete(pidx) = logical(protocolComplete);
    ...
    function row = buildGrantTraceRow(state, grant, direction, slotIdx, frameIdx, ueIdx, feedback)
        ...
        row.IsRetransmission = sixgr.phy.grant.isExplicitHARQRetransmission( ...
            grant, sixgr.util.structGet(grant, "PHYGrant", struct()), struct());
        ...
        row.TBSInputModulation = sixgr.truth.CoupledTruthRuntime.firstString( ...
            sixgr.util.structGet(grant, "TBSInputModulation", row.Modulation), row.Modulation);
        row.TBSInputNumLayers = sixgr.truth.CoupledTruthRuntime.firstNumeric( ...
            sixgr.util.structGet(grant, "TBSInputNumLayers", sixgr.util.structGet(grant, "NumLayers", NaN)), NaN);
        row.TBSInputNPRB = sixgr.truth.CoupledTruthRuntime.firstNumeric( ...
            sixgr.util.structGet(grant, "TBSInputNPRB", row.PRBCount), NaN);
        row.TBSInputNREPerPRB = sixgr.truth.CoupledTruthRuntime.firstNumeric( ...
            sixgr.util.structGet(grant, "TBSInputNREPerPRB", sixgr.util.structGet(grant, "NREPerPRB", NaN)), NaN);
        row.TBSInputTargetCodeRate = sixgr.truth.CoupledTruthRuntime.firstNumeric( ...
            sixgr.util.structGet(grant, "TBSInputTargetCodeRate", row.TargetCodeRate), NaN);
        row.TBSInputXOverhead = sixgr.truth.CoupledTruthRuntime.firstNumeric( ...
            sixgr.util.structGet(grant, "TBSInputXOverhead", sixgr.util.structGet(grant, "XOverhead", NaN)), NaN);
        row.TBSInputSource = sixgr.truth.CoupledTruthRuntime.firstString( ...
            sixgr.util.structGet(grant, "TBSInputSource", ...
            sixgr.util.structGet(grant, "ResourceAccountingSource", "")), "");
        ...
        row.SchedulerCQIRawCQI = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerCQIRawCQI", NaN), NaN);
        row.SchedulerAdjustedSINR_dB = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerAdjustedSINR_dB", NaN), NaN);
        row.SchedulerSINRBackoff_dB = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerSINRBackoff_dB", NaN), NaN);
        row.SchedulerCQISource = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "SchedulerCQISource", ""), "");
        ...
        row.MUMIMOAdmissionReceiveCombiningMatrixSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "MUMIMOAdmissionReceiveCombiningMatrixSHA256", ""), "");
        row.MUMIMOReceiveProcessingMode = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "MUMIMOReceiveProcessingMode", ""), "");
        ...
        row.AppliedPrecoderMatrixSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "AppliedPrecoderMatrixSHA256", ""), "");
        row.RequestedPrecoderSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "RequestedPrecoderSHA256", ""), "");
        row.AppliedPrecoderSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "AppliedPrecoderSHA256", row.AppliedPrecoderMatrixSHA256), row.AppliedPrecoderMatrixSHA256);
        row.PrecoderDigestDomain = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "PrecoderDigestDomain", ""), "");
        row.FrozenGrantContextId = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "FrozenGrantContextId", ""), "");
    ...
    function grant = normalizeGrantSnapshot(grant, direction, state, ueIdx)
        ...
        grant.MUMIMOAdmissionReceiveCombiningMatrixSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "MUMIMOAdmissionReceiveCombiningMatrixSHA256", ""), "");
        grant.MUMIMOReceiveProcessingMode = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "MUMIMOReceiveProcessingMode", ""), "");
        ...
        grant.AppliedPrecoderMatrixSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "AppliedPrecoderMatrixSHA256", ""), "");
        grant.RequestedPrecoderSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "RequestedPrecoderSHA256", ...
            sixgr.util.structGet(grant, "PHYGrant.PrecodingState.AppliedMatrixSHA256", "")), "");
        grant.AppliedPrecoderSHA256 = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "AppliedPrecoderSHA256", grant.AppliedPrecoderMatrixSHA256), grant.AppliedPrecoderMatrixSHA256);
        grant.PrecoderDigestDomain = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "PrecoderDigestDomain", ...
            sixgr.util.structGet(grant, "PHYGrant.PrecodingState.WaveformDomain", "")), "");
        grant.FrozenGrantContextId = sixgr.truth.CoupledTruthRuntime.firstString(sixgr.util.structGet(grant, "FrozenGrantContextId", ...
            sixgr.util.structGet(grant, "PHYGrant.GrantContextId", sixgr.util.structGet(grant, "PHYGrantContextId", ""))), "");
        ...
        grant.SchedulerCQIRawCQI = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerCQIRawCQI", ...
            sixgr.util.structGet(feedback, "SchedulerCQIRawCQI", NaN)), NaN);
        grant.SchedulerAdjustedSINR_dB = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerAdjustedSINR_dB", ...
            sixgr.util.structGet(feedback, "SchedulerAdjustedSINR_dB", NaN)), NaN);
        grant.SchedulerSINRBackoff_dB = sixgr.truth.CoupledTruthRuntime.firstNumeric(sixgr.util.structGet(grant, "SchedulerSINRBackoff_dB", ...
    ...
    function state = applyDecodedULHARQOutcome(state, feedbackRow)
        direction = upper(string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            feedbackRow, "Direction", "")));
        if direction ~= "UL"
            error("sixgr:truth:CoupledTruthRuntime:InvalidULHARQOutcomeDirection", ...
                "Decoded UL HARQ outcome boundary requires Direction=UL.");
        end
        sourceSlot = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            feedbackRow, "SourceSlot", NaN));
        dueSlot = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            feedbackRow, "DueSlot", NaN));
        if ~(isfinite(sourceSlot) && sourceSlot >= 1 && sourceSlot == fix(sourceSlot) && ...
                isfinite(dueSlot) && dueSlot == sourceSlot + 1)
            error("sixgr:truth:CoupledTruthRuntime:InvalidULHARQDecoderTiming", ...
                "UL HARQ decoder outcome must become available at the completed PUSCH slot boundary; it is not a PUCCH/K1 event.");
        end
        rnti = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            feedbackRow, "RNTI", NaN));
        harqId0 = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
    ...
    function grant = applyMeasuredFeedbackAMCToGrant(grant, feedback, scheduler, cfg, direction)
        ...
        % SRS measures the channel subspace required for causal MU pairing,
        % but its pilot-only SINR is not yet a measured post-equalization
        % PDSCH/PUSCH operating point under simultaneous shared-PRB traffic.
        % When YAML enables the policy, cap only the first new-data MU MCS at
        % the direction-specific YAML value.  Rank, measured spatial
        % signatures, block-diagonalization/IRC matrices and raw SRS CQI are
        % preserved.  As soon as a data CSI report replaces SRS as the latest
        % feedback source, ordinary measured CQI/OLLA regains authority.
        muFirstDataBootstrapApplied = false;
        muFirstDataBootstrapMCS = NaN;
        muFirstDataBootstrapPolicy = lower(strtrim(string(sixgr.util.structGet(cfg, ...
            "phy.linkAdaptation.muMIMOFirstDataBootstrapPolicy", ...
            "yaml_mcs_until_first_data_feedback"))));
        muFirstDataBootstrapEnabled = logical(sixgr.util.structGet(cfg, ...
            "phy.linkAdaptation.muMIMOFirstDataBootstrapEnabled", false));
        feedbackSourceSignal = upper(strtrim(string(sixgr.util.structGet( ...
            feedback, "FeedbackSourceSignal", ""))));
        isReferenceSignalOnlyFeedback = any(feedbackSourceSignal == ...
            ["SRS", "SRS_RECIPROCITY"]);
        isSharedMUMIMOGrant = logical(sixgr.util.structGet(grant, ...
            "MUMIMOEnabled", false)) && double(sixgr.util.structGet(grant, ...
            "MUMIMOGroupSize", 1)) > 1;
        if muFirstDataBootstrapEnabled && isSharedMUMIMOGrant && ...
                isReferenceSignalOnlyFeedback
            if muFirstDataBootstrapPolicy ~= "yaml_mcs_until_first_data_feedback"
                error("sixgr:truth:InvalidMUMIMOFirstDataBootstrapPolicy", ...
                    "Unsupported MU-MIMO first-data bootstrap policy '%s'.", ...
                    char(muFirstDataBootstrapPolicy));
            end
            if upper(string(direction)) == "UL"
                configuredFirstDataMCS = double(sixgr.util.structGet(cfg, ...
                    "phy.linkAdaptation.ulMUMIMOFirstDataBootstrapMCSIndex", NaN));
            else
                configuredFirstDataMCS = double(sixgr.util.structGet(cfg, ...
                    "phy.linkAdaptation.dlMUMIMOFirstDataBootstrapMCSIndex", NaN));
            end
            if ~(isscalar(configuredFirstDataMCS) && isfinite(configuredFirstDataMCS) && ...
                    configuredFirstDataMCS >= 0 && configuredFirstDataMCS <= 31 && ...
                    configuredFirstDataMCS == fix(configuredFirstDataMCS))
                error("sixgr:truth:InvalidMUMIMOFirstDataBootstrapMCS", ...
                    "YAML MU-MIMO first-data MCS for %s must be an integer in [0,31].", ...
                    char(upper(string(direction))));
            end
            muFirstDataBootstrapMCS = min(round(double(mcsIndex)), ...
                round(double(configuredFirstDataMCS)));
            bootstrapProfile = sixgr.link.resolveMCSProfile(mcsTable, ...
                muFirstDataBootstrapMCS);
            if ~logical(sixgr.util.structGet(bootstrapProfile, "Valid", false))
                error("sixgr:truth:UnsupportedMUMIMOFirstDataBootstrapMCS", ...
                    "YAML MU-MIMO first-data MCS %d is unavailable in table '%s' for %s.", ...
                    round(muFirstDataBootstrapMCS), char(string(mcsTable)), ...
                    char(upper(string(direction))));
            end
            mcsIndex = double(muFirstDataBootstrapMCS);
            modStr = char(string(bootstrapProfile.Modulation));
            targetCodeRate = double(bootstrapProfile.TargetCodeRate);
            muFirstDataBootstrapApplied = true;
        end
        ...
        if muFirstDataBootstrapApplied
            selectionSource = "yaml_mu_mimo_first_data_mcs_until_data_feedback";
            grant.MCSValueStatus = "yaml_mu_mimo_first_data_bootstrap_pending_data_feedback";
        end
        ...
        grant.SchedulerCQIRawCQI = double(sixgr.util.structGet(feedback, "SchedulerCQIRawCQI", NaN));
        grant.SchedulerAdjustedSINR_dB = double(sixgr.util.structGet(feedback, "SchedulerAdjustedSINR_dB", NaN));
        grant.SchedulerSINRBackoff_dB = double(sixgr.util.structGet(feedback, "SchedulerSINRBackoff_dB", NaN));
        grant.SchedulerCQISource = char(string(sixgr.util.structGet(feedback, "SchedulerCQISource", "")));
        schedulerCQISource = strtrim(string(grant.SchedulerCQISource));
        if strlength(schedulerCQISource) > 0
            grant.CQIProvenance = char(schedulerCQISource);
        else
            grant.CQIProvenance = char(string(sixgr.util.structGet(feedback, "CQIProvenance", ...
                sixgr.util.structGet(grant, "CQIProvenance", ""))));
        end
        ...
        if ~isempty(scheduler) && ismethod(scheduler, "freezePHYGrantForGrant")
            try
                grant = scheduler.freezePHYGrantForGrant(grant);
            catch ME
                grant.Valid = false;
                grant.ExactPHYFeasible = false;
                grant.ExactPHYInfeasibilityReason = char("post_amc_phygrant_refreeze_failed:" + string(ME.message));
                grant.GrantBlocker = grant.ExactPHYInfeasibilityReason;
            end
        end
    end
    ...
    function state = enqueueCSIReport(state, ueIdx, direction, row, cfgExecuted, csiMeasurementRow)
        ...
        if nargin < 6 || ~istable(csiMeasurementRow)
            csiMeasurementRow = table();
        end
        ...
        report.EnqueuedSlot = double(state.CurrentSlot);
        report.SourceSignal = "PDSCH-DMRS";
        if upper(string(direction)) == "UL"
            report.SourceSignal = "PUSCH-DMRS";
        end
        ...
        measuredCSIRSAvailable = upper(string(direction)) == "DL" && ...
            ~isempty(csiMeasurementRow) && ...
            logical(sixgr.truth.CoupledTruthRuntime.rowLogical( ...
            csiMeasurementRow, "Observed", false)) && ...
            (logical(sixgr.truth.CoupledTruthRuntime.rowLogical( ...
            csiMeasurementRow, "CSIMeasurementAvailable", false)) || ...
            logical(sixgr.truth.CoupledTruthRuntime.rowLogical( ...
            csiMeasurementRow, "ChannelEstimateAvailable", false)));
        if measuredCSIRSAvailable
            % Preserve the receiver-observed CSI-RS report verbatim.  Link
            % adaptation may derive a different scheduler CQI/MCS later,
            % but it must never overwrite the primary measured report.
            ...
            report.SINRValueStatus = char(string(sixgr.truth.CoupledTruthRuntime.rowValue(csiMeasurementRow, "SINRValueStatus", "")));
            report.MeasurementSource = char(string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                csiMeasurementRow, "MeasurementSource", "received_csirs_reference_signal_power")));
            report.SourceSignal = "CSI-RS";
            if isfinite(report.CQI) && report.CQI > 0
                [modulation, targetCodeRate, mcsIndex] = sixgr.link.amcFromCQI( ...
                    report.CQI, "", NaN, state.CfgMobility, direction);
                report.MCSIndex = double(mcsIndex);
                report.TargetCodeRate = double(targetCodeRate);
                report.Modulation = char(string(modulation));
            end
        end
        ...
        sourceSignal = string(report.SourceSignal);
        state = sixgr.truth.CoupledTruthRuntime.publishReferenceSignalMeasurementImpl( ...
        ...
        if report.DueSlot <= state.CurrentSlot
            ...
            latest = sixgr.truth.CoupledTruthRuntime.emptyLatestFeedbackRow();
            latest.Valid = true;
            latest.CQI = sixgr.truth.CoupledTruthRuntime.schedulerResolvedCQI(report);
            latest.RI = report.RI;
            latest.PMI = double(sixgr.truth.CoupledTruthRuntime.sanitizeFeedbackPMI( ...
            ...
            state.PendingCSITable.DeliveredSlot(end) = double(state.CurrentSlot);
            state.PendingCSITable.DeliveryStatus(end) = "delivered_to_runtime_scheduler";
        end
    end
    ...
    function [state, report] = applyLinkAdaptationToCSIReport(state, report, ueIdx, direction, row)
        ...
        report.OLLAUpdateCountBeforeEvent = NaN;
        report.OLLAUpdateCountAfterEvent = NaN;
        report.OLLAUpdateAppliedAtThisEvent = false;
        report.OLLAUpdateAuthorized = false;
        report.OLLAFeedbackEventType = "csi_measurement_delivery";
        report.OLLAUpdateAuthoritySource = "metrics.AllowOLLAUpdate";
        ...
        metrics.FeedbackEventType = "csi_measurement_delivery";
        metrics.AllowOLLAUpdate = false;
        ...
        report.OLLAUpdateCountBeforeEvent = double(sixgr.util.structGet( ...
            decision, "OLLAUpdateCountBeforeEvent", report.OLLAUpdateCount));
        report.OLLAUpdateCountAfterEvent = double(sixgr.util.structGet( ...
            decision, "OLLAUpdateCountAfterEvent", report.OLLAUpdateCount));
        report.OLLAUpdateAppliedAtThisEvent = logical(sixgr.util.structGet( ...
            decision, "OLLAUpdateAppliedAtThisEvent", false));
        report.OLLAUpdateAuthorized = logical(sixgr.util.structGet( ...
            decision, "OLLAUpdateAuthorized", false));
        report.OLLAFeedbackEventType = char(string(sixgr.util.structGet( ...
            decision, "OLLAFeedbackEventType", "csi_measurement_delivery")));
        report.OLLAUpdateAuthoritySource = char(string(sixgr.util.structGet( ...
            decision, "OLLAUpdateAuthoritySource", "metrics.AllowOLLAUpdate")));
        ...
        report.SchedulerCQISource = char(string(sixgr.util.structGet(decision, "CQISource", "")));
    ...
    function csi = resolveMeasuredRuntimeCSIForRow(row, cfg, direction)
        ...
        [measuredSINR, sinrSource, sinrRole, sinrStatus] = ...
            sixgr.truth.CoupledTruthRuntime.schedulerMeasuredSINRFromRow(row, cfg, direction);
        subbandSINR = sixgr.truth.CoupledTruthRuntime.rowFirstNumericVector(row, ...
            ["SubbandSINRVector_dB","SubbandSINR_dB","PerRBSINR_dB"]);
        layerSINR = sixgr.truth.CoupledTruthRuntime.rowFirstNumericVector(row, ...
            ["PostEqSINRPerLayer_dB","PerLayerSINR_dB","SelectedLayerSINR_dB","LayerSINRdB"]);
    ...
    function [sinr_dB, source, role, status] = schedulerMeasuredSINRFromRow(row, cfg, direction)
        if nargin < 2
            cfg = struct();
        end
        if nargin < 3
        ...
        validIdx = find(valid);
        [sinr_dB, relIdx] = min(validSINR(validIdx));
        chosenIdx = validIdx(relIdx);
        ...
        if ~isempty(receiverIdx) && ~isempty(postEqIdx)
            ...
            [source, role, status] = sixgr.truth.CoupledTruthRuntime.schedulerCQIResolverSINRProvenance( ...
                validSource(chosenIdx), validRole(chosenIdx), validStatus(chosenIdx));
        end

        [guardedSINR, guardReason] = sixgr.truth.CoupledTruthRuntime.schedulerDecoderQualityGuardedSINR( ...
            row, double(validSINR(valid)), cfg, direction);
        if isfinite(guardedSINR) && guardedSINR < sinr_dB
            sinr_dB = double(guardedSINR);
            source = "measured_scheduler_csi_conservative_min_channel_estimate_posteq_channel_estimation_guarded_sinr:" + string(guardReason);
            role = "measured_post_equalization_scheduling_input";
            status = "OK";
        end
    end

    function [guardedSINR, reason] = schedulerDecoderQualityGuardedSINR(row, candidateSINR_dB, cfg, direction)
        guardedSINR = NaN;
        reason = "";
        candidateSINR_dB = double(candidateSINR_dB(:));
        candidateSINR_dB = candidateSINR_dB(isfinite(candidateSINR_dB));
        if isempty(candidateSINR_dB)
            return;
        end
        baseSINR_dB = min(candidateSINR_dB);
        if ~isfinite(baseSINR_dB)
            return;
        end

        nmse_dB = sixgr.truth.CoupledTruthRuntime.rowFirstFinite(row, ...
            ["NMSE_dB","PilotResidualNMSE_dB","ChannelEstimatePilotResidualNMSE_dB","TrueChannelNMSE_dB"], NaN);
        % MismatchSensitivity_dB is the standard deviation of channel-
        % magnitude samples in dB.  It is a dispersion diagnostic, not a
        % measured loss, and can become very large around legitimate MIMO
        % spatial nulls.  Treating it as an additive SINR penalty clamps AMC
        % by the configured maximum guard even when the receiver has valid
        % post-equalization evidence.  Only an actual measured aging loss is
        % dimensionally valid as an additive penalty here.
        agingLoss_dB = sixgr.truth.CoupledTruthRuntime.rowFirstFinite(row, ...
            ["ChannelAgingLoss_dB"], NaN);
        if ~(isfinite(nmse_dB) || (isfinite(agingLoss_dB) && agingLoss_dB > 0))
            return;
        end

        nmsePenalty_dB = 0;
        if isfinite(nmse_dB)
            gamma = 10 .^ (double(baseSINR_dB) / 10);
            nmseLin = 10 .^ (double(nmse_dB) / 10);
            if isfinite(gamma) && gamma > 0 && isfinite(nmseLin) && nmseLin > 0
                nmsePenalty_dB = 10 * log10(1 + gamma * nmseLin);
            end
        end
        agingPenalty_dB = max(0, double(agingLoss_dB));
        if ~isfinite(agingPenalty_dB)
            agingPenalty_dB = 0;
        end
        maxPenalty_dB = double(sixgr.util.structGet(cfg, "phy.linkAdaptation.maxDecoderQualityGuardPenalty_dB", ...
            sixgr.util.structGet(cfg, "phy.linkAdaptation.maxChannelEstimationGuardPenalty_dB", 18)));
        if ~(isfinite(maxPenalty_dB) && maxPenalty_dB >= 0)
            maxPenalty_dB = 18;
        end
        totalPenalty_dB = min(maxPenalty_dB, max(0, nmsePenalty_dB + agingPenalty_dB));
        if ~(isfinite(totalPenalty_dB) && totalPenalty_dB > 0)
            return;
        end
        guardedSINR = double(baseSINR_dB) - double(totalPenalty_dB);
        reason = "direction=" + upper(string(direction)) + ...
            ";base_sinr_db=" + string(sprintf('%.3f', baseSINR_dB)) + ...
            ";nmse_db=" + string(sprintf('%.3f', nmse_dB)) + ...
            ";nmse_penalty_db=" + string(sprintf('%.3f', nmsePenalty_dB)) + ...
            ";channel_aging_penalty_db=" + string(sprintf('%.3f', agingPenalty_dB)) + ...
            ";total_penalty_db=" + string(sprintf('%.3f', totalPenalty_dB));
    end
    ...
    function grant = buildGrantSnapshot(cfgU, row, direction, ueIdx, rnti)
        ...
        grant = struct( ...
            ...
            "CQIUsed", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "CQIUsed", NaN)), ...
            "RawCQIDerivedMCS", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "RawCQIDerivedMCS", NaN)), ...
            "CQIBasedMCS", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "CQIBasedMCS", NaN)), ...
            "SmoothedCQI", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SmoothedCQI", NaN)), ...
            "InstantaneousCQIMCS", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "InstantaneousCQIMCS", NaN)), ...
            "DeltaMCS", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "DeltaMCS", NaN)), ...
            "StaticDeltaMCS", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "StaticDeltaMCS", NaN)), ...
            "MCSSelectionSource", char(string(sixgr.truth.CoupledTruthRuntime.rowValue(row, "MCSSelectionSource", ""))), ...
            "CQIProvenance", char(string(sixgr.truth.CoupledTruthRuntime.rowValue(row, "CQIProvenance", ""))), ...
            "MCSValueStatus", char(string(sixgr.truth.CoupledTruthRuntime.rowValue(row, "MCSValueStatus", ""))), ...
            "SchedulerCQIRawCQI", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SchedulerCQIRawCQI", NaN)), ...
            "SchedulerAdjustedSINR_dB", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SchedulerAdjustedSINR_dB", NaN)), ...
            "SchedulerSINRBackoff_dB", double(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SchedulerSINRBackoff_dB", NaN)), ...
            "SchedulerCQISource", char(string(sixgr.truth.CoupledTruthRuntime.rowValue(row, "SchedulerCQISource", ""))), ...
    ...
    function [quality_dB, sourceToken] = representativeCurrentLinkQuality(state)
        ...
        if istable(pendingT) && ~isempty(pendingT) && ...
            ...
            rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                pendingT, "RightCensored", false(height(pendingT), 1)));
            finiteMask = isfinite(sinrVals) & isfinite(slotVals) & ~rightCensored;
            if any(finiteMask)
                currentSlot = double(sixgr.util.structGet(state, "CurrentSlot", NaN));
    ...
    function snr_dB = resolveRuntimeSignalSNRForUE(state, cfg, ueIdx, direction)
        ...
        if istable(pendingT) && ~isempty(pendingT) && ...
            ...
            pendingMask = pendingMask & ~logical( ...
                sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                pendingT, "RightCensored", false(height(pendingT), 1)));
            if any(pendingMask)
    ...
    function row = emptyDirectionStatRow()
        row = struct("RNTI", NaN, "Frames", 0, "CRCSum", 0, ...
            "ScheduledBitsSum", 0, "OfferedBitsSum", 0, ...
            "GoodBitsSum", 0, "SINRSum", 0, "SINRCount", 0);
    end
    ...
    function T = annotatePUCCHGrantTraceTableImpl(state, T)
        ...
        canceled = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault(T, "CanceledAtSweepBoundary", false(n, 1)));
        rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault(T, "RightCensored", false(n, 1)));
        standaloneDecodeOk = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault(T, "PUCCHDecodeOk", false(n, 1)));
        ...
        classification(~observed & ~canceled & ~rightCensored) = "active_but_simplified";
        materialization = repmat("active_integrated_waveform_feedback_runtime", n, 1);
        materialization(transferred) = "active_integrated_pusch_uci_feedback_runtime";
        materialization(~observed & ~canceled & ~rightCensored) = "active_scheduled_pending_feedback_due_not_reached";
        materialization(canceled) = "active_scheduled_feedback_canceled_at_independent_snr_boundary";
        materialization(rightCensored) = "active_scheduled_feedback_right_censored_at_finite_horizon";
        ...
        if any(observed)
            ...
            consumer(transferred) = "HARQEntity.onFeedback_via_PUSCH_UCI";
        ...
        valueRole = repmat("explicit_pucch_grant_and_feedback_runtime_state", n, 1);
        valueRole(transferred) = "explicit_pucch_grant_transferred_to_same_waveform_pusch_uci";
        T = sixgr.truth.CoupledTruthRuntime.setStringColumn(T, "ValueSource", valueSource, true);
        T = sixgr.truth.CoupledTruthRuntime.setStringColumn(T, "ValueRole", valueRole, true);
        ...
        T = sixgr.truth.CoupledTruthRuntime.setLogicalColumn(T, "FinalizedFlag", observed | canceled | rightCensored);
        T = sixgr.truth.CoupledTruthRuntime.setLogicalColumn(T, "PlaceholderFlag", false(n, 1));
        T = sixgr.truth.CoupledTruthRuntime.setLogicalColumn(T, "FallbackFlag", false(n, 1));
        naReason = repmat("", n, 1);
        naReason(observed) = "not_applicable_feedback_observed_and_finalized";
        naReason(~observed & ~canceled & ~rightCensored) = "feedback_due_slot_not_reached_in_this_bounded_run";
        naReason(canceled) = "independent_snr_point_boundary_harq_state_reset";
        naReason(rightCensored) = "finite_horizon_right_censor_no_waveform_claim";
    ...
    function T = applyPRACHFullRARuntimeTokens(signalName, T)
        ...
        rrcOk = sixgr.truth.CoupledTruthRuntime.logicalVectorOrDefault( ...
            sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault(T, ...
            "RRCSetupCompleteDecoded", false(n, 1)), n, false) & ...
            sixgr.truth.CoupledTruthRuntime.logicalVectorOrDefault( ...
            sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault(T, ...
            "RRCConnected", false(n, 1)), n, false);
        completedMask = fullAttemptMask & raCompleted & (~rrcRequired | rrcOk);
        if ~any(fullAttemptMask)
            return;
        end
        ...
        % Classification describes execution provenance, not decoder
        % outcome. A waveform-backed attempt that terminates after a missed
        % Msg1 detection is still integrated truth; its status records the
        % causal termination without claiming completion.
        T.SourceClassification(fullAttemptMask) = "active_integrated";
        T.RuntimeMaterializationStatus(fullAttemptMask) = ...
            "active_integrated_four_step_ra_waveform_attempt";
        T.RuntimeMaterializationStatus(fullAttemptMask & ~preambleDetected) = ...
            "active_integrated_four_step_ra_waveform_terminated_at_msg1_detection";
        T.RuntimeMaterializationStatus(fullAttemptMask & preambleDetected & ~raCompleted) = ...
            "active_integrated_four_step_ra_waveform_terminated_before_msg4_completion";
        T.RuntimeMaterializationStatus(fullAttemptMask & raCompleted & rrcRequired & ~rrcOk) = ...
            "active_integrated_initial_access_waveform_terminated_before_rrc_setup_complete";
        T.RuntimeMaterializationStatus(completedMask) = "active_integrated_four_step_ra_waveform_msg1_msg2_msg3_msg4";
        T.RuntimeMaterializationStatus(completedMask & rrcRequired) = ...
            "active_integrated_initial_access_waveform_msg1_msg2_msg3_msg4_rrc_setup_complete";
        T.ControlGatingEffect(fullAttemptMask) = "random_access_gate_full_four_step_ra_attempt";
        T.ControlGatingEffect(completedMask) = "random_access_gate_full_four_step_ra";
        T.RuntimeStateConsumer(fullAttemptMask) = "CoupledTruthRuntime.applyPRACHTrial";
        T.RuntimeConsumer(fullAttemptMask) = "CoupledTruthRuntime.applyPRACHTrial";
        T.RuntimeEvidenceSource(fullAttemptMask) = "sixgr.phy.ra.runFourStepRA";
        T.ValueDefinition(fullAttemptMask) = ...
            "canonical four-step random-access waveform attempt; receiver outcomes and causal termination stage are preserved";
        T.ValueDefinition(completedMask) = "canonical PRACH row backed by Msg1 PRACH, Msg2 RAR, Msg3 PUSCH, and Msg4 contention-resolution waveform evidence";
        T.ValueDefinition(completedMask & rrcRequired) = ...
            "canonical initial-access row backed by Msg1 through Msg4 plus decoded SRB1 RRCSetupComplete PUSCH/UL-SCH waveform evidence";
    end
    ...
    function T = setStringValueAt(T, name, idx, value)
        n = height(T);
        idx = unique(round(double(idx(:))), "stable");
        ...
        if isscalar(value)
            current(idx) = value;
        elseif numel(value) == numel(idx)
            current(idx) = reshape(value, [], 1);
        else
            error("sixgr:truth:InvalidStringAssignment", ...
                "Column %s received %d row indices but %d values.", ...
                char(string(name)), numel(idx), numel(value));
        end
        T.(field) = current;
    end
    ...
    function row = emptyUserPerformanceRow()
        row = struct( ...
            ...
            "DL_OfferedThroughput_Mbps", NaN, "UL_OfferedThroughput_Mbps", NaN, ...
            "DL_Goodput_Mbps", NaN, "UL_Goodput_Mbps", NaN, ...
            "DL_BLER", NaN, "UL_BLER", NaN, ...
            "DL_MeanMeasuredSINR_dB", NaN, "UL_MeanMeasuredSINR_dB", NaN, ...
            "UserThroughput_Mbps", NaN, ...
            "UserOfferedThroughput_Mbps", NaN, "UserGoodput_Mbps", NaN, ...
        ...
        if istable(pendingFeedback) && ~isempty(pendingFeedback)
            for ri = 1:height(pendingFeedback)
                ...
                if logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "Processed", false)) || ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "RightCensored", false))
                    continue;
                end
                deliveryMechanism = lower(strtrim(string( ...
                    sixgr.truth.CoupledTruthRuntime.rowValue(row, "DeliveryMechanism", "pucch"))));
                if deliveryMechanism == "pusch_uci"
                    continue;
                end
        ...
        if istable(pucchGrants) && ~isempty(pucchGrants)
            for ri = 1:height(pucchGrants)
                ...
                if logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "GrantExecutedFlag", false)) || ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "CanceledAtSweepBoundary", false)) || ...
                        logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "RightCensored", false))
                    continue;
                end
                if logical(sixgr.truth.CoupledTruthRuntime.rowLogical(row, "MultiplexedOnPUSCH", false))
                    continue;
                end
                uciType = lower(strtrim(string(sixgr.truth.CoupledTruthRuntime.rowValue(row, "UCIType", "harq_ack"))));
                if strlength(uciType) > 0 && ~contains(uciType, "harq")
    ...
    function [state, grantsOut] = multiplexDueHARQACKOnPUSCHImpl(state, grantsIn, dueSlot)
        ...
        % A retransmitted UL HARQ grant can inherit the GrantSnapshot from
        % its previous transmission.  UCI-on-PUSCH reservation fields are
        % occasion-local runtime state and must never be replayed with that
        % snapshot.  Preserve an existing binding only when every source
        % row is still pending, belongs to this exact due slot and is bound
        % to this exact PUSCH grant context.  Otherwise clear only the
        % transient HARQ-ACK fields before resolving the current occasion.
        grantsOut = sixgr.truth.CoupledTruthRuntime. ...
            sanitizePUSCHHARQACKBindingsForDueSlot(state, grantsOut, dueSlot);
        due = sixgr.truth.CoupledTruthRuntime.collectPUCCHDueHARQACKImpl( ...
            state, dueSlot);
        if isempty(due)
            return;
        end

        assignedGrantIds = strings(0, 1);
        for gi = 1:numel(grantsOut)
            ueIdx = double(sixgr.util.structGet(grantsOut(gi), "UEIndex", NaN));
            rnti = double(sixgr.util.structGet(grantsOut(gi), "RNTI", NaN));
            match = false(numel(due), 1);
            for di = 1:numel(due)
                dueUE = double(sixgr.util.structGet(due(di), "UEIndex", NaN));
                dueRNTI = double(sixgr.util.structGet(due(di), "RNTI", NaN));
                match(di) = (isfinite(ueIdx) && isfinite(dueUE) && ueIdx == dueUE) || ...
                    (isfinite(rnti) && isfinite(dueRNTI) && rnti == dueRNTI);
            end
            matched = due(match);
            if isempty(matched)
                continue;
            end
            sourceSlots = arrayfun(@(x) double(x.SourceSlot), matched(:));
            harqIds = arrayfun(@(x) double(x.HarqID), matched(:));
            [~, order] = sortrows([sourceSlots(:) harqIds(:)], [1 2]);
            matched = matched(order);
            sourceSlots = sourceSlots(order);
            harqIds = harqIds(order);
            grantIds = string({matched.PUCCHGrantId}).';
            if any(strlength(strtrim(grantIds)) == 0) || ...
                    any(ismember(grantIds, assignedGrantIds))
                error("sixgr:truth:InvalidPUSCHHARQACKReservation", ...
                    "Each pending DL HARQ feedback row must bind exactly once to a PUSCH UCI payload.");
            end
            assignedGrantIds = [assignedGrantIds; grantIds]; %#ok<AGROW>
            ackBits = int8(arrayfun(@(x) logical(x.AckBit), matched(:)));
            contextId = string(sixgr.util.structGet(grantsOut(gi), ...
                "GrantContextId", sixgr.util.structGet(grantsOut(gi), ...
                "PHYGrant.GrantContextId", "")));
            grantsOut(gi).ExpectedUCIBits = ackBits(:);
            grantsOut(gi).MultiplexedHARQACKBits = ackBits(:);
            grantsOut(gi).UCIOnPUSCHApplied = true;
            grantsOut(gi).UCIOnPUSCHSource = "pending_dl_harq_ack_nrULSCHMultiplex";
            grantsOut(gi).PUCCHCollisionPolicy = "harq_ack_multiplexed_on_pusch";
    ...
    function grantsOut = sanitizePUSCHHARQACKBindingsForDueSlot(state, grantsIn, dueSlot)
        grantsOut = grantsIn;
        dueSlot = round(double(dueSlot));
        ...
        for gi = 1:numel(grantsOut)
            grantIdsToken = string(sixgr.util.structGet(grantsOut(gi), ...
                "UCIOnPUSCHFeedbackGrantIds", ""));
            if ~(isscalar(grantIdsToken) && strlength(strtrim(grantIdsToken)) > 0)
                continue;
            end
            grantIds = split(grantIdsToken, "|");
            grantIds = grantIds(strlength(strtrim(grantIds)) > 0);
            contextId = string(sixgr.util.structGet(grantsOut(gi), ...
                "GrantContextId", sixgr.util.structGet(grantsOut(gi), ...
                "PHYGrant.GrantContextId", "")));
            bindingValid = isfinite(dueSlot) && dueSlot >= 1 && ...
                ~isempty(grantIds) && numel(unique(grantIds)) == numel(grantIds) && ...
                istable(pending) && ~isempty(pending) && ...
                ismember("PUCCHGrantId", string(pending.Properties.VariableNames)) && ...
                istable(traceT) && ~isempty(traceT) && ...
                ismember("PUCCHGrantId", string(traceT.Properties.VariableNames));
            if bindingValid
                for bitIdx = 1:numel(grantIds)
                    pendingMask = string(pending.PUCCHGrantId) == grantIds(bitIdx) & ...
                        ~logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                        pending, "Processed", false(height(pending), 1))) & ...
                        ~logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                        pending, "RightCensored", false(height(pending), 1)));
                    traceMask = string(traceT.PUCCHGrantId) == grantIds(bitIdx) & ...
                        logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                        traceT, "MultiplexedOnPUSCH", false(height(traceT), 1))) & ...
                        ~logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                        traceT, "GrantExecutedFlag", false(height(traceT), 1))) & ...
                        ~logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                        traceT, "RightCensored", false(height(traceT), 1)));
                    ...
                    pendingDueSlot = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        pending(pendingIdx, :), "DueSlot", NaN));
                    pendingMechanism = lower(strtrim(string( ...
                        sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        pending(pendingIdx, :), "DeliveryMechanism", ""))));
                    pendingContextId = string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        pending(pendingIdx, :), "PUSCHGrantContextId", ""));
                    traceDueSlot = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        traceT(traceIdx, :), "ScheduledAbsoluteSlot", NaN));
                    traceContextId = string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                        traceT(traceIdx, :), "PUSCHGrantContextId", ""));
                    if pendingDueSlot ~= dueSlot || traceDueSlot ~= dueSlot || ...
                            pendingMechanism ~= "pusch_uci" || ...
                            strlength(strtrim(contextId)) == 0 || ...
                            pendingContextId ~= contextId || traceContextId ~= contextId
                        bindingValid = false;
                        break;
                    end
                end
            ...
            grantsOut(gi).ExpectedUCIBits = int8([]);
            grantsOut(gi).MultiplexedHARQACKBits = int8([]);
            grantsOut(gi).UCIOnPUSCHApplied = false;
            grantsOut(gi).UCIOnPUSCHSource = "";
            grantsOut(gi).PUCCHCollisionPolicy = "";
    ...
    function state = markFeedbackRowsReservedForPUSCH(state, grantIds, contextId)
        ...
        if istable(pending) && ~isempty(pending) && ...
                ismember("PUCCHGrantId", string(pending.Properties.VariableNames))
            if ~ismember("DeliveryMechanism", string(pending.Properties.VariableNames))
                pending.DeliveryMechanism = repmat("pucch", height(pending), 1);
            end
            if ~ismember("PUSCHGrantContextId", string(pending.Properties.VariableNames))
                pending.PUSCHGrantContextId = strings(height(pending), 1);
            end
            if ~ismember("MultiplexedBitIndex", string(pending.Properties.VariableNames))
                pending.MultiplexedBitIndex = nan(height(pending), 1);
            end
            for bitIdx = 1:numel(grantIds)
                rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                    pending, "RightCensored", false(height(pending), 1)));
                mask = string(pending.PUCCHGrantId) == grantIds(bitIdx) & ...
                    ~logical(pending.Processed) & ~rightCensored;
                if nnz(mask) ~= 1
                    error("sixgr:truth:InvalidPUSCHHARQACKReservation", ...
                        "PUSCH UCI reservation requires one unprocessed pending row for grant %s.", ...
        ...
        if istable(traceT) && ~isempty(traceT) && ...
                ismember("PUCCHGrantId", string(traceT.Properties.VariableNames))
            if ~ismember("MultiplexedOnPUSCH", string(traceT.Properties.VariableNames))
                traceT.MultiplexedOnPUSCH = false(height(traceT), 1);
            end
            if ~ismember("PUSCHGrantContextId", string(traceT.Properties.VariableNames))
                traceT.PUSCHGrantContextId = strings(height(traceT), 1);
            end
            if ~ismember("MultiplexedBitIndex", string(traceT.Properties.VariableNames))
                traceT.MultiplexedBitIndex = nan(height(traceT), 1);
            end
            if ~ismember("PUSCHUCIDecodeOk", string(traceT.Properties.VariableNames))
                traceT.PUSCHUCIDecodeOk = false(height(traceT), 1);
            end
            for bitIdx = 1:numel(grantIds)
                rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                    traceT, "RightCensored", false(height(traceT), 1)));
                mask = string(traceT.PUCCHGrantId) == grantIds(bitIdx) & ...
                    ~logical(traceT.GrantExecutedFlag) & ~rightCensored;
                if nnz(mask) ~= 1
                    error("sixgr:truth:InvalidPUSCHHARQACKReservation", ...
                        "PUSCH UCI reservation requires one unexecuted control-grant row for grant %s.", ...
                        char(grantIds(bitIdx)));
                end
    ...
    function state = consumeDecodedPUSCHHARQACK(state, harqOut)
        ...
        % score false ACK/NACK; they are never substituted for missing or
        % failed receiver output.
        if ~(isstruct(harqOut) && ~isempty(fieldnames(harqOut)))
            return;
        end
        ...
        if any(~ismember(double(expectedBits), [0 1]))
            error("sixgr:truth:InvalidPUSCHHARQACKEvidence", ...
                "Expected PUSCH HARQ-ACK mapping contains non-binary values.");
        end

        grantIdsToken = string(sixgr.util.structGet(grant, ...
            "UCIOnPUSCHFeedbackGrantIds", ""));
        if ~(isscalar(grantIdsToken) && strlength(strtrim(grantIdsToken)) > 0)
            error("sixgr:truth:MissingPUSCHHARQACKBinding", ...
                "Decoded PUSCH HARQ-ACK requires exact source PUCCH-grant " + ...
                "identities; configured/expected bits cannot stand in for that binding.");
        end
        grantIds = split(grantIdsToken, "|");
        grantIds = grantIds(strlength(strtrim(grantIds)) > 0);
        sourceSlots = double(sixgr.util.structGet(grant, ...
            "UCIOnPUSCHFeedbackSourceSlots", []));
        harqIds = double(sixgr.util.structGet(grant, ...
            "UCIOnPUSCHFeedbackHARQIds", []));
        sourceSlots = sourceSlots(:);
        harqIds = harqIds(:);
        nBits = numel(expectedBits);
        if numel(grantIds) ~= nBits || numel(sourceSlots) ~= nBits || ...
                numel(harqIds) ~= nBits || numel(unique(grantIds)) ~= nBits
            error("sixgr:truth:InvalidPUSCHHARQACKBinding", ...
                "PUSCH HARQ-ACK requires a one-to-one grant/source-slot/" + ...
        ...
        decodeStatus = lower(strtrim(string(sixgr.util.structGet(harqOut, ...
            "HARQACKDecodeStatus", ""))));
        decodedVectorAvailable = numel(decodedBits) == nBits && ...
            all(ismember(double(decodedBits), [0 1])) && ...
            startsWith(decodeStatus, "decoded");
        if decodedVectorAvailable
            observedBits = decodedBits;
        else
            % DTX/missing UCI is a NACK at the HARQ entity.  This is an
            % observed failure outcome, not a replacement with oracle bits.
            observedBits = zeros(nBits, 1, "int8");
        end
        ...
        if ~(istable(pending) && ~isempty(pending) && ...
                ...
                istable(traceT) && ~isempty(traceT) && ...
                ismember("PUCCHGrantId", string(traceT.Properties.VariableNames)))
            error("sixgr:truth:MissingPUSCHHARQACKRuntimeRows", ...
                "PUSCH HARQ-ACK cannot update DL HARQ without its reserved pending and control-grant rows.");
        end
        currentSlot = double(sixgr.util.structGet(state, "CurrentSlot", NaN));
        puschContextId = string(sixgr.util.structGet(grant, "GrantContextId", ...
        ...
        for bitIdx = 1:nBits
            ...
            if rowDirection ~= "DL" || ~isfinite(rowSourceSlot) || ...
                    ...
                    ~isfinite(currentSlot) || rowDueSlot ~= currentSlot || ...
            ...
            if decodedVectorAvailable
                feedbackOutcome = sixgr.truth.CoupledTruthRuntime. ...
                    ternaryString(observedAck, "ACK", "NACK");
            else
                feedbackOutcome = "DTX";
            end
            ...
            if observedAck
                ...
                if isfinite(ueIdx) && ueIdx >= 1 && ueIdx <= size(buffers, 1) && ...
                        pid >= 1 && pid <= size(buffers, 2)
                    buffers{round(ueIdx), round(pid)} = [];
                end
            end
            ...
            bitMatches = decodedVectorAvailable && ...
                observedAck == logical(expectedBits(bitIdx));
            traceT.ObservedAck(traceIdx) = observedAck;
            traceT.DecodedAck(traceIdx) = observedAck;
        ...
        state.PendingFeedbackTable = pending;
        state.PUCCHGrantTraceTable = traceT;
        if istable(sixgr.util.structGet(state, "HARQTimelineTable", table()))
            state.HARQSummaryTable = sixgr.truth.CoupledTruthRuntime. ...
                buildHARQSummary(state.HARQTimelineTable);
        end
    end
    ...
    function row = emptyPacketSDULedgerRow()
        row = struct( ...
            ...
            "ProtocolDecodedBitCount", NaN, "ProtocolDecodedBitExact", false, ...
            "ProtocolDecodedMACSHA256", "", "ProtocolDecodedRLC_SHA256", "", ...
            "ProtocolReceiverEvidenceSource", "", ...
    ...
    function state = rightCensorPendingCausalEventsImpl(state, boundarySlot, reason, scope)
        boundarySlot = round(double(boundarySlot));
        reason = string(reason);
        scope = string(scope);
        isSweepBoundary = scope == "independent_sweep_point";

        csiT = sixgr.util.structGet(state, "PendingCSITable", table());
        if istable(csiT) && ~isempty(csiT)
            csiT = sixgr.truth.CoupledTruthRuntime.setLogicalColumn( ...
                csiT, "RightCensored", false(height(csiT), 1));
            csiT = sixgr.truth.CoupledTruthRuntime.setNumericColumn( ...
                csiT, "CensorBoundarySlot", nan(height(csiT), 1), false);
            csiT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                csiT, "CensorReason", repmat("", height(csiT), 1), false);
            csiT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                csiT, "CensorScope", repmat("", height(csiT), 1), false);
            processed = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                csiT, "Processed", false(height(csiT), 1)));
            alreadyCensored = logical(csiT.RightCensored);
            sourceSlot = double(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                csiT, "SourceSlot", nan(height(csiT), 1)));
            mask = ~processed & ~alreadyCensored;
            if isSweepBoundary
                mask = mask & isfinite(sourceSlot) & sourceSlot < boundarySlot;
            end
            if any(mask)
                idx = find(mask);
                csiT.RightCensored(mask) = true;
                csiT.CensorBoundarySlot(mask) = boundarySlot;
                csiT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    csiT, "CensorReason", idx, reason);
                csiT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    csiT, "CensorScope", idx, scope);
                csiT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    csiT, "DeliveryStatus", idx, "right_censored_" + scope);
            end
            state.PendingCSITable = csiT;
        end

        feedbackT = sixgr.util.structGet(state, "PendingFeedbackTable", table());
        if istable(feedbackT) && ~isempty(feedbackT)
            feedbackT = sixgr.truth.CoupledTruthRuntime.setLogicalColumn( ...
                feedbackT, "RightCensored", false(height(feedbackT), 1));
            feedbackT = sixgr.truth.CoupledTruthRuntime.setNumericColumn( ...
                feedbackT, "CensorBoundarySlot", nan(height(feedbackT), 1), false);
            feedbackT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                feedbackT, "CensorReason", repmat("", height(feedbackT), 1), false);
            feedbackT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                feedbackT, "CensorScope", repmat("", height(feedbackT), 1), false);
            processed = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                feedbackT, "Processed", false(height(feedbackT), 1)));
            sourceSlot = double(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                feedbackT, "SourceSlot", nan(height(feedbackT), 1)));
            mask = ~processed & ~logical(feedbackT.RightCensored);
            if isSweepBoundary
                mask = mask & isfinite(sourceSlot) & sourceSlot < boundarySlot;
            end
            if any(mask)
                idx = find(mask);
                feedbackT.RightCensored(mask) = true;
                feedbackT.CensorBoundarySlot(mask) = boundarySlot;
                feedbackT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    feedbackT, "CensorReason", idx, reason);
                feedbackT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    feedbackT, "CensorScope", idx, scope);
            end
            state.PendingFeedbackTable = feedbackT;
        end

        traceT = sixgr.util.structGet(state, "PUCCHGrantTraceTable", table());
        if istable(traceT) && ~isempty(traceT)
            traceT = sixgr.truth.CoupledTruthRuntime.setLogicalColumn( ...
                traceT, "RightCensored", false(height(traceT), 1));
            traceT = sixgr.truth.CoupledTruthRuntime.setNumericColumn( ...
                traceT, "CensorBoundarySlot", nan(height(traceT), 1), false);
            traceT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                traceT, "CensorReason", repmat("", height(traceT), 1), false);
            traceT = sixgr.truth.CoupledTruthRuntime.setStringColumn( ...
                traceT, "CensorScope", repmat("", height(traceT), 1), false);
            executed = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                traceT, "GrantExecutedFlag", false(height(traceT), 1)));
            transferred = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                traceT, "MultiplexedOnPUSCH", false(height(traceT), 1))) & ...
                logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                traceT, "RuntimeStateUpdated", false(height(traceT), 1)));
            canceled = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                traceT, "CanceledAtSweepBoundary", false(height(traceT), 1)));
            sourceSlot = double(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
                traceT, "SourceSlot", nan(height(traceT), 1)));
            mask = ~executed & ~transferred & ~canceled & ~logical(traceT.RightCensored);
            if isSweepBoundary
                % Sweep-boundary PUCCH grants use the existing explicit
                % cancellation fields below resetIndependentSweepPointLinkState.
                % Right-censoring is reserved for the terminal horizon.
                mask = false(size(mask));
            end
            if any(mask)
                idx = find(mask);
                traceT.RightCensored(mask) = true;
                traceT.CensorBoundarySlot(mask) = boundarySlot;
                traceT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    traceT, "CensorReason", idx, reason);
                traceT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    traceT, "CensorScope", idx, scope);
                traceT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    traceT, "PUCCHGrantState", idx, "right_censored_" + scope);
                traceT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    traceT, "Status", idx, "RIGHT_CENSORED");
                traceT = sixgr.truth.CoupledTruthRuntime.setStringValueAt( ...
                    traceT, "Notes", idx, ...
                    "Scheduled HARQ feedback lies outside the retained causal execution horizon; no waveform result was fabricated.");
            end
            state.PUCCHGrantTraceTable = traceT;
        end
    end
    ...
    function row = emptyPUCCHGrantRow()
        row = struct( ...
            ...
            "MultiplexedOnPUSCH", false, "PUSCHGrantContextId", "", ...
            "MultiplexedBitIndex", NaN, "PUSCHUCIDecodeOk", false, ...
            "CanceledAtSweepBoundary", false, "CancellationSweepPoint", NaN, ...
            "CancellationBoundarySlot", NaN, "CancellationReason", "", ...
            "RightCensored", false, "CensorBoundarySlot", NaN, ...
            "CensorReason", "", "CensorScope", "", ...
    ...
    function row = emptyCSIReportRow()
        row = struct( ...
            ...
            "OLLAUpdateCountBeforeEvent", NaN, "OLLAUpdateCountAfterEvent", NaN, ...
            "OLLAUpdateAppliedAtThisEvent", false, ...
            "OLLAUpdateAuthorized", false, "OLLAFeedbackEventType", "", ...
            "OLLAUpdateAuthoritySource", "", ...
    ...
    function row = emptyGrantRow()
        row = struct( ...
            ...
            "TBSInputModulation", "", "TBSInputNumLayers", NaN, "TBSInputNPRB", NaN, ...
            "TBSInputNREPerPRB", NaN, "TBSInputTargetCodeRate", NaN, ...
            "TBSInputXOverhead", NaN, "TBSInputSource", "", ...
            ...
            "MUMIMOReceiveCombiningMatrixSHA256", "", ...
            "MUMIMOAdmissionReceiveCombiningMatrixSHA256", "", ...
            "MUMIMOReceiveProcessingMode", "", "MUMIMOReceiverAlgorithm", "", ...
            "MUMIMOHybridRFDesignPolicy", "", "MUMIMOHybridRFDesignStatus", "", ...
    ...
    function [state, observed] = observePUCCHFeedback(state, feedbackRow)
        ...
        [state,pucchInterferenceBundle] = ...
            sixgr.truth.CoupledTruthRuntime.buildPUCCHInterferenceBundle( ...
            state,feedbackRow);
        [state,pucchChannelState] = ...
        ...
        trial = sixgr.link.runPUCCHWaveformTrial(cfgU, ...
            ...
            "InitialRuntimeChannelState",pucchChannelState, ...
            "InterferenceBundle",pucchInterferenceBundle);
        state = sixgr.truth.CoupledTruthRuntime.commitRuntimeChannelState( ...
            state,sixgr.util.structGet(trial, ...
            "UpdatedRuntimeChannelState",struct()));
        executedPUCCHTx = sixgr.util.structGet(trial, "Transmitter", struct());
        if isstruct(executedPUCCHTx) && isfield(executedPUCCHTx, "PUCCHIndices")
            pucchRET = sixgr.truth.buildObservedREAllocation(executedPUCCHTx, ...
                "Direction", "UL", "Channel", "PUCCH", ...
                "AbsoluteSlot", max(0, round(dueSlot) - 1), ...
                "CellID", double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
                    row,"ServingCell",NaN)), "UEID", ueIdx, ...
                "AllocationID", "pucch_slot_" + string(max(0, round(dueSlot) - 1)) + ...
                    "_ue_" + string(ueIdx));
            state.ObservedREAllocationTable = ...
                sixgr.truth.CoupledTruthRuntime.appendCompatTable( ...
    ...
    function state = appendPUCCHFeedbackTrial(state, feedbackRow, trial, observed)
        ...
        row = struct( ...
            ...
            "ChannelEstimationMode", char(string(sixgr.util.structGet(trial, "ChannelEstimationMode", ""))), ...
            "NoncoherentSequenceDetection", logical(sixgr.util.structGet(trial, "NoncoherentSequenceDetection", false)), ...
            ...
            "PUCCHTxOutputWaveformHash", char(string(sixgr.util.structGet(trial, "PUCCHTxOutputWaveformHash", ""))), ...
            "PUCCHAppliedTxPower_dBm", double(sixgr.util.structGet(trial, "PUCCHAppliedTxPower_dBm", NaN)), ...
            "PUCCHMeasuredTxOutputPower_dBm", double(sixgr.util.structGet(trial, "PUCCHMeasuredTxOutputPower_dBm", NaN)), ...
            "PUCCHTxPowerClosureError_dB", double(sixgr.util.structGet(trial, "PUCCHTxPowerClosureError_dB", NaN)), ...
            "PUCCHWaveformAmplitudeUnit", char(string(sixgr.util.structGet(trial, "PUCCHWaveformAmplitudeUnit", ""))), ...
            "TxRFExecutionStatus", char(string(sixgr.util.structGet(trial, "TxRFExecutionStatus", ""))), ...
            "TxRFStageOrder", char(string(sixgr.util.structGet(trial, "TxRFStageOrder", ""))), ...
            "TxRFAppliedStageCount", double(sixgr.util.structGet(trial, "TxRFAppliedStageCount", 0)), ...
            "ChannelModel", char(string(sixgr.util.structGet(trial, "ChannelModel", ""))), ...
            "RuntimeIntegrationMode", char(string(sixgr.util.structGet(trial, "RuntimeIntegrationMode", ""))), ...
            "RuntimeTransportMode", char(string(sixgr.util.structGet(trial, "RuntimeTransportMode", ""))), ...
            ...
            "RuntimeChannelStateUsed", logical(sixgr.util.structGet(trial, "RuntimeChannelStateUsed", false)), ...
            "RuntimeChannelLinkKeys", char(string(sixgr.util.structGet(trial, "RuntimeChannelLinkKeys", ""))), ...
            "RuntimeNoiseApplied", logical(sixgr.util.structGet(trial, "RuntimeNoiseApplied", false)), ...
            "RuntimeNoiseVarianceMean", double(sixgr.util.structGet(trial, "RuntimeNoiseVarianceMean", NaN)), ...
            "RuntimeStageCount", double(sixgr.util.structGet(trial, "RuntimeStageCount", NaN)), ...
            "RuntimeChannelPhysicalTxElements", double(sixgr.util.structGet(trial, "Channel.RuntimeChannelPhysicalTxElements", NaN)), ...
            "RuntimeChannelNumRxAntennas", double(sixgr.util.structGet(trial, "Channel.RuntimeChannelNumRxAntennas", NaN)), ...
            "ChannelUsesSameRuntimeAntennaAssumptions", logical(sixgr.util.structGet(trial, "Channel.ChannelUsesSameRuntimeAntennaAssumptions", false)), ...
            "ChannelUsesCountOnlyAntennaModel", logical(sixgr.util.structGet(trial, "Channel.ChannelUsesCountOnlyAntennaModel", false)), ...
            "ChannelArrayHandlingStatus", char(string(sixgr.util.structGet(trial, "Channel.ChannelArrayHandlingStatus", ""))), ...
            "ChannelArrayHandlingBlocker", char(string(sixgr.util.structGet(trial, "Channel.ChannelArrayHandlingBlocker", ""))), ...
    ...
    function [state,bundle] = buildPUCCHInterferenceBundle(state, feedbackRow)
        bundle = struct([]);
        ...
        grants = sixgr.util.structGet(state, ...
            "PUCCHCurrentSlotGrantSnapshot",table());
        frozenSlotSet = istable(grants) && ~isempty(grants);
        if ~frozenSlotSet
        ...
        canceled = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
            grants, "CanceledAtSweepBoundary", false(height(grants), 1)));
        rightCensored = logical(sixgr.truth.CoupledTruthRuntime.tableColumnOrDefault( ...
            grants, "RightCensored", false(height(grants), 1)));
        mask = ~canceled & ~rightCensored & ...
            abs(double(grants.ScheduledAbsoluteSlot) - dueSlot) < 1e-9;
        if ~frozenSlotSet
            mask = mask & ~logical(grants.GrantExecutedFlag);
        end
        currentGrantId = string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            currentRow,"PUCCHGrantId",""));
        if strlength(strtrim(currentGrantId)) > 0 && ...
                ismember("PUCCHGrantId",string(grants.Properties.VariableNames))
            mask = mask & string(grants.PUCCHGrantId) ~= currentGrantId;
        elseif ismember("UEIndex", string(grants.Properties.VariableNames)) && isfinite(currentUE)
        ...
        for i = 1:height(peers)
            peer = peers(i, :);
            resourceOverlap = sixgr.truth.CoupledTruthRuntime.pucchResourcesOverlap( ...
                currentRow,peer);
            ...
            [victimBsEntry, interfererUeEntry] = sixgr.truth.CoupledTruthRuntime.runtimeAntennaEntriesForLink( ...
                state, ueIdx, currentServingCell);
            ...
            bundle(count).InterfererUEAntenna = sixgr.util.structGet(interfererUeEntry, "Antenna", struct()); %#ok<AGROW>
            bundle(count).InterfererUEAntennaMeta = sixgr.util.structGet(interfererUeEntry, "Metadata", struct()); %#ok<AGROW>
            ...
            bundle(count).PUCCHPRBStart = double(sixgr.truth.CoupledTruthRuntime.rowValue(peer,"PUCCHPRBStart",NaN)); %#ok<AGROW>
            bundle(count).PUCCHPRBCount = double(sixgr.truth.CoupledTruthRuntime.rowValue(peer,"PUCCHPRBCount",NaN)); %#ok<AGROW>
            bundle(count).PUCCHSymbolStart = double(sixgr.truth.CoupledTruthRuntime.rowValue(peer,"PUCCHSymbolStart",NaN)); %#ok<AGROW>
            bundle(count).PUCCHNumSymbols = double(sixgr.truth.CoupledTruthRuntime.rowValue(peer,"PUCCHNumSymbols",NaN)); %#ok<AGROW>
            bundle(count).ResourceOverlapsVictim = logical(resourceOverlap); %#ok<AGROW>
            [state,contributionWaveform,contributionMeta] = sixgr.truth.CoupledTruthRuntime.buildPUCCHSharedSlotContribution( ...
    ...
    function tf = pucchResourcesOverlap(a,b)
        aPrb = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            a,"PUCCHPRBStart",NaN));
        aNPrb = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            a,"PUCCHPRBCount",NaN));
        aSym = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            a,"PUCCHSymbolStart",NaN));
        aNSym = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            a,"PUCCHNumSymbols",NaN));
        bPrb = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            b,"PUCCHPRBStart",NaN));
        bNPrb = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            b,"PUCCHPRBCount",NaN));
        bSym = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            b,"PUCCHSymbolStart",NaN));
        bNSym = double(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            b,"PUCCHNumSymbols",NaN));
        values = [aPrb aNPrb aSym aNSym bPrb bNPrb bSym bNSym];
        if any(~isfinite(values)) || any([aNPrb aNSym bNPrb bNSym] < 1)
            error("sixgr:truth:InvalidPUCCHInterferenceResource", ...
                ["Shared-slot PUCCH interference requires exact finite " ...
                 "PRB and symbol intervals for victim and peer grants."]);
        end
        prbOverlap = aPrb < bPrb+bNPrb && bPrb < aPrb+aNPrb;
        symbolOverlap = aSym < bSym+bNSym && bSym < aSym+aNSym;
        tf = logical(prbOverlap && symbolOverlap);
    end

    function assertPUCCHAssignmentMatchesGrant(assignment,grantRow,role)
        if ~isa(assignment,"sixgr.phy.pucch.PUCCHTransmissionAssignment")
            error("sixgr:truth:PUCCHAssignmentTypeMismatch", ...
                "The %s PUCCH execution does not carry a typed assignment.",char(role));
        end
        d = assignment.Resource.Data;
        expected = [ ...
            double(sixgr.truth.CoupledTruthRuntime.rowValue(grantRow,"ResolvedFormat",NaN)), ...
            double(sixgr.truth.CoupledTruthRuntime.rowValue(grantRow,"PUCCHPRBStart",NaN)), ...
            double(sixgr.truth.CoupledTruthRuntime.rowValue(grantRow,"PUCCHPRBCount",NaN)), ...
            double(sixgr.truth.CoupledTruthRuntime.rowValue(grantRow,"PUCCHSymbolStart",NaN)), ...
            double(sixgr.truth.CoupledTruthRuntime.rowValue(grantRow,"PUCCHNumSymbols",NaN))];
        actual = [double(d.Format) double(d.StartPRB) double(d.NumPRBs) ...
            double(d.StartSymbol) double(d.NumSymbols)];
        expectedId = string(sixgr.truth.CoupledTruthRuntime.rowValue( ...
            grantRow,"PUCCHResourceId",""));
        if any(~isfinite(expected)) || any(actual ~= expected) || ...
                strlength(strtrim(expectedId)) == 0 || string(d.ID) ~= expectedId
            error("sixgr:truth:PUCCHFrozenResourceMismatch", ...
                "The %s PUCCH typed assignment differs from its frozen grant: resource expected=%s actual=%s, format/PRB/symbol expected=%s actual=%s.", ...
                char(role),char(expectedId),char(string(d.ID)), ...
                char(mat2str(expected)),char(mat2str(actual)));
        end
    end

    function [state,contributionWaveform,meta] = buildPUCCHSharedSlotContribution(state, cfgIn, bundleEntry, ueIdx, victimCell)
        ...
        sixgr.truth.CoupledTruthRuntime.assertPUCCHAssignmentMatchesGrant( ...
            connected.Assignment,bundleEntry,"interfering");
        ...
        cfgContribution = sixgr.util.structSet(cfgContribution, ...
            "lls6g.userContext.RuntimeSignalFamily","PUCCH");
        [txWaveform,cfgContribution,txBoundary] = ...
            sixgr.link.preparePUCCHTransmitWaveform(tx,cfgContribution);
        [state,initialChannelState] = ...
            sixgr.truth.CoupledTruthRuntime.acquireRuntimeChannelStateForControlImpl( ...
            state,cfgContribution,ueIdx,"UL",victimCell);
        % Same-slot interferers are concurrent.  Materialize a new
        % canonical link once, deep-fork its mutable channel object at the
        % slot origin, and execute only the fork.  This prevents the
        % interference observation from advancing the state subsequently
        % used by the peer's desired waveform.
        if sixgr.channel.ChannelFactory.requiresRuntimeChannelState(cfgContribution)
            if ~logical(sixgr.util.structGet(initialChannelState,"Materialized",false))
                canonicalTruthState = sixgr.link.initWaveformTruthChannelState( ...
                    cfgContribution,tx,txInfo, ...
                    "InitialRuntimeChannelState",initialChannelState);
                initialChannelState = sixgr.util.structGet( ...
                    canonicalTruthState,"RuntimeChannelState",initialChannelState);
                state = sixgr.truth.CoupledTruthRuntime.commitRuntimeChannelStateImpl( ...
                    state,initialChannelState);
            end
            forkedChannelState = sixgr.channel.ChannelFactory.forkRuntimeChannelState( ...
                initialChannelState);
            truthState = sixgr.link.initWaveformTruthChannelState( ...
                cfgContribution,tx,txInfo, ...
                "InitialRuntimeChannelState",forkedChannelState);
        else
            truthState = sixgr.link.initWaveformTruthChannelState( ...
                cfgContribution,tx,txInfo);
        end
        [channelWaveform,channelReplay,truthState] = ...
            sixgr.link.applyRuntimeFadingChannel(txWaveform,truthState);
        [contributionWaveform,rfReplay] = ...
            sixgr.link.applyWaveformImpairments( ...
            channelWaveform,cfgContribution,sampleRateHz, ...
            "ApplyRFChain",false);
        replay = channelReplay;
        rfFields = fieldnames(rfReplay);
        for rfIdx = 1:numel(rfFields)
            replay.(rfFields{rfIdx}) = rfReplay.(rfFields{rfIdx});
        end
        ...
        runtimeState = sixgr.util.structGet(truthState, ...
            "RuntimeChannelState",struct());
        ...
        meta = struct( ...
            ...
            "AppliedTxPower_dBm",double(txBoundary.AppliedPower_dBm), ...
            "MeasuredTxOutputPower_dBm",double(txBoundary.MeasuredTxOutputPower_dBm), ...
            "TxPowerClosureError_dB",double(txBoundary.PowerClosureError_dB), ...
            "TxRFExecutionStatus",char(string(txBoundary.TXRFExecutionStatus)), ...
            "TxRFStageOrder",char(string(txBoundary.TXRFStageOrder)), ...
            "TxRFAppliedStageCount",double(txBoundary.TXRFAppliedStageCount), ...
            "Source", "runtime_pucch_tx_persistent_channel_rf_receiver_contribution", ...
            "ChannelObjectSource", char(string(sixgr.util.structGet( ...
                runtimeMeta,"ChannelObjectSource", ...
                "sixgr.channel.ChannelFactory.runtime_state"))), ...
            "ChannelObjectClass", char(string(sixgr.util.structGet( ...
                replay,"ChannelFadingObjectClass",""))), ...
            "ChannelArrayHandlingStatus", char(string(sixgr.util.structGet( ...
                runtimeMeta,"ChannelArrayHandlingStatus",""))), ...
            "ChannelArrayHandlingBlocker", char(string(sixgr.util.structGet( ...
                runtimeMeta,"ChannelArrayHandlingBlocker",""))), ...
            "ChannelGeometryCouplingLevel", char(string(sixgr.util.structGet( ...
                runtimeMeta,"ChannelGeometryCouplingLevel",""))), ...
            "GeometryAdapterType", char(string(sixgr.util.structGet( ...
                runtimeMeta,"GeometryAdapterType",""))), ...
            "GeometryAdapterSource", char(string(sixgr.util.structGet( ...
                runtimeMeta,"GeometryAdapterSource", ...
                "sixgr.channel.ChannelFactory"))), ...
            "GeometryAdapterLimitation", char(string(sixgr.util.structGet( ...
                runtimeMeta,"GeometryAdapterLimitation",""))), ...
            "GeometryAdapterPortMapping", char(string(sixgr.util.structGet( ...
                runtimeMeta,"GeometryAdapterPortMapping",""))), ...
    ...
    function cfgOut = configurePUCCHContributionLinkBudget(cfgIn, state, ueIdx, victimCell, bundleEntry)
        ...
        if isfield(bundleEntry, "VictimServingBSAntennaMeta")
            userMeta.RuntimeServingBSAntennaMeta = ...
                bundleEntry.VictimServingBSAntennaMeta;
        end
        if isfield(bundleEntry, "InterfererUEAntenna")
            userMeta.RuntimeUEAntenna = bundleEntry.InterfererUEAntenna;
        end
        if isfield(bundleEntry, "InterfererUEAntennaMeta")
            userMeta.RuntimeUEAntennaMeta = bundleEntry.InterfererUEAntennaMeta;
        end
        cfgOut = sixgr.util.structSet(cfgOut, "lls6g.userContext", userMeta);
