...
% Start from a builtin baseline, then apply overrides from cfg.scenario.*.
    ...
    prof.wraparoundEnabled = logical(sixgr.util.structGet(sc, 'geometry.wraparound', ...
        sixgr.util.structGet(sc, 'geometry.wrapAround', ...
        sixgr.util.structGet(sc, 'layout.wrapAround', ...
        sixgr.util.structGet(sc, 'layout.wraparound', prof.wraparoundEnabled)))));
    prof.wraparoundMode = char(string(sixgr.util.structGet(sc, 'geometry.wraparoundMode', ...
        sixgr.util.structGet(sc, 'geometry.wraparound_mode', ...
        sixgr.util.structGet(sc, 'layout.wraparoundMode', ...
        sixgr.util.structGet(sc, 'layout.wraparound_mode', sixgr.util.structGet(prof, 'wraparoundMode', "")))))));
