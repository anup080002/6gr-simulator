...
[dlBler, dlThroughput, dlDistribution, dlScatter, dlSummary, dlKPI] = ...
    localBuildDirectionArtifacts(dl, "DL", dlSourceArtifact, nBins, bandwidthHz, slotDuration_s, runTag);
[ulBler, ulThroughput, ulDistribution, ulScatter, ulSummary, ulKPI] = ...
    localBuildDirectionArtifacts(ul, "UL", ulSourceArtifact, nBins, bandwidthHz, slotDuration_s, runTag);
...
gateTables = localWriteKPIReconciliationGates(layout, kpiSummary, anchorKPIs, ...
    trialData, slotDuration_s, scenarioCfg);
...
function [blerT, throughputT, distT, scatterT, summaryT, kpiT] = localBuildDirectionArtifacts(T, direction, sourceArtifact, nBins, bandwidthHz, slotDuration_s, runTag)
direction = upper(string(direction));
...
for gi = 1:numel(groupMasks)
    ...
    for bi = 1:numel(bins.centers)
        ...
        binSub = sub(inBin, :);
        % A finite SINR observation does not imply a TB CRC observation.
        % Keep it in SINR/throughput populations, but do not invent a BLER
        % point when no binary transport-block outcome was measured.
        if any(isfinite(localCRC(binSub)))
            blerRows{end+1, 1} = localBlerCurveRow(binSub, direction, ueLabel, bins, bi, sourceArtifact); %#ok<AGROW>
        end
        throughputRows{end+1, 1} = localThroughputCurveRow(binSub, direction, ueLabel, bins, bi, sourceArtifact, bandwidthHz, slotDuration_s); %#ok<AGROW>
    end
    % Always retain the aggregate population. Fixed-link campaigns have no
    % geometry UE identity, but their receiver-measured SINR distribution is
    % still a genuine runtime statistic and must not disappear.
    distRows = [distRows; localDistributionRows(sub, direction, ueLabel, bins, sourceArtifact)]; %#ok<AGROW>
    summaryRows{end+1, 1} = localMeasuredSummaryRow(sub, direction, ueLabel, sourceArtifact, bandwidthHz, slotDuration_s); %#ok<AGROW>
    kpiRows{end+1, 1} = localKPISummaryRow(sub, direction, ueLabel, sourceArtifact, bandwidthHz, slotDuration_s, runTag); %#ok<AGROW>
end
...
failures = sum(crc==0);
n = height(T);
observed = sum(isfinite(crc));
[ciLow, ciHigh] = localWilsonCI(failures, observed);
bits = localNumericFirst(T, "BitsCompared", NaN(n, 1));
bitErrors = localNumericFirst(T, "BitErrors", NaN(n, 1));
berEvidence=sixgr.kpi.aggregateBitErrorEvidence(bitErrors,bits);
ber=berEvidence.BER;
...
row = struct( ...
    ...
    "BLER", localSafeDivide(failures, observed), ...
    "BLER_CI_Low", ciLow, ...
    "BLER_CI_High", ciHigh, ...
    "BER", ber, ...
    "BERObservedTrialCount", berEvidence.ObservedTrialCount, ...
    "BERUnavailableTrialCount", berEvidence.UnavailableTrialCount, ...
    "TrialCount", double(observed), ...
    "CRCObservedTrialCount", double(observed), ...
    "CRCUnknownTrialCount", double(n-observed), ...
...
function row = localThroughputCurveRow(T, direction, ueLabel, bins, binIdx, sourceArtifact, bandwidthHz, slotDuration_s)
n = height(T);
throughput = localNumericFirst(T, "Throughput_Mbps", ...
    localRowBitrateMbps(T, ["TBSize_bits","TBSBits","TBS","ScheduledBits"], slotDuration_s));
goodput = localNumericFirst(T, "Goodput_Mbps", ...
    localRowBitrateMbps(T, "GoodBits", slotDuration_s));
offered = localNumericFirst(T, "OfferedThroughput_Mbps", ...
    localRowBitrateMbps(T, "OfferedBits", slotDuration_s));
row = struct( ...
    "Direction", direction, ...
    "UEIndex", double(ueLabel), ...
    "RNTI", localGroupRNTI(T, ueLabel), ...
    ...
    "PostEqSINR_dB_BinMin", double(bins.edges(binIdx)), ...
    "PostEqSINR_dB_BinMax", double(bins.edges(binIdx+1)), ...
    "Throughput_Mbps_mean", localMeanFinite(throughput), ...
    "Throughput_Mbps_p5", localPercentile(throughput, 5), ...
    "Throughput_Mbps_p50", localPercentile(throughput, 50), ...
    "Throughput_Mbps_p95", localPercentile(throughput, 95), ...
    ...
    "SpectralEfficiency_bps_Hz_mean", localSafeDivide(localMeanFinite(goodput) * 1e6, double(bandwidthHz)), ...
    "SpectralEfficiencyDefinition", "goodput_over_channel_bandwidth_same_time_basis", ...
    "TrialCount", double(n), ...
    "SourceArtifact", string(sourceArtifact));
end
...
for i = 1:height(Tin)
    rowT = Tin(i, :);
    propagationDistance = localRepresentativeNumeric( ...
        rowT, "PropagationDistance_m");
    % Distance is a geometry-domain quantity. An isolated AWGN calibration
    % has no propagation geometry, so omit the row instead of exporting a
    % NaN point that falsely claims a distance-vs-SINR observation.
    if ~isfinite(propagationDistance)
        continue;
    end
    rows{end+1, 1} = struct( ...
        "Direction", direction, ...
        ...
        "TrialIndex", double(i), ...
        "PropagationDistance_m", propagationDistance, ...
...
function evidence=localBEREvidence(T)
n=height(T);
evidence=sixgr.kpi.aggregateBitErrorEvidence( ...
    localNumericFirst(T,"BitErrors",NaN(n,1)), ...
    localNumericFirst(T,"BitsCompared",NaN(n,1)));
end

function row = localMeasuredSummaryRow(T, direction, ueLabel, sourceArtifact, bandwidthHz, slotDuration_s)
berEvidence=localBEREvidence(T);
n = height(T);
...
row = struct( ...
    ...
    "BLER_overall", localSafeDivide(sum(crc==0), sum(isfinite(crc))), ...
    "CRCObservedTrialCount", double(sum(isfinite(crc))), ...
    "CRCUnknownTrialCount", double(sum(~isfinite(crc))), ...
    "BER_overall", berEvidence.BER, ...
    "BERObservedTrialCount", berEvidence.ObservedTrialCount, ...
    "BERUnavailableTrialCount", berEvidence.UnavailableTrialCount, ...
    "Throughput_Mbps_mean", localMeanFinite(throughput), ...
    "OfferedThroughput_Mbps_mean", localMeanFinite(offered), ...
    "Goodput_Mbps_mean", localMeanFinite(goodput), ...
    "SpectralEfficiency_mean_bps_Hz", localSafeDivide(localMeanFinite(goodput) * 1e6, bandwidthHz), ...
    "SpectralEfficiencyDefinition", "goodput_over_channel_bandwidth_same_time_basis", ...
    "MCS_dominant", localModeNumeric(T, ["MCS","MCSIndex"]), ...
    "Modulation_dominant", localModeString(T, "Modulation"), ...
    "Rank_dominant", localModeNumeric(T, ["Rank","Layers"]), ...
...
function row = localKPISummaryRow(T, direction, ueLabel, sourceArtifact, bandwidthHz, slotDuration_s, runTag)
berEvidence=localBEREvidence(T);
n = height(T);
sinr = localNumericFirst(T, "PostEqSINR_dB", NaN(n, 1));
crc = localCRC(T);
...
goodputMbps = localSafeDivide(sum(goodBits), radioDuration_s) / 1e6;
throughputMbps = localSafeDivide(sum(scheduledBits(isfinite(scheduledBits)), "omitnan"), radioDuration_s) / 1e6;
offeredMbps = localSafeDivide(sum(offeredBits(isfinite(offeredBits)), "omitnan"), radioDuration_s) / 1e6;
% Goodput already contains its radio-time denominator. Dividing again by a
% TDD duty fraction inflates SE (formerly 5x for UL and 1.25x for DL), and
% falsely applies the same pattern to every numerology and scenario. These
% conditional trial statistics are not a full measurement-window capacity.
se = localSafeDivide(goodputMbps * 1e6, bandwidthHz);
formulaGoodput = localSafeDivide(sum(goodBits), radioDuration_s) / 1e6;
formulaThroughput = localSafeDivide(sum(scheduledBits(isfinite(scheduledBits)), "omitnan"), radioDuration_s) / 1e6;
formulaOffered = localSafeDivide(sum(offeredBits(isfinite(offeredBits)), "omitnan"), radioDuration_s) / 1e6;
reconPass = isfinite(goodputMbps) && isfinite(throughputMbps) && isfinite(offeredMbps) && ...
    ...
    goodputMbps <= offeredMbps + 1e-12;
% Throughput is gross scheduled air-interface payload and therefore counts
% every HARQ transmission. Offered throughput counts unique payload entering
% the queue. A retransmitting link can legitimately have gross throughput
% above offered throughput; only delivered goodput is bounded by both.
strictOk = n > 0 && all(isfinite(crc)) && berEvidence.Complete && ...
    isfinite(localPercentile(sinr, 50)) && reconPass;
failureReason=string(localTernary(strictOk,"", ...
    "measured_sinr_geometry_kpi_unavailable_or_reconciliation_failed"));
if any(~isfinite(crc)), failureReason="transport_crc_evidence_incomplete"; end
if ~berEvidence.Complete, failureReason=failureReason+";paired_bit_error_evidence_incomplete"; end
row = struct( ...
    "RunId", string(runTag), ...
    "Direction", direction, ...
    "UEIndex", localSummaryUELabel(ueLabel), ...
...
mask = strcmpi(string(anchor.Case), caseName);
    ...
    throughput = localFirstFinite([localTableScalar(kpiRow, "Throughput_Mbps"), localTableScalar(summaryRow, "Throughput_Mbps_mean")], NaN);
...
anchor.Throughput_Mbps(mask) = throughput;
...
function gates = localWriteKPIReconciliationGates(layout, kpiSummary, anchor, trialData, slotDuration_s, scenarioCfg)
if ~(istable(anchor) && height(anchor) > 0)
    anchorPath = fullfile(layout.AirInterfaceCSVDir, "link_anchor_kpis.csv");
    if exist(anchorPath, "file") == 2
        anchor = readtable(anchorPath, "VariableNamingRule", "preserve", "TextType", "string");
...
applicability = localKPIReconciliationApplicability(scenarioCfg);
access = localAnchorGateTable(anchor, "AccessKpiReconciliationOk", ...
    ["CellSearch_MIB_SIB1","PRACH_Detection"], applicability.Access, applicability.Authority);
scheduler = localAnchorGateTable(anchor, "SchedulerKpiReconciliationOk", ...
    ["DL_PDSCH_Throughput","UL_PUSCH_Throughput"], applicability.Scheduler, applicability.Authority);
latency = localLatencyReconciliationTable(trialData, slotDuration_s);
canonical = localCanonicalLedgerTable(throughput, blerBer, access, scheduler, latency, applicability);
...
unknownCRC=localColumnIfPresent(kpiSummary,"CRCUnknownTrialCount");
completeCRC=~isempty(unknownCRC) && all(isfinite(unknownCRC) & unknownCRC==0);
unavailableBER=localColumnIfPresent(kpiSummary,"BERUnavailableTrialCount");
completeBER=~isempty(unavailableBER) && all(isfinite(unavailableBER) & unavailableBER==0);
ok=ok && completeCRC && completeBER;
T = struct2table(struct( ...
    ...
    "FailureReason", string(localTernary(ok, "", "bler_or_ber_out_of_range_or_receiver_evidence_incomplete"))), "AsArray", true);
end

function T = localAnchorGateTable(anchor, gateName, cases, applicable, applicabilityAuthority)
if applicable
    [ok, reason, presentCount, okCount] = localAnchorCasesOk(anchor, cases);
    requiredCount = numel(cases);
    evaluationStatus = "evaluated_runtime_anchor_cases";
else
    ok = false;
    reason = "not_applicable_fixed_link_campaign_only";
    presentCount = 0;
    okCount = 0;
    requiredCount = 0;
    evaluationStatus = "not_applicable_fixed_link_campaign_only";
end
S = struct( ...
    "KPIApplicable", logical(applicable), ...
    "RequiredCaseCount", double(requiredCount), ...
    "PresentCaseCount", double(presentCount), ...
    "OkCaseCount", double(okCount), ...
    "EvaluationStatus", string(evaluationStatus), ...
    "ApplicabilityAuthority", string(applicabilityAuthority), ...
    "KPIFormulaVersion", "measured_posteq_sinr_v2", ...
...
function T = localCanonicalLedgerTable(throughput, blerBer, access, scheduler, latency, applicability)
throughputOk = logical(throughput.ThroughputReconciliationOk(1));
blerBerOk = logical(blerBer.BlerBerReconciliationOk(1));
accessObservedOk = logical(access.AccessKpiReconciliationOk(1));
schedulerObservedOk = logical(scheduler.SchedulerKpiReconciliationOk(1));
latencyOk = logical(latency.LatencyReconciliationOk(1));
accessGateOk = ~logical(applicability.Access) || accessObservedOk;
schedulerGateOk = ~logical(applicability.Scheduler) || schedulerObservedOk;
ok = throughputOk && blerBerOk && accessGateOk && schedulerGateOk && latencyOk;
T = struct2table(struct( ...
    "ThroughputReconciliationOk", throughputOk, ...
    "BlerBerReconciliationOk", blerBerOk, ...
    "AccessKpiApplicable", logical(applicability.Access), ...
    "AccessKpiReconciliationOk", accessObservedOk, ...
    "SchedulerKpiApplicable", logical(applicability.Scheduler), ...
    "SchedulerKpiReconciliationOk", schedulerObservedOk, ...
    "LatencyReconciliationOk", latencyOk, ...
    "CanonicalKpiLedgerOk", logical(ok), ...
    "KPIConsistencyGateOk", logical(ok), ...
    "ApplicabilityAuthority", string(applicability.Authority), ...
    "KPIFormulaVersion", "measured_posteq_sinr_v2", ...
...
function applicability = localKPIReconciliationApplicability(scenarioCfg)
fixedLinkOnly = logical(sixgr.util.structGet(scenarioCfg, ...
    "run.fixedLinkCampaignOnly", sixgr.util.structGet(scenarioCfg, ...
    "canonical_control.run.fixed_link_campaign_only", sixgr.util.structGet(scenarioCfg, ...
    "sweeps_and_matrix.fixed_link_calibration.only", false))));
applicability = struct( ...
    "Access", ~fixedLinkOnly, ...
    "Scheduler", ~fixedLinkOnly, ...
    "Authority", localTernary(fixedLinkOnly, ...
        "run.fixedLinkCampaignOnly", "connected_runtime_default"));
end
...
bits(missing & crc==1 & isfinite(tb)) = tb(missing & crc==1 & isfinite(tb));
bits(crc==0) = 0;
end

function mbps = localRowBitrateMbps(T, bitNames, slotDuration_s)
bits = localNumericFirst(T, bitNames, NaN(height(T), 1));
mbps = bits ./ max(double(slotDuration_s), eps) / 1e6;
end
...
function value = localGroupRNTI(T, ueLabel)
% An all-UE aggregate has no single RNTI.  Publishing the first RNTI in
% that group falsely binds a cell-wide curve to one UE.
if ~isfinite(double(ueLabel))
    value = NaN;
else
    value = localRepresentativeNumeric(T, "RNTI");
end
end
...
vars = ["Direction","UEIndex","RNTI","PostEqSINR_dB_BinCenter","PostEqSINR_dB_BinMin","PostEqSINR_dB_BinMax", ...
    ...
    "CRCObservedTrialCount","CRCUnknownTrialCount","BERObservedTrialCount","BERUnavailableTrialCount"];
end
...
types = ["string","double","double","double","double","double","double","double","double","double","double","double", ...
    "double","double","double","double","string","double","string","double","double","double","double"];
end

function vars = localThroughputCurveVars()
vars = ["Direction","UEIndex","RNTI","PostEqSINR_dB_BinCenter","PostEqSINR_dB_BinMin","PostEqSINR_dB_BinMax", ...
    "Throughput_Mbps_mean","Throughput_Mbps_p5","Throughput_Mbps_p50","Throughput_Mbps_p95", ...
    "Goodput_Mbps_mean","Goodput_Mbps_p5","Goodput_Mbps_p50","Goodput_Mbps_p95", ...
    "OfferedThroughput_Mbps_mean","SpectralEfficiency_bps_Hz_mean","TrialCount","SourceArtifact","SpectralEfficiencyDefinition"];
end
...
types = ["string","double","double","double","double","double","double","double","double","double","double","string","double","double","double","string"];
end
...
vars = ["Direction","UEIndex","RNTI","N_Trials","SINR_min_dB","SINR_p5_dB","SINR_p25_dB", ...
    ...
    "BLER_overall","BER_overall","Throughput_Mbps_mean","OfferedThroughput_Mbps_mean", ...
    "Goodput_Mbps_mean","SpectralEfficiency_mean_bps_Hz", ...
    "MCS_dominant","Modulation_dominant","Rank_dominant","Distance_min_m","Distance_max_m", ...
    "KPIFormulaVersion","SourceArtifact","SpectralEfficiencyDefinition", ...
    "CRCObservedTrialCount","CRCUnknownTrialCount","BERObservedTrialCount","BERUnavailableTrialCount"];
end
...
types = ["string","string","double","double","double","double","double","double","double","double","double", ...
    "double","double","double","double","double","double","double","string","double","double","double","string","string","string","double","double","double","double"];
end
...
vars = ["RunId","Direction","UEIndex","RNTI","KPIFormulaVersion","KPIReconciliationPass", ...
    ...
    "Throughput_Mbps","Goodput_Mbps","OfferedThroughput_Mbps","SpectralEfficiency_bps_Hz", ...
    "RadioDuration_s","TrialCount","MCS_dominant","Rank_dominant", ...
    "DistanceRange_m","StrictOk","Status","FailureReason","SourceArtifact","SpectralEfficiencyDefinition", ...
    "CRCObservedTrialCount","CRCUnknownTrialCount","BERObservedTrialCount","BERUnavailableTrialCount"];
end

function types = localKPISummaryTypes()
types = ["string","string","string","double","string","logical","double","double","double","double","double", ...
    "double","double","double","double","double","double","double","double","string","logical","string","string","string","string","double","double","double","double"];
end
