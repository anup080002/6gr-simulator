...
ueFloor = kTBdBm + ueNF;
bsFloor = kTBdBm + bsNF;
mode = localString(cfg, ["run.noise_operating_mode","simulation.noise_operating_mode","noise.operating_mode"], "receiver_noise_figure_thermal_noise");
...
T = table(bwHz, -174, kTBdBm, ueNF, bsNF, ueFloor, bsFloor, string(mode), ok, ...
    "resolved_config_noise_figure_thermal_noise", ...
...
dlRI = localFiniteColumn(dlT, "RankIndicator");
if isempty(dlRI)
    dlRI = localFiniteColumn(dlT, "Layers");
end
ulRI = localFiniteColumn(ulT, "RankIndicator");
if isempty(ulRI)
...
T = table(cfgDLLayers, cfgULLayers, localMean(dlRI), dlMax, dlRank2Frac, ...
    localMean(ulRI), ulMax, ulRank2Frac, ok, "trial_rank_layer_runtime_columns", ...
