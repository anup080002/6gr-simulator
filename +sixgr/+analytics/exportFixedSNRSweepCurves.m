...
% RawBER is an exact row-local arithmetic projection of the persisted
% decoder counters.  Fixed-link trial producers historically exported the
% counters but not this convenience column, which made the normalized
% table fail its own transport arithmetic contract.  Never infer BER when
% the denominator is unavailable or zero.
rawBER = localNumericColumn(T, ["RawBER", "BER"], NaN(n, 1));
rawBERMask = ~isfinite(rawBER) & isfinite(T.BitErrors) & ...
    isfinite(T.BitsCompared) & T.BitsCompared > 0;
rawBER(rawBERMask) = T.BitErrors(rawBERMask) ./ T.BitsCompared(rawBERMask);
T.RawBER = rawBER;
...
% A fixed-link calibration has no adaptive scheduler decision.  Preserve
% that fact explicitly instead of fabricating a scheduled MCS/modulation
% equal to the transmitted fixed operating point.
...
required = struct( ...
    ...
    "FixedLinkConfidenceLevel", NaN(n, 1), ...
    "FixedLinkIntervalMethod", strings(n, 1), ...
    "FixedLinkPointIndex", NaN(n, 1), ...
    "FixedLinkDropIndex", NaN(n, 1), ...
    "FixedLinkTrialIndex", NaN(n, 1), ...
    "FixedLinkSeedHierarchy", strings(n, 1), ...
    "ScheduledMCSIndex", NaN(n, 1), ...
    "ScheduledModulation", strings(n, 1), ...
    "ScheduledOperatingPointMatchesTransmitted", false(n, 1), ...
    "ScheduledOperatingPointEvidenceStatus", ...
        repmat("not_applicable_no_adaptive_scheduled_decision", n, 1), ...
    "RunID", strings(n, 1), ...
    "RunTag", strings(n, 1), ...
    "ScenarioID", strings(n, 1), ...
    "ConfigHash", strings(n, 1), ...
    "ExecutionID", strings(n, 1));
...
T.FixedLinkIntervalMethod = string(T.FixedLinkIntervalMethod);
T.ScheduledModulation = string(T.ScheduledModulation);
T.ScheduledOperatingPointEvidenceStatus = ...
    string(T.ScheduledOperatingPointEvidenceStatus);
T.RunID = string(T.RunID);
T.RunTag = string(T.RunTag);
T.ScenarioID = string(T.ScenarioID);
T.ConfigHash = string(T.ConfigHash);
T.ExecutionID = string(T.ExecutionID);
end
...
for i = 1:height(summary)
    ...
    row.NominalIntervalMethod = localTextScalar(summary(i, :), ...
        [prefixName(direction) + "_BLER_CI_Method", "IntervalMethod"], "");
    row.NominalIntervalSamplingPlan = localTextScalar(summary(i, :), ...
        ["NominalIntervalSamplingPlan"], "FIXED_SAMPLE_DESCRIPTIVE");
    ...
    row.BLER_CI_Method = localTextScalar(summary(i, :), ...
        [prefix + "_BLER_CI_Method", "IntervalMethod"], ...
        row.NominalIntervalMethod);
    ...
    row.BER_CI_Method = localTextScalar(summary(i, :), ...
        [prefix + "_BER_CI_Method", "IntervalMethod"], ...
        row.NominalIntervalMethod);
    row.SequentialIntervalMethod = localTextScalar(summary(i, :), ...
        [prefix + "_SequentialIntervalMethod"], "");
    row.SequentialLookConfidenceLevel = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialLookConfidenceLevel"], NaN);
    row.SequentialSidedness = localTextScalar(summary(i, :), ...
        [prefix + "_SequentialSidedness"], "");
    row.SequentialCI_Low = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialCI_Low"], NaN);
    row.SequentialCI_High = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialCI_High"], NaN);
    row.SequentialCI_HalfWidth = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialCI_HalfWidth"], NaN);
    row.SequentialLookIndex = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialLookIndex"], NaN);
    row.SequentialAlphaSpentThisLook = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialAlphaSpentThisLook"], NaN);
    row.SequentialCumulativeAlphaSpent = localScalarNumeric(summary(i, :), ...
        [prefix + "_SequentialCumulativeAlphaSpent"], NaN);
    row.SequentialDesignID = localTextScalar(summary(i, :), ...
        [prefix + "_SequentialDesignID"], "");
    row.PointStatus = localTextScalar(summary(i, :), ...
        [prefix + "_PointStatus"], "");
...
completeness = table( ...
    ...
    zeroTrials, ...
    string(curveSummary.PointStatus), ...
    string(curveSummary.StopReason), ...
    string(curveSummary.SequentialIntervalMethod), ...
    double(curveSummary.SequentialLookConfidenceLevel), ...
    double(curveSummary.SequentialCI_Low), ...
    double(curveSummary.SequentialCI_High), ...
    double(curveSummary.SequentialLookIndex), ...
    double(curveSummary.SequentialAlphaSpentThisLook), ...
    double(curveSummary.SequentialCumulativeAlphaSpent), ...
    string(curveSummary.SequentialDesignID), ...
    status, ...
    failure, ...
    ...
    'PointStatus','StopReason','SequentialIntervalMethod', ...
    'SequentialLookConfidenceLevel','SequentialCI_Low','SequentialCI_High', ...
    'SequentialLookIndex','SequentialAlphaSpentThisLook', ...
    'SequentialCumulativeAlphaSpent','SequentialDesignID', ...
    'Status','FailureCode'});
end
...
localValidateIntervalEvidence(dirRows, direction);
end

function localValidateIntervalEvidence(T, direction)
allowedNominal = ["CLOPPER_PEARSON_TWO_SIDED", "WILSON_TWO_SIDED"];
nominal = string(T.NominalIntervalMethod);
if any(~ismember(nominal, allowedNominal)) || ...
        any(string(T.BLER_CI_Method) ~= nominal) || ...
        any(string(T.BER_CI_Method) ~= nominal)
    error("sixgr:analytics:exportFixedSNRSweepCurves:InvalidIntervalMethod", ...
        "Fixed SNR sweep direction %s has missing or inconsistent nominal interval evidence.", ...
        char(direction));
end
sequential = string(T.SequentialIntervalMethod);
allowedSequential = [allowedNominal, "CLOPPER_PEARSON_ONE_SIDED_UPPER"];
if any(~ismember(sequential, allowedSequential)) || ...
        any(~isfinite(double(T.SequentialLookConfidenceLevel))) || ...
        any(~isfinite(double(T.SequentialLookIndex))) || ...
        any(~isfinite(double(T.SequentialAlphaSpentThisLook))) || ...
        any(~isfinite(double(T.SequentialCumulativeAlphaSpent))) || ...
        any(strlength(string(T.SequentialDesignID)) == 0) || ...
        any(strlength(string(T.PointStatus)) == 0)
    error("sixgr:analytics:exportFixedSNRSweepCurves:MissingSequentialIntervalEvidence", ...
        "Fixed SNR sweep direction %s lacks reproducible sequential stopping evidence.", ...
        char(direction));
end
localValidateCI(double(T.BLER), double(T.SequentialCI_Low), ...
    double(T.SequentialCI_High), "sequential BLER", direction);
end
...
row = struct( ...
    ...
    "ConfidenceLevel", NaN, ...
    "NominalIntervalMethod", "", ...
    "NominalIntervalSamplingPlan", "", ...
    "BLER_CI_Method", "", ...
    "BER_CI_Method", "", ...
    "SequentialIntervalMethod", "", ...
    "SequentialLookConfidenceLevel", NaN, ...
    "SequentialSidedness", "", ...
    "SequentialCI_Low", NaN, ...
    "SequentialCI_High", NaN, ...
    "SequentialCI_HalfWidth", NaN, ...
    "SequentialLookIndex", NaN, ...
    "SequentialAlphaSpentThisLook", NaN, ...
    "SequentialCumulativeAlphaSpent", NaN, ...
    "SequentialDesignID", "", ...
    "PointStatus", "", ...
    "StopReason", "", ...
    "Incomplete", false, ...
...
function value = prefixName(direction)
value = upper(string(direction));
end
...
T = table(strings(0, 1), zeros(0, 1), zeros(0, 1), zeros(0, 1), zeros(0, 1), ...
    ...
    'MinTrialsRequired','MaxTrialsAllowed','Incomplete','ZeroTrials','PointStatus', ...
    'StopReason','SequentialIntervalMethod','SequentialLookConfidenceLevel', ...
    'SequentialCI_Low','SequentialCI_High','SequentialLookIndex', ...
    'SequentialAlphaSpentThisLook','SequentialCumulativeAlphaSpent', ...
    'SequentialDesignID','Status','FailureCode'});
end
