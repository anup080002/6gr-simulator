...
if direction == "DL"
    ...
    maximumLayers = localFirstFinite(cfg, ...
        ["phy.pdsch.maxLayers","phy.pdsch.maxRankDefault","phy.mimo.maxRank"], layers);
else
    ...
    maximumLayers = localFirstFinite(cfg, ...
        ["phy.pusch.maxLayers","phy.pusch.maxRankDefault","phy.mimo.maxRank"], layers);
end
...
rank = layers;
maximumLayers = localPositiveIntegerOrNaN(maximumLayers);
...
row = struct( ...
    ...
    "ConfiguredRank", rank, "ConfiguredLayers", layers, ...
    "ConfiguredMaximumRank", maximumLayers, "ConfiguredMaximumLayers", maximumLayers, ...
    "ConfiguredCodewords", localCodewords(layers), ...
...
modeTokens = replace(modeTokens,"-","_");
modeTokens = modeTokens(strlength(modeTokens) > 0);
adaptiveTokens = ["amc","adaptive","adaptive_link","cqi_driven","olla","outer_loop", ...
