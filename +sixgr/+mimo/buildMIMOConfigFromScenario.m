...
if direction == "DL"
    ...
    txPorts = localFirstFinite(cfg, ["phy.pdsch.NumAntennaPorts","phy.pdsch.numAntennaPorts","channel.nTxAnt","phy.nTxAnt"], txAnt);
    rxPorts = localFirstFinite(cfg, ["channel.nRxAnt","phy.nRxAnt"], rxAnt);
...
row = struct( ...
    ...
    "TxRFChainCount", localFirstFinite(cfg, ["antenna_and_array.bs_num_txrus","mimo.n_txrus"], txAnt), ...
    "RxRFChainCount", localFirstFinite(cfg, ["antenna_and_array.ue_num_rxrus","mimo.n_rxrus"], rxAnt), ...
...
tokens = lower(strtrim(string([ ...
    ...
    sixgr.util.structGet(cfg, "phy.linkAdaptation.rankPolicy", "")])));
tokens = tokens(strlength(tokens) > 0);
fixedTokens = ["fixed","fixed_mcs","configured_fixed","disabled","off","none","false","fixed_anchor"];
adaptiveTokens = ["amc","adaptive","cqi","cqi_driven","ri_pmi_cqi"];
