...
[~,selectedIndex] = max(rankScores);
selectedRank = rankDomain(selectedIndex);
W = rankW{selectedIndex};
pmi = rankPMI(selectedIndex);
selection = rankInfo{selectedIndex};
% Rank overhead is a selection cost, not receiver noise. Use the selected
% receiver's resource/layer SINRs for the effective scheduling statistic.
% An exact zero-SINR observation stays -Inf; do not insert a finite floor.
sinrDB = 10*log10(expm1(mean(log1p(selection.SelectedSINRPerRELayer),"all")));
cqi = localCQI(sinrDB,cfg,strict);
[li,liEvidence]=sixgr.phy.mimo.selectCSILayerIndicator( ...
    selection.SelectedSINRPerRELayer,cqi);
selection.LayerIndicatorEvidence=liEvidence;
