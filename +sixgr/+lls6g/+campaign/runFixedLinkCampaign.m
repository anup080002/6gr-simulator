...
row.IntervalMethod = string(campaignCfg.IntervalMethod);
row.NominalIntervalSamplingPlan = "FIXED_SAMPLE_DESCRIPTIVE";
row.SequentialSamplingPlan = "BOUNDED_LOOK_BONFERRONI";
...
while true
    ...
    [shouldStop, stopReason, incomplete, decision] = ...
        localShouldStopPoint(stats, campaignCfg);
    stats = localApplySequentialDecision(stats, decision);
    % A statistically admissible stopping boundary is not sufficient when
    % the master YAML declares an explicit independent-seed set.  Every
    % configured seed must contribute at least one executed drop to every
    % direction/operating point before that point can complete.  Otherwise
    % a large trials_per_drop value can satisfy min_tb_per_point while
    % silently omitting the tail of validation.fixed_link_campaign.seeds.
    % This is a seed-coverage requirement, not an artificial extension of
    % a confidence interval: all additional rows still come from the same
    % physical waveform chain and retain their configured seed lineage.
    seedCoverageOk = localConfiguredSeedCoverageOk(stats, campaignCfg);
    if shouldStop && ~seedCoverageOk
        shouldStop = false;
        stopReason = "continue_until_all_configured_seeds_execute";
        incomplete = false;
    end
    if shouldStop
...
T.FixedLinkIntervalMethod = repmat(string(campaignCfg.IntervalMethod), n, 1);
...
intervalMethod = string(campaignCfg.IntervalMethod);
stats.BLER_CI_Method = intervalMethod;
stats.BER_CI_Method = intervalMethod;
...
blerInterval = sixgr.validation.BinomialIntervalEngine.compute( ...
    stats.FailureCount, stats.TrialCount, confidenceLevel, intervalMethod);
stats.BLER_CI_Low = double(blerInterval.Lower);
stats.BLER_CI_High = double(blerInterval.Upper);
stats.BLER_CI_HalfWidth = double(blerInterval.HalfWidth);
stats.BLER_CI_Width = double(blerInterval.Upper - blerInterval.Lower);
...
if isfinite(bitTotal) && bitTotal > 0
    berInterval = sixgr.validation.BinomialIntervalEngine.compute( ...
        bitErrTotal, bitTotal, confidenceLevel, intervalMethod);
    stats.BER_CI_Low = double(berInterval.Lower);
    stats.BER_CI_High = double(berInterval.Upper);
    stats.BER_CI_HalfWidth = double(berInterval.HalfWidth);
    stats.BER_CI_Width = double(berInterval.Upper - berInterval.Lower);
end
...
stats.ConfiguredSeedCount = double(numel(campaignCfg.Seeds));
stats.ExecutedSeedCount = double(localUniqueFiniteCount(localNumericColumn( ...
    Te, ["FixedLinkSeedIndex"], NaN)));
stats.SeedCoverageOk = stats.ExecutedSeedCount >= stats.ConfiguredSeedCount;
end

function tf = localConfiguredSeedCoverageOk(stats, campaignCfg)
required = double(numel(campaignCfg.Seeds));
executed = double(sixgr.util.structGet(stats, "ExecutedSeedCount", 0));
tf = required > 0 && isfinite(executed) && executed >= required;
end

function [shouldStop, stopReason, incomplete, decision] = localShouldStopPoint(stats, campaignCfg)
...
stopReason = "continue";
incomplete = false;
decision = localEmptySequentialDecision();
...
function stats = localApplySequentialDecision(stats, decision)
if ~(isstruct(decision) && isscalar(decision))
    return;
end
stats.SequentialIntervalMethod = string(sixgr.util.structGet( ...
    decision, "Method", "NOT_COMPUTED"));
stats.SequentialLookConfidenceLevel = double(sixgr.util.structGet( ...
    decision, "ConfidenceLevel", NaN));
stats.SequentialSidedness = string(sixgr.util.structGet( ...
    decision, "Sidedness", "NOT_COMPUTED"));
stats.SequentialCI_Low = double(sixgr.util.structGet( ...
    decision, "Lower", NaN));
stats.SequentialCI_High = double(sixgr.util.structGet( ...
    decision, "Upper", NaN));
stats.SequentialCI_HalfWidth = double(sixgr.util.structGet( ...
    decision, "HalfWidth", NaN));
stats.SequentialLookIndex = double(sixgr.util.structGet( ...
    decision, "LookIndex", NaN));
stats.SequentialAlphaSpentThisLook = double(sixgr.util.structGet( ...
    decision, "AlphaSpent", NaN));
stats.SequentialCumulativeAlphaSpent = min(1, ...
    stats.SequentialLookIndex * stats.SequentialAlphaSpentThisLook);
stats.SequentialDesignID = string(sixgr.util.structGet( ...
    decision, "DesignID", ""));
stats.PointStatus = string(sixgr.util.structGet( ...
    decision, "PointStatus", "RUNNING"));
end

function decision = localEmptySequentialDecision()
decision = struct( ...
    "Method", "NOT_COMPUTED", ...
    "ConfidenceLevel", NaN, ...
    "Sidedness", "NOT_COMPUTED", ...
    "Convention", "ERROR_COUNT", ...
    "ErrorCount", 0, ...
    "TrialCount", 0, ...
    "Estimate", NaN, ...
    "Lower", NaN, ...
...
row = struct( ...
    ...
    "DL_SequentialIntervalMethod", "", "DL_SequentialLookConfidenceLevel", NaN, ...
    "DL_SequentialSidedness", "", "DL_SequentialCI_Low", NaN, ...
    "DL_SequentialCI_High", NaN, "DL_SequentialCI_HalfWidth", NaN, ...
    "DL_SequentialLookIndex", NaN, "DL_SequentialAlphaSpentThisLook", NaN, ...
    "DL_SequentialCumulativeAlphaSpent", NaN, ...
    "DL_SequentialDesignID", "", "DL_PointStatus", "", ...
    ...
    "DL_DropCount", NaN, "DL_ConfiguredSeedCount", NaN, ...
    "DL_ExecutedSeedCount", NaN, "DL_SeedCoverageOk", false, ...
    ...
    "UL_SequentialIntervalMethod", "", "UL_SequentialLookConfidenceLevel", NaN, ...
    "UL_SequentialSidedness", "", "UL_SequentialCI_Low", NaN, ...
    "UL_SequentialCI_High", NaN, "UL_SequentialCI_HalfWidth", NaN, ...
    "UL_SequentialLookIndex", NaN, "UL_SequentialAlphaSpentThisLook", NaN, ...
    "UL_SequentialCumulativeAlphaSpent", NaN, ...
    "UL_SequentialDesignID", "", "UL_PointStatus", "", ...
...
row.UL_DropCount = NaN;
row.UL_ConfiguredSeedCount = NaN;
row.UL_ExecutedSeedCount = NaN;
row.UL_SeedCoverageOk = false;
end
...
stats = struct( ...
    ...
    "BLER_CI_Method", "", ...
    "BER_CI_Method", "", ...
    "SequentialIntervalMethod", "NOT_COMPUTED", ...
    "SequentialLookConfidenceLevel", NaN, ...
    "SequentialSidedness", "NOT_COMPUTED", ...
    ...
    "DropCount", NaN, ...
    "ConfiguredSeedCount", NaN, ...
    "ExecutedSeedCount", NaN, ...
    "SeedCoverageOk", false, ...
    "PointSeed", NaN);
end
...
row.(char(prefix + "_SequentialIntervalMethod")) = string(sixgr.util.structGet(stats, "SequentialIntervalMethod", "NOT_COMPUTED"));
row.(char(prefix + "_SequentialLookConfidenceLevel")) = double(sixgr.util.structGet(stats, "SequentialLookConfidenceLevel", NaN));
row.(char(prefix + "_SequentialSidedness")) = string(sixgr.util.structGet(stats, "SequentialSidedness", "NOT_COMPUTED"));
row.(char(prefix + "_SequentialCI_Low")) = double(sixgr.util.structGet(stats, "SequentialCI_Low", NaN));
row.(char(prefix + "_SequentialCI_High")) = double(sixgr.util.structGet(stats, "SequentialCI_High", NaN));
row.(char(prefix + "_SequentialCI_HalfWidth")) = double(sixgr.util.structGet(stats, "SequentialCI_HalfWidth", NaN));
row.(char(prefix + "_SequentialLookIndex")) = double(sixgr.util.structGet(stats, "SequentialLookIndex", NaN));
row.(char(prefix + "_SequentialAlphaSpentThisLook")) = double(sixgr.util.structGet(stats, "SequentialAlphaSpentThisLook", NaN));
row.(char(prefix + "_SequentialCumulativeAlphaSpent")) = double(sixgr.util.structGet(stats, "SequentialCumulativeAlphaSpent", NaN));
row.(char(prefix + "_SequentialDesignID")) = string(sixgr.util.structGet(stats, "SequentialDesignID", ""));
row.(char(prefix + "_PointStatus")) = string(sixgr.util.structGet(stats, "PointStatus", "RUNNING"));
...
row.(char(prefix + "_ConfiguredSeedCount")) = double(sixgr.util.structGet(stats, "ConfiguredSeedCount", NaN));
row.(char(prefix + "_ExecutedSeedCount")) = double(sixgr.util.structGet(stats, "ExecutedSeedCount", NaN));
row.(char(prefix + "_SeedCoverageOk")) = logical(sixgr.util.structGet(stats, "SeedCoverageOk", false));
end
...
T = table( ...
    ...
    string(localTextColumn(sub, ...
        [prefix + "_" + metricKind + "_CI_Method", "IntervalMethod"], "")), ...
    string(localTextColumn(sub, ["NominalIntervalSamplingPlan"], ...
        "FIXED_SAMPLE_DESCRIPTIVE")), ...
    string(localTextColumn(sub, [prefix + "_SequentialIntervalMethod"], "")), ...
    localNumericColumn(sub, [prefix + "_SequentialLookConfidenceLevel"], NaN), ...
    string(localTextColumn(sub, [prefix + "_SequentialSidedness"], "")), ...
    localNumericColumn(sub, [prefix + "_SequentialCI_Low"], NaN), ...
    localNumericColumn(sub, [prefix + "_SequentialCI_High"], NaN), ...
    localNumericColumn(sub, [prefix + "_SequentialCI_HalfWidth"], NaN), ...
    localNumericColumn(sub, [prefix + "_SequentialLookIndex"], NaN), ...
    localNumericColumn(sub, [prefix + "_SequentialAlphaSpentThisLook"], NaN), ...
    localNumericColumn(sub, [prefix + "_SequentialCumulativeAlphaSpent"], NaN), ...
    string(localTextColumn(sub, [prefix + "_SequentialDesignID"], "")), ...
    string(localTextColumn(sub, [prefix + "_PointStatus"], "")), ...
    ...
    'Value','CI_Low','CI_High','CI_HalfWidth','CI_Width','IntervalMethod', ...
    'IntervalSamplingPlan','SequentialIntervalMethod','SequentialLookConfidenceLevel', ...
    'SequentialSidedness','SequentialCI_Low','SequentialCI_High', ...
    'SequentialCI_HalfWidth','SequentialLookIndex','SequentialAlphaSpentThisLook', ...
    'SequentialCumulativeAlphaSpent', ...
    'SequentialDesignID','PointStatus','TrialCount','FailureCount', ...
...
% Preserve the configured seed vector exactly through validation.  The
% generic integer-vector helper intentionally de-duplicates MCS sweeps,
% but doing that to seeds would silently turn an invalid repeated-seed
% campaign into apparently valid independent evidence.
cfg.Seeds = localFiniteRowVector(sixgr.util.structGet(cfg, "Seeds", ...
    sixgr.util.structGet(cfg, "seeds", [])), ...
...
cfg.IntervalMethod = upper(strtrim(string(sixgr.util.structGet(cfg, ...
    "IntervalMethod", sixgr.util.structGet(cfg, "interval_method", ...
...
if isempty(cfg.Seeds) || any(~isfinite(cfg.Seeds)) || ...
        any(cfg.Seeds < 0) || any(cfg.Seeds ~= fix(cfg.Seeds))
    error("sixgr:lls6g:campaign:InvalidConfiguredSeeds", ...
        ["validation.fixed_link_campaign.seeds must contain nonnegative " + ...
         "integer seed values."]);
end
if numel(unique(double(cfg.Seeds(:)))) ~= numel(cfg.Seeds)
    error("sixgr:lls6g:campaign:DuplicateConfiguredSeeds", ...
        ["validation.fixed_link_campaign.seeds must contain distinct " + ...
         "independent seed values; duplicate seeds cannot satisfy a " + ...
         "multi-seed publication claim."]);
end
minimumTrialsForSeedCoverage = ...
    (double(numel(cfg.Seeds)) - 1) * double(cfg.BatchTBCount) + 1;
if cfg.MaxTBPerPoint < minimumTrialsForSeedCoverage
    error("sixgr:lls6g:campaign:InsufficientTrialBudgetForConfiguredSeeds", ...
        ["validation.fixed_link_campaign.max_tb_per_point=%d cannot " + ...
         "exercise all %d configured seeds with trials_per_drop=%d. " + ...
         "Configure at least %d trials per operating point."], ...
        cfg.MaxTBPerPoint, numel(cfg.Seeds), cfg.BatchTBCount, ...
        minimumTrialsForSeedCoverage);
end
...
if ~ismember(cfg.IntervalMethod, ...
        ["CLOPPER_PEARSON_TWO_SIDED", "WILSON_TWO_SIDED"])
    error("sixgr:lls6g:campaign:InvalidIntervalMethod", ...
        ["validation.fixed_link_campaign.interval_method must be " + ...
         "CLOPPER_PEARSON_TWO_SIDED or WILSON_TWO_SIDED."]);
end
...
row = struct( ...
    "Direction", "", "MCSIndex", NaN, "PointIndex", NaN, ...
    "PointSeed", NaN, "SNR_dB", NaN, "Metric", "", ...
    "Value", NaN, "CI_Low", NaN, "CI_High", NaN, ...
    "CI_HalfWidth", NaN, "CI_Width", NaN, "IntervalMethod", "", ...
    "IntervalSamplingPlan", "", "SequentialIntervalMethod", "", ...
    "SequentialLookConfidenceLevel", NaN, "SequentialSidedness", "", ...
    "SequentialCI_Low", NaN, "SequentialCI_High", NaN, ...
    "SequentialCI_HalfWidth", NaN, "SequentialLookIndex", NaN, ...
    "SequentialAlphaSpentThisLook", NaN, ...
    "SequentialCumulativeAlphaSpent", NaN, "SequentialDesignID", "", ...
    "PointStatus", "", "TrialCount", NaN, "FailureCount", NaN, ...
    "MeasuredSINR_dB", NaN, "MeasuredSINR_CI_Low", NaN, ...
    "MeasuredSINR_CI_High", NaN, "Throughput_Mbps", NaN, ...
    "Throughput_CI_Low", NaN, "Throughput_CI_High", NaN, ...
    "Goodput_Mbps", NaN, "OfferedThroughput_Mbps", NaN, ...
    "ConfiguredRank", NaN, "ConfiguredLayers", NaN, ...
    "ConfiguredPRBCount", NaN, "ConfiguredChannelModel", "", ...
    "StopReason", "", "Incomplete", false, "PrimaryTargetBLER", NaN, ...
    "PrimaryTargetCrossingStatus", "", ...
    "PrimaryTargetCrossingSNR_dB", NaN);
