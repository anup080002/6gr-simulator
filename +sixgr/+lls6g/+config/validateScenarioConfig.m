...
ulTable=string(sixgr.util.structGet(cfg,'pusch.mcs_table',''));
if startsWith(ulTable,"experimental_")
    definition=sixgr.phy.research.resolveExperimentalMCSTable(ulTable);
    assert(~isempty(definition) && ...
        string(sixgr.util.structGet(cfg,'modulation_and_mapping.ul_mcs_table',''))==ulTable, ...
        'sixgr:research:InvalidMCSTable','Experimental PUSCH and directional MCS table names must match a catalog definition.');
    assert(string(cfg.meta.research_class)==definition.ResearchClass && ...
        string(sixgr.util.structGet(cfg,'research_pusch_uci.resource_mapping',''))== ...
        "symbol_preserving_single_codeword_ulsch", ...
        'sixgr:research:ExplicitUCIAdapterRequired','Install the explicit experimental class and UCI policy.');
end
assert(~startsWith(string(sixgr.util.structGet(cfg,'pdsch.mcs_table','')),"experimental_"), ...
    'sixgr:research:UnsupportedDLMCSTable','This experimental connected MCS bridge is UL-only; retain the separate DL capability contract.');
localValidateCausalPHYChainAudit(cfg, ctx);
runnerProfile = lower(string(cfg.scenario.runner_profile));
sweepBaseProfile = lower(string(sixgr.util.structGet( ...
    cfg,"scenario.sweep.base_profile","")));
isWaveformExecution = any(runnerProfile==["waveform_bundle","system_level_lls"]) || ...
    (runnerProfile=="generic_sweep" && sweepBaseProfile=="waveform_bundle");
if isfield(cfg,'research_awgn_mimo') && ...
        isfield(cfg.simulation,'awgn_reference_re_energy')
    assert(cfg.simulation.awgn_reference_re_energy== ...
        cfg.research_awgn_mimo.reference_data_re_power, ...
        'sixgr:lls6g:config:ConflictingAWGNReferenceEnergy', ...
        ['simulation.awgn_reference_re_energy must equal the fixed-port ' ...
         'research_awgn_mimo.reference_data_re_power; do not change the noise reference between runners.']);
end
matrixReal=sixgr.util.structGet(cfg,'channels.awgn_spatial_matrix_dl_real',[]);
matrixImag=sixgr.util.structGet(cfg,'channels.awgn_spatial_matrix_dl_imag',[]);
if ~isempty(matrixReal)
    assert(~logical(sixgr.util.structGet(cfg,'channels.shared_identity_awgn_enabled',false)) && ...
        numel(matrixReal)==cfg.mimo.n_tx_ant*cfg.mimo.n_rx_ant && ...
        (isempty(matrixImag) || numel(matrixImag)==numel(matrixReal)), ...
        'sixgr:lls6g:InvalidAWGNSpatialMatrix', ...
        'An explicit row-major RX-by-TX AWGN matrix requires matching dimensions and identity disabled.');
elseif ~isempty(matrixImag)
    error('sixgr:lls6g:InvalidAWGNSpatialMatrix','Imaginary AWGN matrix requires real entries.');
end
if logical(sixgr.util.structGet(cfg,'channels.shared_identity_awgn_enabled',false)) || ~isempty(matrixReal)
    assert(upper(string(cfg.channels.model_type))=="AWGN" && ...
        upper(string(cfg.channels.profile))=="AWGN" && ...
        ~logical(cfg.channels.pathloss_enabled) && ~logical(cfg.channels.shadow_fading_enabled), ...
        'sixgr:lls6g:config:InvalidSharedIdentityAWGN', ...
        'channels.shared_identity_awgn_enabled requires an AWGN profile without pathloss or shadow fading.');
    assert(isWaveformExecution && ...
        logical(cfg.users.enabled) && string(cfg.users.execution_model)=="slot_coupled_truth", ...
        'sixgr:lls6g:config:SharedIdentityAWGNRunnerRequired', ...
        'Shared identity AWGN requires the slot_coupled_truth coordinator, not the ideal-feedback research runner.');
end
for signal=["srs","trs","csi_rs"]
    estimatorPath="reference_signals."+signal+".runtime_channel_estimator";
    if signal=="csi_rs", estimatorPath="reference_signals.csi_rs_runtime_channel_estimator"; end
    estimator=string(sixgr.util.structGet(cfg,estimatorPath,"nr_channel_estimate"));
    if estimator=="flat_static_awgn_ls"
        assert(upper(string(cfg.channels.model_type))=="AWGN" && ...
            upper(string(cfg.channels.profile))=="AWGN" && ...
            (logical(sixgr.util.structGet(cfg,'channels.shared_identity_awgn_enabled',false)) || ~isempty(matrixReal)), ...
            'sixgr:lls6g:config:FlatEstimatorRequiresExplicitAWGN', ...
            'Flat SRS/TRS/CSI-RS estimation requires the explicit shared identity/fixed-matrix AWGN operator.');
    end
end
...
researchMode = logical(localOptionalStructValue(cfg, ...
    "frequency.research_mode", false));
if researchMode && (localScenarioStudyStatus(cfg) ~= "optional_research_experiment" || ...
        upper(string(cfg.frequency.duplex_mode)) ~= "TDD")
    error("sixgr:lls6g:config:ResearchCarrierScope", ...
        "frequency.research_mode requires meta.research_class=" + ...
        "optional_research_experiment and frequency.duplex_mode=TDD in %s.", localCtx(ctx));
end
try
    if researchMode || upper(strtrim(configuredRange)) == "FR3"
        ...
        if ~customAllowed
            error("sixgr:phy:frame:CustomCarrierRequiresOptIn", ...
                "Research carriers require frequency.custom_frequency_override_allowed=true.");
        end
        lowGuard = double(localOptionalStructValue(cfg, "frequency.minimum_low_guardband_hz", NaN));
        highGuard = double(localOptionalStructValue(cfg, "frequency.minimum_high_guardband_hz", NaN));
        if researchMode && (~isfinite(lowGuard) || ~isfinite(highGuard))
            error("sixgr:phy:frame:MissingResearchGuardbands", ...
                "frequency.research_mode requires explicit finite minimum_low_guardband_hz and minimum_high_guardband_hz.");
        end
        sixgr.phy.frame.CarrierGridConfig.custom( ...
            ...
            "CyclicPrefix", cp, ...
            "MinimumLowGuardbandHz", lowGuard, ...
            "MinimumHighGuardbandHz", highGuard, ...
            "NSizeGrid", nSizeGrid, ...
...
if usersEnabled && ~isWaveformExecution
    error("sixgr:lls6g:config:UsersRequireWaveformBundle", ...
        ["users.enabled in %s requires a waveform execution: " + ...
         "waveform_bundle, system_level_lls, or generic_sweep with " + ...
         "scenario.sweep.base_profile='waveform_bundle'."], localCtx(ctx));
end
...
if isnumeric(value) || islogical(value)
    ...
    if isfield(rule, "max_exclusive") && any(v >= double(rule.max_exclusive))
        error("sixgr:lls6g:config:ValueTooLarge", ...
            "%s in %s must be < %g.", fieldPath, localCtx(ctx), double(rule.max_exclusive));
    end
...
tableName = string(localOptionalStructValue(cfg, ...
    "modulation_and_mapping."+lower(direction)+"_mcs_table", ...
    localOptionalStructValue(cfg, "modulation.mcs_table", "")));
...
if ~logical(profile.Valid)
    error("sixgr:lls6g:config:InitialMCSRateRequired", ...
        "%s configured initial MCS %g in %s has no defined rate in directional MCS table=%s. " + ...
...
if round(double(configuredOrder)) ~= round(double(resolvedOrder))
    error("sixgr:lls6g:config:ModulationMCSConsistencyRequired", ...
        "%s modulation order %g in %s does not match directional MCS table=%s, %s_mcs_index=%g, which resolves to %s.", ...
