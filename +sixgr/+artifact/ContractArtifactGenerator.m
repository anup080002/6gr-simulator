classdef ContractArtifactGenerator
    ...
    methods (Static)
        function result = generate(runFolder, evidence, varargin)
            ...
            localAtomicWriteTable(fullfile(auditDir, ...
                "canonical_component_manifest.csv"), manifest);
            result = struct();
            ...
            result.ManifestPath = string(fullfile(auditDir, ...
                "canonical_component_manifest.csv"));
            result.SummaryPath = string(fullfile(auditDir, ...
...
try
    ...
    info = dir(outputPath);
    audit.ByteSize = double(info.bytes);
    audit.Status = "PASS";
...
sixgr.visual.RasterFigureStyle.apply(figureHandle);
drawnow;
end
...
xlabels = strings(0, 1);
ylabels = strings(0, 1);
dataTypes = ["line", "errorbar", "scatter", "image", "surface", "patch", ...
...
function summary = localBuildGenerationSummary(audit)
keys = unique(audit(:, ["Domain","Component","Profile","ArtifactType"]), ...
    "rows", "stable");
rows = repmat(localEmptySummaryRow(), height(keys), 1);
for index = 1:height(keys)
    selected = audit.Domain == keys.Domain(index) & ...
        audit.Component == keys.Component(index) & ...
        audit.Profile == keys.Profile(index) & ...
        audit.ArtifactType == keys.ArtifactType(index);
    subset = audit(selected, :);
    rows(index) = struct( ...
        "Domain", keys.Domain(index), ...
        "Component", keys.Component(index), ...
        "Profile", keys.Profile(index), ...
        "ArtifactType", keys.ArtifactType(index), ...
        "ContractCount", double(height(subset)), ...
        "RequiredCount", double(nnz(subset.Required)), ...
        "GeneratedCount", double(nnz(subset.Status == "PASS")), ...
        "MissingCount", double(nnz(subset.Status == "MISSING")), ...
        "FailedCount", double(nnz(subset.Status == "FAIL")), ...
        "RequiredFailureCount", double(nnz(subset.Required & ...
            subset.Status ~= "PASS")), ...
        "SourceRowCount", double(sum(subset.SourceRows)), ...
        "PublishedByteCount", double(sum(subset.ByteSize)), ...
        "Status", localSummaryStatus(subset));
end
...
function status = localSummaryStatus(subset)
if all(subset.Status == "PASS")
    status = "PASS";
elseif any(subset.Required & subset.Status ~= "PASS")
    status = "FAIL_REQUIRED";
else
    status = "INCOMPLETE_OPTIONAL";
end
end

function manifest = localBuildCanonicalManifest(audit, publishRoot)
manifest = audit(audit.Status == "PASS", :);
prefix = replace(strtrim(string(publishRoot)), string(filesep), "/");
relative = replace(string(manifest.OutputRelativePath), string(filesep), "/");
manifest.PublishedRelativePath = prefix + "/" + relative;
manifest = movevars(manifest, "PublishedRelativePath", ...
    "After", "OutputRelativePath");
if height(manifest) ~= height(audit)
    error("sixgr:artifact:IncompleteCanonicalManifest", ...
        "Canonical manifest cannot be published with %d/%d artifacts.", ...
        height(manifest), height(audit));
end
end
