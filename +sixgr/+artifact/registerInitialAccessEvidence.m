...
if any(~isfinite(peak) | ~isfinite(threshold) | ~isfinite(timing) | ~isfinite(timingError))
    error("sixgr:artifact:InvalidPRACHReceiverMeasurement", ...
        "PRACH peak, threshold, timing estimate and timing error must be finite receiver measurements.");
end
if any(frequencyEnabled & (~frequencyValid | ~isfinite(frequency)))
    error("sixgr:artifact:MissingEnabledPRACHFrequencyEstimate", ...
        "YAML-enabled PRACH CFO estimation requires a finite valid estimate on every eligible trial.");
end

n = height(raw);
T = table( ...
    trialID, occasionID, double(raw.PreambleIndexTx), ...
    double(raw.PreambleIndexDetected), localLogical(raw.PreambleDetected), ...
