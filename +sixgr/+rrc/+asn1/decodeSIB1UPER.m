...
cfg.rrc.sib1.offset_to_point_a = double(s.offset_to_point_a);
for direction = ["dl", "ul"]
    riv = double(s.("initial_" + direction + "_bwp_riv"));
    [startRB, sizeRB] = sixgr.bwop.RIVFDRA.decode(275, riv);
    if sixgr.bwop.RIVFDRA.encode(275, startRB, sizeRB) ~= riv
        error("sixgr:rrc:asn1:DecodeFailed", "Decoded initial BWP RIV is noncanonical.");
    end
    cfg.rrc.sib1.("initial_" + direction + "_bwp") = struct( ...
        "start_rb", startRB, "size_rb", sizeRB, ...
        "scs_khz", double(s.("initial_" + direction + "_bwp_scs_khz")), ...
        "cyclic_prefix", string(s.("initial_" + direction + "_bwp_cyclic_prefix")));
end
