...
%PAPROFILE Calibrated bounded PA models with no output-power restoration.
    ...
    methods(Static)
        ...
        function [y, evidence] = apply(x, profile)
            ...
            validateattributes(profile.InputBackoff_dB,{'numeric'}, ...
                {'scalar','real','finite'});
            % Input backoff changes the PA drive, hence its nonlinear
            % distortion. Attenuating its already compressed output is not
            % equivalent and incorrectly leaves the PA overdriven.
            xin = x.*cast(10.^(-double(profile.InputBackoff_dB)/20),'like',x);
            switch model
                case "rapp"
            ...
            evidence = struct( ...
                ...
                "BackoffReferencePlane","before_pa_nonlinearity", ...
                "InputBackoff_dB",double(profile.InputBackoff_dB), ...
