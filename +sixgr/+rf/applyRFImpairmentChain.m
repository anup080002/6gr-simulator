...
phaseNoise = sixgr.rf.PhaseNoiseModel(cfg, fs, double(sixgr.util.structGet(cfg, "run.seed", 1)) + 173);
phaseNoiseEnabled = logical(phaseNoise.Enable);
...
phaseNoiseBeforeHash = sixgr.channel.hashChannelRFConfig(localWaveformHashPayload(y));
phaseNoiseAfterHash = phaseNoiseBeforeHash;
phaseNoiseApplied = false;
if phaseNoiseEnabled
    y = phaseNoise.apply(y, fs);
    phaseNoiseAfterHash = sixgr.channel.hashChannelRFConfig(localWaveformHashPayload(y));
    phaseNoiseApplied = phaseNoiseAfterHash ~= phaseNoiseBeforeHash;
end
...
failure = "";
if phaseNoiseEnabled && ~phaseNoiseApplied
    strictOk = false;
    failure = localAppendReason(failure, "phase_noise_configured_but_not_applied");
end
if sampleClockOffsetEnabled && abs(sampleClockOffsetPpm) > 0
    strictOk = false;
    failure = localAppendReason(failure, "sample_clock_offset_configured_but_unsupported");
end
if beforeHash == afterHash
    strictOk = false;
    failure = localAppendReason(failure, "rf_configured_waveform_unchanged");
end
out = struct();
out.Waveform = y;
out.Row = struct( ...
    ...
    "CFOHzApplied", cfoHz, ...
    "PhaseNoiseEnabled", phaseNoiseEnabled, ...
    "PhaseNoiseApplied", phaseNoiseApplied, ...
    "PhaseNoiseProfileId", string(phaseNoise.Backend), ...
    "PhaseNoiseLevelApplied", localMeanPhaseNoiseLevel(phaseNoise), ...
    "PhaseNoiseWaveformBeforeHash", phaseNoiseBeforeHash, ...
    "PhaseNoiseWaveformAfterHash", phaseNoiseAfterHash, ...
    "PhaseNoiseExecutionStatus", localString(phaseNoiseEnabled, ...
        localString(phaseNoiseApplied, "applied_sample_domain_phase_noise", "configured_not_applied"), "disabled"), ...
    "PhaseNoiseTruthClassification", string(phaseNoise.TruthClassification), ...
    ...
    "SampleClockOffsetEnabled", sampleClockOffsetEnabled, ...
    "SampleClockOffsetPpm", sampleClockOffsetPpm, ...
...
function reason = localAppendReason(reason, nextReason)
reason = string(reason);
nextReason = string(nextReason);
...
function level = localMeanPhaseNoiseLevel(phaseNoise)
if ~logical(phaseNoise.Enable)
    level = NaN;
    return;
end
vals = double(phaseNoise.Level_dBcHz(:));
