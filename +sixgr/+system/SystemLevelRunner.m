                ...
                elseif noProgressGuardEnabled
                    ...
                    if backlogDemand
                        ...
                        if stalledDemandSlotCount >= noProgressSlotLimit || stagnationElapsed_s >= noProgressTimeout_s
                            ...
                            % Keep the actual failure-boundary state separate
                            % from primary results. Saving diagnostics must
                            % never change or suppress the original failure.
                            try
                                diagnostic = struct('DiagnosticOnly',true,'RunCompleted',false, ...
                                    'Scope',"actual_no_progress_failure_boundary_not_primary_results", ...
                                    'Slot',t,'TotalSlots',nTTI,'SlotDuration_s',tti_s, ...
                                    'LastProductiveSlot',lastUsefulProgressSlot, ...
                                    'StalledDemandSlots',stalledDemandSlotCount, ...
                                    'ElapsedSinceProgress_s',stagnationElapsed_s, ...
                                    'Timeout_s',noProgressTimeout_s,'SlotLimit',noProgressSlotLimit, ...
                                    'QueueBitsDL',queueBitsDL,'QueueBitsUL',queueBitsUL, ...
                                    'GrantsDL',grantsDL,'GrantsUL',grantsUL, ...
                                    'SchedulerDecisionState',schedulerDecisionState, ...
                                    'UEStateDL',ueStateDLAll,'UEStateUL',ueStateULAll, ...
                                    'CQIFeedbackDL',cqiFeedbackDL,'CQIFeedbackUL',cqiFeedbackUL, ...
                                    'PendingULGrantsBySlot',{pendingULGrantsBySlot}, ...
                                    'Errors',out.Errors,'Cfg',cfg,'Params',params);
                                diagnostic.ExecutedGrantTrace = localGrantTraceToTable(grantTrace,grantTraceCount);
                                diagnostic.QueueHistoryDL = queueHistDL(1:t,:);
                                diagnostic.QueueHistoryUL = queueHistUL(1:t,:);
                                diagnostic.DLBudget = dlBudget;
                                diagnostic.ULBudget = ulBudget;
                                diagnostic.SchedulerDLHARQ = cellfun(@(s) sixgr.report.snapshotHARQSchedulingState(s.HARQ), ...
                                    schedDLCells,'UniformOutput',false);
                                diagnostic.SchedulerULHARQ = cellfun(@(s) sixgr.report.snapshotHARQSchedulingState(s.HARQ), ...
                                    schedULCells,'UniformOutput',false);
                                diagnosticRoot = fullfile(ctx.RunFolder,'debug');
                                if ~isfolder(diagnosticRoot), mkdir(diagnosticRoot); end
                                diagnosticPath = fullfile(diagnosticRoot,sprintf('no_progress_slot_%d.mat',t));
                                assert(~isfile(diagnosticPath),'sixgr:system:DiagnosticExists', ...
                                    'Never overwrite earlier watchdog evidence.');
                                save(diagnosticPath,'diagnostic','-v7.3');
                                fprintf('NO_PROGRESS_DIAGNOSTIC=%s\n',diagnosticPath);
                            catch diagnosticError
                                warning('sixgr:system:NoProgressDiagnosticFailed', ...
                                    'Could not retain watchdog diagnostics: %s',diagnosticError.message);
                            end
