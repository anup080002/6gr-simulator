...
ip.addParameter("ControlOnly", false, @(x)islogical(x)||(isnumeric(x)&&isscalar(x)));
ip.parse(varargin{:});
opt = ip.Results;
...
replay = struct( ...
    ...
    "StrictOk", false, ...
    "TruthStatus", "unavailable_waveform_replay_not_executed", ...
    ...
    "PDCCHWaveformExecuted", false, ...
    "PDCCHGrantBindingRequired", false, ...
    "PDCCHGrantBindingOk", false, ...
    "PDCCHGrantBindingStatus", "not_required", ...
    "PDCCHGrantBindingFailureCode", "", ...
    "PDCCHGrantDCIId", "", ...
    "PDCCHGrantDCIFieldsHash", "", ...
    "PDCCHGrantFieldsHash", "", ...
    "PDCCHGrantSearchSpaceId", NaN, ...
    "PDCCHGrantCORESETId", NaN, ...
    "PDCCHGrantAggregationLevel", NaN, ...
    "PDCCHGrantCandidateIndex", NaN, ...
    "PDCCHGrantDCIFormat", "", ...
    "DCICrcPass", false, ...
    "PDCCHPayloadMatch", false, ...
    "PDCCHCausalGrantDecodeOk", false, ...
    "PDCCHMissedDetection", false, ...
    "PDCCHFalseAlarm", false, ...
    "PDCCHControlEvidenceSource", "", ...
    "PDCCHLogicalTxPorts", NaN, ...
    "PDCCHObservedRxBranches", NaN);
replay.BoundGrant = struct();
...
try
    ...
    replay = localReplayPDCCHGrantBinding( ...
        replay, cfgReplay, dir, grant, snr_dB);
    if replay.PDCCHGrantBindingRequired && ...
            ~replay.PDCCHGrantBindingOk
        replay = localMarkReplayUnavailable(replay, ...
            "pdcch_grant_binding_failed", ...
            "waveform_replay_blocked_by_pdcch: " + ...
            replay.PDCCHGrantBindingFailureCode);
        return;
    end
    replay.BoundGrant = localBindDecodedPDCCHToGrant(grant, replay);
    if logical(opt.ControlOnly)
        replay.Ok = logical(~replay.PDCCHGrantBindingRequired || ...
            replay.PDCCHGrantBindingOk);
        replay.PHYDecisionRole = "measured_control";
        replay.PHYDecisionStatus = "OK";
        replay.PHYDecisionSource = ...
            "sixgr.system.waveform.replayGrant.control_only";
        replay.PHYDecisionReason = "physical_pdcch_grant_binding_executed";
        replay.Notes = "Control-only replay executed physical PDCCH Tx/Rx and returned the decoded bound grant; no data waveform was launched.";
        return;
    end
    ...
    replay = localUpdateHARQSoftBuffer( ...
        replay, dir, rx, harqKey, harqPriorLLR, harqIsRetx);
...
function bound = localBindDecodedPDCCHToGrant(grant, replay)
% Carry only measured control evidence into the data-channel grant.  The
% scheduler allocation/DCI payload is not rewritten by the receiver.
bound = grant;
bound.ControlDecodeOk = logical(sixgr.util.structGet( ...
    replay, "PDCCHCausalGrantDecodeOk", false));
for name = ["PDCCHWaveformExecuted", "PDCCHGrantBindingRequired", ...
        "PDCCHGrantBindingOk", "PDCCHGrantBindingStatus", ...
        "PDCCHGrantBindingFailureCode", "PDCCHGrantDCIId", ...
        "PDCCHGrantDCIFieldsHash", "PDCCHGrantFieldsHash", ...
        "PDCCHGrantSearchSpaceId", "PDCCHGrantCORESETId", ...
        "PDCCHGrantAggregationLevel", "PDCCHGrantCandidateIndex", ...
        "PDCCHGrantDCIFormat", "DCICrcPass", "PDCCHPayloadMatch", ...
        "PDCCHCausalGrantDecodeOk", "PDCCHMissedDetection", ...
        "PDCCHFalseAlarm", "PDCCHControlEvidenceSource", ...
        "PDCCHLogicalTxPorts", "PDCCHObservedRxBranches"]
    bound.(char(name)) = sixgr.util.structGet(replay, name, []);
end
end
end

function replay = localReplayPDCCHGrantBinding( ...
        replay, cfg, direction, grant, snr_dB)
% Execute the scheduled DCI through the physical PDCCH waveform chain.
required = logical(sixgr.control.isPDCCHGrantBindingRequired( ...
    cfg, direction)) || logical(sixgr.util.structGet( ...
    grant, "PDCCHGrantBindingRequired", false));
replay.PDCCHGrantBindingRequired = required;
replay.PDCCHGrantDCIFormat = localPDCCHDCIFormat(grant, direction);
if ~required
    replay.PDCCHGrantBindingStatus = "not_required";
    return;
end

failures = strings(0, 1);
if ~logical(sixgr.util.structGet(cfg, "phy.pdcch.enable", false))
    failures(end+1, 1) = "pdcch_disabled_by_config"; %#ok<AGROW>
end
if ~logical(sixgr.util.structGet(cfg, "phy.pdcch.dmrs.enable", false))
    failures(end+1, 1) = "pdcch_dmrs_disabled_by_config"; %#ok<AGROW>
end
dciBits = int8(sixgr.util.structGet(grant, "DCI.Bits", int8([])));
dciBits = dciBits(:);
if isempty(dciBits)
    failures(end+1, 1) = "finalized_grant_dci_bits_missing"; %#ok<AGROW>
elseif any(~ismember(double(dciBits), [0 1]))
    failures(end+1, 1) = "finalized_grant_dci_bits_nonbinary"; %#ok<AGROW>
end
if ~isempty(failures)
    replay.PDCCHGrantBindingStatus = "failed_before_waveform";
    replay.PDCCHGrantBindingFailureCode = strjoin(failures, "|");
    return;
end
...
if ~(isscalar(rnti) && isfinite(rnti) && rnti >= 0 && rnti <= 65535)
    replay.PDCCHGrantBindingStatus = "failed_before_waveform";
    replay.PDCCHGrantBindingFailureCode = "invalid_grant_rnti";
    return;
end

try
    cfgControl = sixgr.util.structSet(cfg, ...
        "phy.pdcch.aggregationLevel", double(sixgr.util.structGet( ...
        grant, "PDCCHAggregationLevel", ...
        sixgr.util.structGet(cfg, "phy.pdcch.aggregationLevel", NaN))));
    cfgControl = sixgr.util.structSet(cfgControl, ...
        "phy.pdcch.searchSpace.id", double(sixgr.util.structGet( ...
        grant, "SearchSpaceID", ...
        sixgr.util.structGet(cfg, "phy.pdcch.searchSpace.id", NaN))));
    cfgControl = sixgr.util.structSet(cfgControl, ...
        "phy.pdcch.coreset.id", double(sixgr.util.structGet( ...
        grant, "CORESETID", ...
        sixgr.util.structGet(cfg, "phy.pdcch.coreset.id", NaN))));
    [tx, txInfo] = sixgr.phy.dl.PDCCH_Tx(cfgControl, ...
        "Grant", grant, "RNTI", rnti, "NumTxAnt", 1);
    controlState = localInitChannelState(cfgControl, tx, txInfo);
    [rxWave, nVar, controlState] = localApplyChannelAndAwgn( ...
        tx.Waveform, snr_dB, controlState);
    [rxWave, nVar, controlState] = ...
        localApplyReceiverIQImbalanceCompensation( ...
        rxWave, nVar, controlState, cfgControl); %#ok<ASGLU>
    [rx, ~] = sixgr.phy.dl.PDCCH_Rx(rxWave, cfgControl, ...
        "Carrier", tx.Carrier, ...
        "PDCCH", tx.PDCCH, ...
        "K", numel(tx.DCIBits), ...
        "ListLength", double(sixgr.util.structGet( ...
            cfgControl, "phy.pdcch.listLength", 16)), ...
        "RNTI", rnti, ...
        "NoiseVar", nVar, ...
        "ExpectedDCIBits", tx.DCIBits);
catch ME
    replay.PDCCHGrantBindingStatus = "waveform_execution_failed";
    replay.PDCCHGrantBindingFailureCode = ...
        "pdcch_waveform_execution_failed:" + string(ME.identifier);
    return;
end

replay.PDCCHWaveformExecuted = true;
replay.PDCCHLogicalTxPorts = size(tx.Waveform, 2);
replay.PDCCHObservedRxBranches = size(rxWave, 2);
replay.PDCCHControlEvidenceSource = ...
    "sixgr.phy.dl.PDCCH_Tx|configured_channel_and_impairments|sixgr.phy.dl.PDCCH_Rx";
replay.DCICrcPass = logical(sixgr.util.structGet(rx, "Ok", false)) && ...
    double(sixgr.util.structGet(rx, "ErrFlag", 1)) == 0;
replay.PDCCHPayloadMatch = logical(sixgr.util.structGet( ...
    rx, "DCIPayloadMatch", false));
replay.PDCCHCausalGrantDecodeOk = logical(sixgr.util.structGet( ...
    rx, "CausalGrantDecodeOk", false));
replay.PDCCHMissedDetection = logical(sixgr.util.structGet( ...
    rx, "MissedDetection", ~replay.DCICrcPass));
replay.PDCCHFalseAlarm = logical(sixgr.util.structGet( ...
...
replay.PDCCHGrantDCIFieldsHash = localPDCCHPayloadHash(decodedPayload);
replay.PDCCHGrantDCIId = string(sixgr.util.structGet( ...
    decoded, "PayloadHash", ""));
if strlength(strtrim(replay.PDCCHGrantDCIId)) == 0
    replay.PDCCHGrantDCIId = localPDCCHBitHash( ...
        sixgr.util.structGet(rx, "DCIBits", int8([])));
end

replay.PDCCHGrantSearchSpaceId = localObjectValue( ...
    tx.PDCCH.SearchSpace, "SearchSpaceID", ...
    localObjectValue(tx.PDCCH.SearchSpace, "ID", NaN));
replay.PDCCHGrantCORESETId = localObjectValue( ...
    tx.PDCCH.CORESET, "CORESETID", ...
    localObjectValue(tx.PDCCH.CORESET, "ID", NaN));
replay.PDCCHGrantAggregationLevel = localObjectValue( ...
    tx.PDCCH, "AggregationLevel", NaN);
replay.PDCCHGrantCandidateIndex = localObjectValue( ...
    tx.PDCCH, "AllocatedCandidate", NaN) - 1;

if ~replay.DCICrcPass
    failures(end+1, 1) = "decoded_dci_crc_failed"; %#ok<AGROW>
end
if ~replay.PDCCHPayloadMatch
    failures(end+1, 1) = "decoded_dci_payload_mismatch"; %#ok<AGROW>
end
if ~replay.PDCCHCausalGrantDecodeOk
    failures(end+1, 1) = "causal_grant_decode_failed"; %#ok<AGROW>
end
if replay.PDCCHFalseAlarm
    failures(end+1, 1) = "pdcch_false_alarm"; %#ok<AGROW>
end
if strlength(replay.PDCCHGrantFieldsHash) == 0 || ...
        strlength(replay.PDCCHGrantDCIFieldsHash) == 0 || ...
        replay.PDCCHGrantFieldsHash ~= replay.PDCCHGrantDCIFieldsHash
    failures(end+1, 1) = "dci_grant_fields_hash_mismatch"; %#ok<AGROW>
end
if strlength(replay.PDCCHGrantDCIId) == 0
    failures(end+1, 1) = "decoded_dci_identity_missing"; %#ok<AGROW>
end
failures = [failures; localPDCCHPayloadMismatches( ...
    grantPayload, decodedPayload)]; %#ok<AGROW>

expectedSearchSpace = localFirstFiniteNumber([ ...
    sixgr.util.structGet(grant, "SearchSpaceID", NaN), ...
    sixgr.util.structGet(grant, "PDCCHGrantSearchSpaceId", NaN)]);
expectedCORESET = localFirstFiniteNumber([ ...
    sixgr.util.structGet(grant, "CORESETID", NaN), ...
    sixgr.util.structGet(grant, "PDCCHGrantCORESETId", NaN)]);
expectedAggregation = localFirstFiniteNumber([ ...
    sixgr.util.structGet(grant, "PDCCHAggregationLevel", NaN), ...
    sixgr.util.structGet(grant, "PDCCHGrantAggregationLevel", NaN)]);
expectedCandidate = localFirstFiniteNumber([ ...
    sixgr.util.structGet(grant, "PDCCHCandidateIndex", NaN), ...
    sixgr.util.structGet(grant, "PDCCHGrantCandidateIndex", NaN)]);
if ~localPDCCHNumberMatch(expectedSearchSpace, ...
        replay.PDCCHGrantSearchSpaceId)
    failures(end+1, 1) = "search_space_mismatch"; %#ok<AGROW>
end
if ~localPDCCHNumberMatch(expectedCORESET, ...
        replay.PDCCHGrantCORESETId)
    failures(end+1, 1) = "coreset_mismatch"; %#ok<AGROW>
end
if ~localPDCCHNumberMatch(expectedAggregation, ...
        replay.PDCCHGrantAggregationLevel)
    failures(end+1, 1) = "aggregation_level_mismatch"; %#ok<AGROW>
end
if ~localPDCCHNumberMatch(expectedCandidate, ...
        replay.PDCCHGrantCandidateIndex)
    failures(end+1, 1) = "candidate_index_mismatch"; %#ok<AGROW>
end

failures = unique(failures(strlength(strtrim(failures)) > 0), ...
    "stable");
replay.PDCCHGrantBindingOk = isempty(failures);
if replay.PDCCHGrantBindingOk
    replay.PDCCHGrantBindingStatus = "bound_physical_waveform";
    replay.PDCCHGrantBindingFailureCode = "";
else
    replay.PDCCHGrantBindingStatus = "failed";
    replay.PDCCHGrantBindingFailureCode = strjoin(failures, "|");
end
end

function format = localPDCCHDCIFormat(source, direction)
format = strtrim(string(sixgr.util.structGet(source, "DCI.Format", ...
...
function payload = localPDCCHGrantPayload(grant, direction)
prbSet = double(sixgr.util.structGet(grant, "PRBSet", []));
prbSet = prbSet(isfinite(prbSet));
symbolAllocation = double(sixgr.util.structGet( ...
    grant, "SymbolAllocation", []));
payload = struct( ...
    "Direction", char(upper(string(direction))), ...
    "DCIFormat", char(localPDCCHDCIFormat(grant, direction)), ...
    "RNTI", double(sixgr.util.structGet(grant, "RNTI", NaN)), ...
    "HARQProcessId", localFirstFiniteNumber([ ...
    ...
    "PRBStart", localFirstFiniteNumber([prbSet, ...
        sixgr.util.structGet(grant, "PRBStart", NaN)]), ...
    "AllocatedPRBCount", double(numel(prbSet)), ...
    "SymbolStart", localVectorValue(symbolAllocation, 1), ...
    "NumSymbols", localVectorValue(symbolAllocation, 2), ...
    "TimeDomainAssignmentIndex", localFirstFiniteNumber([ ...
        sixgr.util.structGet(grant, "DCI.TimeDomainAssignmentIndex", NaN), ...
        sixgr.util.structGet(grant, "TimeDomainResourceAssignmentIndex", NaN), ...
        sixgr.util.structGet(grant, "TDRAIndex", NaN), ...
        sixgr.util.structGet(grant, "TimeResourceAssignment", NaN)]), ...
    "MCSIndex", localFirstFiniteNumber([ ...
        sixgr.util.structGet(grant, "MCSIndex", NaN), ...
        sixgr.util.structGet(grant, "MCS", NaN)]), ...
    "RV", localFirstFiniteNumber([ ...
        sixgr.util.structGet(grant, "HARQ.RV", NaN), ...
        sixgr.util.structGet(grant, "RV", NaN)]), ...
    "NDI", localFirstFiniteNumber([ ...
        sixgr.util.structGet(grant, "HARQ.NDI", NaN), ...
        sixgr.util.structGet(grant, "NDI", NaN)]));
end

function payload = localPDCCHDecodedPayload(decoded, rnti, direction)
payload = struct( ...
    "Direction", char(upper(string(direction))), ...
    "DCIFormat", char(string(sixgr.util.structGet( ...
        decoded, "Format", localPDCCHDCIFormat(struct(), direction)))), ...
    "RNTI", double(rnti), ...
    "HARQProcessId", double(sixgr.util.structGet( ...
        decoded, "Fields.harq_process", NaN)), ...
    "PRBStart", double(sixgr.util.structGet( ...
        decoded, "Fields.prb_start", NaN)), ...
    "AllocatedPRBCount", double(sixgr.util.structGet( ...
        decoded, "Fields.num_prb", NaN)), ...
    "SymbolStart", double(sixgr.util.structGet( ...
        decoded, "Fields.symbol_start", NaN)), ...
    "NumSymbols", double(sixgr.util.structGet( ...
        decoded, "Fields.num_symbols", NaN)), ...
    "TimeDomainAssignmentIndex", double(sixgr.util.structGet( ...
        decoded, "Fields.time_resource_assignment", NaN)), ...
    "MCSIndex", double(sixgr.util.structGet( ...
        decoded, "Fields.mcs", NaN)), ...
    "RV", double(sixgr.util.structGet(decoded, "Fields.rv", NaN)), ...
    "NDI", double(sixgr.util.structGet(decoded, "Fields.ndi", NaN)));
end

function failures = localPDCCHPayloadMismatches(expected, observed)
failures = strings(0, 1);
numericFields = ["RNTI","HARQProcessId","PRBStart", ...
    "AllocatedPRBCount","SymbolStart","NumSymbols", ...
    "TimeDomainAssignmentIndex","MCSIndex","RV","NDI"];
failureNames = ["rnti_mismatch","harq_process_mismatch", ...
    "prb_start_mismatch","prb_count_mismatch", ...
    "symbol_start_mismatch","symbol_count_mismatch", ...
    "time_domain_assignment_mismatch","mcs_mismatch", ...
    "rv_mismatch","ndi_mismatch"];
for i = 1:numel(numericFields)
    if ~localPDCCHNumberMatch(expected.(numericFields(i)), ...
            observed.(numericFields(i)))
        failures(end+1, 1) = failureNames(i); %#ok<AGROW>
    end
end
if string(expected.Direction) ~= string(observed.Direction)
    failures(end+1, 1) = "direction_mismatch"; %#ok<AGROW>
end
if string(expected.DCIFormat) ~= string(observed.DCIFormat)
    failures(end+1, 1) = "dci_format_mismatch"; %#ok<AGROW>
end
end

function hash = localPDCCHPayloadHash(payload)
hash = "";
try
    bytes = uint8(unicode2native(char(jsonencode( ...
        orderfields(payload))), "UTF-8"));
    hash = string(sixgr.util.sha256Hex(bytes));
catch
end
end

function hash = localPDCCHBitHash(bits)
hash = "";
bits = uint8(bits(:));
if isempty(bits)
    return;
...
try
    hash = string(sixgr.util.sha256Hex(bits));
catch
end
end

function tf = localPDCCHNumberMatch(lhs, rhs)
tf = isscalar(lhs) && isscalar(rhs) && isfinite(double(lhs)) && ...
    isfinite(double(rhs)) && abs(double(lhs) - double(rhs)) < 1e-9;
end
...
function value = localVectorValue(values, index)
values = double(values(:));
...
if isstruct(priorValue)
    if isfield(priorValue, "LLRSum") || isfield(priorValue, "SoftBuffer")
        priorLLR = priorValue;
        if upper(string(dir)) == "DL"
            priorLayout = sixgr.util.structGet( ...
                priorValue, "CodingPlans", []);
        else
            priorLayout = sixgr.util.structGet( ...
                priorValue, "CodingLayout", struct());
        end
    else
...
if upper(string(dir)) == "DL" ...
        && localHARQSoftBufferAvailable(priorLLR) ...
        && isempty(priorLayout)
    error("sixgr:pdsch:ReplayHARQMissingPriorCodingPlan", ...
        ("A cached DL HARQ soft buffer lacks the immutable prior " + ...
        "DLSCHCodingPlan identity required for position-aware combining."));
end
end

function replay = localUpdateHARQSoftBuffer( ...
        replay, dir, rx, key, priorLLR, isRetx)
...
if isstruct(softBuffer) && ~isempty(fieldnames(softBuffer))
    if upper(string(dir)) == "DL"
        softBuffer.CodingPlans = sixgr.util.structGet( ...
            rx, "CodingPlans", []);
    end
...
elseif isempty(softLLR)
    ...
    if upper(string(dir)) == "DL"
        softBuffer = struct( ...
            "LLR", softLLR, ...
            "CodingPlans", {sixgr.util.structGet( ...
                rx, "CodingPlans", [])});
    else
        softBuffer = struct( ...
            "LLR", softLLR, ...
            "CodingLayout", sixgr.util.structGet( ...
                rx, "CodingLayout", struct()));
    end
    localHARQSoftBufferCache("set", key, softBuffer);
    replay.HARQSoftBufferStored = true;
end
...
replay.NoiseVariance = double(sixgr.util.structGet(rx, ...
    "PreEqualizationNoiseVariance", sixgr.util.structGet(rx, "NoiseVar", NaN)));
replay.PreEqualizationNoiseVariance = replay.NoiseVariance;
replay.PostEqualizationNoiseVariance = double(sixgr.util.structGet(rx, ...
    "PostEqualizationNoiseVariance", sixgr.util.structGet(rx, "PostEqualizationNoiseVar", NaN)));
replay.LLRNoiseVariance = double(sixgr.util.structGet(rx, "LLRNoiseVariance", NaN));
replay.StrictReceiverEvidenceOk = logical(sixgr.util.structGet(rx, ...
    "StrictReceiverEvidenceOk", false));
replay.StrictOk = logical(sixgr.util.structGet(rx, "StrictOk", false));
replay.TruthStatus = string(sixgr.util.structGet(rx, "TruthStatus", ""));
replay.ApproximationMode = "none";
...
replay.EVM_rms = double(sixgr.util.structGet(decoderMeta, "EVM_rms", NaN));
replay.EVMProxySINR_dB = double(sixgr.util.structGet( ...
    decoderMeta, "DiagnosticEVMProxySINR_dB", NaN));
...
if dir == "UL"
    ...
    cfgOut = localConfigureDLReplayCalibrationOwnership( ...
        cfgOut, grant, numLayers);
end
...
function cfgOut = localConfigureDLReplayCalibrationOwnership( ...
        cfgOut, grant, numLayers)
...
if any(modulation == "1024QAM")
    % The adapter intentionally remains fail-closed here. A 1024QAM
    % replay must carry its own explicit UE/RRC/DCI, band, range and
    % deployment eligibility context in cfgOut or the grant.
    return;
end
...
cfgOut.phy.pdsch.mcsContext = struct( ...
    ...
    "FrequencyRange", "not_applicable_non1024_replay", ...
    "OperatingBand", "not_applicable_non1024_replay", ...
    "DeploymentClass", "waveform_replay_calibration");
end
