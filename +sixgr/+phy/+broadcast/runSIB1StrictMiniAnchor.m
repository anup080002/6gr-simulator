...
if runRandomAccess && logical(rx.StrictOk)
    [raRuntimeMode, raUseRuntimeChannel, raRequireStageWaveforms, ...
        raAllowStageComposition] = localResolveRARuntimeTransport(cfg);
    ra = sixgr.phy.ra.runFourStepRA(cfg, ...
        ...
        "RuntimeIntegrationMode", raRuntimeMode, ...
        "UseRuntimeChannel", raUseRuntimeChannel, ...
        "RequireRuntimeStageWaveforms", raRequireStageWaveforms, ...
        "AllowRuntimeStageWaveformComposition", raAllowStageComposition, ...
        "RuntimeNoiseSNR_dB", snrDB, ...
        "WriteArtifacts", true);
...
function [mode, useRuntimeChannel, requireStageWaveforms, allowComposition] = ...
        localResolveRARuntimeTransport(cfg)
% A strict SIB1 anchor may continue into the scenario's real four-step RA
% chain.  When YAML requires runtime stage waveforms, the anchor must
% propagate each generated Msg1--Msg4/RRCSetupComplete waveform through the
% persistent runtime channel composer; leaving runFourStepRA in its default
% standalone mode would correctly fail closed for a missing RX waveform.
requireStageWaveforms = localStrictBoolean(cfg, ...
    "random_access.require_runtime_stage_waveforms", false);
allowComposition = localStrictBoolean(cfg, ...
    "random_access.allow_runtime_stage_waveform_composition", false);
configuredRuntimeChannel = localStrictBoolean(cfg, ...
    "random_access.use_runtime_channel", false);
useRuntimeChannel = configuredRuntimeChannel || requireStageWaveforms;
if requireStageWaveforms && ~allowComposition
    error("sixgr:phy:broadcast:SIB1RARuntimeCompositionDisabled", ...
        ["SIB1-to-RA continuation requires runtime stage waveforms, but " + ...
         "random_access.allow_runtime_stage_waveform_composition is false " + ...
         "and no external stage waveform source exists in this anchor."]);
end
if useRuntimeChannel
    mode = "coupled_truth_runtime";
else
    mode = "standalone_self_loop";
end
end

function value = localStrictBoolean(cfg, path, defaultValue)
raw = sixgr.util.structGet(cfg, path, defaultValue);
if ~((islogical(raw) || isnumeric(raw)) && isscalar(raw) && ...
        isfinite(double(raw)) && any(double(raw) == [0 1]))
    error("sixgr:phy:broadcast:InvalidSIB1RATransportAuthority", ...
        "%s must be an explicit scalar boolean.", char(path));
end
value = logical(raw);
end
