...
p.addParameter('DetectionObserver', [], @(x) isempty(x) || isa(x,'function_handle'));
...
[timingSearchGuard,timingSearchGuardSource]= ...
    sixgr.phy.sync.resolveTimingSearchGuard(cfg,fs);
try
    [rxF, fOffHz, NID2, finfo] = sixgr.phy.sync.freqOffsetCorrect(rxWaveform, blockPattern, fs, ...
        ...
        'FineCFOMaxResidualHz', sixgr.util.structGet(cfg,'phy.sync.fineCFOMaxResidualHz',Inf), ...
         ...
         'TimingSearchGuardSamples', timingSearchGuard, ...
...
if ~isempty(p.Results.DetectionObserver)
    p.Results.DetectionObserver("PSS",pssDecision);
end
...
rxGrid = sixgr.phy.waveform.ofdmDemodulate( ...
    ...
    "CarrierFrequency", double(timing.CarrierFrequencyHz));
% SSB_Tx uses this configured carrier frequency for the TS 38.211 5.4
% per-symbol phase rotation. Reverse it at the actual acquired slot; an
% estimator must not absorb an omitted deterministic receiver operation.
...
if ~isempty(p.Results.DetectionObserver)
    p.Results.DetectionObserver("SSS",sssDecision);
end
...
sync.nRBSSB = nrbSSB;
sync.Nfft = double(ssbSampling.Nfft);
sync.CyclicPrefixLengthsPerSlot = ...
    double(ssbSampling.CyclicPrefixLengthsPerSlot(:).');
...
% Attach debug info
sync.FreqInfo = finfo;
sync.TimingSearchGuardSamples = timingSearchGuard;
sync.TimingSearchGuardSource = timingSearchGuardSource;
...
sync.SymbolPhaseCompensationCarrierFrequencyHz = double(timing.CarrierFrequencyHz);
sync.SymbolPhaseCompensationSource = 'configured_carrier_nr_ofdm_demodulation';
