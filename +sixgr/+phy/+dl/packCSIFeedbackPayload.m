...
if isempty(report)
    ...
    if ~isa(reportConfig,"sixgr.phy.mimo.CSIReportConfiguration")
        error("sixgr:mimo:MissingCSIReportConfig", ...
            "Strict CSI packing requires CSIReportConfiguration.");
    end
    values = struct( ...
        "CRI",double(sixgr.util.structGet(csi,"CRI",0)), ...
        ...
        "LI",double(sixgr.util.structGet(csi,"LI",0)));
    report = reportConfig.build(values);
else
    reportConfig = sixgr.util.structGet(csi,"CSIReportConfiguration",[]);
end
...
if isa(reportConfig,"sixgr.phy.mimo.CSIReportConfiguration")
    reportConfig.validateDecoded(report.Part1Bits,report.Part2Bits);
end
