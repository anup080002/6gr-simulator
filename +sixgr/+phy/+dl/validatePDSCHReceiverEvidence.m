...
% A receiver channel estimate is a required input to a real equalizer, so
% the computation-method provenance may legitimately mention DM-RS/Hest.
% What is forbidden is promoting the old raw receiver-Hest diagnostic (or
% any configured/proxy/oracle quantity) into the scheduling SINR field.
blockedSourceRole = ["configured", "reference_snr", "cqi", "mcs", ...
    "oracle", "perfect", "proxy", "fallback", "evm", "sweep", ...
    "receiver_hest"];
blockedMethod = ["configured", "reference_snr", "cqi", "mcs", ...
    "oracle", "perfect", "proxy", "fallback", "evm", "sweep"];
blockedHit = any(contains(source, blockedSourceRole)) || ...
    any(contains(role, blockedSourceRole)) || ...
    any(contains(method, blockedMethod));
