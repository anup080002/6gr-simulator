...
ip.parse(varargin{:});
opt = ip.Results;
if ~isempty(opt.ReceiverHARQState) && isempty(opt.Assignment)
...
priorReceivedLLR=[]; priorReceivedPlan=[]; receiverHARQKey="";
if ~isempty(opt.ReceiverHARQState)
    [priorReceivedLLR,priorReceivedPlan,receiverHARQKey]=opt.ReceiverHARQState.priorFor(assignment);
end
canonical = sixgr.pdsch.PDSCHReceiver( ...
    ...
    "HARQManager", opt.HARQManager, ...
    "PriorRecoveredLLR", priorReceivedLLR, ...
    "PriorCodingPlan", priorReceivedPlan, ...
    "HARQKey", receiverHARQKey, ...
    "UseMexLDPC",logical(sixgr.util.structGet( ...
        cfg,"phy.ldpc.useMexBatchDecode",false)), ...
    "EnablePTRSCPECorrection", logical(sixgr.util.structGet( ...
        cfg, "phy.pdsch.ptrs.enableCPECorrection", false)), ...
