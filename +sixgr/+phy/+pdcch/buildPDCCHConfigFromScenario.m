...
numCand = numCand(1:5);
configuredNumCand = double(sixgr.util.structGet(cfg, ...
    "phy.pdcch.searchSpace.configuredNumCandidates", numCand));
configuredNumCand = reshape(configuredNumCand, 1, []);
if numel(configuredNumCand) ~= 5
    error("sixgr:phy:pdcch:invalid_candidate_count", ...
        ["Configured PDCCH candidate intent must contain AL " ...
         "1/2/4/8/16 counts."]);
end
...
strictCfg.StatisticalQualification = localStatisticalQualification(operatorStrict);
strictCfg.Seed = double(sixgr.util.structGet(cfg, "run.seed", 5489));
...
function design = localStatisticalQualification(operatorStrict)
raw = localRequired(operatorStrict, "statistical_qualification");
design = struct( ...
    ...
    "MinimumWaveformTrials", double(localRequired(raw, "minimum_waveform_trials")), ...
    "MaximumWaveformTrials", double(localRequired(raw, "maximum_waveform_trials")), ...
    "BatchSizeWaveformTrials", double(localRequired(raw, "batch_size_waveform_trials")), ...
    "MinimumCandidateDecisions", double(localRequired(raw, "minimum_candidate_decisions")), ...
...
if ~design.Enabled
    error("sixgr:phy:pdcch:StatisticalQualificationDisabled", ...
        "Strict PDCCH validation requires statistical_qualification.enabled=true.");
end
integerValues = [design.MinimumWaveformTrials design.MaximumWaveformTrials ...
    design.BatchSizeWaveformTrials design.MinimumCandidateDecisions ...
    design.MinimumFalseAlarmEvents design.PlannedLooks design.DeterministicSeeds(:).'];
if any(~isfinite(integerValues) | integerValues ~= fix(integerValues)) || ...
        design.MinimumWaveformTrials < 1 || ...
        design.MaximumWaveformTrials < design.MinimumWaveformTrials || ...
        design.BatchSizeWaveformTrials < 1 || design.MinimumCandidateDecisions < 1 || ...
        design.MinimumFalseAlarmEvents < 0 || design.PlannedLooks < 1 || ...
        numel(design.DeterministicSeeds) < 2
    error("sixgr:phy:pdcch:InvalidStatisticalQualification", ...
        "PDCCH statistical qualification requires valid integer budgets and at least two deterministic seeds.");
end
if ~(isfinite(design.TargetProbability) && design.TargetProbability > 0 && design.TargetProbability < 1 && ...
    ...
    error("sixgr:phy:pdcch:InvalidStatisticalQualification", ...
        "PDCCH target probability, confidence level and CI width must be explicit valid probabilities/widths.");
end
requiredLooks = ceil(design.MaximumWaveformTrials/design.BatchSizeWaveformTrials);
if design.PlannedLooks < requiredLooks
    error("sixgr:phy:pdcch:InvalidStatisticalQualification", ...
        "PDCCH planned_looks must cover every configured batch look (%d required).", requiredLooks);
end
end
...
cfg = sixgr.util.structSet(cfg, ...
    ...
    double(strictCfg.ConfiguredNumCandidates));
cfg = sixgr.util.structSet(cfg, ...
    "phy.pdcch.searchSpace.candidateResolutionSource", ...
    char(string(strictCfg.CandidateResolutionSource)));
