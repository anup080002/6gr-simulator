classdef PDCCHTDocStudyCampaignRunner
    ...
    methods (Static)
        function result=run(configPath,varargin)
            ...
            p.parse(varargin{:});campaign=sixgr.phy.pdcch.tdoc.loadPDCCHTDocStudyCampaignConfig(string(configPath));
            ...
            analytical=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyAnalyticalSuite.run(campaign.Config);
            scheduler=sixgr.phy.pdcch.tdoc.PDCCHTDocStudySchedulerSuite.run(campaign.Config);
            waveform=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyWaveformSuite.run(campaign);
...
data=struct("run_id",string(c.Config.run_id),"scenario_id",c.ScenarioID,"config_hash",c.ConfigHash, ...
    ...
    "command","runPDCCHTDoc10521StudyCampaign('"+c.ConfigPath+"')","utc",sixgr.util.utcNowISO8601());
...
sixgr.phy.pdcch.tdoc.PDCCHTDocStudyEvidenceClass.assertPrimaryTruth(T);sixgr.util.csvWriteTable(fullfile(root,"tables",name+".csv"),T);
save(fullfile(root,"raw",name+".mat"),"T","-v7");
end
function T=localTraceability(c,a,s,w)
catalog=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyScenarioRegistry.catalog();n=height(catalog);TDocSection=strings(n,1);ObservationOrProposal=strings(n,1);
...
for i=1:n
 ...
 if startsWith(id,"ANA-")
  ImplementationModule(i)="sixgr.phy.pdcch.tdoc.PDCCHTDocStudyAnalyticalSuite";Status(i)="PASS";EvidenceClass(i)=localAnalyticalClass(id);ArtifactPaths(i)="tables/ANA"+extractAfter(id,"-")+"*.csv";
 elseif startsWith(id,"SCH-")
  ImplementationModule(i)="sixgr.phy.pdcch.tdoc.PDCCHTDocStudySchedulerSuite";Status(i)="PASS";EvidenceClass(i)=localSchedulerClass(id);ArtifactPaths(i)="tables/SCH"+extractAfter(id,"-")+"*.csv";
 elseif startsWith(id,"LLS-")
  row=w.ScenarioStatus(w.ScenarioStatus.ScenarioID==id,:);ImplementationModule(i)="sixgr.phy.pdcch.tdoc.PDCCHTDocStudyWaveformSuite";
...
figureRegistry=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyScenarioRegistry.figures();
...
rows=table();names=string(fieldnames(a.Figures));registry=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyScenarioRegistry.figures();
for i=1:numel(names)
 ...
 render=localRender(n);[x,y,g]=localAxes(n);row=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyFigurePublisher.publish(root,n,a.Figures.(n),evidence,c.ConfigHash, ...
...
names=string(fieldnames(s.Figures));for i=1:numel(names),n=names(i);scenario=localTernary(startsWith(n,"tdoc_fig"),"SCH-01",localSchedulerScenario(n));evidence=localSchedulerClass(scenario);[x,y,g]=localAxes(n);
 row=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyFigurePublisher.publish(root,n,s.Figures.(n),evidence,c.ConfigHash,"ScenarioID",scenario,"Title",replace(n,"_"," "), ...
...
names=string(fieldnames(w.Figures));for i=1:numel(names),n=names(i);source=w.Figures.(n);if isempty(source),continue;end;[x,y,g]=localAxes(n);
 row=sixgr.phy.pdcch.tdoc.PDCCHTDocStudyFigurePublisher.publish(root,n,source,string(c.Config.waveform.evidence_class),c.ConfigHash,"ScenarioID","LLS-00", ...
...
pass(7)=true;try,sixgr.phy.pdcch.tdoc.PDCCHTDocStudyEvidenceClass.assertPrimaryTruth(tables.LLS00Trials);catch,pass(7)=false;end;detail(7)="primary tables scanned";
