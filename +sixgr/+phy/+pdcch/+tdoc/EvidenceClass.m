    ...
    methods (Static)
        ...
        function value = require(value)
            ...
            if ~isscalar(value) || ~ismember(value, ...
                    sixgr.phy.pdcch.tdoc.EvidenceClass.allowed())
                error("sixgr:phy:pdcch:tdoc:InvalidEvidenceClass", ...
                    "Evidence class '%s' is invalid. Allowed: %s.", ...
                    value, strjoin(sixgr.phy.pdcch.tdoc.EvidenceClass.allowed(), ", "));
            end
        end

        function tf = tdocEligible(value, status, statisticsQualified)
            value = sixgr.phy.pdcch.tdoc.EvidenceClass.require(value);
        ...
        function assertPrimaryTruth(T)
            ...
            for field = ["Source","ExecutionBackend","ApproximationMode","EvidenceClass"]
