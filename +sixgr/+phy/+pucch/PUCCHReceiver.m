classdef PUCCHReceiver
    ...
    methods (Static)
        function rx = receive(waveform,carrier,assignment,reportContext,varargin)
            ...
            receiveOnly=isa(assignment,"sixgr.phy.pucch.PUCCHReceptionAssignment");
            if ~isscalar(assignment) || ~(receiveOnly || isa(assignment,"sixgr.phy.pucch.PUCCHTransmissionAssignment"))
                error("sixgr:phy:pucch:WrongResource", ...
                    "PUCCH RX requires a typed assignment.");
            end
            if receiveOnly
                assert(assignment.ReportContextDigest==reportContext.Digest, ...
                    'sixgr:phy:pucch:StaleConfiguration','Receiver length/layout hypothesis has changed.');
                slot0=double(assignment.Data.AbsoluteSlot0);
                assert(double(carrier.NSlot)==mod(slot0,double(carrier.SlotsPerFrame)) && ...
                    double(carrier.NFrame)==floor(slot0/double(carrier.SlotsPerFrame)), ...
                    'sixgr:phy:pucch:WrongReceiveSlot','Carrier must identify the scheduled receive slot.');
                assignmentSource=string(assignment.Data.Source);
            else
                assignmentSource="legacy_transmission_assignment";
            end
            ...
            timing=struct('TimingOffsetSamples',NaN,'AppliedTimingCorrectionSamples',0, ...
                'TimingSource',"caller_aligned_observation_not_measured", ...
                'OracleTimingUsed',false,'ReceiverZeroPaddingUsed',false);
            if ~isempty(opt.TimingSearchWindowSamples)
                window=opt.TimingSearchWindowSamples;
                validateattributes(window,{'numeric'},{'real','finite','nonnegative','integer','numel',2});
                if isempty(dmrs.Indices)
                    assert(all(window==0),'sixgr:phy:pucch:PUCCHTimingReferenceRequired', ...
                        'Format 0 needs a received UL timing reference before uncertain-window FFT processing; no transmitted-payload timing oracle.');
                else
                    [waveform,timing]=sixgr.phy.sync.alignULReferenceObservation( ...
                        carrier,waveform,dmrs.Indices,dmrs.Symbols,window);
                end
            end
            ofdmTic = tic;
            ...
            if assignment.Format <= 1
                ...
                crcPassed = true;
                crcApplicable=false; crcErrors=false(0,1);
                crcBitCount=0;
            else
                decodedResult = sixgr.phy.pucch.UCIDecoder.decode(soft{1},totalA);
                decoded = decodedResult.Bits;
                crcPassed = decodedResult.CRCPassed;
                crcApplicable=decodedResult.CRCApplicable;
                crcErrors=decodedResult.CodeBlockCRCError;
                crcBitCount=decodedResult.Plan.CRCBits;
            end
            decodeLatency_ms = 1e3 .* toc(decodeTic);
            energyRatio = mean(abs(eq(:)).^2)/max(decodeNoiseVariance,eps);
            [detectionMetric,detectionMetricSource] = ...
                sixgr.phy.pucch.PUCCHDetector.selectMetric( ...
                assignment.Format,noncoherent,metric,energyRatio,totalA);
            ...
            rx = struct( ...
                "ReceiveTiming",timing, ...
                "ReceiverUsable",~decision.DTX, ...
                "DetectionAttempted",true,"DTX",decision.DTX, ...
                "DetectionOutcome",decision.DetectionOutcome, ...
                "DetectionMetric",decision.DetectionMetric, ...
                "DetectionMetricValid",decision.DetectionMetricValid, ...
                "DetectionMetricSource",detectionMetricSource, ...
                "ResourceExtractionAvailable",~isempty(eq) && all(isfinite(eq(:))), ...
                ...
                "CRCPassed",crcPassed,"CRCApplicable",crcApplicable, ...
                "CodeBlockCRCError",crcErrors,"WrongRNTI",false, ...
                "ExpectedPayloadBitCount",totalA,"UCICRCBitCount",crcBitCount, ...
                ...
                "ReceiverOnlyAssignment",receiveOnly, ...
                "ReceiverAssignmentSource",assignmentSource, ...
...
% Keep the UCISequence column-vector contract even for zero-length fields.
one = reshape(bits(1:n1),[],1);
two = reshape(bits(n1+(1:n2)),[],1);
end
