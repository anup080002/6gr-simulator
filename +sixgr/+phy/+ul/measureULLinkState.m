...
[sinr_dB, sinrSource, sinrStatus, pilotNMSE_dB, perRBSINR_dB, reference] = localMeasureReferenceSINR(Hest, nVar, ...
    opt.ReceivedGrid, opt.ReferenceIndices, opt.ReferenceSymbols, cfg);
metrics.ReferenceSignalPower = double(sixgr.util.structGet(reference,"SignalPower",NaN));
metrics.ReferenceEffectiveDisturbancePower = double(sixgr.util.structGet(reference,"EffectiveDisturbancePower",NaN));
...
function [sinr_dB, source, status, pilotNMSE_dB, perRBSINR_dB, reference] = localMeasureReferenceSINR(Hest, nVar, rxGrid, refInd, refSym, cfg)
sinr_dB = NaN;
