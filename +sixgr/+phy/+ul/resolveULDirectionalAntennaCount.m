...
if role == "TX"
    ...
    cfgCandidates = [ ...
        ...
        sixgr.util.structGet(cfg, "random_access.num_tx_antennas", NaN), ...
    ...
    cfgCandidates = [ ...
        ...
        sixgr.util.structGet(cfg, "random_access.num_rx_antennas", NaN), ...
...
count = NaN;
waveformDomain = lower(strtrim(string(sixgr.util.structGet(meta, ...
    "WaveformDomain", sixgr.util.structGet(antenna, "WaveformDomain", "")))));
hybridElementDomain = waveformDomain == "element" || ...
    logical(sixgr.util.structGet(meta, "HybridBeamformingEnabled", false)) || ...
    logical(sixgr.util.structGet(antenna, "HybridBeamformingEnabled", false));
if hybridElementDomain
    physicalCandidates = [ ...
        sixgr.util.structGet(meta, "NumWaveformColumns", NaN), ...
        sixgr.util.structGet(antenna, "NumWaveformColumns", NaN), ...
        sixgr.util.structGet(meta, "NumElements", NaN), ...
        sixgr.util.structGet(antenna, "NumElements", NaN), ...
        sixgr.util.structGet(antenna, "Nant", NaN), ...
        localArraySizeCount(sixgr.util.structGet(antenna, "Size", []))];
    count = localFirstValidCount(physicalCandidates);
    if isfinite(count)
        return;
    end
end
