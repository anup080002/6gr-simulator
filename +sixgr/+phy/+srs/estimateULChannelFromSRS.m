...
[pilotResidualNMSEdB, pilotResidualSINRDb, pilotResidualNoiseVar, pilotResidualPower, pilotSignalPower] = ...
...
truthRequired = logical(sixgr.util.structGet(srsCfg, "RequireTrueChannelOracleForValidation", true));
truthMetricOk = truthAvailable && isfinite(nmseDb) && nmseDb <= double(srsCfg.ChannelNMSEThresholddB);
available = logical(det.DetectionSuccess) && perReAvailable && perPrbAvailable && perPortAvailable && ...
    ((~truthRequired && ~truthAvailable) || truthMetricOk);
...
producerSlot = localFirstFinite(meta.Slot);
row = struct("RunId", string(srsCfg.RunId), "TrialId", NaN, "UEId", double(srsCfg.UEId), ...
    ...
    "ProducerSlot", double(producerSlot), "AvailableSlot", double(producerSlot), ...
    "MeasurementTargetType", "UE", ...
    "CausalConsumerContract", "SRS measurement may feed UL RI/TPMI/CSI only when AvailableSlot<=grant slot and age is within SRS freshness", ...
    ...
    "PilotResidualSINR_dB", double(pilotResidualSINRDb), ...
    "PilotResidualReferenceSource", "ls_minus_practical_estimator", ...
    "TrueChannelNMSE_dB", double(nmseDb), ...
    "TrueChannelSINR_dB", double(sinrDb), ...
    "NumChannelSamples", double(sum(valid)), ...
...
signalPower = mean(abs(hLS(mask)).^2, "omitnan");
