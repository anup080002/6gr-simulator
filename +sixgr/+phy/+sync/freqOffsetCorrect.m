...
function [segments, windows, retainedCandidateSymbols] = ...
        localSSBCandidateSearchSegments( ...
        x, ssbTiming, fs, guardSamples, candidateSSBIndex)
candidateSymbols = double( ...
    ssbTiming.CandidateStartSymbolsWithinHalfFrame(:));
candidateIndices = double(ssbTiming.CandidateIndices(:));
if numel(candidateIndices) ~= numel(candidateSymbols)
    error("sixgr:phy:sync:InvalidSSBCandidateMap", ...
        "Canonical SSB candidate indices and start symbols must have equal length.");
end
if ~isempty(candidateSSBIndex)
    requestedIndex = round(double(candidateSSBIndex));
    selectedMask = candidateIndices == requestedIndex;
    if nnz(selectedMask) ~= 1
        error("sixgr:phy:sync:UnknownSSBCandidateIndex", ...
            "SSB candidate index %g does not resolve to exactly one canonical candidate window.", ...
            requestedIndex);
    end
    candidateSymbols = candidateSymbols(selectedMask);
end
