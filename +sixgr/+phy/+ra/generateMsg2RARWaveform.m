...
cfgTx.phy.pdcch.KBits = double(raCfg.Msg2DCIPayloadBits);
cfgTx.phy.pdcch.dciPayloadBits = double(raCfg.Msg2DCIPayloadBits);
...
[commonPDCCH,commonEvidence] = sixgr.phy.ra.buildRARCommonPDCCH(cfgTx,raCfg,carrier);

[pdcchTx, pdcchInfo] = sixgr.phy.dl.PDCCH_Tx(cfgTx, "Carrier", carrier, ...
    "PDCCH",commonPDCCH, ...
    "DCIBits", sched.DCIBits, "K", double(raCfg.Msg2DCIPayloadBits), ...
    "RNTI", double(raCfg.RARNTI), "PDCCHScramblingRNTI",0, ...
    "NCellID", double(raCfg.NCellID), ...
    "OFDMModulate", true);
...
tx.RARDCIFieldTable = sched.DCIFieldTable;
tx.RARDCIContextDigest = sched.DCIContext.Digest;
tx.RARDCIReferenceSource = sched.DCIContext.Data.FrequencyReferenceSource;
