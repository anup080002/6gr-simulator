...
p.addParameter("PreamblePowerRampingCounter",[],@(x)isempty(x) || ...
    (isnumeric(x) && isreal(x) && isscalar(x) && isfinite(x) && x>=1 && x==fix(x)));
...
if isempty(opt.PreamblePowerRampingCounter)
    if raCfg.AttemptId~=1
        error('sixgr:phy:ra:MissingPowerRampingCounter','Retries require the UE power-ramping counter; an attempt identifier is not power authority.');
    end
    raCfg.PreamblePowerRampingCounter=1; % TS 38.321 MAC initialization.
else
    raCfg.PreamblePowerRampingCounter=double(opt.PreamblePowerRampingCounter);
end
...
rarStart=sixgr.phy.frame.AbsoluteTime.fromAbsoluteSlotSymbol( ...
    raCfg.Msg2Slot,raCfg.Msg2PDSCH.SymbolStart,numerology);
rarEnd=rarStart.plusSymbols(raCfg.Msg2PDSCH.NumSymbols,numerology);
...
powerState = localResolveRATransmitPower(cfg, raCfg, runtime.Mode);
...
try
    if receiveRAR || expireRAR
        ...
        if expireRAR
            ...
            result.ExpiredRARReceiver=rarReceiver;
        ...
        if ~isempty(fieldnames(rarPreparedTransmission)) && ...
            ...
            % The stage provenance validator consumes raw post-RF samples;
            % it derives the gain-compensated decoder plane itself. Keep
            % the already decoded observation distinct from that raw plane.
            runtime.StageWaveforms.Msg2RxWaveform=rawPost;
    ...
    msg1Tx.WaveformHashPlane = "prach_after_preamble_power_control_before_spatial_mapping_and_rf";
    ...
    result = localAppendRuntimeStage(result, stageInfo, runtime);
    if string(runtime.Mode)=="shared_physical_waveform_stream"
        retained=runtime.StageWaveforms.Msg1PhysicalExecution.Prepared;
        msg1Tx.TransmitPortResourceGrid=retained.TransmitPortResourceGrid;
        msg1Tx.TransmitProjectionMatrix=retained.TransmitProjectionMatrix;
        msg1Tx.TransmitProjectionMatrixSHA256=retained.TransmitProjectionMatrixSHA256;
        origin=retained.StartTime_s*retained.SampleRate_Hz;
        assert(abs(origin-round(origin))<1e-7,'sixgr:truth:PRACHNativeOriginMismatch', ...
            'Shared PRACH origin must lie on its actual sample clock.');
        msg1Tx.WaveformStartSample=round(origin);
    end
    ...
    det = localAttachMsg1ReferenceEvidence(det, stageInfo, cfgMsg1Rx);
    ...
    timerRows = [timerRows; localTimer(raCfg, "ra-ResponseWindow", "stop", raCfg.RARMonitoringWindow.StartSlot, double(raCfg.Msg2Slot), ...
        raCfg.RARMonitoringWindow.LastSlot, false, double(raCfg.RAResponseWindowSlots), "OK")]; %#ok<AGROW>
...
fields = { ...
    ...
    "PreambleDetectionThresholdCalibrationStatus", "", "Msg1ConfiguredTargetFalseAlarmProbability", NaN, ...
    ...
    "RAResponseWindowStartSlot", double(raCfg.RARMonitoringWindow.StartSlot), ...
    ...
    "Msg3PayloadHex", "", "Msg3TransmittedPayloadHex", "", ...
    ...
    "Msg4PayloadHex", "", "Msg4TransmittedPayloadHex", "", ...
...
row = sixgr.phy.ra.bindStagePowerEvidence(row,powerContext);
...
row.TxSSBAssociationJSON = localSSBAssociationJSON(txRuntimeMeta);
...
if ~row.PhysicalDevicePowerClaim
    row.DesiredReferencePowerBeforeNoise_re_UnitOccupiedRE_Es = ...
        row.DesiredReferencePowerBeforeNoise_mW;
    row.NoiseVariancePreFrontEnd_re_UnitOccupiedRE_Es = ...
        row.NoiseVariancePreFrontEnd_mW;
    row.DesiredReferencePowerBeforeNoise_mW = NaN;
    row.NoiseVariancePreFrontEnd_mW = NaN;
    row.RequestedAWGNReferenceSNR_dB = double(runtime.RuntimeNoiseSNR_dB);
end
...
function det = localAttachMsg1ReferenceEvidence(det, stageInfo, cfg)
...
elseif localIsNormalizedFixedSNRSharedPRACH(stageInfo, cfg)
    % In configured-Es/N0 LLS the shared waveform is deliberately detached
    % from geometry and absolute pathloss.  ChannelFactory removes the
    % implementation/filter delay before PRACH scoring and CDL profiles use
    % a zero-delay first path, so the resolved PRACH occasion is the exact
    % propagation-timing reference.  This is not valid for a geometry or
    % externally timed capture; those modes remain unavailable below.
    trueOffset = 0;
    referenceSource = "normalized_fixed_esn0_zero_geometry_prach_occasion_origin";
else
    trueOffset = NaN;
...
function tf = localIsNormalizedFixedSNRSharedPRACH(stageInfo, cfg)
tf = startsWith(string(sixgr.util.structGet(stageInfo, ...
    "WaveformSource", "")), "shared_physical_stream") && ...
    strcmpi(string(sixgr.util.structGet(cfg, ...
    "integration.run_mode", "")), "FIXED_SNR_SWEEP") && ...
    logical(sixgr.util.structGet(cfg, ...
    "integration.configured_snr_is_link_authority", false)) && ...
    ~logical(sixgr.util.structGet(cfg, "channel.pathlossEnabled", true)) && ...
    ~logical(sixgr.util.structGet(cfg, ...
...
[projection, mappingSource, beamId, association] = localStagePortProjection( ...
...
meta.CommonDownlinkSSBAssociation = association;
end

function value = localSSBAssociationJSON(meta)
value = "";
association = sixgr.util.structGet(meta,"CommonDownlinkSSBAssociation",struct());
if isstruct(association) && ~isempty(fieldnames(association))
    matrix = meta.PortToElementMatrix;
    if string(sixgr.phy.mimo.MatrixContract.digest(matrix)) ~= association.ProjectionMatrixSHA256
        error('sixgr:phy:ra:CommonDownlinkProjectionMismatch', ...
            'Physical RA projection no longer matches its associated SSB matrix.');
    end
    value = string(jsonencode(association));
end
end
...
function [projection, mappingSource, beamId, association] = localStagePortProjection( ...
        cfg, ant, meta, direction, endpoint, portCount, stageName)
projection = [];
...
switch mappingSource
    case "selected_ssb_beam"
        ...
        [projection,beamId,association] = sixgr.phy.ra.resolveAssociatedSSBProjection(cfg,numElements);
...
row = struct( ...
    ...
    "TxPortToElementMappingSource", "", "TxBeamId", "", "TxSSBAssociationJSON", "", ...
    "AppliedTxPower_dBm", NaN, "MeasuredTxPowerBeforeRF_dBm", NaN, ...
    "PhysicalDevicePowerClaim", true, ...
    "AppliedTxPower_dB_re_UnitOccupiedRE_Es", NaN, ...
    "MeasuredTxPowerBeforeRF_dB_re_UnitOccupiedRE_Es", NaN, ...
    "ReferenceOutputPower_dB_re_UnitOccupiedRE_Es", NaN, ...
    "ExpectedWaveformPower_re_UnitOccupiedRE_Es", NaN, ...
    "AppliedTxPowerValueRole", "", "PowerNormalizationPolicy", "", ...
    "FullBWPActivityFactor", NaN, "ReferenceOutputPower_dBm", NaN, ...
    "ExpectedEmittedPower_mW", NaN, "ReferencePowerDomain", "", ...
    "ReferenceSampleCount", NaN, "ActualEmittedPowerBackoffFromBudget_dB", NaN, ...
    ...
    "DesiredReferencePowerBeforeNoise_re_UnitOccupiedRE_Es", NaN, ...
    "NoiseVariancePreFrontEnd_mW", NaN, "ThermalSampleNoiseBandwidth_Hz", NaN, ...
    "NoiseVariancePreFrontEnd_re_UnitOccupiedRE_Es", NaN, ...
    "RequestedAWGNReferenceSNR_dB", NaN, ...
    "ReferenceAWGNGridNoiseVariance", NaN, ...
    "ReferenceAWGNSampleNoiseVariance", NaN, ...
    "SampleToGridNoiseVarianceGain", NaN, ...
    "SharedNoiseCalibrationSource", "", ...
    "ThermalNoisePSD_mWPerHz", NaN, ...
...
fixedNormalizedEsN0 = strcmpi(string(sixgr.util.structGet( ...
    cfgStage, "integration.run_mode", "")), "FIXED_SNR_SWEEP") && ...
    logical(sixgr.util.structGet(cfgStage, ...
    "integration.configured_snr_is_link_authority", false));
if fixedNormalizedEsN0
    power=0;
    source="normalized_fixed_esn0_unit_occupied_re_es";
elseif ~isfinite(power) || startsWith(source,"fallback")
...
if name=="Msg1"
    native=tx.WaveformPortResourceGrid;
    assert(size(native,3)==size(samples,2),'sixgr:truth:PRACHNativePortMismatch', ...
        'The PRACH native port grid must match the waveform input to the applied spatial mapping.');
    prepared.TransmitPortResourceGrid=reshape( ...
        reshape(native,[],size(native,3))*cast(matrix,'like',native).', ...
        size(native,1),size(native,2),size(matrix,1));
end
prepared.TxSSBAssociationJSON=localSSBAssociationJSON(meta);
...
row.TxSSBAssociationJSON=p.TxSSBAssociationJSON;
row=sixgr.phy.ra.bindStagePowerEvidence(row,ctx);
row.NoiseVariancePreFrontEnd_mW=replay.InjectedNoiseVariance;
row.NoiseVariancePreFrontEnd_re_UnitOccupiedRE_Es=NaN;
row.NoiseApplied=isfinite(replay.InjectedNoiseVariance) && replay.InjectedNoiseVariance>0;
if ~row.PhysicalDevicePowerClaim
    row.NoiseVariancePreFrontEnd_re_UnitOccupiedRE_Es=double(replay.InjectedNoiseVariance);
    row.NoiseVariancePreFrontEnd_mW=NaN;
    row.DesiredReferencePowerBeforeNoise_re_UnitOccupiedRE_Es = ...
        double(sixgr.util.structGet(replay, ...
        "DesiredReferencePowerBeforeNoise_mW", ...
        sixgr.util.structGet(replay,"DesiredSignalPowerBeforeNoise",NaN)));
    row.DesiredReferencePowerBeforeNoise_mW=NaN;
end
...
for name=["RequestedAWGNReferenceSNR_dB", ...
        "ReferenceAWGNGridNoiseVariance", ...
        "ReferenceAWGNSampleNoiseVariance", ...
...
mapping={ 'TxRFExecutionStatus',txRF,'RFExecutionStatus'; ...
    ...
    'RequestedAWGNReferenceSNR_dB',rxRF,'RequestedAWGNReferenceSNR_dB'; ...
    'ReferenceAWGNGridNoiseVariance',rxRF,'ReferenceAWGNGridNoiseVariance'; ...
    'ReferenceAWGNSampleNoiseVariance',rxRF,'ReferenceAWGNSampleNoiseVariance'; ...
    'SampleToGridNoiseVarianceGain',rxRF,'SampleToGridNoiseVarianceGain'; ...
    'SharedNoiseCalibrationSource',rxRF,'SharedNoiseCalibrationSource'; ...
...
function pc = localResolveRATransmitPower(cfg, raCfg, runtimeMode)
...
[pathloss_dB, basePathloss_dB, pathlossSource] = localResolveRATxPathloss(cfg, raCfg, runtimeMode);
powerRampingCounter = double(raCfg.PreamblePowerRampingCounter);
...
if isfinite(pathloss_dB) && pathloss_dB >= 0
    msg1Power = sixgr.phy.ia.RAPowerController.resolve( ...
        ...
        "PreamblePowerRampingCounter", powerRampingCounter, ...
...
pc = struct( ...
    ...
    "PowerPathlossMeasurementId",string(sixgr.util.structGet(cfg,'lls6g.userContext.RuntimeServingPathlossMeasurementId','')), ...
    "PowerPathlossMeasurementSlot",double(sixgr.util.structGet(cfg,'lls6g.userContext.RuntimeServingPathlossMeasurementSlot',NaN)), ...
    "PowerPathlossMeasurementAgeSlots",double(sixgr.util.structGet(cfg,'lls6g.userContext.RuntimeServingPathlossMeasurementAgeSlots',NaN)), ...
...
fixedNormalizedEsN0 = strcmpi(string(sixgr.util.structGet( ...
    ...
    "integration.configured_snr_is_link_authority", false));
if fixedNormalizedEsN0
    % TS 38.213 power-control parameters remain decoded/configured
    % metadata, but a normalized configured-Es/N0 LLS has no absolute
    % pathloss or device-power reference. Applying dBm scaling here would
    % mix two incompatible operating-point authorities and make Msg2/Msg4
    % tens of dB stronger than the common waveform stream.
    pc.PowerPathloss_dB = NaN;
    pc.PowerBasePathloss_dB = NaN;
    pc.PowerPathlossSource = ...
        "not_applicable_normalized_fixed_esn0";
    pc.PowerPathlossMeasurementId = "";
    pc.PowerPathlossMeasurementSlot = NaN;
    pc.PowerPathlossMeasurementAgeSlots = NaN;
    pc.ReferenceTxPower_dBm = NaN;
    pc.PreambleTargetReceivedPower_dBm = NaN;
    pc.PreambleRequestedTxPower_dBm = NaN;
    pc.PreambleTxPower_dBm = NaN;
    pc.PreamblePowerHeadroom_dB = NaN;
    pc.Msg3Pathloss_dB = NaN;
    pc.Msg3RequestedTxPower_dBm = NaN;
    pc.Msg3TxPower_dBm = NaN;
    pc.Msg3PowerHeadroom_dB = NaN;
    pc.DownlinkTxPower_dBm = NaN;
    pc.DownlinkTxPowerSource = ...
        "not_applicable_normalized_fixed_esn0";
    pc.Msg2TxPower_dBm = NaN;
    pc.Msg4TxPower_dBm = NaN;
    pc.PowerControlStatus = ...
        "not_applied_normalized_fixed_esn0_unit_occupied_re_es";
end
end
...
function [pathloss_dB, basePathloss_dB, source] = localResolveRATxPathloss(cfg, raCfg, runtimeMode)
userMeta = sixgr.util.structGet(cfg, "lls6g.userContext", struct());
fixedNormalizedEsN0 = strcmpi(string(sixgr.util.structGet( ...
    cfg, "integration.run_mode", "")), "FIXED_SNR_SWEEP") && ...
    logical(sixgr.util.structGet(cfg, ...
    "integration.configured_snr_is_link_authority", false));
if string(runtimeMode)=="shared_physical_waveform_stream" && fixedNormalizedEsN0
    pathloss_dB=NaN;
    basePathloss_dB=NaN;
    source="not_applicable_normalized_fixed_esn0";
    return;
end
if string(runtimeMode)=="shared_physical_waveform_stream"
    id=string(sixgr.util.structGet(userMeta,'RuntimeServingPathlossMeasurementId',''));
    source=string(sixgr.util.structGet(userMeta,'RuntimeServingPathlossSource',''));
    signal=string(sixgr.util.structGet(userMeta,'RuntimeServingPathlossReferenceSignalType',''));
    reference=sixgr.util.structGet(userMeta,'RuntimeServingPathlossReferenceSignalId',NaN);
    pathloss_dB=sixgr.util.structGet(userMeta,'RuntimeServingPathloss_dB',NaN);
    if strlength(id)==0 || strlength(source)==0 || signal~="SSB" || ...
            reference~=cfg.random_access.associated_ssb_index || ~isfinite(pathloss_dB) || pathloss_dB<0
        error('sixgr:phy:ra:MissingSharedPreamblePathlossMeasurement', ...
            'Shared PRACH requires the selected SSB measurement identity and physical pathloss; model/base pathloss is not a substitute.');
    end
    basePathloss_dB=sixgr.util.structGet(userMeta,'RuntimeServingBasePathloss_dB',NaN);
    return;
end
...
startTicks=NaN; expiryTicks=NaN; clockSource="slot_fields_only";
if string(name)=="ra-ResponseWindow"
    startTicks=double(raCfg.RARMonitoringWindow.StartTicks);
    expiryTicks=double(raCfg.RARMonitoringWindow.ExpiryTicksExclusive);
    clockSource="38.213_8.2_actual_prach_end_and_type1_css_Tc";
end
row = struct("RunId", string(raCfg.RunId), "CellId", double(raCfg.CellId), ...
    ...
    "StartTicks",startTicks,"ExpiryTicksExclusive",expiryTicks,"ClockSource",clockSource);
end
...
result.PreambleDetectionThresholdMode=string(det.ThresholdMode);
result.PreambleDetectionThresholdSource=string(det.ThresholdSource);
result.PreambleDetectionThresholdCalibrationStatus=string(det.ThresholdCalibrationStatus);
result.Msg1ConfiguredTargetFalseAlarmProbability=double(det.ConfiguredTargetFalseAlarmProbability);
...
result.Msg3TransmittedPayloadHex = string(msg3TxPayload.PayloadHex);
result.Msg3PayloadHex = "";
if result.Msg3PUSCHCrcPass
    result.Msg3PayloadHex = string(sixgr.util.structGet(msg3Decoded,"PayloadHex",""));
end
...
result.Msg4DCIFields = sixgr.phy.ra.commonDCIFieldEvidence( ...
    result,raCfg.Msg4Slot,raCfg.TempCRNTI,msg4Tx,pdschRx,"Msg4");
result.Msg4DCIContextDigest = string(sixgr.util.structGet(pdschRx,'Msg4DCIContextDigest',""));
result.Msg4MACEncodingProfile = string(sixgr.util.structGet(pdschRx,'Msg4MACEncodingProfile',""));
result.Msg4MACRRCStandardsQualified = false; % Bounded MAC/RRC codec is still unmigrated.
...
result.Msg4TransmittedPayloadHex = string(msg4TxPayload.PayloadHex);
result.Msg4PayloadHex = "";
if result.Msg4PDSCHCrcPass
    result.Msg4PayloadHex = string(sixgr.util.structGet(msg4Decoded,"PayloadHex",""));
end
...
tables.msg4_dci_fields = sixgr.util.structGet(result,'Msg4DCIFields', ...
    sixgr.phy.ra.commonDCIFieldEvidence(result,raCfg.Msg4Slot, ...
    raCfg.TempCRNTI,struct(),struct(),"Msg4"));
...
names = ["RunId","ScenarioName","CellId","UEId","AttemptId","RAProcedureType","RABindingSource", ...
    ...
    "PreambleDetectionThresholdMode","PreambleDetectionThresholdSource", ...
    "PreambleDetectionThresholdCalibrationStatus","Msg1ConfiguredTargetFalseAlarmProbability", ...
    ...
    "PreambleReceivedTargetPower_dBm","PowerRampingStep_dB","PreambleTransMax","PreambleAttemptNumber","PreamblePowerRampingCounter", ...
    "PowerPathloss_dB","PowerBasePathloss_dB","PowerPathlossSource","ReferenceTxPower_dBm", ...
    "PowerPathlossMeasurementId","PowerPathlossMeasurementSlot","PowerPathlossMeasurementAgeSlots", ...
...
row = struct("RunId", string(r.RunId), "CellId", double(r.CellId), "UEId", double(r.UEId), ...
    ...
    "PreamblePowerRampingCounter", double(r.PreamblePowerRampingCounter), ...
    "PowerBasePathloss_dB", double(r.PowerBasePathloss_dB), ...
    "PowerPathlossSource", string(r.PowerPathlossSource), ...
    "PowerPathlossMeasurementId",string(r.PowerPathlossMeasurementId), ...
    "PowerPathlossMeasurementSlot",double(r.PowerPathlossMeasurementSlot), ...
    "PowerPathlossMeasurementAgeSlots",double(r.PowerPathlossMeasurementAgeSlots), ...
...
row = struct("RunId", string(r.RunId), "CellId", double(r.CellId), "UEId", double(r.UEId), ...
    ...
    "RARBytesHex", string(r.RARBytesHex), ...
    "BackoffIndicator",double(sixgr.util.structGet(rarRx,'BackoffIndicator',NaN)), ...
    "BackoffIndicatorPresent",double(sixgr.util.structGet(rarRx,'BackoffIndicatorPresent',NaN)), ...
    "BackoffParameter_ms",double(sixgr.util.structGet(rarRx,'BackoffParameter_ms',NaN)), ...
    "RAPIDDecoded", double(r.RAPIDDecoded), ...
...
row = struct("RunId", string(r.RunId), "CellId", double(r.CellId), "UEId", double(r.UEId), ...
    ...
    "PayloadEvidenceRole", "crc_valid_receiver_decoded_payload", ...
    "TransmittedPayloadHex", string(r.Msg3TransmittedPayloadHex), ...
...
row = struct("RunId", string(r.RunId), "CellId", double(r.CellId), "UEId", double(r.UEId), ...
    ...
    "PayloadEvidenceRole", "crc_valid_receiver_decoded_payload", ...
    "TransmittedPayloadHex", string(r.Msg4TransmittedPayloadHex), ...
    ...
    "FailureReason", string(r.FailureReason));
row.DCIContextDigest = string(sixgr.util.structGet(r,'Msg4DCIContextDigest',""));
row.MACEncodingProfile = string(sixgr.util.structGet(r,'Msg4MACEncodingProfile',""));
row.MACRRCStandardsQualified = logical(sixgr.util.structGet(r,'Msg4MACRRCStandardsQualified',false));
end
