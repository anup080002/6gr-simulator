...
row.NoiseSNR_dB = double(sixgr.util.structGet( ...
    replay,"AppliedAWGNSNR_dB",runtime.RuntimeNoiseSNR_dB));
...
switch mode
    ...
    case "receiver_noise_figure_thermal_noise"
        ...
        if ~(isfinite(servingRxPower_dBm) && isfinite(thermalNoisePower_dBm) && ...
            ...
            error("sixgr:phy:ra:MissingThermalNoiseAnchor", ...
                "Runtime RA requires measured serving receive power, receiver " + ...
                "thermal-noise power, and finite post-channel waveform power.");
        end
        nVar = referencePower*10.^((thermalNoisePower_dBm-servingRxPower_dBm)/10);
        noise = sqrt(nVar/2).*(randn(size(desiredWaveform), ...
            "like",real(desiredWaveform))+1i*randn(size(desiredWaveform), ...
            "like",real(desiredWaveform)));
        waveform = desiredWaveform+cast(noise,"like",desiredWaveform);
