...
localRequire(numel(dmrsPortSet) == numLayers && ...
    all(isfinite(dmrsPortSet)) && all(dmrsPortSet == fix(dmrsPortSet)) && ...
    all(dmrsPortSet >= 0) && numel(unique(dmrsPortSet)) == numLayers, ...
    "BadDMRSPortSet", ...
    "CodingLayout.DMRSPortSet must contain one unique logical port per layer at stage '%s'.", ...
    stage);
ptrsEnabled = logical(sixgr.util.structGet(cl, "PTRSEnabled", false));
ptrsPortSet = double(sixgr.util.structGet(cl, "PTRSPortSet", []));
if ptrsEnabled
    localRequire(numel(ptrsPortSet) == 1 && isfinite(ptrsPortSet) && ...
        ptrsPortSet == fix(ptrsPortSet) && ptrsPortSet >= 0 && ...
        ismember(ptrsPortSet, dmrsPortSet), ...
        "BadPTRSPortAssociation", ...
        "CodingLayout.PTRSPortSet must identify one scheduled DM-RS port at stage '%s'.", ...
        stage);
else
    localRequire(isempty(ptrsPortSet), ...
        "DisabledPTRSPortPresent", ...
        "A PT-RS-disabled PHYGrant must not freeze a PT-RS port at stage '%s'.", ...
        stage);
end
...
if ~isempty(opts.Waveform)
    ...
    if opts.AllowAuxiliaryWaveformColumns
        localRequire(size(opts.Waveform, 2) >= numWaveformColumns, ...
            "WaveformColumnMismatch", ...
            ["Composite waveform has %d column(s), but the frozen PHYGrant " ...
            "requires at least %d primary-signal column(s) at stage '%s'."], ...
...
if opts.AllowAuxiliaryWaveformColumns && ~isempty(opts.Waveform) && ~isempty(opts.Grid)
    localRequire(size(opts.Waveform, 2) == size(opts.Grid, 3), ...
        "CompositeWaveformGridColumnMismatch", ...
        ["Composite waveform has %d column(s), but its resource grid has %d " ...
        "page(s) at stage '%s'."], ...
        size(opts.Waveform, 2), size(opts.Grid, 3), stage);
end
...
opts = struct("PDSCH", [], "PUSCH", [], "Precoding", [], ...
    ...
    "AllowAuxiliaryWaveformColumns", false);
...
for i = 1:2:numel(varargin)
    ...
    switch key
        ...
        case "grid"
            opts.Grid = val;
        case "allowauxiliarywaveformcolumns"
            if ~(islogical(val) || (isnumeric(val) && isscalar(val) && isfinite(double(val))))
                error("sixgr:phy:grant:BadAssertOption", ...
                    "AllowAuxiliaryWaveformColumns must be a scalar logical value.");
            end
            opts.AllowAuxiliaryWaveformColumns = logical(val);
        otherwise
