...
persistent metaTemplate;
if isempty(metaTemplate)
    metaTemplate = struct('LCID',NaN,'IsControl',false,'SN',NaN,'SI',NaN,'SO',NaN,'Poll',false,'Length',0, ...
        'PktId',NaN,'FlowId',NaN,'QFI',NaN,'CreationSlot',NaN,'CreationTime_s',NaN, ...
        'PDCP_SN',NaN,'RLC_SN',NaN,'SegmentOffset',NaN, ...
        'HARQProcess',NaN,'GrantSlot',NaN,'DeliverySlot',NaN,'DropCause',"");
end
meta = metaTemplate;
meta.PktId = tr.PktId;
meta.FlowId = tr.FlowId;
meta.QFI = tr.QFI;
meta.CreationSlot = tr.CreationSlot;
meta.CreationTime_s = tr.CreationTime_s;
meta.PDCP_SN = tr.PDCP_SN;
meta.RLC_SN = tr.RLC_SN;
meta.SegmentOffset = tr.SegmentOffset;
meta.HARQProcess = tr.HARQProcess;
meta.GrantSlot = tr.GrantSlot;
meta.DeliverySlot = tr.DeliverySlot;
meta.DropCause = tr.DropCause;
end
...
if isfield(in, "PktId"), meta.PktId = in.PktId; end
if isfield(in, "FlowId"), meta.FlowId = in.FlowId; end
if isfield(in, "QFI"), meta.QFI = in.QFI; end
if isfield(in, "CreationSlot"), meta.CreationSlot = in.CreationSlot; end
if isfield(in, "CreationTime_s"), meta.CreationTime_s = in.CreationTime_s; end
if isfield(in, "PDCP_SN"), meta.PDCP_SN = in.PDCP_SN; end
if isfield(in, "RLC_SN"), meta.RLC_SN = in.RLC_SN; end
if isfield(in, "SegmentOffset"), meta.SegmentOffset = in.SegmentOffset; end
if isfield(in, "HARQProcess"), meta.HARQProcess = in.HARQProcess; end
if isfield(in, "GrantSlot"), meta.GrantSlot = in.GrantSlot; end
if isfield(in, "DeliverySlot"), meta.DeliverySlot = in.DeliverySlot; end
if isfield(in, "DropCause") && strlength(string(in.DropCause)) > 0
    meta.DropCause = in.DropCause;
end
end
