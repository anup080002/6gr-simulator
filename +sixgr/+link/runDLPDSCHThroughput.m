...
[txRuntimeAntenna, txRuntimeMeta] = localRuntimeSignalAntennaView(userMeta, ...
    "RuntimeServingBSAntenna", "RuntimeServingBSAntennaMeta", runtimeNumTx, ...
...
grant = struct( ...
    ...
    "TBSInputModulation", localSafeCharToken(pdschMod), ...
...
isRetx = isstruct(harqContext) && logical(sixgr.util.structGet(harqContext, "IsRetransmission", false));
if ~isstruct(grant)
    grant = struct();
end
...
if ~(isstruct(phyGrant) && ~isempty(fieldnames(phyGrant)) && ...
        ...
        isfinite(phyTBS) && round(phyTBS) == round(replayBits) && (~isRetx || phyRetx))
...
if localGrantHasExplicitDLPrecoding(grant, precodingMatrix)
