...
for n = 1:numFrames
    ...
    try
        [cfgDyn, laState, laApplyEvent] = sixgr.link.updateLinkAdaptationState(cfgDyn, laState, "UL", n, "Phase", "before");
        cfgFrame = cfgDyn;
        ...
        [tx, txInfo] = sixgr.phy.ul.PUSCH_Tx(cfgFrame);
        if isfield(tx, "PUSCH")
            try
        ...
        [rx, ~] = sixgr.phy.ul.PUSCH_Rx(rxWave, cfgFrame, ...
            "Carrier", tx.Carrier, ...
            "PUSCH", tx.PUSCH, ...
        ...
        [cfgDyn, laState, laObserveEvent] = sixgr.link.updateLinkAdaptationState(cfgDyn, laState, "UL", n, ...
...
configuredPMI = double(sixgr.util.structGet(cfg, "phy.pusch.PMI", sixgr.util.structGet(cfg, "phy.pdsch.PMI", NaN)));
