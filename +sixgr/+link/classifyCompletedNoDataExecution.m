function evidence=classifyCompletedNoDataExecution(raw,expectedSlots)
% Explain unavailable data measurements; never declare PHY/scenario success.
% A completed physical clock with no issued or transmitted data is distinct
% from missing receiver rows. This fact does not depend on acquisition CRC.
...
if ~isa(owner,'sixgr.truth.CoupledWaveformStream') || isempty(owner.Events), return; end
...
evidence.Reason="completed_physical_execution_zero_data_grants_and_transmissions";
