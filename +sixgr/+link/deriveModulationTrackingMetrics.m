...
metrics = struct( ...
    ...
    "EqualizerCovarianceFactorizationCount", NaN, ...
    "DataRECount", NaN, ...
...
% An RE count or diagnostic H-grid shape is not an operation count.
% Report the work instrumented by the receiver, without inventing FLOPs.
for name=["EqualizerSolveCount","EqualizerUniqueSolveCount", ...
        "EqualizerCovarianceFactorizationCount"]
    value=sixgr.util.structGet(rx,name,NaN);
    validateattributes(value,{'numeric'},{'scalar','real'});
    assert(isnan(value) || (isfinite(value) && value>=0 && value==fix(value)), ...
        'sixgr:link:InvalidEqualizerWorkEvidence', ...
        'Executed equalizer counter %s must be an integer or unavailable.',name);
