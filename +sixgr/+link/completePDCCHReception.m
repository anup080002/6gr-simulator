...
if ~isempty(fieldnames(alignment))
    expected=sixgr.phy.frame.pdcchReceivedClockAlignment(alignment.Reference, ...
        sixgr.phy.grid.makeCarrier(prepared.ReceiverConfig),prepared.RuntimeStartSample,prepared.SampleRateHz, ...
...
if isfield(sixgr.util.structGet(cfg,'phy.pdcch.operatorControl',struct()),'connected_dci')
    monitoringArgs={}; % No TX object, identity, format, or payload length.
else
    monitoringArgs={'Carrier',tx.Carrier,'PDCCH',tx.PDCCH,'K',numel(tx.DCIBits), ...
        'RNTI',txInfo.RNTI,'PDCCHScramblingRNTI',txInfo.PDCCHScramblingRNTI};
end
if ~logical(sixgr.util.structGet(cfg,'phy.pdcch.blindSearch',false)) && isempty(fieldnames(alignment))
    % Search only offsets for which the complete monitored symbols really
    % arrived. Select the correlation maximum within this interval; do not
    % clip an out-of-window noise peak or append unreceived samples.
    monitoringArgs=[monitoringArgs,{'TimingSearchWindowSamples', ...
        [0 size(samples,1)-prepared.MinimumReceiveSamples]}];
end
