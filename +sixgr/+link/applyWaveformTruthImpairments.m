function [y, replay, state, reference] = applyWaveformTruthImpairments(x, snr_dB, state, cfg, tx, txInfo, varargin)
...
assert(~capture || (isstruct(state) && isscalar(state)), ...
    'sixgr:link:ChannelReferenceStateRequired','Channel reference capture requires the executed physical state.');
...
if isstruct(state)
    if sixgr.channel.ChannelFactory.requiresRuntimeChannelState(cfg) || ...
        ...
        [y, channelReplay, state, reference] = sixgr.link.applyRuntimeFadingChannel( ...
            x,state,"OutputSampleAlignment","continuous_raw_samples", ...
            "InputSampleDomain",ip.Results.InputSampleDomain,"CaptureChannelReference",capture);
    else
        [y, channelReplay, state, reference] = sixgr.link.applyRuntimeFadingChannel( ...
            x,state,"InputSampleDomain",ip.Results.InputSampleDomain,"CaptureChannelReference",capture);
    end
...
if noiseMode == "receiver_noise_figure_thermal_noise"
    ...
    if fixedSNRReference
        ...
        validateattributes(sampleVariance, {'numeric'}, ...
            {'real','scalar','finite','positive'});
        noiseState = sixgr.util.structGet(state, "ReceiverNoiseState", struct());
        origin = double(sixgr.util.structGet(noiseState, "OriginSample", startSample));
        runSeed = sixgr.util.structGet(cfg, "run.seed", []);
        validateattributes(runSeed, {'numeric'}, ...
            {'real','scalar','finite','integer','nonnegative'});
        identity = struct("RunSeed",runSeed,"LinkKey", ...
            string(sixgr.util.structGet(state, ...
            "RuntimeChannelState.LinkKey","standalone_receiver")), ...
            "UEIndex",sixgr.util.structGet(cfg, ...
            "lls6g.userContext.UEIndex",sixgr.util.structGet(cfg, ...
            "lls6g.userContext.RuntimeUEIndex",[])), ...
            "ServingCell",sixgr.util.structGet(cfg, ...
            "lls6g.userContext.RuntimeServingCellIndex",[]), ...
            "CarrierFrequencyHz",sixgr.util.structGet(cfg,"phy.fc_Hz", ...
            sixgr.util.structGet(cfg,"channel.fc_Hz",[])), ...
            "OriginSample",origin,"SampleRateHz",fs,"Direction",direction, ...
            "Role","fixed_occupied_re_awgn");
        ...
        replay.NoiseStreamOriginSample = double(origin);
        replay.RequestedAWGNReferenceSNR_dB = double(appliedSnr_dB);
        replay.SignalEnergyPerOccupiedRE = double(signalEnergy);
        replay.GridNoiseVariance = double(gridVariance);
        replay.ReferenceAWGNGridNoiseVariance = double(gridVariance);
        replay.ReferenceAWGNSampleNoiseVariance = double(sampleVariance);
...
userMeta.RuntimeServingPathloss_dB = double(sixgr.util.structGet( ...
    state, "Pathloss_dB", NaN));
% A direction/link-specific channel state must not retain a different
% link's propagation value inherited through the input configuration.
fields=intersect(fieldnames(userMeta), ...
    {'RuntimePropagationPathloss_dB','RuntimePropagationPathlossSource'});
userMeta=rmfield(userMeta,fields);
if isfinite(userMeta.RuntimeServingPathloss_dB)
    userMeta.RuntimePropagationPathloss_dB=userMeta.RuntimeServingPathloss_dB;
    userMeta.RuntimePropagationPathlossSource="waveform_truth_channel_state.Pathloss_dB";
end
