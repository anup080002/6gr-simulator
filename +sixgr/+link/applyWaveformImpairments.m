...
servingRSRPSource = string(sixgr.util.structGet( ...
    userMeta, "RuntimeServingRSRPSource", ""));
if strlength(strtrim(servingRSRPSource)) == 0
    servingRSRPSource = localAvailableSource( ...
        servingRSRP_dBm, "large_scale_pathloss_applied_per_reference_re_power");
end
...
replay = struct( ...
    ...
    "ServingRSRPSource", char(servingRSRPSource), ...
...
% same large-scale loss/gains applied to the waveform.
signalSpecific = logical(sixgr.util.structGet(powerContext, ...
    "SignalSpecificPowerControl",false));
if signalSpecific && isfinite(referenceTxPower_dBm) && ...
        isfinite(loss_dB) && loss_dB >= 0 && ...
        (isfinite(pathloss_dB) || isfinite(basePathloss_dB))
    ...
    servingRxPower_dBm = referenceTxPower_dBm+txGain_dB+rxGain_dB- ...
        loss_dB-additionalLoss_dB;
    source = "signal_specific_power_context_link_budget";
    return;
end
