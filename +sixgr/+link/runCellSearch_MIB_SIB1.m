...
if wantSIB1
    try
        ...
        receiveOrigin = double(sixgr.util.structGet(out, ...
            "RuntimeChannelReplay.RuntimeChannelStartSample", 0));
        receiver = sixgr.phy.waveform.WaveformReceiveDispatcher( ...
            double(tx.SampleRateHz), size(rxWaveform,2), receiveOrigin);
        receiver.register("cell_search_broadcast", ...
            receiveOrigin, receiveOrigin + size(rxWaveform,1));
        completed = receiver.dispatch(sixgr.phy.waveform.WaveformChunk( ...
            rxWaveform, receiveOrigin), double(tx.SampleRateHz));
        observation = completed(1).Observation;
        out = sixgr.link.completeCellSearchBroadcast( ...
            prepared, observation, out, p.Results, tStart);
        return;
    catch ME
        out.Ok = false;
        out.BLER = 1;
