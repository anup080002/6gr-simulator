...
[grant.DMRSPortSet, grant.DMRSPortSetSource] = ...
    ...
    cfg, direction, grant.NumLayers, grant);
...
grant.K1 = NaN;
grant.K2 = NaN;
grant.SearchSpaceID = double(sixgr.util.structGet(cfg, ...
    "phy.pdcch.searchSpace.id", ...
    sixgr.util.structGet(cfg, "phy.dl.pdcch.SearchSpaceID", 1)));
grant.CORESETID = double(sixgr.util.structGet(cfg, ...
    "phy.pdcch.coreset.id", ...
    sixgr.util.structGet(cfg, "phy.dl.pdcch.CORESETID", 0)));
grant.BWPId = NaN;
