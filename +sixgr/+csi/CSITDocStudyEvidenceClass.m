classdef CSITDocStudyEvidenceClass
    ...
    methods (Static)
        ...
        function value = validate(value)
            ...
            if ~isscalar(value) || ~any(value == sixgr.csi.CSITDocStudyEvidenceClass.allowed())
                error("sixgr:csi:InvalidEvidenceClass", ...
                    "CSITDocStudyEvidenceClass must be one of: %s.", ...
                    strjoin(sixgr.csi.CSITDocStudyEvidenceClass.allowed(),", "));
            end
        end

        function tf = tdocQuantitativeEligible(value)
            value = sixgr.csi.CSITDocStudyEvidenceClass.validate(value);
        ...
        function value = validateFigure(value)
            value = upper(strtrim(string(value)));
            if ~isscalar(value) || ...
                    ~any(value == sixgr.csi.CSITDocStudyEvidenceClass.allowedFigures())
                error("sixgr:csi:InvalidFigureEvidenceClass", ...
                    "Figure CSITDocStudyEvidenceClass must be one of: %s.", ...
                    strjoin(sixgr.csi.CSITDocStudyEvidenceClass.allowedFigures(),", "));
            end
        end
    end
end
