function paths=writeSuiteTables(runFolder,context,registry,inventory,gaps,a,p,w)
...
paths=strings(0,1);
    function emit(name,T,subfolder)
        ...
        path=fullfile(folder,name); sixgr.csi.ResultWriter.write(path,T,context);
        paths(end+1,1)=string(path); %#ok<AGROW>
    end
sixgr.csi.ResultWriter.write(fullfile(manifestRoot,"repository_inventory.csv"),inventory,context);
sixgr.csi.ResultWriter.write(fullfile(manifestRoot,"gap_analysis.csv"),gaps,context);
sixgr.csi.ResultWriter.write(fullfile(manifestRoot,"scenario_registry.csv"),registry,context);
